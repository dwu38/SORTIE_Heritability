...
modelMsg CheckForMessage( char * cAppPath )
{
  ...
  const char * cFileName = "messages.txt";
  char * cFilePath = new char[strlen( cAppPath ) + strlen( cFileName ) + 1];
  strcpy( cFilePath, cAppPath );
  strcat( cFilePath, cFileName );
  FILE * messages = fopen( cFilePath, "rt" );
  ...
  if ( messages )
  {
    ...
    //Empty the file by closing it and reopening in truncate mode
    fclose( messages );
    fopen( cFilePath, "wt" );
    fclose( messages );
...
{
  using namespace std;	
  cout << "I do not understand that command.\n";
}
...
void WriteRunComplete( int iCurrentTimestep, int iNumTimesteps, unsigned long iRunTime )
{
  ...
  if ( iRunTime > 0 )
  {
    iTemp = (int)floor( iRunTime / 3600 );
    sprintf( cTime, "%d", iTemp );
    strcat( cTime, ":" );
    iRunTime -= ( iTemp * 3600 );

    //Calc number of minutes
    iTemp = (int)floor( iRunTime / 60 );
    sprintf( cTime2, "%s%d", cTime, iTemp );
    strcat( cTime2, ":" );
    iRunTime -= ( iTemp * 60 );

    //Add number of seconds
    sprintf( cTime, "%s%d", cTime2, iRunTime );

    if (iNumTimesteps > 0) {
      cout << "Run is complete. Completed " << iCurrentTimestep << " out of " << iNumTimesteps
           << " timesteps.  Run time:  " << cTime << ".\n";
    }
    else {
      cout << "Run is complete.  Run time:  " << cTime << "\n";
    }
  }
  else
  {
    if (iNumTimesteps > 0) {
      cout << "Run is complete. Completed " << iCurrentTimestep << " out of " << iNumTimesteps << " timesteps.\n";
    }
    else {
...
{
  using namespace std;	
  //Don't change this wording too much- the Java interface depends on the
  //presence of the word "paused" to know the model is paused.
  cout << "The run is paused.\n";
}
