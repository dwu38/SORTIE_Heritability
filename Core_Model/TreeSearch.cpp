...
void clTreeSearch::Setup() {
	try {
		...
		if (!((m_bAllUsed && !m_bDistanceHeightUsed && !m_bSpeciesUsed
				&& !m_bTypeUsed)
		...
		if (m_bDistanceHeightUsed) {
			...
			if (m_fDistanceCutoff < 0 || m_fHeightCutoff < 0) { //nope - throw error
				...
				stcErr.sFunction = "clTreeSearch::Setup";
				stcErr.sMoreInfo =
						"Either distance or height search criterion is a negative number.";
				throw(stcErr);
			}
			//Make sure our "from" point is within the plot
			if (m_fFromX < 0 || m_fFromX >= mp_oTreePop->m_fPlotLengthX
					|| m_fFromY < 0
					|| m_fFromY >= mp_oTreePop->m_fPlotLengthY) {
				modelErr stcErr;
				stcErr.iErrorCode = BAD_DATA;
				stcErr.sFunction = "clTreeSearch::Setup";
				stcErr.sMoreInfo =
						"Either X or Y search point is outside plot.";
				throw(stcErr);
			}
		...
		if (m_bTypeUsed) {
			...
			if (m_bSeeds || m_bSeedlings || m_bSaplings)
				...
				//start at the height div of the shortest adult height
				m_iStartHeightDiv = (int) floor(
						mp_oTreePop->m_fMinAdultHeight
								/ mp_oTreePop->m_iSizeHeightDivs);
			...
			if (!m_bAdults && !m_bSnags && !m_bWoody_Debris && !m_bSaplings) {
				...
				for (int m = 0; m < mp_oTreePop->GetNumberOfSpecies(); m++) {
					if (fMaxSeedlingHeight
							< mp_oTreePop->mp_fMaxSeedlingHeight[m])
						fMaxSeedlingHeight =
								mp_oTreePop->mp_fMaxSeedlingHeight[m];
				}
				m_iEndHeightDiv = (int) floor(
						fMaxSeedlingHeight / mp_oTreePop->m_iSizeHeightDivs);
			}
			//If there are saplings then the tallest height div is the sapling height
			//cutoff one
			else if (!m_bAdults && !m_bSnags && !m_bWoody_Debris)
				m_iEndHeightDiv = (int) floor(
						mp_oTreePop->m_fMaxSaplingHeight
								/ mp_oTreePop->m_iSizeHeightDivs);
			//Otherwise it's the tallest one
			else
				m_iEndHeightDiv = mp_oTreePop->m_iNumHeightDivs - 1;
...
clTree* clTreeSearch::FindNextAllTree() {
	...
	if (p_oTree)
		...
		if (!p_oTree) {
			for (iX = m_iCurrentXGrid + 1; iX < mp_oTreePop->m_iNumXCells;
					iX++) {
				for (iY = 0; iY < mp_oTreePop->m_iNumYCells; iY++) {
					for (iDiv = 0; iDiv < mp_oTreePop->m_iNumHeightDivs;
							iDiv++) {
						p_oTree = mp_oTreePop->mp_oTreeShortest[iX][iY][iDiv];
						if (p_oTree != NULL)
							break;
...
clTree* clTreeSearch::FindNextTSTree() {
	...
	while (p_oNextTree) {
		...
		if (mp_bWhatSpecies[p_oNextTree->GetSpecies()]
				&& (iTypeTest & iBitMask))
			return p_oNextTree;
		else {
			p_oNextTree->GetValue(
					mp_oTreePop->mp_iHeightCode[p_oNextTree->GetSpecies()][p_oNextTree->GetType()],
					&fHeight);
			//Check to see if we're taller than the cutoff height division
			if (floor(fHeight / mp_oTreePop->m_iSizeHeightDivs)
					> m_iEndHeightDiv)
				break;
		}
	...
	for (iY = m_iCurrentYGrid + 1; iY < mp_oTreePop->m_iNumYCells; iY++) {
		...
		if (p_oNextTree == NULL)
			for (int i = m_iStartHeightDiv + 1; i <= m_iEndHeightDiv; i++) {
				p_oNextTree =
						mp_oTreePop->mp_oTreeShortest[m_iCurrentXGrid][iY][i];
				if (p_oNextTree != NULL)
					break;
			}
	...
	for (iX = m_iCurrentXGrid + 1; iX < mp_oTreePop->m_iNumXCells; iX++)
		for (iY = 0; iY < mp_oTreePop->m_iNumYCells; iY++) {
			...
			//above it
			p_oNextTree =
					mp_oTreePop->mp_oTreeShortest[iX][iY][m_iStartHeightDiv];
			if (p_oNextTree == NULL)
				for (int i = m_iStartHeightDiv + 1; i <= m_iEndHeightDiv; i++) {
		...
		for (iY = 0; iY < mp_oTreePop->m_iNumYCells; iY++) {
			...
			while (p_oTree != NULL) {
				...
				if (iTypeTest & iBitMask) {
					...
					p_oTree->GetValue(
							mp_oTreePop->mp_iHeightCode[p_oTree->GetSpecies()][p_oTree->GetType()],
							&fHeight);
					//Check to see if we're taller than the cutoff height division
					if (floor(fHeight / mp_oTreePop->m_iSizeHeightDivs)
							> m_iEndHeightDiv)
...
clTree* clTreeSearch::FindFirstTSTree() {
	...
	for (iX = 0; iX < mp_oTreePop->m_iNumXCells; iX++)
		for (iY = 0; iY < mp_oTreePop->m_iNumYCells; iY++) {
			...
			while (p_oTree != NULL) {
				iTypeTest = m_iWhatTypes >> p_oTree->GetType();
				if (mp_bWhatSpecies[p_oTree->GetSpecies()]
						&& (iTypeTest & iBitMask)) {
					//set this as the tree to return
					m_iCurrentXGrid = iX;
					m_iCurrentYGrid = iY;
...
clTree* clTreeSearch::FindFirstDHTree() {
	...
	m_iStartHeightDiv = (int) floor(
			m_fHeightCutoff / mp_oTreePop->m_iSizeHeightDivs);
	...
	if (m_fDistanceCutoff >= mp_oTreePop->m_fPlotLengthY / 2) {
		m_iMaxY = (int) floor(
				(m_fFromY + mp_oTreePop->m_fPlotLengthY / 2) / iGrLen);
		m_iMinY = (int) floor(
				(m_fFromY - mp_oTreePop->m_fPlotLengthY / 2) / iGrLen) + 1;
	} else {
	...
	for (iOldY = m_iMinY; iOldY <= m_iMaxY; iOldY++) {
		...
		for (iOldX = m_iMinX; iOldX <= m_iMaxX; iOldX++) {
			...
			while (p_oTree) {
				unsigned int iSp = p_oTree->GetSpecies(), iTp =
						p_oTree->GetType();
				p_oTree->GetValue(mp_oTreePop->mp_iHeightCode[iSp][iTp],
						&fHeight);
...
void clTreeSearch::GetMinMaxX(const int &iY, const int &iGrLen) {
	...
	//DECORRECT for torus
	if (fMinX < 0)
		m_iMinX -= mp_oTreePop->GetNumXCells();
	if (fMinX >= mp_oPlot->GetXPlotLength())
		m_iMinX += mp_oTreePop->GetNumXCells();
	if (fMaxX < 0)
		m_iMaxX -= mp_oTreePop->GetNumXCells();
	if (fMaxX >= mp_oPlot->GetXPlotLength())
		m_iMaxX += mp_oTreePop->GetNumXCells();
