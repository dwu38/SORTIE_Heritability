...
public:
	 ...
	 *
	 * changed to allow for the SizeClass to be updated and the corresponding diameter (Diam10)
	 * and height to be updated with allometric relations
	 */
	void SetValue(short int iCode, int iValue, bool bUpdateNow = false,
			bool bUpdateAllometry = false);
	 ...
	 */
	void SetValue(short int iCode, float fValue, bool bUpdateNow = true,
...
protected:
	 ...
	 */
	clTree(int iType, int iSpecies, int iNumFloats, int iNumInts,
