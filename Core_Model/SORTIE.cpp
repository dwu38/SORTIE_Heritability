		...
		try {
			...
			if (batchfile == iFileType) {
				...
				if (p_oSimManager.GetSimState() == clSimManager::Run_Complete
						|| p_oSimManager.GetSimState()
								== clSimManager::No_Data) {
					timer = time( NULL);
					stcTime = localtime(&timer);
				...
				if ((p_oSimManager.GetSimState() == clSimManager::Run_Complete
						&& p_oSimManager.GetCurrentTimestep()
								== p_oSimManager.GetNumberOfTimesteps())
						|| p_oSimManager.GetSimState()
								== clSimManager::No_Data) {
					stringstream msg;
					msg << p_oSimManager.GetCurrentTimestep() << ","
							<< p_oSimManager.GetNumberOfTimesteps() << ","
							<< iRunTime;
				...
				cout
						<< "The file passed to SORTIE is not of a valid type.  Exiting.\n";
				//An error occurred - exit
				return 0;
			}
		} //end of try block
				...
				if (clSimManager::No_Data == p_oSimManager.GetSimState()) {
					...
					if ("" == strArg1) {
						...
						if (2 == argc || 3 == argc) {
							...
							if ((p_oSimManager.GetSimState()
									== clSimManager::Run_Complete
									&& p_oSimManager.GetCurrentTimestep()
											== p_oSimManager.GetNumberOfTimesteps()
						...
						if (0 == iNumTimesteps && strArg1.length() > 1) {
							...
							stcMsg.sMoreInfo =
									"Couldn't read number of timesteps";
							SendMessage(stcMsg);
						} else {
							iRunTime = p_oSimManager.RunSim(iNumTimesteps);
							if (2 == argc || 3 == argc) {
