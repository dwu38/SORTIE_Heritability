...
public:
	 ...
	 */
	static float CalculateWeibullFunction(float fDispersal, float fTheta,
			float fDistance);
	 ...
	 * zero.
	 */
	static float CalculateLognormalFunction(float fX0, float fXb,
			float fDistance);

	/**
	 * Calculates the exponential of the 2Dt probability distribution
	 * function at a particular point.
	 *
	 * @param fX0 Value of u variable
	 * @param fDistance Distance, in meters, at which to calculate the function
	 * @return Value of function
	 * @throw modelErr if fX0 or fDistance are not greater than 0, or if fXb is
	 * zero.
	 */
	static float CalculatetwodtFunction(float fX0, float fDistance);
	...
	/**
	 * Returns a random zero-inflated Poisson-distributed number. This just wraps the appropriate
	 * GSL function.
	 *
	 * @param fLambda Lambda (mean) of the Poisson function.
	 * @return Integer random draw.
	 */
	static int ZeroInflatedPoissonRandomDraw(float fLambda, float fp);
