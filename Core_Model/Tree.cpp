...
clTree::clTree(int iType, int iSpecies, int iNumFloats, int iNumInts,
	...
	try {
		...
		if (iType < clTreePopulation::seed
		...
		if (iNumFloats < 0 || iNumInts < 0 || iNumStrings < 0
				|| iNumBools < 0) {
			modelErr stcErr;
			stcErr.iErrorCode = BAD_DATA;
			stcErr.sFunction = "clTree::clTree";
...
// SetValue
///////////////////////////////////////////////////////////////////////////*/
void clTree::SetValue(short int iCode, int iValue, bool bUpdateNow,
		bool bUpdateAllometry) {
	mp_oTreePop->UpdateTree(this, iCode, iValue, bUpdateNow, bUpdateAllometry);
}

////////////////////////////////////////////////////////////////////////////
// SetValue
///////////////////////////////////////////////////////////////////////////*/
void clTree::SetValue(short int iCode, float fValue, bool bUpdateNow,
		bool bUpdateAllometry) {
...
void clTree::GetValue(short int iCode, int *p_iValHolder) {
	if (iCode < 0
			|| iCode > mp_oTreePop->mp_iNumTreeIntVals[m_iSpecies][m_iType]) {
		modelErr stcErr;
		stcErr.iErrorCode = BAD_DATA;
		stcErr.sFunction = "clTree::clTree";
...
clDeadTree* clTree::MakeDeadClone() {
	...
	for (i = 0; i < mp_oTreePop->mp_iNumTreeFloatVals[m_iSpecies][m_iType];
			i++) {
	...
	for (i = 0; i < mp_oTreePop->mp_iNumTreeBoolVals[m_iSpecies][m_iType];
			i++) {
		p_oClone->mp_bBoolValues[i] = mp_bBoolValues[i];
	}

	for (i = 0; i < mp_oTreePop->mp_iNumTreeStringVals[m_iSpecies][m_iType];
			i++) {
