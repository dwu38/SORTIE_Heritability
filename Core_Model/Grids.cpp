...
// CreateGrid
/////////////////////////////////////////////////////////////////////////////
clGrid* clGridManager::CreateGrid(string sGridName, short int iNumIntVals,
	...
	//Create our grid
	p_oReturnGrid = new clGrid(mp_oSimManager, sGridName, iNumIntVals,
			iNumFloatVals, iNumStringVals, iNumBoolVals, fXCellLength,
			fYCellLength);
...
void clGridManager::CreateObjects(DOMDocument *p_oDoc) {
	try {
		...
		int iNumGrids,                //Number of grid objects to initialize
				i, j;                     //loop counters
		short int iNumInts, iNumFloats, iNumChars, iNumBools, iNumPackageInts,
				iNumPackageFloats, iNumPackageChars, iNumPackageBools;
		...
		for (i = 0; i < iNumGrids; i++) {
			...
			sGridName = XMLString::transcode(
					p_oGridEl->getAttributeNode(sVal)->getNodeValue());
			XMLString::release(&sVal);
			...
			if (0 == iNumPackageInts && 0 == iNumPackageFloats
					&& 0 == iNumPackageChars && 0 == iNumPackageBools)
				goto NextGrid;

			//Otherwise, register the package data members
			p_oGrid->ChangePackageDataStructure(iNumPackageInts,
					iNumPackageFloats, iNumPackageChars, iNumPackageBools);
