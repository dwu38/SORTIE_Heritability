...
void clTempDependentNeighborhoodDisperse::DoShellSetup(DOMDocument *p_oDoc) {
	...
	try {
		...
		FillSpeciesSpecificValue(p_oElement, "di_tempDepNeighFecM",
				"di_tdnfmVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		...
		FillSpeciesSpecificValue(p_oElement, "di_tempDepNeighFecN",
				"di_tdnfnVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		FillSpeciesSpecificValue(p_oElement, "di_tempDepNeighPresM",
				"di_tdnpmVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		FillSpeciesSpecificValue(p_oElement, "di_tempDepNeighPresB",
				"di_tdnpbVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		FillSpeciesSpecificValue(p_oElement, "di_tempDepNeighPresThreshold",
				"di_tdnptVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		//A
		FillSpeciesSpecificValue(p_oElement, "di_tempDepNeighA", "di_tdnaVal",
		...
		//B
		FillSpeciesSpecificValue(p_oElement, "di_tempDepNeighB", "di_tdnbVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
...
void clTempDependentNeighborhoodDisperse::AddSeeds() {
	float *p_fBAC = NULL, *p_fFecundity = NULL, *p_fFunctionalA = NULL; //Intercept actually used - zero this out if a
	...
	try {
			...
			for (i = 0; i < m_iNumBehaviorSpecies; i++) {
				...
				if (false == p_bPresent[iSpecies]) {
					fTemp = exp(
							-0.5
									* pow(
											(fMeanTempC - mp_fPresM[iSpecies])
													/ mp_fPresB[iSpecies], 2));
		...
		for (iX = 0; iX < iNumXCells; iX++) {
			for (iY = 0; iY < iNumYCells; iY++) {
				...
				for (i = 0; i < m_iNumBehaviorSpecies; i++) {
					iSpecies = mp_iWhatSpecies[i];
					fNumGridSeeds = p_fFunctionalA[iSpecies]
