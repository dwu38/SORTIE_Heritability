...
void clMastingNonSpatialDisperse::DoShellSetup(DOMDocument *p_oDoc) {
	...
	try {
		...
		FillSpeciesSpecificValue(p_oElement, "di_nonSpatialMastBinomialP",
				"di_nsmbpVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		FillSpeciesSpecificValue(p_oElement, "di_nonSpatialMastMastFunction",
				"di_nsmmfVal", p_iTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		...
		FillSpeciesSpecificValue(p_oElement, "di_nonSpatialNonMastFunction",
				"di_nsnmfVal", p_iTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		...
		FillSpeciesSpecificValue(p_oElement, "di_nonSpatialMastGroup",
				"di_nsmgVal", p_iTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		if (iNumFunctionSpecies > 0) {
			...
			FillSpeciesSpecificValue(p_oElement, "di_nonSpatialMastInvGaussMu",
					"di_nsmigmVal", p_fTempValues, iNumFunctionSpecies, p_oPop,
					true);
			...
			for (i = 0; i < iNumFunctionSpecies; i++) {
				if (p_fTempValues[i].val <= 0) {
					...
					stcErr.iErrorCode = BAD_DATA;
					stcErr.sFunction =
							"clMastingNonSpatialDisperse::DoShellSetup";
					stcErr.sMoreInfo =
							"Inverse gaussian mu values must be greater than zero.";
					throw(stcErr);
				}
			}
			...
			FillSpeciesSpecificValue(p_oElement,
					"di_nonSpatialMastInvGaussLambda", "di_nsmiglVal",
					p_fTempValues, iNumFunctionSpecies, p_oPop, true);
			...
			for (i = 0; i < iNumFunctionSpecies; i++) {
				if (p_fTempValues[i].val <= 0) {
					...
					stcErr.sFunction =
							"clMastingNonSpatialDisperse::DoShellSetup";
					stcErr.sMoreInfo =
							"Inverse gaussian lambda values must be greater than zero.";
					throw(stcErr);
				}
			}
		...
		if (iNumFunctionSpecies > 0) {
			...
			FillSpeciesSpecificValue(p_oElement, "di_nonSpatialMastNormalMean",
					"di_nsmnmVal", p_fTempValues, iNumFunctionSpecies, p_oPop,
					true);
			for (i = 0; i < iNumFunctionSpecies; i++) {
				for (j = 0; i < m_iNumBehaviorSpecies; j++) {
					if (p_fTempValues[i].code == mp_iWhatSpecies[j]) {
			...
			//Standard deviation
			FillSpeciesSpecificValue(p_oElement,
					"di_nonSpatialMastNormalStdDev", "di_nsmnsdVal",
					p_fTempValues, iNumFunctionSpecies, p_oPop, true);
			for (i = 0; i < iNumFunctionSpecies; i++) {
				for (j = 0; i < m_iNumBehaviorSpecies; j++) {
					if (p_fTempValues[i].code == mp_iWhatSpecies[j]) {
		...
		if (iNumFunctionSpecies > 0) {
			//Mu
			FillSpeciesSpecificValue(p_oElement,
					"di_nonSpatialNonMastInvGaussMu", "di_nsnmigmVal",
					p_fTempValues, iNumFunctionSpecies, p_oPop, true);
			...
			//Lambda
			FillSpeciesSpecificValue(p_oElement,
					"di_nonSpatialNonMastInvGaussLambda", "di_nsnmiglVal",
					p_fTempValues, iNumFunctionSpecies, p_oPop, true);
		...
		if (iNumFunctionSpecies > 0) {
			//Mean
			FillSpeciesSpecificValue(p_oElement,
					"di_nonSpatialNonMastNormalMean", "di_nsnmnmVal",
					p_fTempValues, iNumFunctionSpecies, p_oPop, true);
			for (i = 0; i < iNumFunctionSpecies; i++) {
				for (j = 0; i < m_iNumBehaviorSpecies; j++) {
					if (p_fTempValues[i].code == mp_iWhatSpecies[j]) {
			...
			//Standard deviation
			FillSpeciesSpecificValue(p_oElement,
					"di_nonSpatialNonMastNormalStdDev", "di_nsnmnsdVal",
					p_fTempValues, iNumFunctionSpecies, p_oPop, true);
			for (i = 0; i < iNumFunctionSpecies; i++) {
				for (j = 0; i < m_iNumBehaviorSpecies; j++) {
					if (p_fTempValues[i].code == mp_iWhatSpecies[j]) {
						mp_fNormalStandardDev[nonmast][j] =
								p_fTempValues[i].val;
						break;
					}
				}
...
void clMastingNonSpatialDisperse::AddSeeds() {
	...
	float
			...
			fMidX = mp_oSeedGrid->GetLengthXCells(), fMidY =
					mp_oSeedGrid->GetLengthYCells(), fMidArea = fMidX * fMidY, //shortcut
	...
	try {
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			...
			if (false == p_bDone[i]) {
				...
				iDraw = gsl_ran_binomial(clModelMath::randgen, mp_fBinomialP[i],
						1);
				...
				for (j = 0; j < m_iNumBehaviorSpecies; j++) {
					if (mp_iGroup[j] == mp_iGroup[i]) {
						p_fNumNewSeeds[j] = fNumSeeds
								* (mp_fBasalArea[j] / fTotalBasalArea);
						p_bDone[j] = true;
						if (iDraw == mast)
							mp_iEvent[j] = mast;
						else
			...
			for (iY = 0; iY < iNumYCells; iY++) {
				...
				for (i = 0; i < m_iNumBehaviorSpecies; i++) {
					...
					mp_oSeedGrid->GetValueOfCell(iX, iY,
							mp_iNumSeedsCode[mp_iWhatSpecies[i]],
							&fNumGridSeeds);
	...
	try {
		...
		while (p_oTree) {
			...
			if (p_iIndexes[iSp] > -1) {
				...
				p_oTree->GetValue(p_oPop->GetDbhCode(iSp, p_oTree->GetType()),
						&fDbh);
