	...
	try {
		...
		if (!mp_oGapGrid) {
			...
			if (-1 == m_iGapCode) {
				...
				stcErr.sMoreInfo =
						"Gap Light grid was incorrectly set up in the parameter file.  Missing bool \"Is Gap\".";
				throw(stcErr);
			}
		}
	} catch (modelErr &err) {
...
void clGapLight::CalculateGapStatus(clTreePopulation *p_oPop) {
	try {
		...
		short int iSp, iType, //species and type of a given tree
				iNumXCells = mp_oGapGrid->GetNumberXCells(), iNumYCells =
						mp_oGapGrid->GetNumberYCells(), iDeadCode, //code for whether or not a adult's dead
				i, j; //loop counters
		bool bNotGap = true;
