			...
			if (-1 != mp_iGrowthCodes[iSpecies][iType]) {
				...
				if (notdead == iIsDead) {
					...
					for (i = 0; i < fNumberYearsPerTimestep; i++) {
						...
						fCrowdingEffect =
								mp_oCrowdingEffect->CalculateCrowdingEffect(
										p_oTree, fDiam, nci, iSpecies);

						//Get the tree's size effect
						fSizeEffect = mp_oSizeEffect->CalculateSizeEffect(
								p_oTree, fDiam);

						//Calculate actual growth in cm/yr
						fTempDiamIncrease = mp_fMaxPotentialValue[iSpecies]
