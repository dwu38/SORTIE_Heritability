...
void clStormDamageApplier::GetParameterFileData(xercesc::DOMDocument *p_oDoc) {
	...
	FillSpeciesSpecificValue(p_oElement, "st_minDBH", "st_mdVal", p_fTempValues,
	...
	FillSpeciesSpecificValue(p_oElement, "st_stmDmgInterceptMed", "st_sdimVal",
	...
	FillSpeciesSpecificValue(p_oElement, "st_stmDmgInterceptFull", "st_sdifVal",
			p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
	//Transfer to the appropriate array buckets
	for (i = 0; i < m_iNumBehaviorSpecies; i++)
	...
	FillSpeciesSpecificValue(p_oElement, "st_stmIntensityCoeff", "st_sicVal",
			p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
	//Transfer to the appropriate array buckets
	for (i = 0; i < m_iNumBehaviorSpecies; i++)
	...
	FillSpeciesSpecificValue(p_oElement, "st_stmDBHCoeff", "st_sdcVal",
			p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
	//Transfer to the appropriate array buckets
	for (i = 0; i < m_iNumBehaviorSpecies; i++)
	...
	if (m_iNumYearsToHeal <= 0) {
		...
		stcErr.sMoreInfo =
				"The number of years to heal parameter must be greater than 0.";
...
void clStormDamageApplier::GetData(xercesc::DOMDocument *p_oDoc) {
	try {
		...
		if ( NULL == mp_oStormDamageGrid) {
			...
			stcErr.sMoreInfo =
					"The storm damage applier behavior must be used with the storm behavior.";
			throw(stcErr);
		}

		m_iDmgIndexCode = mp_oStormDamageGrid->GetPackageFloatDataCode(
				"1dmg_index");
		if (-1 == m_iDmgIndexCode) {
			...
			stcErr.sFunction = "clStormDamageApplier::GetData";
			stcErr.sMoreInfo =
					"The grid \"Storm Damage\" is set up in an unexpected way.";
			throw(stcErr);
		}
	} catch (modelErr &err) {
		throw(err);
...
void clStormDamageApplier::Action() {
	try {
		...
		int iTreeDamage, iYearsToHeal, iThisDamage, iNumYearsPerTimestep =
			...
			if ( NULL != p_oPkg) {
				...
				while (p_oPkg && iThisDamage < 2) {
					...
					if (fStormSeverity > 0) {
						...
						fMediumDmgProb =
								exp(
										mp_fStmDmgInterceptMed[iSp]
												+ (mp_fStmIntensityCoeff[iSp]
														* fStormSeverity
														* pow(fDbh,
																mp_fStmDBHCoeff[iSp])))
										/ (1
