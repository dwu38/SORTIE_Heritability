...
void clCarbonValueCalculator::GetParameterFileData(xercesc::DOMDocument *p_oDoc,
	...
	try {
		...
		FillSpeciesSpecificValue(p_oElement, "an_carbonPercentBiomassCarbon",
				"an_cpbcVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets, and
		//convert to proportion; check to make sure it's between 0 and 1
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			mp_fCPercentBiomass[p_fTempValues[i].code] = p_fTempValues[i].val
					/ 100.0;
			if (mp_fCPercentBiomass[p_fTempValues[i].code] > 1
					|| mp_fCPercentBiomass[p_fTempValues[i].code] < 0) {
				modelErr stcErr;
				stcErr.sFunction =
						"clCarbonValueCalculator::GetParameterFileData";
				stcErr.sMoreInfo =
						"Percent of biomass that is carbon must be between 0 and 100.";
				stcErr.iErrorCode = BAD_DATA;
				throw(stcErr);
			}
		}

		FillSingleValue(p_oElement, "an_carbonPricePerMetricTonCarbon",
				&m_fPricePerTonCarbon, true);
...
void clCarbonValueCalculator::SetupGrid() {
	...
	mp_oValueGrid = mp_oSimManager->CreateGrid("Carbon Value", 0, //number of ints
...
void clCarbonValueCalculator::Action() {
	try {
		...
		//grid
		for (iSp = 0; iSp < m_iNumTotalSpecies; iSp++) {
			mp_oValueGrid->SetValueOfCell(0, 0, mp_iCarbonCodes[iSp],
					p_fSpeciesTotals[iSp]);
			p_fSpeciesTotals[iSp] *= m_fPricePerTonCarbon;
			mp_oValueGrid->SetValueOfCell(0, 0, mp_iValueCodes[iSp],
					p_fSpeciesTotals[iSp]);
		}
...
void clCarbonValueCalculator::GetBiomassCodes(clTreePopulation *p_oPop) {
	...
	for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
		mp_iBiomassCodes[mp_whatSpeciesTypeCombos[i].iSpecies][mp_whatSpeciesTypeCombos[i].iType
				- clTreePopulation::sapling] = p_oPop->GetFloatDataCode(
				"Biomass", mp_whatSpeciesTypeCombos[i].iSpecies,
				mp_whatSpeciesTypeCombos[i].iType);
		...
		if (-1
			...
			stcErr.sMoreInfo =
					"The carbon value behavior must be used with the dimension analysis behavior for all species.";
			stcErr.iErrorCode = BAD_DATA;
			throw(stcErr);
		}
	}
...
void clCarbonValueCalculator::RegisterTreeDataMembers() {
	...
	for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
		...
		if (clTreePopulation::adult != mp_whatSpeciesTypeCombos[i].iType
			...
			modelErr stcErr;
			stcErr.sFunction =
					"clCarbonValueCalculator::RegisterTreeDataMembers";
			stcErr.sMoreInfo =
		...
		mp_iTreeCarbonCodes[mp_whatSpeciesTypeCombos[i].iSpecies][mp_whatSpeciesTypeCombos[i].iType
				- clTreePopulation::sapling] = p_oPop->RegisterFloat("Carbon",
