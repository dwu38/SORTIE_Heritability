...
void clSailLight::DoShellSetup(xercesc::DOMDocument *p_oDoc) {
	try {
		...
		//Max shading radius
		FillSingleValue(p_oElement, "li_maxShadingRadius", &m_fMaxShadingRadius,
				true);
		...
		if (iVal == clLightOrg::mid)
			m_iPhotoDepth = clLightOrg::mid;
		else
			m_iPhotoDepth = clLightOrg::top;
...
float clSailLight::CalcLightValue(clTree *p_oTree, clTreePopulation *p_oPop) {
	...
	sprintf(cQuery, "%s%f%s%f%s%f%s%f", "distance=", m_fMaxShadingRadius,
			" FROM x=", fTargetX, "y=", fTargetY, "::height=", fTreeHeight);
	...
	while (p_oNeighbor != NULL) {
		...
		if (clTreePopulation::seedling != p_oNeighbor->GetType()
			...
			AddTreeToSailFishEye(fTargetX, fTargetY, fTreeHeight, p_oNeighbor,
