...
////////////////////////////////////////////////////////////////////////////
void clHeightGLIWeibullMortality::ReadParameterFileData(
	...
	try {
		...
		//"a"
		FillSpeciesSpecificValue(p_oElement, "mo_heightGLIWeibA", "mo_hgwaVal",
		...
		//"b"
		FillSpeciesSpecificValue(p_oElement, "mo_heightGLIWeibB", "mo_hgwbVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		...
		//"c"
		FillSpeciesSpecificValue(p_oElement, "mo_heightGLIWeibC", "mo_hgwcVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer values to our permanent array
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		//"d"
		FillSpeciesSpecificValue(p_oElement, "mo_heightGLIWeibD", "mo_hgwdVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer values to our permanent array
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		FillSpeciesSpecificValue(p_oElement, "mo_heightGLIWeibMaxMort",
				"mo_hgwmmVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			if (p_fTempValues[i].val < 0 || p_fTempValues[i].val > 1) {
				...
				stcErr.sFunction =
						"clHeightGLIWeibullMortality::ReadParameterFileData";
				stcErr.sMoreInfo =
						"All values in max mortality must be between 0 and 1.";
				throw(stcErr);
			}
			mp_fMaxMort[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;
		}
		...
		if (bBrowsing) {
			...
			FillSpeciesSpecificValue(p_oElement, "mo_heightGLIWeibBrowsedA",
					"mo_hgwbaVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
					true);
			//Transfer values to our permanent array - flip the sign
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
				mp_fBrowsedA[mp_iIndexes[p_fTempValues[i].code]] =
						-p_fTempValues[i].val;

			//"b"
			FillSpeciesSpecificValue(p_oElement, "mo_heightGLIWeibBrowsedB",
					"mo_hgwbbVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
					true);
			//Transfer values to our permanent array
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
				mp_fBrowsedB[mp_iIndexes[p_fTempValues[i].code]] =
						p_fTempValues[i].val;

			//"c"
			FillSpeciesSpecificValue(p_oElement, "mo_heightGLIWeibBrowsedC",
					"mo_hgwbcVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
					true);
			//Transfer values to our permanent array
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
				mp_fBrowsedC[mp_iIndexes[p_fTempValues[i].code]] =
						p_fTempValues[i].val;

			//"d"
			FillSpeciesSpecificValue(p_oElement, "mo_heightGLIWeibBrowsedD",
					"mo_hgwbdVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
					true);
			//Transfer values to our permanent array
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
				mp_fBrowsedD[mp_iIndexes[p_fTempValues[i].code]] =
						p_fTempValues[i].val;

			//Max mortality
			FillSpeciesSpecificValue(p_oElement,
					"mo_heightGLIWeibBrowsedMaxMort", "mo_hgwbmmVal",
					p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
			//Transfer values to our permanent array, making sure all values are
			//between 0 and 1
			for (i = 0; i < m_iNumBehaviorSpecies; i++) {
				...
				mp_fBrowsedMaxMort[mp_iIndexes[p_fTempValues[i].code]] =
						p_fTempValues[i].val;
			}
		}
...
// DoMort()
///////////////////////////////////////////////////////////////////////////
deadCode clHeightGLIWeibullMortality::DoMort(clTree *p_oTree,
		const float &fDiam, const short int &iSp) {
	...
	if (bBrowsed) {
		fMortProb =
				mp_fBrowsedMaxMort[mp_iIndexes[iSp]]
						* exp(
								mp_fBrowsedA[mp_iIndexes[iSp]]
										* pow(fHeight,
												mp_fBrowsedB[mp_iIndexes[iSp]])
										- mp_fBrowsedC[mp_iIndexes[iSp]]
												* pow(fGLI,
														mp_fBrowsedD[mp_iIndexes[iSp]]));
	} else {
		fMortProb = mp_fMaxMort[mp_iIndexes[iSp]]
				* exp(
						mp_fA[mp_iIndexes[iSp]]
								* pow(fHeight, mp_fB[mp_iIndexes[iSp]])
								- mp_fC[mp_iIndexes[iSp]]
										* pow(fGLI, mp_fD[mp_iIndexes[iSp]]));
	}
...
void clHeightGLIWeibullMortality::GetTreeDataMemberCodes() {
	try {
		clTreePopulation *p_oPop =
				(clTreePopulation*) mp_oSimManager->GetPopulationObject(
						"treepopulation");
		char cLightLabel[] = "Light", cBrowsedLabel[] = "Browsed";
		...
		for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
			...
			mp_iLightCodes[mp_iIndexes[iSp]][iTp] = p_oPop->GetFloatDataCode(
					cLightLabel, iSp, iTp);
			if (-1 == mp_iLightCodes[mp_iIndexes[iSp]][iTp]) {
				modelErr stcErr;
				stcErr.iErrorCode = BAD_DATA;
				stcErr.sFunction =
						"clHeightGLIWeibullMortality::GetTreeDataMemberCodes";
				stcErr.sMoreInfo =
						"All trees that use Height GLI Mortality must also use a light behavior.";
				throw(stcErr);
			}
			mp_iHeightCodes[mp_iIndexes[iSp]][iTp] = p_oPop->GetHeightCode(iSp,
					iTp);
			mp_iBrowsedCodes[mp_iIndexes[iSp]][iTp] = p_oPop->GetBoolDataCode(
					cBrowsedLabel, iSp, iTp);
		}
	} catch (modelErr &err) {
		throw(err);
	} catch (modelMsg &msg) {
