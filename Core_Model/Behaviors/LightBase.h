...
protected:
	...
	inline float GetDeclination(float &fDayAngle) {
		return 0.006918 - (0.399912 * cos(fDayAngle))
				...
				+ (0.000907 * sin(2 * fDayAngle))
				- (0.002697 * cos(3 * fDayAngle))
	...
	inline float GetEccentricity(float &fDayAngle) {
		return 1.000110 + (0.034221 * cos(fDayAngle))
				+ (0.001280 * sin(fDayAngle)) - (0.000719 * cos(2 * fDayAngle))
				+ (0.000077 * sin(2 * fDayAngle));
	}
	;
	...
	inline float GetSunrise(float &fLatInRadians, float &fDeclination) {
		...
		float x = (-1.0 * sin(fLatInRadians) * sin(fDeclination))
				/ (cos(fLatInRadians) * cos(fDeclination));
	 ...
	 */
	inline float GetCosineOfZenithAngle(float &fDeclination,
			float &fLatInRadians, float &fTimeNow) {
	...
	inline float GetAltitudeAngle(float &fCosZenAng) {
		...
		if (fCosZenAng <= -1.0)
			...
			return ((M_PI / 2.0) - acos(fCosZenAng));
	}
	;
	...
	inline float GetAzimuthAngle(float &fDeclination, float &fLatInRadians,
		...
		float fCosAzi = (((sin(fAltInRad) * sin(fLatInRadians))
				- sin(fDeclination)) / (cos(fLatInRadians) * cos(fAltInRad)));
	...
	inline float GetAirmassEffect(float &fAltInDeg, float &fCosZenAng) {
		if (fAltInDeg <= 5)
			return 10.39;
		else if ((fAltInDeg > 5) && (fAltInDeg <= 15))
			return 5.60;
		else if ((fAltInDeg > 15) && (fAltInDeg <= 25))
			return 2.90;
		else
			return (1 / fCosZenAng);
	}
