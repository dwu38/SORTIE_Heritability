...
void clClimateImporter::GetData(DOMDocument *p_oDoc) {
	...
	double **p_fPpt = NULL, **p_fTemp = NULL, *p_fRad = new double[12],
			*p_fPET = new double[12], *p_fAET = new double[12], *p_fSMS =
					new double[12];
	...
	try {
		...
		if (fAWS < 0) {
			...
			stcErr.sMoreInfo =
					"Available water storage value cannot be negative.";
			throw(stcErr);
		}
		...
		for (i = 0; i < iNumTimesteps; i++) {
			...
			for (j = 0; j < 12; j++) {
				if (p_fTemp[j][i] >= 0)
					p_fPET[j] = 0.013 * (p_fTemp[j][i] / (p_fTemp[j][i] + 15))
							* (p_fRad[j] + 50);
				else
					p_fPET[j] = 0;
			}
			...
			for (j = 1; j < 12; j++) {
				if (p_fSMS[j] > 0)
					...
					p_fAET[j] = p_fSMS[j - 1] + p_fPpt[j][i];
			}
...
void clClimateImporter::ReadParameterFileData(DOMElement *p_oElement,
	...
	FillSingleValue(p_oElement, "sc_ciJanRad", &fVal, true);
	p_fRad[0] = fVal;
	FillSingleValue(p_oElement, "sc_ciFebRad", &fVal, true);
	p_fRad[1] = fVal;
	FillSingleValue(p_oElement, "sc_ciMarRad", &fVal, true);
	p_fRad[2] = fVal;
	FillSingleValue(p_oElement, "sc_ciAprRad", &fVal, true);
	p_fRad[3] = fVal;
	FillSingleValue(p_oElement, "sc_ciMayRad", &fVal, true);
	p_fRad[4] = fVal;
	FillSingleValue(p_oElement, "sc_ciJunRad", &fVal, true);
	p_fRad[5] = fVal;
	FillSingleValue(p_oElement, "sc_ciJulRad", &fVal, true);
	p_fRad[6] = fVal;
	FillSingleValue(p_oElement, "sc_ciAugRad", &fVal, true);
	p_fRad[7] = fVal;
	FillSingleValue(p_oElement, "sc_ciSepRad", &fVal, true);
	p_fRad[8] = fVal;
	FillSingleValue(p_oElement, "sc_ciOctRad", &fVal, true);
	p_fRad[9] = fVal;
	FillSingleValue(p_oElement, "sc_ciNovRad", &fVal, true);
	p_fRad[10] = fVal;
	FillSingleValue(p_oElement, "sc_ciDecRad", &fVal, true);
	p_fRad[11] = fVal;

	//Precipitation for each month and timestep
	ReadMonthlyData(p_oElement, "sc_ciMonthlyPptJan", "sc_cimpjanVal",
			p_fPpt[0]);
	ReadMonthlyData(p_oElement, "sc_ciMonthlyPptFeb", "sc_cimpfebVal",
			p_fPpt[1]);
	ReadMonthlyData(p_oElement, "sc_ciMonthlyPptMar", "sc_cimpmarVal",
			p_fPpt[2]);
	ReadMonthlyData(p_oElement, "sc_ciMonthlyPptApr", "sc_cimpaprVal",
			p_fPpt[3]);
	ReadMonthlyData(p_oElement, "sc_ciMonthlyPptMay", "sc_cimpmayVal",
			p_fPpt[4]);
	ReadMonthlyData(p_oElement, "sc_ciMonthlyPptJun", "sc_cimpjunVal",
			p_fPpt[5]);
	ReadMonthlyData(p_oElement, "sc_ciMonthlyPptJul", "sc_cimpjulVal",
			p_fPpt[6]);
	ReadMonthlyData(p_oElement, "sc_ciMonthlyPptAug", "sc_cimpaugVal",
			p_fPpt[7]);
	ReadMonthlyData(p_oElement, "sc_ciMonthlyPptSep", "sc_cimpsepVal",
			p_fPpt[8]);
	ReadMonthlyData(p_oElement, "sc_ciMonthlyPptOct", "sc_cimpoctVal",
			p_fPpt[9]);
	ReadMonthlyData(p_oElement, "sc_ciMonthlyPptNov", "sc_cimpnovVal",
			p_fPpt[10]);
	ReadMonthlyData(p_oElement, "sc_ciMonthlyPptDec", "sc_cimpdecVal",
			p_fPpt[11]);

	//Temperature for each month and timestep
	ReadMonthlyData(p_oElement, "sc_ciMonthlyTempJan", "sc_cimtjanVal",
			p_fTemp[0]);
	ReadMonthlyData(p_oElement, "sc_ciMonthlyTempFeb", "sc_cimtfebVal",
			p_fTemp[1]);
	ReadMonthlyData(p_oElement, "sc_ciMonthlyTempMar", "sc_cimtmarVal",
			p_fTemp[2]);
	ReadMonthlyData(p_oElement, "sc_ciMonthlyTempApr", "sc_cimtaprVal",
			p_fTemp[3]);
	ReadMonthlyData(p_oElement, "sc_ciMonthlyTempMay", "sc_cimtmayVal",
			p_fTemp[4]);
	ReadMonthlyData(p_oElement, "sc_ciMonthlyTempJun", "sc_cimtjunVal",
			p_fTemp[5]);
	ReadMonthlyData(p_oElement, "sc_ciMonthlyTempJul", "sc_cimtjulVal",
			p_fTemp[6]);
	ReadMonthlyData(p_oElement, "sc_ciMonthlyTempAug", "sc_cimtaugVal",
			p_fTemp[7]);
	ReadMonthlyData(p_oElement, "sc_ciMonthlyTempSep", "sc_cimtsepVal",
			p_fTemp[8]);
	ReadMonthlyData(p_oElement, "sc_ciMonthlyTempOct", "sc_cimtoctVal",
			p_fTemp[9]);
	ReadMonthlyData(p_oElement, "sc_ciMonthlyTempNov", "sc_cimtnovVal",
			p_fTemp[10]);
	ReadMonthlyData(p_oElement, "sc_ciMonthlyTempDec", "sc_cimtdecVal",
			p_fTemp[11]);
