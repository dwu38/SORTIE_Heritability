...
void clInsectInfestation::ReadParFile(xercesc::DOMDocument *p_oDoc,
	...
	try {
		...
		FillSpeciesSpecificValue(p_oElement, "di_insectMaxInfestation",
				"di_imiVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets and make sure all values are
		//between 0 and 1
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			...
			if (mp_fMax[p_fTempValues[i].code] < 0
				...
				stcErr.sFunction = "clInsectInfestation::ReadParFile";
				stcErr.sMoreInfo =
						"Max infestation value must be between 0 and 1.";
				stcErr.iErrorCode = BAD_DATA;
				throw(stcErr);
			}
		}
		...
		FillSingleValue(p_oElement, "di_insectStartTimestep", &m_iFirstTimestep,
				true);
		if (m_iFirstTimestep < 0) {
			modelErr stcErr;
			stcErr.sFunction = "clInsectInfestation::ReadParFile";
			stcErr.sMoreInfo =
					"Timestep to start infestation cannot be negative.";
			stcErr.iErrorCode = BAD_DATA;
			throw(stcErr);
		}

		//Timestep to end infestation - for backwards compatibility, not required
		FillSingleValue(p_oElement, "di_insectEndTimestep", &m_iLastTimestep,
				false);
...
void clInsectInfestation::Action() {
	...
	try {
		...
		float fDbh;
		long iTot = 0;
		int i, iSp, iTp, iInf, iNumYrsTimestep =
