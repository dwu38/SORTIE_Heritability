...
void clWeibullSnagMort::ValidateTypes() {
	...
	for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
		if (clTreePopulation::snag != mp_whatSpeciesTypeCombos[i].iType) {
			...
			stcErr.sMoreInfo =
					"Snag weibull mortality can only be applied to snags.";
...
void clWeibullSnagMort::ReadParameterFile(xercesc::DOMDocument *p_oDoc) {
	try {
		...
		FillSpeciesSpecificValue(p_oElement, "mo_snag1WeibullA", "mo_s1waVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer values to our permanent array 0
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fAParameter[0][mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;

		//Weibull parameter "a" for snag age class 2
		FillSpeciesSpecificValue(p_oElement, "mo_snag2WeibullA", "mo_s2waVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer values to our permanent array
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fAParameter[1][mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;

		//Weibull parameter "a" for snag age class 3
		FillSpeciesSpecificValue(p_oElement, "mo_snag3WeibullA", "mo_s3waVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer values to our permanent array
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fAParameter[2][mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;

		//Weibull parameter "b" for snag age class 1
		FillSpeciesSpecificValue(p_oElement, "mo_snag1WeibullB", "mo_s1wbVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer values to our permanent array
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fBParameter[0][mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;

		//Weibull parameter "b" for snag age class 2
		FillSpeciesSpecificValue(p_oElement, "mo_snag2WeibullB", "mo_s2wbVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer values to our permanent array
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fBParameter[1][mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;

		//Weibull parameter "b" for snag age class 3
		FillSpeciesSpecificValue(p_oElement, "mo_snag3WeibullB", "mo_s3wbVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer values to our permanent array
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fBParameter[2][mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;
		...
		FillSpeciesSpecificValue(p_oElement, "mo_snagSizeClass1DBH",
				"mo_sc1dVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				false);
		...
		if (bFound) {
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
				mp_fSnagSizeClasses[0][mp_iIndexes[p_fTempValues[i].code]] =
						p_fTempValues[i].val;
		} else {
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			p_fTempValues[i].val = -1;
		FillSpeciesSpecificValue(p_oElement, "mo_snagSizeClass2DBH",
				"mo_sc2dVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				false);
		//Was the value present as a species-specific?
		bFound = false;
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
		...
		if (bFound) {
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
				mp_fSnagSizeClasses[1][mp_iIndexes[p_fTempValues[i].code]] =
						p_fTempValues[i].val;
		} else {
			//The value wasn't present as a species-specific - so find it as a
			//single value and transfer the single value to all species
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			if (mp_fSnagSizeClasses[0][i] < 0 || mp_fSnagSizeClasses[1][i] < 0
					|| mp_fSnagSizeClasses[1][i] < mp_fSnagSizeClasses[0][i]) {
				modelErr stcErr;
				stcErr.iErrorCode = BAD_DATA;
				stcErr.sFunction = "clWeibullSnagMort::ReadParameterFile";
				stcErr.sMoreInfo =
						"Snag size classes must be positive and cannot overlap.";
		...
		for (i = 0; i < m_iNumSizeClasses; i++) {
			for (j = 0; j < m_iNumBehaviorSpecies; j++) {
				if (0 > mp_fAParameter[i][j]
						&& (0 >= mp_fBParameter[i][j]
								|| mp_fBParameter[i][j]
										- floor(mp_fBParameter[i][j]) != 0)) {
					modelErr stcErr;
					stcErr.iErrorCode = BAD_DATA;
					stcErr.sFunction = "clWeibullSnagMort::ReadParameterFile";
					stcErr.sMoreInfo =
							"All values for \"a\" must be a positive number.";
					delete[] p_fTempValues;
					throw(stcErr);
				}
			}
...
void clWeibullSnagMort::GetAgeCodes() {
	try {
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			mp_iAgeCodes[i] = p_oPop->GetIntDataCode(cLabel, mp_iWhatSpecies[i],
		...
		for (i = 0; i < m_iNumSizeClasses; i++) {
			...
			for (j = 0; j < m_iNumBehaviorSpecies; j++) {
				...
				for (k = 0; k < m_iMaxPrecalcAge; k++) {
					mp_fProbabilityOfDeath[i][j][k] = CalculateDeathProbability(
							i, mp_iWhatSpecies[j],
							(int) m_fNumYearsPerTimestep * k);
				}
			}
		}
	} catch (modelErr &err) {
...
////////////////////////////////////////////////////////////////////////////
float clWeibullSnagMort::CalculateDeathProbability(int iSizeClass, int iSpecies,
		int iAge) {

	float fMinValueAllowed = 1.0E-5, fMaxValueAllowed = 1 - fMinValueAllowed,
			fMinExp = -11.51, fFunc1, fFunc2, fProb;
...
// DoMort()
////////////////////////////////////////////////////////////////////////////
deadCode clWeibullSnagMort::DoMort(clTree *p_oTree, const float &fDbh,
	...
	if (iAge < m_iMaxPrecalcAge) {
		...
		fProb =
				mp_fProbabilityOfDeath[iSizeClass][mp_iIndexes[iSpecies]][iAgeInTS];
