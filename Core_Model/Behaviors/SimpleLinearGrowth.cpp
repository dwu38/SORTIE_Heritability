	...
	try {
		...
		if (height_only == m_iGrowthMethod) {
			...
			FillSpeciesSpecificValue(p_oElement,
					"gr_heightSimpleLinearIntercept", "gr_hsliVal",
			...
			//Growth slope
			FillSpeciesSpecificValue(p_oElement, "gr_heightSimpleLinearSlope",
					"gr_hslsVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
					true);
			...
			//Growth intercept
			FillSpeciesSpecificValue(p_oElement, "gr_diamSimpleLinearIntercept",
					"gr_dsliVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
					true);
			...
			//Growth slope
			FillSpeciesSpecificValue(p_oElement, "gr_diamSimpleLinearSlope",
					"gr_dslsVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
					true);
...
inline float clSimpleLinearGrowth::CalculateFunctionValue(clTree *p_oTree) {
	...
	float fGli;
	short int iSpecies = p_oTree->GetSpecies(), iLightCode =
			mp_oGrowthOrg->GetLightCode(iSpecies, p_oTree->GetType());

	//Get the gli value for this tree
	p_oTree->GetValue(iLightCode, &fGli);

	//Calculate the function value
	return clModelMath::CalcPointValue(fGli, mp_fSlope[iSpecies],
			mp_fIntercept[iSpecies]);
}
