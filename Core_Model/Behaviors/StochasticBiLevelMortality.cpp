...
void clStochasticBiLevelMortality::DoShellSetup(xercesc::DOMDocument *p_oDoc) {
	...
	try {
		...
		short int iNumSpecies = mp_oPop->GetNumberOfSpecies(), iNumTypes =
				mp_oPop->GetNumberOfTypes(), iSp, iTp, i, j;
		...
		if (m_bIsGLI) {
			...
			for (i = 0; i < iNumSpecies; i++) {
				...
				for (j = 0; j < iNumTypes; j++)
					mp_iGLILightCodes[i][j] = -1;
			}
			for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
				iSp = mp_whatSpeciesTypeCombos[i].iSpecies;
				iTp = mp_whatSpeciesTypeCombos[i].iType;
				mp_iGLILightCodes[iSp][iTp] = mp_oPop->GetFloatDataCode("Light",
						iSp, iTp);
				if (-1 == mp_iGLILightCodes[iSp][iTp]) {
					modelErr stcErr;
					stcErr.iErrorCode = CANT_FIND_OBJECT;
					stcErr.sFunction =
							"clStochasticBiLevelMortality::DoShellSetup";
					stcErr.sMoreInfo =
							"A light behavior must be assigned to all trees for Stochastic Bi-Level Mortality.";
					throw(stcErr);
				}
			}
		...
		FillSpeciesSpecificValue(p_oElement, "mo_stochBilevLoLiteMortProb",
				"mo_sbllmpVal", p_fTempValues, m_iNumBehaviorSpecies, mp_oPop,
				true);
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			if (0 > p_fTempValues[i].val || 1 < p_fTempValues[i].val) {
				...
				stcErr.sFunction = "clStochasticBiLevelMortality::DoShellSetup";
				stcErr.sMoreInfo =
						"All values for probability of mortality must be between 0 and 1.";
				throw(stcErr);
			}
			mp_fLoLightMortProb[mp_iIndexes[p_fTempValues[i].code]] = 1
					- pow(1 - p_fTempValues[i].val, fYearsPerTimestep);
		}

		//Translate the values

		//High-light mortality probability
		FillSpeciesSpecificValue(p_oElement, "mo_stochBilevHiLiteMortProb",
				"mo_sbhlmpVal", p_fTempValues, m_iNumBehaviorSpecies, mp_oPop,
				true);
		//Transfer values to our permanent array, making sure all values are
		//between 0 and 1 and translating to a timestep probability
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			...
			mp_fHiLightMortProb[mp_iIndexes[p_fTempValues[i].code]] = 1
					- pow(1 - p_fTempValues[i].val, fYearsPerTimestep);
		}

		//High-light threshold
		FillSpeciesSpecificValue(p_oElement, "mo_stochBilevHiLiteThreshold",
				"mo_sbhltVal", p_fTempValues, m_iNumBehaviorSpecies, mp_oPop,
				true);
		//Transfer values to our permanent array, making sure all values are
		//between 0 and 100
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
...
deadCode clStochasticBiLevelMortality::DoMort(clTree *p_oTree,
		const float &fDiam, const short int &iSpecies) {
