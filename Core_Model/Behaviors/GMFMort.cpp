...
void clGMFMort::DoShellSetup(xercesc::DOMDocument *p_oDoc) {
	try {
		...
		if (5 != mp_oSimManager->GetNumberOfYearsPerTimestep()) {
			...
			stcErr.sMoreInfo =
					"GMF Mortality cannot be used if the timestep length is not 5 years.";
		...
		FillSpeciesSpecificValue(p_oElement, "mo_mortAtZeroGrowth",
				"mo_mazgVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		...
		FillSpeciesSpecificValue(p_oElement, "mo_lightDependentMortality",
				"mo_ldmVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		for (i = 0; i < m_iNumTotalSpecies; i++)
			for (j = 0; j < iTotalTypes; j++)
				if (mp_bUsesThisMortality[i][j]) {
					mp_iGrowthCodes[i][j] = p_oPop->GetFloatDataCode(cLabel, i,
							j);
...
// DoMort()
////////////////////////////////////////////////////////////////////////////
deadCode clGMFMort::DoMort(clTree *p_oTree, const float &fDbh,
