...
clAbsoluteGrowth::clAbsoluteGrowth(clSimManager *p_oSimManager) :
		clWorkerBase(p_oSimManager), clBehaviorBase(p_oSimManager), clGrowthBase(
				p_oSimManager), clMichMenBase(p_oSimManager) {
...
void clAbsoluteGrowth::DoShellSetup(xercesc::DOMDocument *p_oDoc) {
	try {
		...
		FillSpeciesSpecificValue(p_oElement, "gr_lengthLastSuppFactor",
				"gr_llsfVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);

		//Transfer values to our permanent array
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fLengthLastSuppFactor[p_fTempValues[i].code] =
					p_fTempValues[i].val;

		//Length of current release factor
		FillSpeciesSpecificValue(p_oElement, "gr_lengthCurrReleaseFactor",
				"gr_lcrfVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);

		//Transfer values to our permanent array
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fLengthCurrReleaseFactor[p_fTempValues[i].code] =
					p_fTempValues[i].val;
		...
		//later
		if (1 == m_fMortRateAtSuppression)
			m_fMortRateAtSuppression -= 0.01;
...
//////////////////////////////////////////////////////////////////////////////
void clAbsoluteGrowth::CalculateSuppressionThresholds(
	...
	try {
		...
		FillSpeciesSpecificValue(p_oElement, "mo_mortAtZeroGrowth",
				"mo_mazgVal", p_fMortAtZeroGrowth, m_iNumBehaviorSpecies,
				p_oPop, true);

		FillSpeciesSpecificValue(p_oElement, "mo_lightDependentMortality",
				"mo_ldmVal", p_fLightDepMort, m_iNumBehaviorSpecies, p_oPop,
				true);
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			...
			if (0 < m_iYrsExceedThresholdBeforeSup * p_fMortAtZeroGrowth[i].val
				...
				fGrowthAtMortThreshold = log(
						m_iYrsExceedThresholdBeforeSup
								* p_fMortAtZeroGrowth[i].val
								/ log(1 / (1 - m_fMortRateAtSuppression)))
						/ p_fLightDepMort[i].val + 1;
			} else {
				modelErr stcErr;
				stcErr.iErrorCode = BAD_DATA;
				stcErr.sMoreInfo = "Cannot calculate suppression thresholds.";
				stcErr.sFunction =
						"clAbsoluteGrowth::CalculateSuppressionThresholds";
				throw(stcErr);
			}
...
void clAbsoluteGrowth::RegisterTreeDataMembers() {
	try {
		...
		short int *p_iTypesList,  //array holding unique types
				...
				iNumTotalSpecies = p_oPop->GetNumberOfSpecies(), i, j; //loop counters
...
float clAbsoluteGrowth::CalcDiameterGrowthValue(clTree *p_oTree,
	...
	fAmountDiamIncrease = ApplyGrowthLimits(iSpecies, fAmountDiamIncrease,
			fDiam);
...
float clAbsoluteGrowth::CalculateSuppressionFactor(clTree *p_oTree,
	...
	short int iIndex = 0,       //array index for species/type combo
			iSpecies = p_oTree->GetSpecies(), iType = p_oTree->GetType(), i; //loop counter
	...
	fFactor = exp(
			(mp_fLengthCurrReleaseFactor[iSpecies] * iYlr)
