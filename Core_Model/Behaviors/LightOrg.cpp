...
void clLightOrg::DoSetup(clSimManager *p_oSimManager,
	...
	try {
		...
		float fNumYearsPerTimestep =
				p_oSimManager->GetNumberOfYearsPerTimestep();
		if (fabs(
				clModelMath::Round(fNumYearsPerTimestep, 0)
						- fNumYearsPerTimestep) > 0) {
			modelErr stcErr;
			stcErr.sFunction = "clLightOrg::DoSetup";
			stcErr.sMoreInfo =
					"Light cannot be used if there is not an integer number of years per timestep.";
			stcErr.iErrorCode = BAD_DATA;
			throw(stcErr);
		}

		//Get the tree population object, and from it the numbers of species and types
		mp_oPop = (clTreePopulation*) p_oSimManager->GetPopulationObject(
				"treepopulation");
		...
		for (i = 0; i < iNumBehaviors; i++) {
			...
			if (sBehaviorName.find(sShellMarker) != std::string::npos) {
				...
				for (j = 0; j < iNumCombos; j++) {
					...
					if (mp_oLightFunctionTable[combo.iSpecies][combo.iType]
							!= NULL) {
						modelErr stcErr;
						stcErr.sFunction = "clLightOrg::DoSetup";
						std::stringstream s;
						...
						mp_oLightFunctionTable[combo.iSpecies][combo.iType] =
								p_oLightShell;
				}
...
void clLightOrg::GetParameterFileData(clSimManager *p_oSimManager,
	...
	try {
		...
		for (i = 0; i < iNumBehaviors; i++) {
			...
			p_oTemp =
					dynamic_cast<clLightBase*>(p_oSimManager->GetBehaviorObject(
							i));
		...
		FillSingleValue(p_oElement, "li_beamFractGlobalRad",
				&m_fBeamFracGlobRad, true);
		//Light extinction coefficient
		FillSpeciesSpecificValue(p_oElement, "li_lightExtinctionCoefficient",
				"li_lecVal", mp_fLightExtCoef, mp_oPop, true);
		//Clear sky transmission coefficient
		FillSingleValue(p_oElement, "li_clearSkyTransCoeff",
				&m_fClearSkyTransCoeff, true);
		//Make sure not zero
		if (fabs(m_fClearSkyTransCoeff - 0) < 0.001) {
			...
			stcErr.sMoreInfo =
					"The value for clear sky transmission coefficient cannot be 0.";
			throw(stcErr);
		}

		//Start of growing season
		FillSingleValue(p_oElement, "li_julianDayGrowthStarts", &m_iFirstJulDay,
				true);
		//End of growing season
		FillSingleValue(p_oElement, "li_julianDayGrowthEnds", &m_iLastJulDay,
				true);
		...
		if (mp_fSnagLightExtCoef) {
			...
			if (mp_iSnagAgeClasses[0] < 0 || mp_iSnagAgeClasses[1] < 0
					|| mp_iSnagAgeClasses[1] < mp_iSnagAgeClasses[0]) {
				modelErr stcErr;
				stcErr.iErrorCode = BAD_DATA;
				stcErr.sFunction = "clLightOrg::GetParameterFileData";
				stcErr.sMoreInfo =
						"Snag age classes must be positive and cannot overlap.";
				throw(stcErr);
			}
		}
		...
		for (i = 0; i < m_iTotalSpecies; i++) {
			if (mp_fLightExtCoef[i] < 0.0 || mp_fLightExtCoef[i] > 1.0) {
				...
				stcErr.sFunction = "clLightOrg::GetParameterFileData";
				stcErr.sMoreInfo =
						"All light extinction coefficient values must be between 0 and 1.";
				throw(stcErr);
			}
		}
		...
		if (mp_fSnagLightExtCoef) {
			for (i = 0; i < m_iNumSnagAgeClasses; i++) {
				for (j = 0; j < m_iTotalSpecies; j++) {
					if (mp_fSnagLightExtCoef[i][j] < 0.0
							|| mp_fSnagLightExtCoef[i][j] > 1.0) {
						modelErr stcErr;
						stcErr.iErrorCode = BAD_DATA;
						stcErr.sFunction = "clLightOrg::GetParameterFileData";
						stcErr.sMoreInfo =
		...
		while (p_oTree) {
			...
			if ( NULL != mp_oLightFunctionTable[iSp][iType]) {
				...
				if (notdead == iIsDead) {
					...
					fLightVal =
							mp_oLightFunctionTable[iSp][iType]->CalcLightValue(
									p_oTree, mp_oPop);

					//Assign the value to the "Light" float data member of the tree
					p_oTree->SetValue(mp_iLightCodes[iSp][iType], fLightVal,
							false);
				}
			}
