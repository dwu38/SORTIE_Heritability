...
// Constructor
//////////////////////////////////////////////////////////////////////////////
clClimateGenGrowth::clClimateGenGrowth(clSimManager * p_oSimManager) :
  clWorkerBase(p_oSimManager), clBehaviorBase(p_oSimManager),
      clGrowthBase(p_oSimManager), clNCIBehaviorBase() {

  //Set namestring
  m_sNameString = "Climategengrowthshell";
  m_sXMLRoot = "ClimateGenGrowth";
...
// Destructor
////////////////////////////////////////////////////////////////////////////
clClimateGenGrowth::~clClimateGenGrowth() {
...
// GetTreeMemberCodes()
////////////////////////////////////////////////////////////////////////////
void clClimateGenGrowth::GetTreeMemberCodes() {
...
// DoShellSetup()
////////////////////////////////////////////////////////////////////////////
void clClimateGenGrowth::DoShellSetup(xercesc::DOMDocument * p_oDoc) {
  clTreePopulation * p_oPop = ( clTreePopulation * ) mp_oSimManager->GetPopulationObject( "treepopulation" );
  ...
  FillSpeciesSpecificValue( p_oElement, "gr_climategenIntercept", "gr_cgiVal", p_fTempValues,
  ...
  FillSpeciesSpecificValue( p_oElement, "gr_climategenDBHSlope", "gr_cgdVal", p_fTempValues,
      m_iNumBehaviorSpecies, p_oPop, true );
  //Transfer to the appropriate array buckets
  for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
  ...
  FillSpeciesSpecificValue( p_oElement, "gr_climategenNCISlope", "gr_cgnVal", p_fTempValues,
      m_iNumBehaviorSpecies, p_oPop, true );
  //Transfer to the appropriate array buckets
  for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
  ...
  FillSpeciesSpecificValue( p_oElement, "gr_climatenciTempSlope", "gr_cntVal", p_fTempValues,
      m_iNumBehaviorSpecies, p_oPop, true );
  //Transfer to the appropriate array buckets
  for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
  ...
  FillSpeciesSpecificValue( p_oElement, "gr_climategenPrecipSlope", "gr_cgpVal", p_fTempValues,
      m_iNumBehaviorSpecies, p_oPop, true );
  //Transfer to the appropriate array buckets
  for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
  ...
  if (deterministic_pdf == iTemp) {
    ...
    m_iStochasticGrowthMethod = deterministic_pdf;
    Adjust = & clClimateGenGrowth::DeterministicAdjust;

  } else if (lognormal_pdf == iTemp) {

    m_iStochasticGrowthMethod = lognormal_pdf;
    Adjust = & clClimateGenGrowth::LognormalAdjust;

  } else if (normal_pdf == iTemp) {

    m_iStochasticGrowthMethod = normal_pdf;
    Adjust = & clClimateGenGrowth::NormalAdjust;

  } else {
    modelErr stcErr;
    stcErr.sFunction = "clClimateGenGrowth::DoShellSetup";
    std::stringstream s;
...
// SetNameData()
//////////////////////////////////////////////////////////////////////////////
void clClimateGenGrowth::SetNameData(std::string sNameString) {
  try
  {

    //Check the string passed and set the flags accordingly
    if (sNameString.compare("ClimateGenGrowth") == 0 )
    {
      m_iGrowthMethod = diameter_auto;
    }
    else if (sNameString.compare("ClimateGenGrowth diam only") == 0 )
    {
    ...
    else
    {
      ...
      stcErr.sMoreInfo = s.str();
      stcErr.sFunction = "clClimateGenGrowth::SetNameData" ;
      throw( stcErr );
    }
  }
  catch ( modelErr & err )
...
// CalcGrowthValue
////////////////////////////////////////////////////////////////////////////
float clClimateGenGrowth::CalcDiameterGrowthValue(clTree * p_oTree,
...
// PreGrowthCalcs
////////////////////////////////////////////////////////////////////////////
void clClimateGenGrowth::PreGrowthCalcs(clTreePopulation * p_oPop) {
  float *p_fTempEffect,
        *p_fPrecipEffect;
  try
  {
    ...
    float fDiam, //tree's diameter
	fParamG1, // tree's paramG1
	fParamG2, // tree's paramG2
    fX, fY,
	...
	fPlotTemp = p_oPlot->GetJanMinTemp(), //Current plot temperature
    ...
    while ( p_oTree )
    {
      ...
      if ( -1 != mp_iGrowthCodes[iSpecies][iType]) {
        ...
        if (notdead == iIsDead) {
          ...
          // get the tree's paramG1
          p_oTree->GetValue(p_oPop->GetParamG1Code(iSpecies, iType), &fParamG1);

          // get the tree's paramG2
          p_oTree->GetValue(p_oPop->GetParamG2Code(iSpecies, iType), &fParamG2);
          ...
          for ( i = 0; i < fNumberYearsPerTimestep; i++ ) {
            ...
            fTempDiamIncrease = mp_fIntercept[iSpecies] + mp_fDBHSlope[iSpecies] * fDiam +
            		mp_fNCISlope[iSpecies] * nci.fNCI1 +
					mp_fTempSlope[iSpecies] * fPlotTemp +
					fParamG1 * fPlotTemp +
					fParamG2 * fPlotPrecip;
            fTempDiamIncrease = (fTempDiamIncrease>0) ? fTempDiamIncrease : 0;
  ...
  catch ( ... )
  {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    stcErr.sFunction = "clClimateGenGrowth::PreCalcGrowth" ;
    throw( stcErr );
  }
}
...
// DeterministicAdjust()
/////////////////////////////////////////////////////////////////////////////
float clClimateGenGrowth::DeterministicAdjust(float fNumber, int iSpecies) {
  return fNumber;
}
...
// NormalAdjust()
/////////////////////////////////////////////////////////////////////////////
float clClimateGenGrowth::NormalAdjust(float fNumber, int iSpecies) {
...
// LognormalAdjust()
/////////////////////////////////////////////////////////////////////////////
float clClimateGenGrowth::LognormalAdjust(float fNumber, int iSpecies) {
