...
/////////////////////////////////////////////////////////////////////////////
void clConditOmegaCalculator::GetParameterFileData(
		xercesc::DOMDocument *p_oDoc) {
	try {
		DOMElement *p_oElement = GetParentParametersElement(p_oDoc);

		//Get the max distance
		FillSingleValue(p_oElement, "an_ConditsOmegaMaxDistance",
				&m_fMaxDistance, true);

		//Get the increment
		FillSingleValue(p_oElement, "an_ConditsOmegaDistanceInc", &m_fIncrement,
				true);
		...
		if (m_fIncrement <= 0) {
			...
			stcErr.sFunction = "clConditOmegaCalculator::GetData";
			stcErr.sMoreInfo =
					"The distance increment must be greater than zero.";
			stcErr.iErrorCode = BAD_DATA;
			throw(stcErr);
		}
		...
		if (m_fMaxDistance <= m_fIncrement) {
			...
			stcErr.sFunction = "clConditOmegaCalculator::GetData";
			stcErr.sMoreInfo =
					"The distance increment must be less than the max distance.";
			stcErr.iErrorCode = BAD_DATA;
			throw(stcErr);
		}
	} catch (modelErr &err) {
...
void clConditOmegaCalculator::SetupGrid() {
	...
	//Create the grid
	mp_oGrid = mp_oSimManager->CreateGrid("Relative Neighborhood Density", 0, //number of ints
	...
	mp_oGrid->SetValueOfCell(0, 0, mp_oGrid->GetFloatDataCode("inc"),
			m_fIncrement);
	mp_oGrid->SetValueOfCell(0, 0, mp_oGrid->GetFloatDataCode("dist"),
			m_fMaxDistance);
}
...
void clConditOmegaCalculator::Action() {
	...
	try {
		...
		float fPlotArea, //in square meters
				fValue; //grid values
		int iNumTotCells = m_iNumXCells * m_iNumYCells, iMinX, iMaxX, iMinY,
				iMaxY, //search limits
				...
				if (p_oFirstTree) {
					...
					for (iLoopX = iMinX; iLoopX <= iMaxX; iLoopX++) {
						for (iLoopY = iMinY; iLoopY <= iMaxY; iLoopY++) {
							...
							iNeighX =
									iLoopX >= m_iNumXCells ?
											iLoopX - m_iNumXCells : iLoopX;
							iNeighX =
									iNeighX < 0 ?
											iNeighX + m_iNumXCells : iNeighX;
							iNeighY =
									iLoopY >= m_iNumYCells ?
											iLoopY - m_iNumYCells : iLoopY;
							iNeighY =
									iNeighY < 0 ?
											iNeighY + m_iNumYCells : iNeighY;

							//Continue if the cell pair has not already been searched and
							//is worth searching
							if (!mp_bCellsSearched[(iX * m_iNumYCells) + iY][(iNeighX
									* m_iNumYCells) + iNeighY]) {
								ProcessCell(p_oPop, p_oFirstTree, iX, iY,
										iNeighX, iNeighY);
							}
						}
					}
		...
		for (i = 0; i < m_iNumTotalSpecies; i++) {
			for (j = 0; j < m_iNumIncs; j++) {
				mp_oGrid->SetValueOfCell(0, 0, mp_iGridCodes[i][j],
						mp_fOmegaValues[i][j]);
			}
		}
...
void clConditOmegaCalculator::ProcessCell(clTreePopulation *p_oPop,
			...
			if (clTreePopulation::sapling == iTp
				...
				while (p_oNeighbor) {
					...
					if (iNeighSp == iSp
						...
						p_oNeighbor->GetValue(mp_iXCodes[iNeighSp][iNeighTp],
								&fNeighX);
						p_oNeighbor->GetValue(mp_iYCodes[iNeighSp][iNeighTp],
								&fNeighY);

						fDistance = p_oPlot->GetDistance(fX, fY, fNeighX,
								fNeighY);
	...
	mp_bCellsSearched[(iX * m_iNumYCells) + iY][(iNeighX * m_iNumYCells)
			+ iNeighY] = true;
	mp_bCellsSearched[(iNeighX * m_iNumYCells) + iNeighY][(iX * m_iNumYCells)
			+ iY] = true;
}
...
void clConditOmegaCalculator::ProcessOwnCell(clTree *p_oFirstTree,
	...
	mp_bCellsSearched[(iX * m_iNumYCells) + iY][(iX * m_iNumYCells) + iY] =
			true;
}
...
void clConditOmegaCalculator::SetUpSearching(clTreePopulation *p_oPop) {
	int iCellSize = p_oPop->GetGridCellSize(), iNumTotCells = m_iNumXCells
			* m_iNumYCells, i, iLimit,           //effective search limit
	...
	for (i = 1; i < m_iNumIncs; i++) {
		mp_fAnnulusAreas[i] = M_PI
				* (pow(mp_fIncs[i], 2) - pow(mp_fIncs[i - 1], 2));
	}
}
