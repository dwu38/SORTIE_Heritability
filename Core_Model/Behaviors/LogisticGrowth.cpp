...
void clLogisticGrowth::DoShellSetup(DOMDocument *p_oDoc) {
	try {
		...
		if (height_only == m_iGrowthMethod) {
			...
			//annual to timestep
			m_fConversionFactor = mp_oSimManager->GetNumberOfYearsPerTimestep()
					/ 100;
		} else {
		...
		if (height_only == m_iGrowthMethod) {
			...
			FillSpeciesSpecificValue(p_oElement,
					"gr_heightAsympGrowthAtFullLight", "gr_hagaflVal",
					p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);

			//Transfer values to our permanent array
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
				mp_fAsympGrowthAtFullLight[mp_iIndexes[p_fTempValues[i].code]] =
						p_fTempValues[i].val;

			//Shape parameter 1
			FillSpeciesSpecificValue(p_oElement, "gr_heightShape1b",
					"gr_hs1bVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
					true);
			...
			//Shape parameter 2
			FillSpeciesSpecificValue(p_oElement, "gr_heightShape2c",
					"gr_hs2cVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
					true);
			...
			//Asymptotic growth at full light
			FillSpeciesSpecificValue(p_oElement,
					"gr_diamAsympGrowthAtFullLight", "gr_dagaflVal",
					p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
			...
			//Shape parameter 1
			FillSpeciesSpecificValue(p_oElement, "gr_diamShape1b", "gr_ds1bVal",
					p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
			...
			//Shape parameter 2
			FillSpeciesSpecificValue(p_oElement, "gr_diamShape2c", "gr_ds2cVal",
					p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
...
float clLogisticGrowth::CalculateFunctionValue(clTree *p_oTree) {
	float fGli;
	short int iSpecies = p_oTree->GetSpecies(), iIndex = mp_iIndexes[iSpecies],
			iLightCode = mp_oGrowthOrg->GetLightCode(iSpecies,
					p_oTree->GetType());
