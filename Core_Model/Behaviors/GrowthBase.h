...
public:
	...
	enum growthType {
		diameter_auto, /**<The object calculates a diameter increase,
	 ...
	 */
	virtual float CalcDiameterGrowthValue(clTree *p_oTree,
	 ...
	 */
	virtual float CalcHeightGrowthValue(clTree *p_oTree,
	...
	/**
	 * Calculates the size class for a particular tree (seedlings only),
	 * if applicable.  If overridden, this function must not change the tree's
	 * height (clGrowthOrg will do that).
	 * <p>
	 * <b>REMEMBER to appropriately compound growth by the number of years per
	 ...
	 * @return Amount, in m, by which to increase the tree's height.
	 */
	virtual int CalcSizeClassGrowthValue(clTree *p_oTree,
			clTreePopulation *p_oPop, float fDiameterGrowth) {
		return 0;
	}
	;
	...
	virtual float GetGrowthMemberValue(clTree *p_oTree, float fDiameterGrowth) {
		return fDiameterGrowth * m_fConvertCmPerTSToMmPerYr;
	}
	;
