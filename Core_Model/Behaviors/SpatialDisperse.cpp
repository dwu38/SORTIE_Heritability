		...
		if (iNumFunctionSpecies > 0) {
			...
			for (i = 0; i < iNumFunctionSpecies; i++) {
				...
				if (p_fTemp[i].val >= 50) {
					...
					stcErr.iErrorCode = BAD_DATA;
					stcErr.sFunction =
							"clSpatialDispersal::GetParameterFileData";
					stcErr.sMoreInfo =
...
void clSpatialDispersal::CalculateProbabilityDistribution(float *p_fProbArray,
		int iMaxDistance, int iSpecies, function iFunction, cover iCover) {
	try {
...
///////////////////////////////////////////////////////////////////////////*/
float clSpatialDispersal::CalculateFunctionValue(float fDistance, int iSpecies,
...
// GetNumberOfSeeds()
////////////////////////////////////////////////////////////////////////////
float clSpatialDispersal::GetNumberOfSeeds(float fDbh, short int iSp,
		int iCover, int iFunc) {
	return clModelMath::RandomRound(
			mp_fFecundity[iFunc][iCover][mp_iIndexes[iSp]]
					* pow(fDbh, mp_fBeta[iFunc][iCover][mp_iIndexes[iSp]])
					* m_fNumYearsPerTimestep);
}
