...
void clBasalAreaLight::DoShellSetup(xercesc::DOMDocument *p_oDoc) {
	...
	try {
		...
		int i, j, iNumXCells, iNumYCells, iNumSpecies =
				p_oPop->GetNumberOfSpecies();
		bool bMap;
		...
		FillSingleValue(p_oElement, "li_baAngiospermLightB", &m_fAngiospermB,
				true);
		...
		if (0 == m_fConiferC) {
			...
			stcErr.sMoreInfo =
					"Basal area light conifer \"c\" parameter cannot equal 0.";
			stcErr.iErrorCode = BAD_DATA;
			throw(stcErr);
		}

		//Angiosperm "c" parameter
		FillSingleValue(p_oElement, "li_baAngiospermLightC", &m_fAngiospermC,
				true);
		//Error if this is equal to 0
		if (0 == m_fAngiospermC) {
			modelErr stcErr;
			stcErr.sFunction = "clBasalAreaLight::DoShellSetup";
			stcErr.sMoreInfo =
					"Basal area light angiosperm \"c\" parameter cannot equal 0.";
			stcErr.iErrorCode = BAD_DATA;
			throw(stcErr);
		}
		...
		FillSingleValue(p_oElement, "li_baLightChangeThreshold",
				&m_fChangeThreshold, true);
		//Error if this is less than 0
		if (m_fChangeThreshold < 0) {
			modelErr stcErr;
			stcErr.sFunction = "clBasalAreaLight::DoShellSetup";
			stcErr.sMoreInfo =
					"Basal area light change threshold cannot be less than 0.";
			stcErr.iErrorCode = BAD_DATA;
			throw(stcErr);
		}
		...
		if (m_fMinDbh < 0) {
			...
			stcErr.sFunction = "clBasalAreaLight::DoShellSetup";
			stcErr.sMoreInfo =
					"Basal area light minimum DBH cannot be less than 0.";
			stcErr.iErrorCode = BAD_DATA;
			throw(stcErr);
		}
		...
		FillSpeciesSpecificValue(p_oElement, "li_baTreeType", "li_bttVal",
				p_iTemp, p_oPop, true);
		for (i = 0; i < iNumSpecies; i++) {
			if (angiosperm == p_iTemp[i])
				mp_iSpeciesTypes[i] = angiosperm;
			else if (conifer == p_iTemp[i])
				mp_iSpeciesTypes[i] = conifer;
			else {
				modelErr stcErr;
				stcErr.sFunction = "clBasalAreaLight::DoShellSetup";
				std::stringstream s;
		...
		if (!mp_oLightGrid) {
			...
			mp_oLightGrid = mp_oSimManager->CreateGrid("Basal Area Light", 0, 3,
					0, 0);
			...
			if (-1 == m_iGridLightCode || -1 == m_iGridConBACode
					|| -1 == m_iGridAngBACode) {
				modelErr stcErr;
				stcErr.iErrorCode = BAD_DATA;
				stcErr.sFunction = "clBasalAreaLight::DoShellSetup";
				stcErr.sMoreInfo =
						"\"Basal Light\" grid was incorrectly set up in the parameter file.  Data members missing.";
				throw(stcErr);
			}
		}
		...
		if (!bMap) {
			...
			for (i = 0; i < iNumXCells; i++) {
				for (j = 0; j < iNumYCells; j++) {
					mp_oLightGrid->SetValueOfCell(i, j, m_iGridConBACode,
							fConBA);
				}
			}
		}
...
void clBasalAreaLight::UpdateGridValues(clTreePopulation *p_oPop) {
	try {
		...
		int iSp, iTp, iNumXCells = mp_oLightGrid->GetNumberXCells(),
				iNumYCells = mp_oLightGrid->GetNumberYCells(), iX, iY; //loop counter
		...
		for (iX = 0; iX < iNumXCells; iX++) {
			for (iY = 0; iY < iNumYCells; iY++) {
				...
				mp_oLightGrid->GetValueOfCell(iX, iY, m_iGridConBACode,
						&fLastConBA);
				mp_oLightGrid->GetValueOfCell(iX, iY, m_iGridAngBACode,
						&fLastAngBA);
				if (fabs((fAngBA + fConBA) - (fLastConBA + fLastAngBA))
						> m_fChangeThreshold) {
					...
					mp_oLightGrid->SetValueOfCell(iX, iY, m_iGridLightCode,
							fLight);

					//Assign the new basal areas to the grid
					mp_oLightGrid->SetValueOfCell(iX, iY, m_iGridConBACode,
							fConBA);
					mp_oLightGrid->SetValueOfCell(iX, iY, m_iGridAngBACode,
							fAngBA);
				}
			}
		}
