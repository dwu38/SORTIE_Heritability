...
void clWindstorm::ReadParFile(xercesc::DOMDocument *p_oDoc,
	...
	try {
		...
		FillSpeciesSpecificValue(p_oElement, "ws_stmInterceptA", "ws_siaVal",
		...
		FillSpeciesSpecificValue(p_oElement, "ws_stmDBHExpB", "ws_sdebVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		FillSpeciesSpecificValue(p_oElement, "ws_stmIntensCoeffC", "ws_sicVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		FillSpeciesSpecificValue(p_oElement, "ws_minDBH", "ws_mdVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			...
			if (mp_fMinDBH[p_fTempValues[i].code] < 0) {
				...
				stcErr.sMoreInfo =
						"Minimum DBH for windstorms cannot be negative.";
		...
		FillSingleValue(p_oElement, "ws_stmTSToStartStorms",
				&m_iFirstStormTimestep, true);
		if (m_iFirstStormTimestep < 0) {
			modelErr stcErr;
			stcErr.sFunction = "clWindstorm::ReadParFile";
			stcErr.sMoreInfo =
					"Timestep to start windstorms cannot be negative.";
			stcErr.iErrorCode = BAD_DATA;
			throw(stcErr);
		}
		...
		for (i = 1; i < m_iNumReturnIntervals; i++) {
			...
			if (fTemp < 0 || fTemp > 1) {
				...
				stcErr.sFunction = "clWindstorm::ReadParFile";
				stcErr.sMoreInfo =
						"Windstorm severities must be between 0 and 1.";
				stcErr.iErrorCode = BAD_DATA;
				throw(stcErr);
			}
		...
		FillSingleValue(p_oElement, "ws_stmTrendInterceptI",
...
void clWindstorm::DoGridSetup() {
	...
	for (i = 0; i < m_iTotalNumSpecies; i++) {
		...
		sLabel.str("");
		sLabel << "density_" << i;
		mp_iDensity_xCodes[i] = mp_oResultsGrid->RegisterPackageFloat(
				sLabel.str());
		sLabel.str("");
	}
...
void clWindstorm::Action() {
	...
	try {
		...
		fX = 4 * (fCurrentTimestep - m_iFirstStormTimestep)
				* fNumYearsPerTimestep / m_fSSTPeriod;
			...
			for (j = 0; j < fNumYearsPerTimestep; j++) {
				if (clModelMath::GetRand()
					...
					while (p_oTree) {
						...
						p_oTree->GetValue(
								mp_iDeadCodes[iSp][iTp
										- clTreePopulation::sapling], &iDead);
						if (iDead > notdead)
							goto nextTree;
						...
						if (mp_fStormSeverities[i] < WINDSTORMMIN) {
							...
							fMortProb = mp_fA[iSp]
									+ (mp_fC[iSp] * mp_fStormSeverities[i]
											* pow(fDbh, mp_fB[iSp]));

							if (fMortProb >= 10)
								fMortProb = 1.0;
							else if (fMortProb <= -10)
								fMortProb = 0.0;
							else {
					...
					if (NULL == p_oStormPackage) {
						p_oStormPackage = mp_oResultsGrid->CreatePackageAtPoint(
								0, 0);
					} else {
						p_oStormPackage = mp_oResultsGrid->CreatePackage(
								p_oStormPackage);
					}
					p_oStormPackage->SetValue(m_iSeverityCode,
							mp_fStormSeverities[i]);
					for (iSp = 0; iSp < m_iTotalNumSpecies; iSp++) {
						p_oStormPackage->SetValue(mp_iBa_xCodes[iSp],
								p_fDeadBA[iSp] / fPlotArea);
						p_oStormPackage->SetValue(mp_iDensity_xCodes[iSp],
								p_fDeadDen[iSp] / fPlotArea);
					}
				}
			}
		}
...
void clWindstorm::GetDeadCodes(clTreePopulation *p_oPop) {
	...
	for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
		...
		mp_iDeadCodes[mp_whatSpeciesTypeCombos[i].iSpecies][mp_whatSpeciesTypeCombos[i].iType
				- clTreePopulation::sapling] = p_oPop->GetIntDataCode("dead",
		...
		if (-1
			...
			stcErr.sFunction = "clWindstorm::GetDeadCodes";
			stcErr.sMoreInfo =
					"You must use tree mortality behaviors along with Windstorm.";
			throw(stcErr);
		}
	}
}
