	...
	try {
		...
		for (iDayCount = iFirstJulDay; iDayCount < iLastJulDay; iDayCount++) {
			...
			while (fTimeNow >= 0.0) //while still morning and not yet noon (noon is 0 in solar time)
			{
				...
				//the zenith
				fCosZenAng = GetCosineOfZenithAngle(fDeclination, fLatInRadians,
						fTimeNow);
				...
				if (iAltCoord >= m_iNumAltAng)
					iAltCoord = m_iNumAltAng - 1;

				//Compute azimuth angle of the sun in radians
				fAzimuth = GetAzimuthAngle(fDeclination, fLatInRadians,
						fAltInRad, fTimeNow);
				...
				if (iAziCoord >= m_iNumAziAng)
					iAziCoord = m_iNumAziAng - 1;
				...
				if (iAltCoord >= m_iMinAngRow) { //Compute beam transmission as function of path length and eccentricity
					fBeam = GetBeamRadiation(fClearSkyTransCoef, fAirmass,
							fEccentricity, fCosZenAng);
					fTotalBeam += fBeam;
		...
		for (iAltCoord = 0; iAltCoord < m_iNumAltAng; iAltCoord++)
			for (iAziCoord = 0; iAziCoord < iEWDivideRow; iAziCoord++) {
				...
				mp_fBrightness[iAltCoord][iWestCol] =
						mp_fBrightness[iAltCoord][iAziCoord];
			}
		...
		for (i = 0; i < m_iNumAltAng; i++)
			for (j = 0; j < m_iNumAziAng; j++) {
				if (i >= m_iMinAngRow) {
					mp_fBrightness[i][j] += (1.0 - fBeamFracGlobalRad)
							/ (m_iNumAziAng * (m_iNumAltAng - m_iMinAngRow));
				} else
					mp_fBrightness[i][j] = 0.0;
			}
		...
		if (m_fAzimuthOfNorth > 0) {
			...
			for (j = 0; j < m_iNumAziAng; j++) {
				iNewCol = j + iOffset;
				if (iNewCol >= m_iNumAziAng)
					iNewCol -= m_iNumAziAng;
				for (i = 0; i < m_iNumAltAng; i++) {
	...
	try {
		...
		for (i = 1; i <= m_iNumAltAng; i++) {
			...
			p_fDiffuseRad[i - 1] = sin((i - 0.5) * CONVERT_TO_RADIANS)
					* ((sin(CONVERT_TO_RADIANS * i)
							- sin(CONVERT_TO_RADIANS * (i - 1))) / 360);
		...
		for (i = 0; i < m_iNumAltAng; i++) {
			...
			if (i >= m_iMinAngRow) {
				...
				for (j = 0; j < m_iNumAziAng; j++)
					mp_fBrightness[i][j] = fBeamFracGlobalRad
							* mp_fBrightness[i][j]
							+ (1.0 - fBeamFracGlobalRad) * p_fDiffuseRad[i];
