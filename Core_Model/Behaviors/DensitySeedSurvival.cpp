...
void clDensitySeedSurvival::GetData(DOMDocument *p_oDoc) {
	try {
		...
		FillSpeciesSpecificValue(p_oElement, "es_densDepSteepness",
				"es_ddstVal", p_fTemp, m_iNumBehaviorSpecies, p_oPop, true);
		//Now transfer the values to the permanent array
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		if (mp_oSeedGrid == NULL) {
			...
			stcErr.sMoreInfo =
					"Disperse behaviors must be used with density-dependent seed survival.";
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			...
			mp_iSeedGridCode[mp_iWhatSpecies[i]] =
					mp_oSeedGrid->GetFloatDataCode(sLabel.str());
		...
		if (m_bSeeds) {
			...
			fXTemp = p_oPlot->GetXPlotLength()
					- (mp_oSeedGrid->GetLengthXCells()
							* floor(
									p_oPlot->GetXPlotLength()
											/ mp_oSeedGrid->GetLengthXCells()));
			fXTemp = (fXTemp == 0 ? mp_oSeedGrid->GetLengthXCells() : fXTemp);
			fYTemp = p_oPlot->GetYPlotLength()
					- (mp_oSeedGrid->GetLengthYCells()
							* floor(
			...
			FillSpeciesSpecificValue(p_oElement, "es_densDepMinNeighHeight",
					"es_ddmnhVal", p_fTemp, m_iNumBehaviorSpecies, p_oPop,
					true);
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
				mp_fMinHeight[p_fTemp[i].code] = p_fTemp[i].val;

			//Neighbor search radius
			FillSingleValue(p_oElement, "es_densDepSearchRadius",
					&m_fSearchRadius, true);
			...
			for (i = 0; i < m_iNumBehaviorSpecies; i++) {
				...
				if (fNumSeeds > 1) {
					fNewSeeds =
							fNumSeeds
									* exp(
											-mp_fDensDepSlope[mp_iWhatSpecies[i]]
													* pow(
															(fNumSeeds
																	/ fCellArea),
															mp_fDensDepSteepness[mp_iWhatSpecies[i]]));
				} else {
					fNewSeeds = fNumSeeds;
				}
...
void clDensitySeedSurvival::ActionUseTrees() {
	...
	int iNumXCells = mp_oSeedGrid->GetNumberXCells(), iNumYCells =
			mp_oSeedGrid->GetNumberYCells(), iNumberTotalSpecies =
			p_oPop->GetNumberOfSpecies(), iX, iY, i;
	...
	for (i = 0; i < iNumberTotalSpecies; i++)
		p_iIndexes[i] = -1;
	for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		for (iY = 0; iY < iNumYCells; iY++) {
			...
			while (p_oTree) {
				...
				if (p_iIndexes[p_oTree->GetSpecies()] > -1) {
					...
					p_oTree->GetValue(
							p_oPop->GetHeightCode(p_oTree->GetSpecies(),
					...
					fNewSeeds =
							fNumSeeds
									* exp(
											-mp_fDensDepSlope[mp_iWhatSpecies[i]]
													* pow(
															(p_iNeighCount[mp_iWhatSpecies[i]]
																	/ m_fNormalSearchArea),
															mp_fDensDepSteepness[mp_iWhatSpecies[i]]));
				} else {
					fNewSeeds = fNumSeeds;
	...
	try {
		...
		if (sNameString.compare("DensityDependentSeedSurvival") == 0) {
			m_bSeeds = true;
				...
				"ConspecificTreeDensityDependentSeedSurvival") == 0) {
			m_bSeeds = false;
		} else {
			modelErr stcErr;
			stcErr.iErrorCode = BAD_DATA;
