...
void clLinearBiLevelGrowth::DoShellSetup(DOMDocument *p_oDoc) {
	...
	try {
		...
		FillSpeciesSpecificValue(p_oElement, "gr_linearBilevLoLiteIntercept",
				"gr_lblliVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		...
		FillSpeciesSpecificValue(p_oElement, "gr_linearBilevLoLiteSlope",
				"gr_lbllsVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer values to our permanent array
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		if (NULL != mp_oStormLight) {
			...
			FillSpeciesSpecificValue(p_oElement,
					"gr_linearBilevHiLiteThreshold", "gr_lbhltVal",
			...
			FillSpeciesSpecificValue(p_oElement,
					"gr_linearBilevHiLiteIntercept", "gr_lbhliVal",
					p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
			//Transfer values to our permanent array
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
				mp_fHiLightIntercept[mp_iIndexes[p_fTempValues[i].code]] =
						p_fTempValues[i].val;

			//High-light growth slope
			FillSpeciesSpecificValue(p_oElement, "gr_linearBilevHiLiteSlope",
					"gr_lbhlsVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
					true);
			//Transfer values to our permanent array
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
				mp_fHiLightSlope[mp_iIndexes[p_fTempValues[i].code]] =
						p_fTempValues[i].val;
...
float clLinearBiLevelGrowth::CalcDiameterGrowthValue(clTree *p_oTree,
	...
	return m_fYearsPerTimestep
			* clModelMath::CalcPointValue(fDiam,
					mp_fHiLightSlope[mp_iIndexes[iSp]],
					mp_fHiLightIntercept[mp_iIndexes[iSp]]);
}
