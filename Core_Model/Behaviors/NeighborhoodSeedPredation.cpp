...
void clNeighborhoodSeedPredation::Action() {
	...
	int iNumXCells = mp_oSeedGrid->GetNumberXCells(), iNumYCells =
			mp_oSeedGrid->GetNumberYCells(), iX, iY, iSp;
		...
		for (iY = 0; iY < iNumYCells; iY++) {
			for (iSp = 0; iSp < m_iNumBehaviorSpecies; iSp++) {
				mp_oSeedGrid->GetValueOfCell(iX, iY, mp_iSeedGridCode[iSp],
						&fSeeds);
				mp_oOutputGrid->SetValueOfCell(iX, iY,
						mp_iStartSeedsCode[mp_iWhatSpecies[iSp]], fSeeds);
			}
		}
	}
	...
	for (iX = 0; iX < iNumXCells; iX++) {
		for (iY = 0; iY < iNumYCells; iY++) {
			for (iSp = 0; iSp < m_iNumBehaviorSpecies; iSp++) {
				mp_oSeedGrid->GetValueOfCell(iX, iY, mp_iSeedGridCode[iSp],
						&fProp);
				mp_oOutputGrid->GetValueOfCell(iX, iY,
						mp_iStartSeedsCode[mp_iWhatSpecies[iSp]], &fSeeds);
				if (fSeeds == 0)
					fProp = 0;
				else
					fProp = 1 - (fProp / fSeeds);
				mp_oOutputGrid->SetValueOfCell(iX, iY,
						mp_iPropEatenCode[mp_iWhatSpecies[iSp]], fProp);
			}
		}
	}
	...
	try {
		...
		if (NULL == p_oTree)
			...
			if (m_bUseThresholdToDecideMast) {
						...
						for (iSp = 0; iSp < m_iNumBehaviorSpecies; iSp++) {
							if (mp_bCountInMast[iSp]) {
								mp_oSeedGrid->GetValueOfCell(iX, iY,
										mp_iSeedGridCode[iSp], &fNewSeeds);
								fNumSeeds += fNewSeeds;
							}
						}
					}
				...
				if (fNumSeeds / m_fPlotArea <= m_fMastingThreshold)
					bMasting = false;
				else
					bMasting = true;
			} else {
				...
				if (p_oTemp != NULL) {
					clMastingSpatialDisperse *p_oDisperse1 =
							dynamic_cast<clMastingSpatialDisperse*>(p_oTemp);
					for (iSp = 0; iSp < m_iNumBehaviorSpecies; iSp++) {
						if (p_oDisperse1->GetMastEvent(mp_iWhatSpecies[iSp])
								== mast) {
							bMasting = true;
							break;
						}
				...
				if (bMasting == false) {
					p_oTemp = mp_oSimManager->GetBehaviorObject(
							"MastingNonSpatialDisperse");
					if (p_oTemp != NULL) {
						clMastingNonSpatialDisperse *p_oDisperse2 =
								dynamic_cast<clMastingNonSpatialDisperse*>(p_oTemp);
						for (iSp = 0; iSp < m_iNumBehaviorSpecies; iSp++) {
							if (p_oDisperse2->GetMastEvent(mp_iWhatSpecies[iSp])
									== mast) {
								bMasting = true;
								break;
							}
...
void clNeighborhoodSeedPredation::DoLinkedPredation() {
	...
	try {
		...
		clFuncResponseSeedPredation *p_oPred =
				dynamic_cast<clFuncResponseSeedPredation*>(p_oTemp);
		float fZ = p_oPred->GetOfftakeRate(), fMinY = 100000, fAvgY = 0,
				fCorrectionFactor, fNumSeeds = 0, fNewSeeds;
		int iSp;
		...
		for (iX = 0; iX < iNumXCells; iX++) {
			for (iY = 0; iY < iNumYCells; iY++) {
				...
				for (iSp = 0; iSp < m_iNumBehaviorSpecies; iSp++) {
					p_fOfftakes[iX][iY][iSp] = log(
							p_fOfftakes[iX][iY][iSp]
									/ (1 - p_fOfftakes[iX][iY][iSp]));
					if (fMinY > p_fOfftakes[iX][iY][iSp])
						fMinY = p_fOfftakes[iX][iY][iSp];
				}
			}
		}
...
void clNeighborhoodSeedPredation::GetOfftakes(clTreePopulation *p_oPop,
	...
	int iNumSpecies = p_oPop->GetNumberOfSpecies(), iSp, iTp, i;
...
void clNeighborhoodSeedPredation::SetupGrids() {
	try {
		...
		if (mp_oSeedGrid == NULL) {
			...
			stcErr.sMoreInfo =
					"Disperse behaviors must be used with neighborhood seed predation.";
		...
		if (mp_oOutputGrid == NULL) {
			...
			for (i = 0; i < iTotalSpecies; i++) {
				...
				mp_iStartSeedsCode[i] = mp_oOutputGrid->RegisterFloat(
						sLabel.str());
				sLabel.str("");
			}
			for (i = 0; i < iTotalSpecies; i++) {
				sLabel << "propeaten_" << i;
				mp_iPropEatenCode[i] = mp_oOutputGrid->RegisterFloat(
						sLabel.str());
				sLabel.str("");
			}
			...
			if (fabs(
					mp_oOutputGrid->GetLengthXCells()
							- mp_oSeedGrid->GetLengthXCells()) > 0.001
					|| fabs(
							mp_oOutputGrid->GetLengthYCells()
									- mp_oSeedGrid->GetLengthYCells())
							> 0.001) {
				mp_oOutputGrid = mp_oSimManager->CreateGrid(
						"Neighborhood Seed Predation", //grid name
				...
				for (i = 0; i < iTotalSpecies; i++) {
					sLabel << "startseeds_" << i;
					mp_iStartSeedsCode[i] = mp_oOutputGrid->GetFloatDataCode(
							sLabel.str());
					sLabel.str("");
					sLabel << "propeaten_" << i;
					mp_iPropEatenCode[i] = mp_oOutputGrid->GetFloatDataCode(
							sLabel.str());
					sLabel.str("");
				}
			}
...
////////////////////////////////////////////////////////////////////////////
void clNeighborhoodSeedPredation::ReadParameterFileData(
	...
	try {
		...
		if (m_bIsLinked) {
			//"p0"
			FillSpeciesSpecificValue(p_oElement, "pr_neighPredP0",
					"pr_npmp0Val", p_fTemp, m_iNumBehaviorSpecies, p_oPop,
					true);
			...
			//"pns"
			for (i = 0; i < iNumSpecies; i++) {
				sLabel << "pr_neighPredPn"
						<< p_oPop->TranslateSpeciesCodeToName(i);
				FillSpeciesSpecificValue(p_oElement, sLabel.str(), "pr_nppnVal",
			...
			//Masting "p0"
			FillSpeciesSpecificValue(p_oElement, "pr_neighPredMastingP0",
					"pr_npmp0Val", p_fTemp, m_iNumBehaviorSpecies, p_oPop,
					true);
			//Now transfer the values to the permanent array
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
			...
			//Masting "pns"
			for (i = 0; i < iNumSpecies; i++) {
				sLabel << "pr_neighPredMastingPn"
						<< p_oPop->TranslateSpeciesCodeToName(i);
				FillSpeciesSpecificValue(p_oElement, sLabel.str(),
						"pr_npmpnVal", p_fTemp, m_iNumBehaviorSpecies, p_oPop,
						true);
				sLabel.str("");
				for (j = 0; j < m_iNumBehaviorSpecies; j++) {
			...
			//Non-masting "p0"
			FillSpeciesSpecificValue(p_oElement, "pr_neighPredNonMastingP0",
					"pr_npnmp0Val", p_fTemp, m_iNumBehaviorSpecies, p_oPop,
					true);
			//Now transfer the values to the permanent array
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
				mp_fNonMastingP0[i] = p_fTemp[i].val;
			...
			//Non-masting "pns"
			for (i = 0; i < iNumSpecies; i++) {
				sLabel << "pr_neighPredNonMastingPn"
						<< p_oPop->TranslateSpeciesCodeToName(i);
				FillSpeciesSpecificValue(p_oElement, sLabel.str(),
						"pr_npnmpnVal", p_fTemp, m_iNumBehaviorSpecies, p_oPop,
						true);
				sLabel.str("");
				for (j = 0; j < m_iNumBehaviorSpecies; j++) {
					mp_fNonMastingPn[j][i] = p_fTemp[j].val;
			...
			FillSingleValue(p_oElement, "pr_neighPredMastDecisionMethod",
					&iTemp, true);
			if (iTemp == 0)
				m_bUseThresholdToDecideMast = true;
			else
				m_bUseThresholdToDecideMast = false;

			if (m_bUseThresholdToDecideMast) {
				//Density of seeds for masting
				FillSingleValue(p_oElement, "pr_neighPredMastingDensity",
						&m_fMastingThreshold, true);
				if (m_fMastingThreshold < 0) {
					modelErr stcErr;
					stcErr.sFunction =
							"clNeighborhoodSeedPredation::ReadParameterFileData";
					stcErr.sMoreInfo =
							"Masting seed density threshold cannot be negative.";
					stcErr.iErrorCode = BAD_DATA;
					throw(stcErr);
				}
				...
				FillSpeciesSpecificValue(p_oElement, "pr_neighPredCounts4Mast",
						"pr_npc4mVal", p_bTemp, m_iNumBehaviorSpecies, p_oPop,
						true);
				//Now transfer the values to the permanent array
				for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		FillSingleValue(p_oElement, "pr_neighPredRadius", &m_fRadius, true);
		if (m_fRadius < 0) {
			modelErr stcErr;
			stcErr.sFunction =
					"clNeighborhoodSeedPredation::ReadParameterFileData";
