...
void clDimensionAnalysis::GetData(xercesc::DOMDocument *p_oDoc) {
	...
	try {
		...
		FillSpeciesSpecificValue(p_oElement, "bi_a", "bi_aVal", p_fTempValues,
				m_iNumBehaviorSpecies, p_oPop, true);
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fA[i] = p_fTempValues[i].val;

		//b in the biomass equation
		FillSpeciesSpecificValue(p_oElement, "bi_b", "bi_bVal", p_fTempValues,
				m_iNumBehaviorSpecies, p_oPop, true);
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fB[i] = p_fTempValues[i].val;

		//c in the biomass equation
		FillSpeciesSpecificValue(p_oElement, "bi_c", "bi_cVal", p_fTempValues,
				m_iNumBehaviorSpecies, p_oPop, true);
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fC[i] = p_fTempValues[i].val;

		//d in the biomass equation
		FillSpeciesSpecificValue(p_oElement, "bi_d", "bi_dVal", p_fTempValues,
				m_iNumBehaviorSpecies, p_oPop, true);
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fD[i] = p_fTempValues[i].val;

		//e in the biomass equation
		FillSpeciesSpecificValue(p_oElement, "bi_e", "bi_eVal", p_fTempValues,
				m_iNumBehaviorSpecies, p_oPop, true);
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fE[i] = p_fTempValues[i].val;

		//Use correction factor
		FillSpeciesSpecificValue(p_oElement, "bi_useCorrectionFactor",
				"bi_ucfVal", p_bTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_bUseCorrectionFactor[i] = p_bTempValues[i].val;

		//Correction factor
		FillSpeciesSpecificValue(p_oElement, "bi_correctionFactorValue",
				"bi_cfvVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fCorrectionFactor[i] = p_fTempValues[i].val;

		//Equation ID
		FillSpeciesSpecificValue(p_oElement, "bi_eqID", "bi_eiVal",
				p_iTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		//Meaning of "dia"
		FillSpeciesSpecificValue(p_oElement, "bi_whatDia", "bi_wdVal",
				p_iTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		//DBH units
		FillSpeciesSpecificValue(p_oElement, "bi_dbhUnits", "bi_duVal",
				p_iTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		...
		//Biomass units
		FillSpeciesSpecificValue(p_oElement, "bi_biomassUnits", "bi_buVal",
				p_iTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Go through and assign the appropriate correction factor for each
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			if (g == p_iTempValues[i].val) {
				...
				stcErr.sFunction = "clDimensionAnalysis::GetData";
				std::stringstream s;
				s << "Unidentified biomass units code \""
						<< p_iTempValues[i].val << "\" for species \""
						<< mp_iWhatSpecies[i] << "\".";
				stcErr.sMoreInfo = s.str();
				throw(stcErr);
			}
...
void clDimensionAnalysis::Action() {
	try {
		...
		while (p_oTree) {
			...
			if (-1
					!= mp_iBiomassCodes[mp_iIndexes[iSp]][iTp
					...
					fBiomass = mp_fA[mp_iIndexes[iSp]]
							* exp(
									mp_fB[mp_iIndexes[iSp]]
											+ (mp_fC[mp_iIndexes[iSp]]
													* log(fDBH))
											+ (mp_fD[mp_iIndexes[iSp]] * fDBH));
				} else if (7 == mp_iEquationID[mp_iIndexes[iSp]]) {
					fBiomass = mp_fA[mp_iIndexes[iSp]]
							+ ((mp_fB[mp_iIndexes[iSp]]
									* pow(fDBH, mp_fC[mp_iIndexes[iSp]]))
									/ (pow(fDBH, mp_fC[mp_iIndexes[iSp]])
											+ mp_fD[mp_iIndexes[iSp]]));
				} else if (8 == mp_iEquationID[mp_iIndexes[iSp]]) {
					fBiomass = pow(100,
							mp_fA[mp_iIndexes[iSp]]
									+ (mp_fB[mp_iIndexes[iSp]] * log10(fDBH)));
				} else {
					fBiomass = exp(
							log(mp_fA[mp_iIndexes[iSp]])
									+ (mp_fB[mp_iIndexes[iSp]] * log(fDBH)));
				}
				...
				p_oTree->SetValue(
						mp_iBiomassCodes[mp_iIndexes[iSp]][iTp
								- clTreePopulation::sapling], fBiomass);
			}
...
void clDimensionAnalysis::RegisterTreeDataMembers() {
	...
	for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
		...
		mp_iBiomassCodes[mp_iIndexes[mp_whatSpeciesTypeCombos[i].iSpecies]][mp_whatSpeciesTypeCombos[i].iType
				- clTreePopulation::sapling] = p_oPop->RegisterFloat("Biomass",
