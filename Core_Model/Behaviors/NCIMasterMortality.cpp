...
void clNCIMasterMortality::DoShellSetup(xercesc::DOMDocument *p_oDoc) {
	...
	for (i = 0; i < m_iNumBehaviorSpecies; i++) {
		if (mp_fMaxPotentialValue[mp_iWhatSpecies[i]] < 0
			...
			stcErr.sFunction = "clNCIMasterMortality::DoShellSetup";
			stcErr.sMoreInfo =
					"All values for max survival probability must be between 0 and 1.";
			throw(stcErr);
		}
	}

	//Get survival period length
	FillSingleValue(p_oElement, "mo_nciMortSurvPeriod", &m_fMaxSurvivalPeriod,
			true);
...
// DoMort
////////////////////////////////////////////////////////////////////////////
deadCode clNCIMasterMortality::DoMort(clTree *p_oTree, const float &fDbh,
...
void clNCIMasterMortality::PreMortCalcs(clTreePopulation *p_oPop) {
    ...
    out << "Timestep\tSpecies\tDBH\tNCI\tSize Effect\tCrowding Effect\tDamage Effect\tSurv Prob\tDead?\n";
		...
		float
				fDamageEffect, //this tree's damage effect
		...
		while (p_oTree) {
			...
			if (mp_bUsesThisMortality[iSpecies][iType]) {
				...
				if (iDead <= natural) {
					...
					//Get crowding effect
					fCrowdingEffect =
							mp_oCrowdingEffect->CalculateCrowdingEffect(p_oTree,
									fDbh, nci, iSpecies);

					//Get the tree's damage effect
					fDamageEffect = mp_oDamageEffect->CalculateDamageEffect(
							p_oTree);

					//Get the tree's shading effect
					fShadingEffect = mp_oShadingEffect->CalculateShadingEffect(
							p_oTree);

					//Get the tree's size effect
					fSizeEffect = mp_oSizeEffect->CalculateSizeEffect(p_oTree,
							fDbh);

					//Get the tree's infection effect
					fInfectionEffect =
							mp_oInfectionEffect->CalculateInfectionEffect(
									p_oTree);

					//Determine whether tree survives
					fSurvivalProb = mp_fMaxPotentialValue[iSpecies]
							* fSizeEffect * fCrowdingEffect * fShadingEffect
							* fDamageEffect * p_fPrecipEffect[iSpecies]
