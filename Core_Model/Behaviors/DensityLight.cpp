...
void clDensityLight::DoShellSetup(xercesc::DOMDocument *p_oDoc) {
	try {
		...
		FillSingleValue(p_oElement, "li_densLightChangeThreshold",
				&m_iChangeThreshold, true);
		//Error if this is less than 0
		if (m_iChangeThreshold < 0) {
			modelErr stcErr;
			stcErr.sFunction = "clDensityLight::GetData";
			stcErr.sMoreInfo =
					"Density light change threshold cannot be less than 0.";
			stcErr.iErrorCode = BAD_DATA;
			throw(stcErr);
		}
		...
		if (m_fMinDbh < 0) {
			...
			stcErr.sFunction = "clDensityLight::GetData";
			stcErr.sMoreInfo =
					"Density light minimum DBH cannot be less than 0.";
			stcErr.iErrorCode = BAD_DATA;
			throw(stcErr);
		}
		...
		if (!mp_oLightGrid) {
			...
			mp_oLightGrid = mp_oSimManager->CreateGrid("Density Light", 1, 1, 0,
					0);
			...
			if (-1 == m_iGridLightCode || -1 == m_iGridCountCode) {
				...
				stcErr.sFunction = "clDensityLight::GetData";
				stcErr.sMoreInfo =
						"\"Density Light\" grid was incorrectly set up in the parameter file.  Data members missing.";
				throw(stcErr);
			}
		}
		...
		if (p_oPlot->GetXPlotLength()
				- (iNumXCells * mp_oLightGrid->GetLengthXCells()) > 0.01
				|| p_oPlot->GetYPlotLength()
						- (iNumYCells * mp_oLightGrid->GetLengthYCells())
						> 0.01) {
			modelErr stcErr;
			stcErr.iErrorCode = BAD_DATA;
			stcErr.sFunction = "clDensityLight::GetData";
			stcErr.sMoreInfo =
					"\"Density Light\" grid cell lengths must divide evenly into the plot lengths.";
			throw(stcErr);
		}
		...
		if (!bMap) {
			...
			for (i = 0; i < iNumXCells; i++) {
				for (j = 0; j < iNumYCells; j++) {
					mp_oLightGrid->SetValueOfCell(i, j, m_iGridCountCode,
							iCount);
				}
			}
		}
...
void clDensityLight::UpdateGridValues(clTreePopulation *p_oPop) {
	try {
		...
		float fLight, //light level
				...
				fXCellLength = mp_oLightGrid->GetLengthXCells(), fYCellLength =
						mp_oLightGrid->GetLengthYCells();
		int iSp, iTp, iNumXCells = mp_oLightGrid->GetNumberXCells(),
				iNumYCells = mp_oLightGrid->GetNumberYCells(), iCount, iX, iY; //loop counter
		...
		sQuery << "type=" << clTreePopulation::sapling << ","
		...
		for (iX = 0; iX < iNumXCells; iX++) {
			for (iY = 0; iY < iNumYCells; iY++) {
				...
				mp_oLightGrid->GetValueOfCell(iX, iY, m_iGridCountCode,
						&iCount);
				if (fabs(p_iCountGrid[iX][iY] - iCount) > m_iChangeThreshold) {
					...
					mp_oLightGrid->SetValueOfCell(iX, iY, m_iGridCountCode,
							p_iCountGrid[iX][iY]);
				}
			}
		}
