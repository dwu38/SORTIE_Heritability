...
void clSnagDecomp::GetData(xercesc::DOMDocument *p_oDoc) {
	try {
		...
		FillSingleValue(p_oElement, "sd_snagDecompTreefallAlpha",
				&m_fTreefallAlpha, true);

		FillSpeciesSpecificValue(p_oElement, "sd_snagDecompTreefallBeta",
				"sd_sdtbVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fTreefallBeta[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;

		FillSingleValue(p_oElement, "sd_snagDecompTreefallDelta",
				&m_fTreefallDelta, true);
		FillSingleValue(p_oElement, "sd_snagDecompTreefallTheta",
				&m_fTreefallTheta, true);
		FillSingleValue(p_oElement, "sd_snagDecompTreefallIota",
				&m_fTreefallIota, true);
		FillSingleValue(p_oElement, "sd_snagDecompTreefallLambda",
				&m_fTreefallLamda, true);

		FillSingleValue(p_oElement, "sd_snagDecompSnagfallAlpha",
				&m_fSnagfallAlpha, true);

		FillSingleValue(p_oElement, "sd_minSnagBreakHeight", &m_fMinBreakHeight,
				true);
		FillSingleValue(p_oElement, "sd_maxSnagBreakHeight", &m_fMaxBreakHeight,
				true);

		//Make sure min<max and both >0
		if (m_fMinBreakHeight < 0 || m_fMaxBreakHeight < 0
				|| m_fMaxBreakHeight < m_fMinBreakHeight) {
			modelErr stcErr;
			stcErr.sFunction = "clSnagDecomp::GetData";
			stcErr.sMoreInfo =
					"Max break height must be greater than or equal to min break height, and both values must be non-negative.";
			stcErr.iErrorCode = BAD_DATA;
			delete[] p_fTempValues;
			throw(stcErr);
		} //end if

		FillSpeciesSpecificValue(p_oElement, "sd_snagDecompSnagfallBeta",
				"sd_sdsbVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fSnagfallBeta[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;

		mp_fSnagfallGamma[0] = 0;
		mp_fSnagfallGamma[1] = 0;
		FillSingleValue(p_oElement, "sd_snagDecompSnagfallGamma2",
				&mp_fSnagfallGamma[2], true);
		FillSingleValue(p_oElement, "sd_snagDecompSnagfallGamma3",
				&mp_fSnagfallGamma[3], true);
		FillSingleValue(p_oElement, "sd_snagDecompSnagfallGamma4",
				&mp_fSnagfallGamma[4], true);
		FillSingleValue(p_oElement, "sd_snagDecompSnagfallGamma5",
				&mp_fSnagfallGamma[5], true);
		FillSingleValue(p_oElement, "sd_snagDecompSnagfallZeta",
				&m_fSnagfallZeta, true);
		FillSingleValue(p_oElement, "sd_snagDecompSnagfallEta", &m_fSnagfallEta,
				true);
		FillSingleValue(p_oElement, "sd_snagDecompSnagfallKappa",
				&m_fSnagfallKappa, true);
		...
		for (j = 0; j <= 5; j++) {
			...
			for (k = j; k <= 5; k++) {
				if (k == 0) {
					...
					sprintf(cConcatenatedCode, "sd_snagDecomp%sTo%sProb",
							cDCArrayCodes[j], cDCArrayCodes[k]);
					FillSingleValue(p_oElement, cConcatenatedCode,
							&mp_fSnagTransProb[j][k], true);
					...
					if (mp_fSnagTransProb[j][k] < 0.0) {
						...
						stcErr.sFunction = "clSnagDecomp::GetData";
						stcErr.sMoreInfo =
								"Values for transition probabilites cannot be less than 0.";
						stcErr.iErrorCode = BAD_DATA;
						throw(stcErr);
					} //end if
			...
			if (fabs(fCumulativeProb - 1.0) > 0.0001) {
				...
				stcErr.sFunction = "clSnagDecomp::GetData";
				stcErr.sMoreInfo =
						"Sum of transition probabilities must add up to 1 within each column.";
				stcErr.iErrorCode = BAD_DATA;
				throw(stcErr);
			} //end if
			...
			if (fabs((fYearsPerTimestep) - 5.0) > 0.1) {
				...
				for (k = 5; k >= j; k--) {
					...
					p_fProbGEk[k] = 1
							- pow((1 - p_fProbGEk[k]), fYearsPerTimestep / 5.0);
		...
		sprintf(cQueryPiece, "%d,%d", clTreePopulation::adult,
		...
		if ((400 - pow(i - 1, 2) * fPopGridCellArea)
				< (pow(i, 2) * fPopGridCellArea - 400))
			m_fGridCellLength = (i - 1) * pow(fPopGridCellArea, 0.5);
		else
			m_fGridCellLength = i * pow(fPopGridCellArea, 0.5);

		//create the basal area grid
		mp_oBasalAreaGrid = mp_oSimManager->CreateGrid(
				"Snag Decay Class Dynamics Basal Area", //grid name
...
void clSnagDecomp::CalcGridValues() {
	try {
		...
		sprintf(cQuery, "type=%d,%d", clTreePopulation::adult,
				clTreePopulation::snag);
		...
		int iSp, iOldSnag;
		float fDBH, fXPosition, fYPosition, fTempBAHolder; //temp value used in calculating basal area
		...
		short int iNumXCells = mp_oBasalAreaGrid->GetNumberXCells(),
				iNumYCells = mp_oBasalAreaGrid->GetNumberYCells(), i, j; //loop counters
		...
		while (p_oTree) {
			...
			if (p_oTree->GetType() == clTreePopulation::snag)
				p_oTree->GetValue(mp_iSnagDCCode[iSp][clTreePopulation::snag],
						&iOldSnag);
			else
				iOldSnag = 0;
			...
			if (!iOldSnag) {
				...
				p_oTree->GetValue(
						p_oPop->GetDbhCode(iSp, clTreePopulation::adult),
						&fDBH);
				p_oTree->GetValue(
						p_oPop->GetXCode(iSp, clTreePopulation::adult),
						&fXPosition);
				p_oTree->GetValue(
						p_oPop->GetYCode(iSp, clTreePopulation::adult),
						&fYPosition);

				//get the BA so far in that tree's cell
				mp_oBasalAreaGrid->GetValueAtPoint(fXPosition, fYPosition,
						m_iLiveBACode, &fTempBAHolder);

				//add the current tree's BA (converted to per ha) to it
				fTempBAHolder += (clModelMath::CalculateBasalArea(fDBH))
						/ (pow(m_fGridCellLength / 100.0, 2));

				//write the new BA back to the grid
				mp_oBasalAreaGrid->SetValueAtPoint(fXPosition, fYPosition,
						m_iLiveBACode, fTempBAHolder);
		...
		for (i = 0; i < iNumXCells; i++) {
			mp_oBasalAreaGrid->GetValueOfCell(i, iNumYCells - 1, m_iLiveBACode,
					&fTempBAHolder);
			fTempBAHolder *= m_fGridCellLength; //multiply by old length
			fTempBAHolder /= mp_oBasalAreaGrid->GetEndYOfCell(iNumYCells - 1)
					- mp_oBasalAreaGrid->GetOriginYOfCell(iNumYCells - 1); //divide by new length
			mp_oBasalAreaGrid->SetValueOfCell(i, iNumYCells - 1, m_iLiveBACode,
					fTempBAHolder);
		}
		for (j = 0; j < iNumYCells; j++) {
			mp_oBasalAreaGrid->GetValueOfCell(iNumXCells - 1, j, m_iLiveBACode,
					&fTempBAHolder);
			fTempBAHolder *= m_fGridCellLength; //multiply by old length
			fTempBAHolder /= mp_oBasalAreaGrid->GetEndXOfCell(iNumXCells - 1)
					- mp_oBasalAreaGrid->GetOriginXOfCell(iNumXCells - 1); //divide by new length
			mp_oBasalAreaGrid->SetValueOfCell(iNumXCells - 1, j, m_iLiveBACode,
					fTempBAHolder);
		}
		...
		clGrid *p_oHarvestResultsGrid = mp_oSimManager->GetGridObject(
				"Harvest Results");
		...
		float fCellBACut, //temp value used in calculating cut basal area
				...
				fHarvestResultsXPosition, fHarvestResultsYPosition;
		...
		if (p_oHarvestResultsGrid) {
			...
			iNumHarvestResultsXCells = p_oHarvestResultsGrid->GetNumberXCells(),
					iNumHarvestResultsYCells =
							p_oHarvestResultsGrid->GetNumberYCells(), k; //loop counter

			for (i = 0; i < iNumHarvestResultsXCells; i++)
				for (j = 0; j < iNumHarvestResultsYCells; j++) {

					p_oHarvestResultsGrid->GetValueOfCell(i, j,
							p_oHarvestResultsGrid->GetIntDataCode(
									"Harvest Type"), &iCellCut);
					...
					if (iCellCut != -1) {
						...
						for (k = 0;
								k
										< p_oHarvestResultsGrid->GetNumberFloatDataMembers();
								k++) {

							p_oHarvestResultsGrid->GetValueOfCell(i, j, k,
									&fTempCellBACut);
						...
						p_oHarvestResultsGrid->GetPointOfCell(i, j,
								&fHarvestResultsXPosition,
								&fHarvestResultsYPosition);

						//correct basal area if we're on one of the far edges
						mp_oBasalAreaGrid->GetCellOfPoint(
								fHarvestResultsXPosition,
								fHarvestResultsYPosition, &iXCell, &iYCell);

						if (iXCell == (iNumXCells - 1)) {
							fCellBACut *= m_fGridCellLength; //multiply by old length
							fCellBACut /= mp_oBasalAreaGrid->GetEndXOfCell(
									iNumXCells - 1)
									- mp_oBasalAreaGrid->GetOriginXOfCell(
											iNumXCells - 1); //divide by new length
						}
						if (iXCell == (iNumXCells - 1)) {
							fCellBACut *= m_fGridCellLength; //multiply by old length
							fCellBACut /= mp_oBasalAreaGrid->GetEndYOfCell(
									iNumYCells - 1)
									- mp_oBasalAreaGrid->GetOriginYOfCell(
											iNumYCells - 1); //divide by new length
						}

						//add to cut ba
						mp_oBasalAreaGrid->GetValueAtPoint(
								fHarvestResultsXPosition,
								fHarvestResultsYPosition, m_iCutBACode,
								&fTempCellBACut);
						fTempCellBACut += fCellBACut;
						mp_oBasalAreaGrid->SetValueAtPoint(
								fHarvestResultsXPosition,
								fHarvestResultsYPosition, m_iCutBACode,
								fTempCellBACut);
						//add to live ba
						mp_oBasalAreaGrid->GetValueAtPoint(
								fHarvestResultsXPosition,
								fHarvestResultsYPosition, m_iLiveBACode,
								&fTempCellBACut);
						fTempCellBACut += fCellBACut;
						mp_oBasalAreaGrid->SetValueAtPoint(
								fHarvestResultsXPosition,
...
void clSnagDecomp::Action() {
	try {
		...
		float fRandom, //random number
				...
				fBreakHeight, fHeight, fDBH, fXPosition, fYPosition, //snag info
		...
		int iInitCondition, //the snag's starting decay class
				iDead, iNewCondition, //the snag's new decay class for the cell the snag is in
				iCutOrNot; //indicator of cutting in the cell the snag is in

		clGrid *p_oDetailedSubstrateGrid;

		p_oDetailedSubstrateGrid = mp_oSimManager->GetGridObject(
				"DetailedSubstrate");
		...
		while (p_oTree) {
			...
			mp_oBasalAreaGrid->GetValueAtPoint(fXPosition, fYPosition,
					m_iLiveBACode, &fLiveBasalArea);
			mp_oBasalAreaGrid->GetValueAtPoint(fXPosition, fYPosition,
					m_iCutBACode, &fCutBasalArea);
			...
			if (iTp == clTreePopulation::adult) { // tree died this timestep, apply tree fall model
				fFallProb = m_fTreefallAlpha
						+ mp_fTreefallBeta[mp_iIndexes[iSp]]
						+ m_fTreefallDelta * fDBH
						+ m_fTreefallTheta * fLiveBasalArea
						+ (m_fTreefallIota + m_fTreefallLamda * fDBH)
								* iCutOrNot;
				...
				fFallProb = m_fSnagfallAlpha
						+ mp_fSnagfallBeta[mp_iIndexes[iSp]]
			...
			if (fRandom < fFallProb) { //it fell over
				...
				if (iNewCondition > iInitCondition) { // it advanced to a later snag DC
					...
					if (iNewCondition == 5) {
						...
						if (fRandom < fHeight)
							p_oTree->SetValue(mp_iNewBreakCode[iSp][iTp],
									fRandom);
			...
			nextTree: p_oTree = p_oBehaviorTrees->NextTree();
...
void clSnagDecomp::RegisterTreeDataMembers() {
	...
	for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
		...
		if (clTreePopulation::snag != mp_whatSpeciesTypeCombos[i].iType
			...
			stcErr.sMoreInfo =
					"This behavior can only be applied to adults and snags.";
			stcErr.iErrorCode = BAD_DATA;
			throw(stcErr);
		}
