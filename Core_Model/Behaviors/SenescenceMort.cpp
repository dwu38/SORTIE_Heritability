...
void clSenescenceMort::DoShellSetup(xercesc::DOMDocument *p_oDoc) {
	try {
		...
		FillSpeciesSpecificValue(p_oElement, "mo_dbhAtOnsetOfSenescence",
				"mo_daoosVal", mp_fDbhAtOnset, m_iNumBehaviorSpecies, p_oPop,
				true);
		...
		for (i = 0; i < m_iNumSpeciesTypeCombos; i++)
			if (clTreePopulation::sapling != mp_whatSpeciesTypeCombos->iType
					&& clTreePopulation::adult
							!= mp_whatSpeciesTypeCombos->iType) {
				modelErr stcErr;
...
// DoMort()
////////////////////////////////////////////////////////////////////////////
deadCode clSenescenceMort::DoMort(clTree *p_oTree, const float &fDiam,
	...
	if (fIntDbh >= m_iMaxDbh - 1)
		...
		fDeathProb = (mp_fMortProb[iSpecies][(int) fIntDbh]
				+ fFracDbh
						* (mp_fMortProb[iSpecies][(int) fIntDbh + 1]
								- mp_fMortProb[iSpecies][(int) fIntDbh]));
...
void clSenescenceMort::CalculateMortalityProbability() {
	try {
		float fNumberYearsPerTimestep =
				mp_oSimManager->GetNumberOfYearsPerTimestep(), fDeathProb;
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			...
			for (iDbh = 0; iDbh < m_iMaxDbh; iDbh++) {
				...
				fDeathProb =
						exp(
								mp_fRandomAlpha[i].val
										+ mp_fRandomBeta[i].val
												* (iDbh - mp_fDbhAtOnset[i].val))
								/ (1
										+ exp(
												mp_fRandomAlpha[i].val
														+ mp_fRandomBeta[i].val
																* (iDbh
																		- mp_fDbhAtOnset[i].val)));

				mp_fMortProb[iSpecies][iDbh] = 1
						- pow(1 - fDeathProb, fNumberYearsPerTimestep);
			}
		}
	} catch (modelErr &err) {
		throw(err);
