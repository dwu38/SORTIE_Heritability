...
clDisperseBase::~clDisperseBase() {
	...
	delete[] mp_iNumSeedsCode;
	mp_iNumSeedsCode = NULL;
...
void clDisperseBase::TimestepCleanup() {
	try {
		if (m_bHooked) {
			short int iNumXCells = mp_oSeedGrid->GetNumberXCells(), iNumYCells =
					mp_oSeedGrid->GetNumberYCells(), iX, iY, iSp; //loop counters
			float fZero = 0;
	...
	try {
		...
		if (mp_oSeedGrid != NULL) {
			...
			for (i = 0; i < m_iTotalSpecies; i++) {
				sLabel << "seeds_" << i;
				mp_iNumSeedsCode[i] = mp_oSeedGrid->GetFloatDataCode(
						sLabel.str());
				sLabel.str("");
				if (-1 == mp_iNumSeedsCode[i]) {
					...
					stcErr.sMoreInfo =
							"The seed grid was incorrectly set up in the parameter file.  It's missing a seed count data member for each species.";
					throw(stcErr);
				}
			}
			...
			if (-1 == m_iGapCountCode || -1 == m_iIsGapCode) {
				...
				stcErr.sFunction = "clDisperseBase::SetUpBase";
				stcErr.sMoreInfo =
						"The seed grid was incorrectly set up in the parameter file.  It's missing an adult count or gap status data member.";
				throw(stcErr);
			}
		} else {
