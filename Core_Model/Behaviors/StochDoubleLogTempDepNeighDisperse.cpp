...
void clStochDoubleLogTempDepNeighDisperse::DoShellSetup(DOMDocument *p_oDoc) {
	...
	try {
		...
		FillSpeciesSpecificValue(p_oElement, "di_stochDoubLogTempDepNeighPA",
				"di_sdltdnpaVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		...
		FillSpeciesSpecificValue(p_oElement, "di_stochDoubLogTempDepNeighPB",
				"di_sdltdnpbVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		FillSpeciesSpecificValue(p_oElement, "di_stochDoubLogTempDepNeighPM",
				"di_sdltdnpmVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		FillSpeciesSpecificValue(p_oElement, "di_stochDoubLogTempDepNeighA",
				"di_sdltdnaVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		FillSpeciesSpecificValue(p_oElement, "di_stochDoubLogTempDepNeighB",
				"di_sdltdnbVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		FillSingleValue(p_oElement, "di_stochDoubLogTempDepNeighRadius",
				&m_fRadius, true);

		//Original analysis plot size, square meters
		FillSingleValue(p_oElement, "di_stochDoubLogTempDepNeighPlotSize",
				&m_fAnalysisPlotSize, true);

		//Deterministic or Poisson seed distribution
		FillSingleValue(p_oElement, "di_stochDoubLogTempDepNeighDeterministic",
				&m_bDeterministic, false);

		//Use temperature dependent portion of fecundity
		FillSingleValue(p_oElement, "di_stochDoubLogTempDepNeighTempFec",
				&m_bFecTempDep, true);

		//T, in years, to go from cumulative to annualized probability
		FillSingleValue(p_oElement, "di_stochDoubLogTempDepNeighT",
				&m_fAnnualizePeriod, true);

		//If we are using temperature dependent fecundity, get those values
		if (m_bFecTempDep) {
			//Al
			FillSpeciesSpecificValue(p_oElement,
					"di_stochDoubLogTempDepNeighAl", "di_sdltdnalVal",
			...
			//Bl
			FillSpeciesSpecificValue(p_oElement,
					"di_stochDoubLogTempDepNeighBl", "di_sdltdnblVal",
					p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
			//Transfer to the appropriate array buckets
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
			...
			//Cl
			FillSpeciesSpecificValue(p_oElement,
					"di_stochDoubLogTempDepNeighCl", "di_sdltdnclVal",
					p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
			//Transfer to the appropriate array buckets
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
			...
			//Ah
			FillSpeciesSpecificValue(p_oElement,
					"di_stochDoubLogTempDepNeighAh", "di_sdltdnahVal",
					p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
			//Transfer to the appropriate array buckets
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
			...
			//Bh
			FillSpeciesSpecificValue(p_oElement,
					"di_stochDoubLogTempDepNeighBh", "di_sdltdnbhVal",
					p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
			//Transfer to the appropriate array buckets
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
			...
			//Ch
			FillSpeciesSpecificValue(p_oElement,
					"di_stochDoubLogTempDepNeighCh", "di_sdltdnchVal",
					p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
			//Transfer to the appropriate array buckets
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
...
void clStochDoubleLogTempDepNeighDisperse::AddSeeds() {
	float *p_fBAC = NULL, *p_fFecundity = NULL;
	...
	try {
		...
		float
				...
				fTemp, fNumYrs = mp_oSimManager->GetNumberOfYearsPerTimestep(),
				fMeanTempC = p_oPlot->GetMeanAnnualTemp(), fMeanTempK =
						fMeanTempC + 273.15,
				fNeighDbh, //DBH of neighbors for calculating basal area
				//grid cell lengths in the middle of the grid
				fMidX = mp_oSeedGrid->GetLengthXCells(), fMidY =
						mp_oSeedGrid->GetLengthYCells(), fMidArea = fMidX
						* fMidY, //shortcut
				...
				fPCc, fPCa, fCellX, fCellY; //one cell's X and Y dimensions
		short int i, iSpecies, iNeighSpecies, iNeighType, iX, iY;
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			...
			if (false == p_bPresent[iSpecies]) {
				...
				fPCc = mp_fPA[iSpecies]
						* exp(
								-0.5
										* pow(
												(fMeanTempC - mp_fPM[iSpecies])
														/ mp_fPB[iSpecies], 2));
		...
		for (iX = 0; iX < iNumXCells; iX++) {
			for (iY = 0; iY < iNumYCells; iY++) {
				...
				if (iX != iNumXCells - 1 && iY != iNumYCells - 1)
					fArea = fMidArea;
				else {
					if (iX == iNumXCells - 1)
						fCellX = fEndX;
				...
				while (p_oNeighbor) {
					...
					if (clTreePopulation::adult == iNeighType) {
						...
						p_oNeighbor->GetValue(
								p_oPop->GetDbhCode(iNeighSpecies, iNeighType),
								&fNeighDbh);
						p_fBAC[iNeighSpecies] +=
								clModelMath::CalculateBasalArea(fNeighDbh);
					}
					p_oNeighbor = p_oAllNeighbors->NextTree();
				}
				...
				for (i = 0; i < m_iNumBehaviorSpecies; i++) {
					...
					if (p_bPresent[iSpecies]) {
						...
						fNumGridSeeds = mp_fA[iSpecies]
								+ p_fFecundity[iSpecies] * p_fBAC[iSpecies];

						//Poisson draw if appropriate
						if (!m_bDeterministic) {
							fNumGridSeeds = clModelMath::PoissonRandomDraw(
									fNumGridSeeds);
						}
						...
						mp_oSeedGrid->GetValueOfCell(iX, iY,
								mp_iNumSeedsCode[mp_iWhatSpecies[i]], &fTemp);
						fTemp += fNumGridSeeds;
