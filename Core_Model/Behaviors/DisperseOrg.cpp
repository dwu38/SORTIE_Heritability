...
void clDisperseOrg::DoSetup(clSimManager *p_oSimManager,
	...
	try {
		...
		clTreePopulation *p_oPop =
				(clTreePopulation*) p_oSimManager->GetPopulationObject(
						"treepopulation");
...
void clDisperseOrg::SetFunctionPointer() {
	...
	} else if (zeroinflated_poisson_pdf == m_iSeedDistributionMethod) {
		Adjust = &clDisperseOrg::ZeroInflatedPoissonAdjust;
	}
}
...
void clDisperseOrg::GetParameterFileData(clSimManager *p_oSimManager,
	...
	try {
		...
		clPopulationBase *p_oTempPop = p_oSimManager->GetPopulationObject(
				"treepopulation");
		...
		} else if (zeroinflated_poisson_pdf == iTemp) {
			m_iSeedDistributionMethod = zeroinflated_poisson_pdf;
		} else {
			modelErr stcErr;
		...
		//If lognormal, normal, or negative binomial, populate the parameters array (or zero inflated Poisson!)
		if (lognormal_pdf == m_iSeedDistributionMethod
				...
				|| zeroinflated_poisson_pdf == m_iSeedDistributionMethod) {
			...
			for (i = 0; i < m_iNumDispObjects; i++) {
				for (j = 0; j < mp_oDispObjects[i]->GetNumBehaviorSpecies();
						j++) {
					p_bSpeciesUsed[mp_oDispObjects[i]->GetBehaviorSpecies(j)] =
							true;
				}
			}
			...
			if (negative_binomial_pdf == m_iSeedDistributionMethod) {
				FillSpeciesSpecificValue(p_oElement, "di_clumpingParameter",
						"di_cpVal", p_fTemp, iTemp, p_oPop, true);
			} else if (zeroinflated_poisson_pdf == m_iSeedDistributionMethod) {
				FillSpeciesSpecificValue(p_oElement, "di_zeroInflatedP",
						"di_zpVal", p_fTemp, iTemp, p_oPop, true);
			} else {
				FillSpeciesSpecificValue(p_oElement, "di_standardDeviation",
						"di_sdVal", p_fTemp, iTemp, p_oPop, true);
			}
...
void clDisperseOrg::GetDisperseObjects(clSimManager *p_oSimManager) {
	try {
		...
		std::string sBehaviorName, //behavior namestrings
				sShellMarker1 = "disperse", sShellMarker2 = "Disperse";
		short int iNumBehaviors = p_oSimManager->GetNumberOfBehaviors(), iTemp,
				i; //loop counters
		...
		for (i = 0; i < iNumBehaviors; i++) {
			...
			if (std::string::npos != sBehaviorName.find(sShellMarker1)
					|| std::string::npos != sBehaviorName.find(sShellMarker2)) {
				mp_oDispObjects[iTemp] =
						dynamic_cast<clDisperseBase*>(p_oTempBehavior);
				iTemp++;
			}
		}
	} catch (modelErr &err) {
...
void clDisperseOrg::AdjustSeedNumbers() {
	try {
		float fNumSeeds = 0;
		int iNumXCells = mp_oSeedsGrid->GetNumberXCells(), iNumYCells =
				mp_oSeedsGrid->GetNumberYCells(), iX, iY, iSp;

		//Loop through all cells and all species and adjust each
		for (iX = 0; iX < iNumXCells; iX++) {
			for (iY = 0; iY < iNumYCells; iY++) {
				for (iSp = 0; iSp < m_iTotalSpecies; iSp++) {
					mp_oSeedsGrid->GetValueOfCell(iX, iY, mp_iCodes[iSp],
							&fNumSeeds);
					if (fNumSeeds > 0) {
						mp_oSeedsGrid->SetValueOfCell(iX, iY, mp_iCodes[iSp],
								(*this.*Adjust)(fNumSeeds, iSp));
					}
				}
			}
		}
...
float clDisperseOrg::LognormalAdjust(float fNumber, int iSpecies) {
	...
	return clModelMath::LognormalRandomDraw(fNumber,
...
//////////////////////////////////////////////////////////////////////////////
// Zero inflated PoissonAdjust()
/////////////////////////////////////////////////////////////////////////////
float clDisperseOrg::ZeroInflatedPoissonAdjust(float fNumber, int iSpecies) {
	return clModelMath::ZeroInflatedPoissonRandomDraw(fNumber,
			mp_fRandParameter[iSpecies]);
}
...
float clDisperseOrg::NegativeBinomialAdjust(float fNumber, int iSpecies) {
	return clModelMath::NegBinomialRandomDraw(fNumber,
			mp_fRandParameter[iSpecies]);
}
