...
void clVolumeCalculator::GetData(xercesc::DOMDocument *p_oDoc) {
	try {
		...
		FillSpeciesSpecificValue(p_oElement, "vo_taperA", "vo_taVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fTaperA[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;

		//b in the taper equation (also known as a1)
		FillSpeciesSpecificValue(p_oElement, "vo_taperB", "vo_tbVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fTaperB[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;

		//c in the taper equation (also known as a2)
		FillSpeciesSpecificValue(p_oElement, "vo_taperC", "vo_tcVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fTaperC[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;

		//d in the taper equation (also known as b1)
		FillSpeciesSpecificValue(p_oElement, "vo_taperD", "vo_tdVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fTaperD[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;

		//f in the taper equation (also known as b2)
		FillSpeciesSpecificValue(p_oElement, "vo_taperF", "vo_tfVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fTaperF[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;

		//g in the taper equation (also known as b3)
		FillSpeciesSpecificValue(p_oElement, "vo_taperG", "vo_tgVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fTaperG[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;

		//i in the taper equation (also known as b4)
		FillSpeciesSpecificValue(p_oElement, "vo_taperI", "vo_tiVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fTaperI[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;

		//j in the taper equation (also known as b5)
		FillSpeciesSpecificValue(p_oElement, "vo_taperJ", "vo_tjVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fTaperJ[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;

		//k in the taper equation (also known as b6)
		FillSpeciesSpecificValue(p_oElement, "vo_taperK", "vo_tkVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fTaperK[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;

		//a in the math library's AddBarkToDBH() function (also known as a1)
		FillSpeciesSpecificValue(p_oElement, "vo_barkA", "vo_baVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fBarkA[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;

		//b in the math library's AddBarkToDBH() function (also known as a2)
		FillSpeciesSpecificValue(p_oElement, "vo_barkB", "vo_bbVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fBarkB[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;

		//c in the math library's AddBarkToDBH() function (also known as a3)
		FillSpeciesSpecificValue(p_oElement, "vo_barkC", "vo_bcVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fBarkC[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;
		...
		FillSingleValue(p_oElement, "vo_minUsableDiam", &m_fMinUsableDiam,
				true);
		//Make sure it's greater than 0
		...
		FillSingleValue(p_oElement, "vo_segmentLength", &m_fSegmentLength,
				true);
		//Make sure it's greater than 0
...
void clVolumeCalculator::Action() {
	try {
		...
		while (p_oTree) {
			...
			if (-1
					!= mp_iVolumeCodes[mp_iIndexes[iSp]][iTp
				...
				p_oTree->SetValue(
						mp_iVolumeCodes[mp_iIndexes[iSp]][iTp
								- clTreePopulation::sapling], fVolume);
			}
...
void clVolumeCalculator::RegisterTreeDataMembers() {
	...
	for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
		...
		mp_iVolumeCodes[mp_iIndexes[mp_whatSpeciesTypeCombos[i].iSpecies]][mp_whatSpeciesTypeCombos[i].iType
				- clTreePopulation::sapling] = p_oPop->RegisterFloat("Volume",
...
float clVolumeCalculator::GetTreeVolume(const float &fTreeHeight,
		const float &fDBH, const int &iSpecies) {
	...
	float fX, fP, fQ, fZ, fTaper1, //taper equation first term
			...
			fOneThird = 0.333333, fXExp1, //Xi exponent pieces that we can calculate ahead of time
	...
	if (fTreeHeight <= m_fStumpHeight)
		return fVolume;

	//Calculate values that only need to be calculated once
	fP = 1.3 / fTreeHeight;
	fOBDBH = clModelMath::AddBarkToDBH(fDBH, mp_fBarkA[iSpInd],
			mp_fBarkB[iSpInd], mp_fBarkC[iSpInd]);
	fXExp1 = mp_fTaperF[iSpInd] * (1 / exp(fOBDBH / fTreeHeight))
			+ mp_fTaperI[iSpInd] * (1 / fOBDBH);
	fTaper1 = mp_fTaperA[iSpInd] * pow(fOBDBH, mp_fTaperB[iSpInd])
			* pow(fTreeHeight, mp_fTaperC[iSpInd]);
	...
	fXExp2 = mp_fTaperD[iSpInd] * pow(fZ, 4) + mp_fTaperG[iSpInd] * pow(fX, 0.1)
	...
	for (i = 0; i < iNumSegments; i++) {
		...
		fVolume += (M_PI
				* (pow(fBeginDiam * 0.005, 2) + pow(fEndDiam * 0.005, 2)) * 0.5
				* fSegmentLength);
