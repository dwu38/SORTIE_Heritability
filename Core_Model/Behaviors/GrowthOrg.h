 ...
 * expectation that another growth object will explicitly update the diameter.
...
protected:
	...
	/**
	 * Return codes for size class where to apply growth to each type/species combo.
	 * Array size is # species by # types
	 */
	short int **mp_iSizeClassCodes;
	...
	/**
	 * Queries for the return code for size class for each type/species combo.  Return
	 * codes are captured in the mp_iSizeClassCodes array.
	 *
	 * @throw Error if there is no code for any tree which uses growth.
	 */
	void GetSizeClassVariableCodes();
