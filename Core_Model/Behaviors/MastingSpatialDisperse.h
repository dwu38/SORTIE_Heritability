...
public:
	...
	mastEvent GetMastEvent(int iSp) {
		if (mp_iIndexes[iSp] > -1)
			return mp_iEvent[mp_iIndexes[iSp]];
		else
			return nonmast;
	}
	;
	...
	int GetTimestepsSinceLastMast(int iSp) {
		return mp_iTimestepsSinceLastMast[iSp];
	}
	;
...
protected:
	...
	typedef float (clMastingSpatialDisperse::*Ptr2GetNumberOfSeeds)(
			const float&, const short int&);
	 ...
	 * @param p_oPop Tree Population object
	 */
	void DisperseOneParentSeeds(clTree *p_oTree, clTreePopulation *p_oPop,
	...
	void CalculateProbabilityDistribution(float *p_fProbArray,
			const int &iMaxDistance, const function &iFunction,
	 ...
	 * @return Number of seeds.
	 */
	float GetNumberOfSeedsDrawLognormal(const float &fDbh,
			const short int &iSp);
