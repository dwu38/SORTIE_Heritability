...
void clGLIMap::SetUpGrid() {
	...
	if (!mp_oMapGrid) {
		...
		mp_oMapGrid = mp_oSimManager->CreateGrid(sMapName.str().c_str(), 0, 1,
				0, 0);
		...
		if (-1 == m_iGridGliCode) {
			...
			stcErr.sMoreInfo =
					"\"GLI Map\" grid was incorrectly set up in the parameter file.  Missing float \"GLI\".";
			throw(stcErr);
		}
	}
...
void clGLIMap::DoSetupCalculations() {
	...
	clTreePopulation *p_oPop =
			(clTreePopulation*) mp_oSimManager->GetPopulationObject(
					"treepopulation");
	m_fMaxSearchDistance = (mp_oLightOrg->GetMaxTreeHeight() - m_fLightHeight)
			* m_fRcpTanMinAng
...
void clGLIMap::ReadParameterFileData(xercesc::DOMDocument *p_oDoc) {
	...
	if (0 > m_fLightHeight) {
		...
		stcErr.sFunction = "clGLIMap::ReadParameterFileData";
		stcErr.sMoreInfo =
				"The height of the GLI point for the GLI map cannot be less than 0.";
		throw(stcErr);
	}
...
void clGLIMap::Action() {
	try {
		...
		int iNumXCells = mp_oMapGrid->GetNumberXCells(), iNumYCells =
				mp_oMapGrid->GetNumberYCells(), iX, iY, i, j; //loop counters
		...
		for (iX = 0; iX < iNumXCells; iX++) {
			for (iY = 0; iY < iNumYCells; iY++) {
				...
				sprintf(cQuery, "%s%f%s%f%s%f%s%f", "distance=",
						m_fMaxSearchDistance, " FROM x=", fX, "y=", fY,
				...
				while (p_oNeighbor != NULL) {
					...
					if (clTreePopulation::seedling != p_oNeighbor->GetType())
						...
						AddTreeToGliFishEye(fX, fY, m_fLightHeight, p_oNeighbor,
								p_oPlot, p_oPop, p_oAllom);
