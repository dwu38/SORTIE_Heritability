...
void clMichMenNegGrowth::RegisterTreeDataMembers() {
	try {
		...
		for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
			...
			mp_iAutoCorrCodes[mp_iIndexes[mp_whatSpeciesTypeCombos[i].iSpecies]][mp_whatSpeciesTypeCombos[i].iType] =
					p_oPop->RegisterFloat("autocorr",
...
void clMichMenNegGrowth::DoShellSetup(DOMDocument *p_oDoc) {
	...
	try {
		...
		FillSpeciesSpecificValue(p_oElement, "gr_mmNegGrowthAlpha",
				"gr_mmngaVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer values to our permanent array
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			mp_fAlpha[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;
		}

		//Beta
		FillSpeciesSpecificValue(p_oElement, "gr_mmNegGrowthBeta",
				"gr_mmngbVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		...
		FillSpeciesSpecificValue(p_oElement, "gr_mmNegGrowthGamma",
				"gr_mmnggVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer values to our permanent array
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			mp_fGamma[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;
		}

		//Phi
		FillSpeciesSpecificValue(p_oElement, "gr_mmNegGrowthPhi", "gr_mmngpVal",
		...
		FillSpeciesSpecificValue(p_oElement, "gr_mmNegGrowthStdDev",
				"gr_mmngsdVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer values to our permanent array
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			mp_fStdDev[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;
		}

		//one year probability of autocorrelation
		FillSpeciesSpecificValue(p_oElement, "gr_mmNegGrowthAutoCorrProb",
				"gr_mmngacpVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer values to our permanent array
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			if (p_fTempValues[i].val < 0 || p_fTempValues[i].val > 1) {
				...
				stcErr.sFunction = "clMichMenNegGrowth::DoShellSetup";
				stcErr.sMoreInfo =
						"Autocorrelation probability must be between 0 and 1.";
				throw(stcErr);
			}
			mp_fProbAutoCorr[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;
		}
...
float clMichMenNegGrowth::CalcHeightGrowthValue(clTree *p_oTree,
	...
	short int iSpecies = p_oTree->GetSpecies(), iType = p_oTree->GetType(),
			iLightCode = mp_oGrowthOrg->GetLightCode(iSpecies, iType), ind =
					mp_iIndexes[iSpecies], iYr;
	...
	for (iYr = 0; iYr < m_fYearsPerTimestep; iYr++) {
		if (mp_fStdDev[ind] > 0) {
			if (clModelMath::GetRand() > mp_fProbAutoCorr[ind])
				...
				fStochasticFactor = clModelMath::NormalRandomDraw(
						mp_fStdDev[ind]);
		}
