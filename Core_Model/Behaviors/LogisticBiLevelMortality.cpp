...
void clLogisticBiLevelMortality::DoShellSetup(xercesc::DOMDocument *p_oDoc) {
	...
	try {
		...
		FillSpeciesSpecificValue(p_oElement, "mo_logBilevLoLiteA",
				"mo_lbllaVal", p_fTempValues, m_iNumBehaviorSpecies, mp_oPop,
				true);
		//Transfer values to our permanent array
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fLoLightA[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;

		//Low-light growth "b"
		FillSpeciesSpecificValue(p_oElement, "mo_logBilevLoLiteB",
				"mo_lbllbVal", p_fTempValues, m_iNumBehaviorSpecies, mp_oPop,
				true);
		//Transfer values to our permanent array
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fLoLightB[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;
		...
		if (NULL != mp_oStormLight) {
			...
			FillSpeciesSpecificValue(p_oElement, "mo_logBilevHiLiteThreshold",
					"mo_lbhltVal", p_fTempValues, m_iNumBehaviorSpecies,
					mp_oPop, true);
			...
			for (i = 0; i < m_iNumBehaviorSpecies; i++) {
				if (p_fTempValues[i].val < 0 || p_fTempValues[i].val > 100) {
					...
					stcErr.sFunction =
							"clLogisticBiLevelMortality::DoShellSetup";
					stcErr.sMoreInfo =
			...
			FillSpeciesSpecificValue(p_oElement, "mo_logBilevHiLiteA",
					"mo_lbhlaVal", p_fTempValues, m_iNumBehaviorSpecies,
					mp_oPop, true);
			//Transfer values to our permanent array
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
			...
			FillSpeciesSpecificValue(p_oElement, "mo_logBilevHiLiteB",
					"mo_lbhlbVal", p_fTempValues, m_iNumBehaviorSpecies,
					mp_oPop, true);
			//Transfer values to our permanent array
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
...
deadCode clLogisticBiLevelMortality::DoMort(clTree *p_oTree, const float &fDiam,
	...
	if (fLightLevel < mp_fHiLightThreshold[mp_iIndexes[iSpecies]])
		fSurvivalProb = exp(
				mp_fLoLightA[mp_iIndexes[iSpecies]]
						+ mp_fLoLightB[mp_iIndexes[iSpecies]] * fMortDiam);
	else
		fSurvivalProb = exp(
				mp_fHiLightA[mp_iIndexes[iSpecies]]
						+ mp_fHiLightB[mp_iIndexes[iSpecies]] * fMortDiam);
