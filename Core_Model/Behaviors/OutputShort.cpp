...
clShortOutput::~clShortOutput() {
	...
	for (i = 0; i < m_iNumSubplotsToSave; i++) {
		delete[] mp_subplots[i].p_cellList;
...
void clShortOutput::ExtractLiveTreeInfo(xercesc::DOMDocument *p_oDoc) {
	try {
		...
		p_oTypeList = p_oDoc->getElementsByTagName(
				XMLString::transcode("so_treeTypeInfo"));
		...
		for (i = 0; i < iNumTypes; i++) {
			...
			cData =
					XMLString::transcode(
							p_oTreeInfo->getAttributeNode(
									XMLString::transcode("type"))->getNodeValue());
...
void clShortOutput::ExtractDeadTreeInfo(xercesc::DOMDocument *p_oDoc) {
	try {
		...
		DOMElement *p_oTreeInfo; //one tree type's info
		short int iNumTypes, //how many types we have output data for
				iType, iDeadReason, i, j; //loop counter
		char *cData; //extracting text data
		...
		//Go through each type and break out its info
		p_oTypeList = p_oDoc->getElementsByTagName(
				XMLString::transcode("so_deadTreeTypeInfo"));
		iNumTypes = p_oTypeList->getLength();
		for (i = 0; i < iNumTypes; i++) {
			p_oDocNode = p_oTypeList->item(i);
			p_oTreeInfo = (DOMElement*) p_oDocNode;
			...
			cData = XMLString::transcode(
					p_oTreeInfo->getAttributeNode(
							XMLString::transcode("reason"))->getNodeValue());
			...
			//Check the data save tags
			mp_bSaveDeadABA[iType][iDeadReason] = TestForSave(p_oTreeInfo,
					"so_saveABA");
			mp_bSaveDeadADN[iType][iDeadReason] = TestForSave(p_oTreeInfo,
					"so_saveADN");
...
void clShortOutput::ExtractSubplotInfo(xercesc::DOMDocument *p_oDoc) {
	try {
		...
		p_oSubplotList = p_oDoc->getElementsByTagName(
				XMLString::transcode("so_subplot"));
		...
		for (i = 0; i < m_iNumSubplotsToSave; i++) {
			...
			//Get subplot name
			FillSingleValue(p_oSubplot, "so_subplotName",
			...
			p_oGridCellList = p_oElement->getElementsByTagName(
					XMLString::transcode("po_point"));
			...
			for (j = 0; j < iNumCells; j++) {
				...
				bDuplicate = false;
				for (m = 0; m < j; m++)
					if (iX == p_gridListForVal[m].iX
							&& iY == p_gridListForVal[m].iY)
						bDuplicate = true;
				if (!bDuplicate) { //our values are valid and not duplicates
...
bool clShortOutput::TestForSave(DOMElement *p_oParentElement,
	...
	p_oNodeList = p_oParentElement->getElementsByTagName(
			XMLString::transcode(cNodeName));
	...
	cData =
			XMLString::transcode(
					p_oElement->getAttributeNode(XMLString::transcode("save"))->getNodeValue());
	if (strcmp("true", cData) == 0)
		return true;
...
void clShortOutput::WriteFileHeader() {
	try {
		...
		char cABAString[80], cABATotalString[80], cRBAString[80],
				cADNString[80], cADNTotalString[80], cRDNString[80],
				cABARoot[80], cABATotalRoot[80], cADNRoot[80],
				cADNTotalRoot[80];
		...
		for (i = 0; i < m_iNumTypes; i++) {
			...
			if (mp_bSaveLiveABA[i]) {
				for (j = 0; j < m_iNumSpecies; j++)
					out << "\t" << cABAString
							<< p_oPop->TranslateSpeciesCodeToName(j);
			...
			for (j = 0; j <= remove_tree; j++) {
				...
				if (mp_bSaveDeadABA[i][j]) {
					for (k = 0; k < m_iNumSpecies; k++)
						out << "\t" << cABAString
								<< p_oPop->TranslateSpeciesCodeToName(k);
	...
	try {
		...
		if (0 == m_iNumSubplotsToSave) {
			...
			while (p_oTree) {
				...
				if (mp_bSaveAnyLive[iTypeIndex]) {
					...
					if (mp_bSaveAnyLiveBA[iTypeIndex]) { //Basal area
						//Get the dbh
						p_oTree->GetValue(
								p_oTrees->GetDbhCode(iSpecies, iTypeIndex),
								&fDbh);
			...
			while (p_oTree) {
				...
				if (mp_bSaveAnyLive[iTypeIndex]) {
					...
					p_oTree->GetValue(p_oTrees->GetXCode(iSpecies, iTypeIndex),
							&fCoord);
					iTreeX = (int) floor(fCoord / m_fXCellLength);
					p_oTree->GetValue(p_oTrees->GetYCode(iSpecies, iTypeIndex),
							&fCoord);
					...
					for (iSubplotIndex = 0;
							iSubplotIndex < m_iNumSubplotsToSave;
							iSubplotIndex++) {
						for (i = 0; i < mp_subplots[iSubplotIndex].iNumCells;
								i++)
							if (iTreeX
									== mp_subplots[iSubplotIndex].p_cellList[i].iX
									&& iTreeY
											== mp_subplots[iSubplotIndex].p_cellList[i].iY) {
								if (mp_bSaveAnyLiveBA[iTypeIndex])
									mp_fSubABA[iSubplotIndex][iTypeIndex][iSpecies] +=
											fBA;
		...
		for (iTypeIndex = 0; iTypeIndex < m_iNumTypes; iTypeIndex++) {
			...
			if (mp_bSaveLiveRBA[iTypeIndex]) {
				...
				for (i = 0; i < m_iNumSpecies; i++) {
					if (fTotal > 0) {
						mp_fLiveRBA[iTypeIndex][i] = mp_fLiveABA[iTypeIndex][i]
								/ fTotal;
					} else {
				...
				//Now do subplots
				for (iSubplotIndex = 0; iSubplotIndex < m_iNumSubplotsToSave;
						iSubplotIndex++) {
					fTotal = 0;
					for (i = 0; i < m_iNumSpecies; i++)
					...
					for (i = 0; i < m_iNumSpecies; i++) {
						if (fTotal > 0) {
							mp_fSubRBA[iSubplotIndex][iTypeIndex][i] =
									mp_fSubABA[iSubplotIndex][iTypeIndex][i]
											/ fTotal;
						} else {
			...
			if (mp_bSaveLiveRDN[iTypeIndex]) {
				...
				for (i = 0; i < m_iNumSpecies; i++) {
					if (fTotal > 0) {
						mp_fLiveRDN[iTypeIndex][i] = mp_fLiveADN[iTypeIndex][i]
								/ fTotal;
					} else {
					...
					for (i = 0; i < m_iNumSpecies; i++) {
						if (fTotal > 0) {
							mp_fSubRDN[iSubplotIndex][iTypeIndex][i] =
									mp_fSubADN[iSubplotIndex][iTypeIndex][i]
											/ fTotal;
						} else {
...
void clShortOutput::GetDeadTreeStats() {
	try {
		...
		clTreePopulation *p_oTrees =
				(clTreePopulation*) mp_oSimManager->GetPopulationObject(
						"treepopulation");
		clGhostTreePopulation *p_oGhostTrees =
		...
		while (p_oTree) {
			...
			if (mp_bSaveDeadADN[iTypeIndex][iDeadIndex]
				...
				if (mp_bSaveDeadABA[iTypeIndex][iDeadIndex]) { //Basal area
					//Get the dbh
					p_oTree->GetValue(
							p_oTrees->GetDbhCode(iSpecies, iTypeIndex), &fDbh);
					//Calculate basal area and add to array total
					fBA = clModelMath::CalculateBasalArea(fDbh);
...
void clShortOutput::WriteTimestepData() {
	try {
			...
			for (iDeadReason = 0; iDeadReason <= remove_tree; iDeadReason++) {
				if (mp_bSaveDeadABA[iType][iDeadReason]) {
					...
					for (iSpecies = 0; iSpecies < m_iNumSpecies; iSpecies++) {
						...
						out << "\t"
								<< mp_fDeadABA[iType][iSpecies][iDeadReason];
					}
					//Total line
					out << "\t" << fTotal;
				}
				...
				if (mp_bSaveDeadADN[iType][iDeadReason]) {
					...
					for (iSpecies = 0; iSpecies < m_iNumSpecies; iSpecies++) {
						...
						out << "\t"
								<< mp_fDeadADN[iType][iSpecies][iDeadReason];
					}
					//Total line
					out << "\t" << fTotal;
				}
		...
		for (iSubplot = 0; iSubplot < m_iNumSubplotsToSave; iSubplot++) {
			...
			for (iType = 0; iType < m_iNumTypes; iType++) {
				for (iDeadReason = 0; iDeadReason <= remove_tree;
						iDeadReason++) {
					if (mp_bSaveDeadABA[iType][iDeadReason]) {
						for (iSpecies = 0; iSpecies < m_iNumSpecies;
								iSpecies++) {
							out << "\tNA";
						}
						//Total line
