...
// ReadParameterFile()
////////////////////////////////////////////////////////////////////////////
void clLaggedPostHarvestGrowth::ReadParameterFile(
	...
	try {
		...
		for (i = 0; i < m_iNumSpeciesTypeCombos; i++)
			if (clTreePopulation::sapling != mp_whatSpeciesTypeCombos[i].iType
				...
				stcErr.sFunction =
						"clLaggedPostHarvestGrowth::ReadParameterFile";
				stcErr.sMoreInfo =
		...
		FillSpeciesSpecificValue(p_oElement, "gr_lagMaxGrowthConstant",
				"gr_lmgcVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fMaxGrowthConstant[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;

		//Effect of DBH on max potential growth term
		FillSpeciesSpecificValue(p_oElement, "gr_lagMaxGrowthDbhEffect",
				"gr_lmgdbheVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fMaxGrowthDbhEffect[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;

		//Effect of NCI on growth
		FillSpeciesSpecificValue(p_oElement, "gr_lagNciConstant", "gr_lncicVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fNciConstant[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;

		//Effect of DBH on NCI term
		FillSpeciesSpecificValue(p_oElement, "gr_lagNciDbhEffect",
				"gr_lncidbheVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fNciDbhEffect[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;

		//Rate parameter for time since harvest lag effect
		FillSpeciesSpecificValue(p_oElement, "gr_lagTimeSinceHarvestRateParam",
				"gr_ltshrpVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fTimeSinceHarvestRateParam[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;

		//NCI radius
		FillSingleValue(p_oElement, "gr_lagNciDistanceRadius",
				&m_fNciDistanceRadius, true);
	...
	try {
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			...
			if (mp_fMaxGrowthConstant[i] < 0) {
				modelErr stcErr;
				stcErr.sMoreInfo =
						"All values for maximum growth constant must be non-negative.";
				throw(stcErr);
			}
			...
			if (mp_fMaxGrowthDbhEffect[i] <= 0) {
				modelErr stcErr;
				stcErr.sMoreInfo =
						"All values for maximum growth dbh effect must be non-negative.";
				throw(stcErr);
			}
			...
			if (mp_fNciConstant[i] <= 0) {
				modelErr stcErr;
				stcErr.sMoreInfo =
						"All values for NCI constant must be non-negative.";
				throw(stcErr);
			}
			...
			if (mp_fNciDbhEffect[i] <= 0) {
				modelErr stcErr;
				stcErr.sMoreInfo =
						"All values for NCI dbh effect must be non-negative.";
				throw(stcErr);
			}
		...
		if (m_fNciDistanceRadius <= 0) {
			modelErr stcErr;
			stcErr.sMoreInfo =
					"The NCI distance radius must be greater than 0.";
			throw(stcErr);
		}
...
void clLaggedPostHarvestGrowth::GetTreeMemberCodes() {
	...
	for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
		...
		mp_iGrowthCodes[mp_iIndexes[mp_whatSpeciesTypeCombos[i].iSpecies]][mp_whatSpeciesTypeCombos[i].iType
				- clTreePopulation::sapling] = mp_oGrowthOrg->GetGrowthCode(
...
void clLaggedPostHarvestGrowth::AssembleUniqueTypes() {
	...
	for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
		...
		if (!bFound) {
			...
			//by one
			p_iTypesList[m_iNumBehaviorTypes] =
					mp_whatSpeciesTypeCombos[i].iType;
			m_iNumBehaviorTypes++;
		}
...
void clLaggedPostHarvestGrowth::SetNameData(std::string sNameString) {
	try {
		...
		} else if (sNameString.compare("LaggedPostHarvestGrowth diam only")
				== 0) {
...
float clLaggedPostHarvestGrowth::CalcDiameterGrowthValue(clTree *p_oTree,
	...
	p_oTree->GetValue(
			mp_iGrowthCodes[mp_iIndexes[p_oTree->GetSpecies()]][p_oTree->GetType()
					- clTreePopulation::sapling], &fAmountDiamIncrease);
...
void clLaggedPostHarvestGrowth::PreGrowthCalcs(clTreePopulation *p_oPop) {
	try {
		...
		sprintf(cQueryPiece, "%d",
				mp_iWhatBehaviorTypes[m_iNumBehaviorTypes - 1]);
		strcat(cQuery, cQueryPiece);
		...
		while (p_oTree) {
			...
			if (-1
					!= mp_iGrowthCodes[mp_iIndexes[iSpecies]][iType
							- clTreePopulation::sapling]) {

				//Make sure tree's not dead
				iDeadCode = p_oPop->GetBoolDataCode("dead",
				...
				if (!bIsDead) {
					...
					fLocalNCI =
							LocalBasalAreaAroundTree(p_oTree)
									* exp(
											-1.0
													* mp_fNciDbhEffect[mp_iIndexes[iSpecies]]
													* fDbh);

					iTimeSinceHarvest = GetTimeSinceHarvest(p_oTree);

					fMaxGrowth =
							mp_fMaxGrowthConstant[mp_iIndexes[iSpecies]]
									* exp(
											-1.0
													* mp_fMaxGrowthDbhEffect[mp_iIndexes[iSpecies]]
													* fDbh);
					...
					fPostHarvestGrowth =
							fMaxGrowth
									* exp(
											-1.0
													* mp_fNciConstant[mp_iIndexes[iSpecies]]
													* fLocalNCI);
					...
					if (iTimeSinceHarvest >= 1000) { //no harvest yet
						...
						p_oTree->GetValue(
								...
								&fPreHarvestGrowth);
						p_oTree->SetValue(
								mp_iPreHarvGrowthCodes[iSpecies][iType],
								fPreHarvestGrowth);
								...
								fPreHarvestGrowth
										+ (fPostHarvestGrowth
												...
												* (1.0
														- exp(
																-1.0
																		* mp_fTimeSinceHarvestRateParam[mp_iIndexes[iSpecies]]
																		* iTimeSinceHarvest));
...
float clLaggedPostHarvestGrowth::LocalBasalAreaAroundTree(clTree *p_oTree) {
	...
	float fTreeX, fTreeY, //the target tree's location
			fDbh, fBasalArea = 0.0; //NCI within crowding radius
	...
	sprintf(cQuery, "distance=%4.4f FROM x=%4.4f,y=%4.4f::height=1.35",
			m_fNciDistanceRadius, fTreeX, fTreeY);
...
void clLaggedPostHarvestGrowth::SetupTimeSinceHarvestGrid() {
	try {
		...
		stcErr.iErrorCode = UNKNOWN;
		stcErr.sFunction =
				"clLaggedPostHarvestGrowth::SetupTimeSinceHarvestGrid";
		throw(stcErr);
	}
}
