...
void clRandomBrowse::GetData(xercesc::DOMDocument *p_oDoc) {
	...
	try {
		...
		int iNumTypes = p_oPop->GetNumberOfTypes(), i, iTemp;
		...
		FillSpeciesSpecificValue(p_oElement, "di_randBrowseProb", "di_rbpVal",
		...
		if (normal_pdf == m_iPDF) {
			FillSpeciesSpecificValue(p_oElement, "di_randBrowseStdDev",
					"di_rbsdVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
					true);
		...
		for (i = 0; i < m_iNumSpecies; i++) {
			...
			mp_fBrowseProb[i] = 1
					- pow(1 - mp_fBrowseProb[i], fYearsPerTimestep);
		}
