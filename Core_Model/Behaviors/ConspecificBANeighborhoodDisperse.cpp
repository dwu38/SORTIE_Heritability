...
void clConspecificBANeighborhoodDisperse::DoShellSetup(DOMDocument *p_oDoc) {
	...
	try {
		...
		//A
		FillSpeciesSpecificValue(p_oElement, "di_conspBANeighA", "di_cbnaVal",
		...
		//B
		FillSpeciesSpecificValue(p_oElement, "di_conspBANeighB", "di_cbnbVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			...
			for (iY = 0; iY < iNumYCells; iY++) {
				...
				for (i = 0; i < m_iNumBehaviorSpecies; i++) {
					...
					fNumGridSeeds = mp_fA[iSpecies]
							+ mp_fB[iSpecies] * p_fBAC[iSpecies];
