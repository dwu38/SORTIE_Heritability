	...
	try {
		clTreePopulation *p_oPop =
				(clTreePopulation*) mp_oSimManager->GetPopulationObject(
						"treepopulation");
		short int iNumTotalSpecies = p_oPop->GetNumberOfSpecies(), i; //loop counters
		...
		for (i = 0; i < m_iNumSpeciesTypeCombos; i++)
			if (clTreePopulation::adult != mp_whatSpeciesTypeCombos[i].iType) {
				...
				stcErr.sFunction =
						"clQualityVigorClassifier::RegisterTreeDataMembers";
				stcErr.sMoreInfo =
						"This behavior can only be applied to adults.";
				throw(stcErr);
			}
		...
		for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
			...
			mp_iVigorousInd[i] = p_oPop->RegisterBool("vigorous",
					mp_whatSpeciesTypeCombos[i].iSpecies,
					mp_whatSpeciesTypeCombos[i].iType);

			mp_iSawlogInd[i] = p_oPop->RegisterBool("sawlog",
					mp_whatSpeciesTypeCombos[i].iSpecies,
					mp_whatSpeciesTypeCombos[i].iType);

			mp_iTreeclassInd[i] = p_oPop->RegisterInt("treeclass",
					mp_whatSpeciesTypeCombos[i].iSpecies,
					mp_whatSpeciesTypeCombos[i].iType);
		}
	} catch (modelErr &err) {
...
// ReadParameterFile
///////////////////////////////////////////////////////////////////////////////
void clQualityVigorClassifier::ReadParameterFile(DOMDocument *p_oDoc,
		clTreePopulation *p_oPop) {
	...
	try {
		...
		FillSpeciesSpecificValue(p_oElement, "ma_classifierVigBeta0",
				"ma_cvb0Val", p_fTemp, m_iNumBehaviorSpecies, p_oPop, true);
		...
		FillSpeciesSpecificValue(p_oElement,
				"ma_classifierNewAdultProbVigorous", "ma_cnapvVal", p_fTemp,
		...
		FillSpeciesSpecificValue(p_oElement, "ma_classifierDeciduous",
				"ma_cdVal", p_bTemp, m_iNumBehaviorSpecies, p_oPop, true);
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
...
void clQualityVigorClassifier::AssignClass(clTreePopulation *p_oPop,
	...
	if (mp_bDeciduous[iSpInd]) {
		...
		bSawlog = false; //no quality for conifers
	...
	if (mp_bDeciduous[iSpInd]) {
		if (bVigorous) {
			...
			if (bSawlog) {
				p_oTree->GetValue(
						p_oPop->GetDbhCode(p_oTree->GetSpecies(),
...
void clQualityVigorClassifier::EvolveVigorQuality(clTreePopulation *p_oPop,
	...
	switch (iClass) {
	case 1:
		fBeta1 = mp_fVigBeta11[iSpInd];
		break;
	case 2:
		fBeta1 = mp_fVigBeta12[iSpInd];
		break;
	case 3:
		fBeta1 = mp_fVigBeta13[iSpInd];
		break;
	case 4:
		fBeta1 = mp_fVigBeta14[iSpInd];
		break;
	case 5:
		fBeta1 = mp_fVigBeta15[iSpInd];
		break;
	case 6:
		fBeta1 = mp_fVigBeta16[iSpInd];
		break;
	default:
		modelErr stcErr;
		stcErr.iErrorCode = BAD_DATA;
	...
	if (mp_bDeciduous[iSpInd]) {
		...
		switch (iClass) {
		case 1:
			fBeta1 = mp_fQualBeta11[iSpInd];
			break;
		case 2:
			fBeta1 = mp_fQualBeta12[iSpInd];
			break;
		case 3:
			fBeta1 = mp_fQualBeta13[iSpInd];
			break;
		case 4:
			fBeta1 = mp_fQualBeta14[iSpInd];
			break;
		default:
			modelErr stcErr;
			stcErr.iErrorCode = BAD_DATA;
...
///////////////////////////////////////////////////////////////////////////////
void clQualityVigorClassifier::HandleInitialConditionsTrees(DOMDocument *p_oDoc,
		clTreePopulation *p_oPop) {
	floatVal *p_fTemp = NULL; //for getting species-specific values
	...
	try {
		...
		XMLCh *sVal;
		float fDBH, fTemp; //workhorse variable
		...
		if (0 == p_oNodeList->getLength()) {
			...
			for (i = 0; i < iNumClasses; i++) {
				...
				FillSingleValue(p_oElement, "ma_classifierBeginDBH", &fTemp,
						true);
				p_fMinDBH[i] = fTemp;

				FillSingleValue(p_oElement, "ma_classifierEndDBH", &fTemp,
						true);
				p_fMaxDBH[i] = fTemp;

				FillSpeciesSpecificValue(p_oElement,
						"ma_classifierProbVigorous", "ma_cpvVal", p_fTemp,
						m_iNumBehaviorSpecies, p_oPop, true);
				for (j = 0; j < m_iNumBehaviorSpecies; j++)
					p_fPropVig[i][j] = p_fTemp[j].val;

				FillSpeciesSpecificValue(p_oElement, "ma_classifierProbSawlog",
						"ma_cpsVal", p_fTemp, m_iNumBehaviorSpecies, p_oPop,
						true);
				for (j = 0; j < m_iNumBehaviorSpecies; j++)
		...
		while (p_oTree) {
			...
			if (iClass == 0) {
				...
				if (!bFound) {
					//Vigor:
					bVal = clModelMath::GetRand()
							<= mp_fNewAdultProbVigorous[iSpInd];
					p_oTree->SetValue(mp_iVigorousInd[iSpInd], bVal);

					//Sawlog:
					if (mp_bDeciduous[iSpInd]) {
						bVal = clModelMath::GetRand()
								<= mp_fNewAdultProbSawlog[iSpInd];
						p_oTree->SetValue(mp_iSawlogInd[iSpInd], bVal);
					}
				}
