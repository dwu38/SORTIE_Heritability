...
void clPostHarvestSkiddingMort::DoShellSetup(xercesc::DOMDocument *p_oDoc) {
	...
	FillSpeciesSpecificValue(p_oElement, "mo_postHarvPreHarvestBackgroundMort",
			"mo_phphbmVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);

	//Transfer to the appropriate array buckets
	for (i = 0; i < m_iNumBehaviorSpecies; i++) {
		mp_fPreHarvestBackgroundMort[p_fTempValues[i].code] =
				p_fTempValues[i].val;
		...
		if (mp_fPreHarvestBackgroundMort[p_fTempValues[i].code] < 0) {
			...
			stcErr.sFunction = "clPostHarvestSkiddingMort::DoShellSetup";
			stcErr.sMoreInfo =
					"The Post-harvest skidding mortality parameter mo_preHarvestBackgroundMort must be non-negative.";
			throw(stcErr);
		}
	}
	...
	FillSpeciesSpecificValue(p_oElement, "mo_postHarvWindthrowHarvestBasicProb",
			"mo_phwhbpVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);

	//Transfer to the appropriate array buckets
	for (i = 0; i < m_iNumBehaviorSpecies; i++) {
		mp_fWindthrowHarvestBasicProb[p_fTempValues[i].code] =
				p_fTempValues[i].val;
		...
		if (mp_fWindthrowHarvestBasicProb[p_fTempValues[i].code] < 0) {
			...
			stcErr.sFunction = "clPostHarvestSkiddingMort::DoShellSetup";
			stcErr.sMoreInfo =
					"The Post-harvest skidding mortality parameter mo_windthrowHarvestBasicProb must be non-negative.";
			throw(stcErr);
		}
	}
	...
	FillSpeciesSpecificValue(p_oElement,
			"mo_postHarvSnagRecruitHarvestBasicProb", "mo_phsrhbpVal",
			p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);

	//Transfer to the appropriate array buckets
	for (i = 0; i < m_iNumBehaviorSpecies; i++) {
		mp_fSnagRecruitHarvestBasicProb[p_fTempValues[i].code] =
				p_fTempValues[i].val;
		...
		if (mp_fSnagRecruitHarvestBasicProb[p_fTempValues[i].code] < 0) {
			...
			stcErr.sFunction = "clPostHarvestSkiddingMort::DoShellSetup";
			stcErr.sMoreInfo =
					"The Post-harvest skidding mortality parameter mo_snagRecruitHarvestBasicProb must be non-negative.";
			throw(stcErr);
		}
	}
	...
	FillSpeciesSpecificValue(p_oElement, "mo_postHarvWindthrowSizeEffect",
			"mo_phwseVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
	...
	for (i = 0; i < m_iNumBehaviorSpecies; i++) {
		...
		if (mp_fWindthrowSizeEffect[p_fTempValues[i].code] < 0) {
			...
			stcErr.sFunction = "clPostHarvestSkiddingMort::DoShellSetup";
			stcErr.sMoreInfo =
					"The Post-harvest skidding mortality parameter mo_windthrowSizeEffect must be non-negative.";
			throw(stcErr);
		}
	}
	...
	FillSpeciesSpecificValue(p_oElement,
			"mo_postHarvWindthrowHarvestIntensityEffect", "mo_phwhieVal",
			p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);

	//Transfer to the appropriate array buckets
	for (i = 0; i < m_iNumBehaviorSpecies; i++) {
		mp_fWindthrowHarvestIntensityEffect[p_fTempValues[i].code] =
				p_fTempValues[i].val;
		...
		if (mp_fWindthrowHarvestIntensityEffect[p_fTempValues[i].code] < 0) {
			...
			stcErr.sFunction = "clPostHarvestSkiddingMort::DoShellSetup";
			stcErr.sMoreInfo =
					"The Post-harvest skidding mortality parameter mo_windthrowHarvestIntensityEffect must be non-negative.";
			throw(stcErr);
		}
	}
	...
	FillSpeciesSpecificValue(p_oElement,
			"mo_postHarvSnagRecruitHarvestIntensityEffect", "mo_phsrhieVal",
			p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);

	//Transfer to the appropriate array buckets
	for (i = 0; i < m_iNumBehaviorSpecies; i++) {
		mp_fSnagRecruitHarvestIntensityEffect[p_fTempValues[i].code] =
				p_fTempValues[i].val;
		...
		if (mp_fSnagRecruitHarvestIntensityEffect[p_fTempValues[i].code] < 0) {
			...
			stcErr.sFunction = "clPostHarvestSkiddingMort::DoShellSetup";
			stcErr.sMoreInfo =
					"The Post-harvest skidding mortality parameter mo_snagRecruitHarvestIntensityEffect must be non-negative.";
			throw(stcErr);
		}
	}
	...
	FillSpeciesSpecificValue(p_oElement, "mo_postHarvWindthrowCrowdingEffect",
			"mo_phwceVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);

	//Transfer to the appropriate array buckets
	for (i = 0; i < m_iNumBehaviorSpecies; i++) {
		mp_fWindthrowCrowdingEffect[p_fTempValues[i].code] =
				p_fTempValues[i].val;
		...
		if (mp_fWindthrowCrowdingEffect[p_fTempValues[i].code] < 0) {
			...
			stcErr.sFunction = "clPostHarvestSkiddingMort::DoShellSetup";
			stcErr.sMoreInfo =
					"The Post-harvest skidding mortality parameter mo_windthrowCrowdingEffect must be non-negative.";
			throw(stcErr);
		}
	}
	...
	FillSpeciesSpecificValue(p_oElement, "mo_postHarvSnagRecruitCrowdingEffect",
			"mo_phsrceVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);

	//Transfer to the appropriate array buckets
	for (i = 0; i < m_iNumBehaviorSpecies; i++) {
		mp_fSnagRecruitCrowdingEffect[p_fTempValues[i].code] =
				p_fTempValues[i].val;
		...
		if (mp_fSnagRecruitCrowdingEffect[p_fTempValues[i].code] < 0) {
			...
			stcErr.sFunction = "clPostHarvestSkiddingMort::DoShellSetup";
			stcErr.sMoreInfo =
					"The Post-harvest skidding mortality parameter mo_snagRecruitCrowdingEffect must be non-negative.";
			throw(stcErr);
		}
	}
	...
	FillSpeciesSpecificValue(p_oElement, "mo_postHarvWindthrowHarvestRateParam",
			"mo_phwhrpVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);

	//Transfer to the appropriate array buckets
	for (i = 0; i < m_iNumBehaviorSpecies; i++) {
		mp_fWindthrowHarvestRateParam[p_fTempValues[i].code] =
				p_fTempValues[i].val;
		...
		if (mp_fWindthrowHarvestRateParam[p_fTempValues[i].code] < 0) {
			...
			stcErr.sFunction = "clPostHarvestSkiddingMort::DoShellSetup";
			stcErr.sMoreInfo =
					"The Post-harvest skidding mortality parameter mo_windthrowHarvestRateParam must be non-negative.";
			throw(stcErr);
		}
	}
	...
	FillSpeciesSpecificValue(p_oElement,
			"mo_postHarvSnagRecruitHarvestRateParam", "mo_phsrhrpVal",
			p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);

	//Transfer to the appropriate array buckets
	for (i = 0; i < m_iNumBehaviorSpecies; i++) {
		mp_fSnagRecruitHarvestRateParam[p_fTempValues[i].code] =
				p_fTempValues[i].val;
		...
		if (mp_fSnagRecruitHarvestRateParam[p_fTempValues[i].code] < 0) {
			...
			stcErr.sFunction = "clPostHarvestSkiddingMort::DoShellSetup";
			stcErr.sMoreInfo =
					"The Post-harvest skidding mortality parameter mo_snagRecruitHarvestRateParam must be non-negative.";
			throw(stcErr);
		}
	}
	...
	FillSpeciesSpecificValue(p_oElement, "mo_postHarvWindthrowBackgroundProb",
			"mo_phwbpVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);

	//Transfer to the appropriate array buckets
	for (i = 0; i < m_iNumBehaviorSpecies; i++) {
		mp_fWindthrowBackgroundProb[p_fTempValues[i].code] =
				p_fTempValues[i].val;
		...
		if (mp_fWindthrowBackgroundProb[p_fTempValues[i].code] < 0) {
			...
			stcErr.sFunction = "clPostHarvestSkiddingMort::DoShellSetup";
			stcErr.sMoreInfo =
					"The Post-harvest skidding mortality parameter mo_windthrowBackgroundProb must be non-negative.";
			throw(stcErr);
		}
	}
	...
	FillSpeciesSpecificValue(p_oElement, "mo_postHarvSnagRecruitBackgroundProb",
			"mo_phsrbpVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);

	//Transfer to the appropriate array buckets
	for (i = 0; i < m_iNumBehaviorSpecies; i++) {
		mp_fSnagRecruitBackgroundProb[p_fTempValues[i].code] =
				p_fTempValues[i].val;
		...
		if (mp_fSnagRecruitBackgroundProb[p_fTempValues[i].code] < 0) {
			...
			stcErr.sFunction = "clPostHarvestSkiddingMort::DoShellSetup";
			stcErr.sMoreInfo =
					"The Post-harvest skidding mortality parameter mo_snagRecruitBackgroundProb must be non-negative.";
			throw(stcErr);
		}
	}
	...
	if (m_fCrowdingEffectRadius <= 0) {
		...
		stcErr.sFunction = "clPostHarvestSkiddingMort::DoShellSetup";
		stcErr.sMoreInfo =
				"The Post-harvest skidding mortality parameter mo_crowdingEffectRadius must be greater than zero.";
		throw(stcErr);
	}
	...
	for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
		...
		mp_iHarvestIntensityCodes[mp_whatSpeciesTypeCombos[i].iSpecies][mp_whatSpeciesTypeCombos[i].iType] =
				p_oPop->GetFloatDataCode("HarvInten",
...
// DoMort()
////////////////////////////////////////////////////////////////////////////
deadCode clPostHarvestSkiddingMort::DoMort(clTree *p_oTree, const float &fDbh,
	...
	int i, iType = p_oTree->GetType(), iTimeSinceHarvest; //time since last harvest for this grid cell
	...
	if (iTimeSinceHarvest < 1000) { //there has been harvesting in this grid cell to date
		...
		if (mp_iHarvestIntensityCodes[iSpecies][iType] > -1) {
			p_oTree->GetValue(mp_iHarvestIntensityCodes[iSpecies][iType],
					&fHarvestIntensity);
		} else {
			fHarvestIntensity = 0;
		}
		...
		for (i = 0; i < m_fNumberYearsPerTimestep; i++) {
			...
			fAnnualWindthrowRisk = fImmediateWindthrowRisk
					* exp(
							-1.0 * mp_fWindthrowHarvestRateParam[iSpecies]
									* iTimeSinceHarvest)
					+ mp_fWindthrowBackgroundProb[iSpecies];

			fAnnualSnagRecruitRisk = fImmediateSnagRecruitRisk
					* exp(
							-1.0 * mp_fSnagRecruitHarvestRateParam[iSpecies]
									* iTimeSinceHarvest)
		...
		fBackgroundMort = 1.0
				- pow(1.0 - mp_fPreHarvestBackgroundMort[iSpecies],
...
float clPostHarvestSkiddingMort::LocalBasalAreaAroundTree(clTree *p_oTree) {
	...
	float fTreeX, fTreeY, //the target tree's location
			fDbh, fBasalArea = 0.0; //total basal area within crowding radius
	...
	sprintf(cQuery, "distance=%4.4f FROM x=%4.4f,y=%4.4f::height=1.35",
			m_fCrowdingEffectRadius, fTreeX, fTreeY);
	...
	while (p_oNeighborTree) { //loop through trees within crowding radius
		...
		if ((p_oNeighborTree != p_oTree)
				&& (p_oNeighborTree->GetType() == clTreePopulation::adult)) {
			//if the tree is an adult and isn't the focal tree itself

			//add it to the calculated basal area
			p_oNeighborTree->GetValue(
					p_oPop->GetDbhCode(p_oNeighborTree->GetSpecies(),
							p_oNeighborTree->GetType()), &fDbh);
...
void clPostHarvestSkiddingMort::CalcTimeSinceHarvest() {
	try {
		...
		if (mp_oHarvestResultsGrid) { //if there is cutting in this simulation
			...
			mp_oTimeSinceHarvestGrid->GetValueOfCell(0, 0, m_iLastUpdated,
					&iLastUpdated);
			...
			if (iLastUpdated < iCurrentTimestep) { //if the grid hasn't yet been updated
				...
				for (i = 0; i < m_iNumXCells; i++)
					for (j = 0; j < m_iNumYCells; j++) {
						...
						mp_oHarvestResultsGrid->GetValueOfCell(i, j,
								m_iHarvestTypeCode, &iHarvestedThisTimestep);
						...
						if (iHarvestedThisTimestep != -1) {
							mp_oTimeSinceHarvestGrid->SetValueOfCell(i, j,
									m_iTimeCode, 0);
							...
							mp_oTimeSinceHarvestGrid->GetValueOfCell(i, j,
									m_iTimeCode, &iNewTimeSinceHarvest);
				...
				mp_oTimeSinceHarvestGrid->SetValueOfCell(0, 0, m_iLastUpdated,
						iCurrentTimestep);
...
void clPostHarvestSkiddingMort::SetupTimeSinceHarvestGrid() {
	try {
		...
		mp_oTimeSinceHarvestGrid = mp_oSimManager->GetGridObject(
				"Years Since Last Harvest");

		if (!mp_oTimeSinceHarvestGrid) {
			//if it doesn't, then create it
			mp_oTimeSinceHarvestGrid = mp_oSimManager->CreateGrid(
					"Years Since Last Harvest", //grid name
			...
			m_iLastUpdated = mp_oTimeSinceHarvestGrid->RegisterInt(
					"LastUpdated");
			...
			for (i = 0; i < m_iNumXCells; i++)
				for (j = 0; j < m_iNumYCells; j++) {
					mp_oTimeSinceHarvestGrid->SetValueOfCell(i, j, m_iTimeCode,
							1000);
			...
			m_iLastUpdated = mp_oTimeSinceHarvestGrid->GetIntDataCode(
					"LastUpdated");

			//figure out its size
			m_iNumXCells = mp_oTimeSinceHarvestGrid->GetNumberXCells(), m_iNumYCells =
					mp_oTimeSinceHarvestGrid->GetNumberYCells();

		}

		//Get a pointer to the harvest results grid
		mp_oHarvestResultsGrid = mp_oSimManager->GetGridObject(
				"Harvest Results");

		//if harvest results grid exists, get the code for harvest type
		if (mp_oHarvestResultsGrid)
			m_iHarvestTypeCode = mp_oHarvestResultsGrid->GetPackageIntDataCode(
					"Harvest Type");
		...
		stcErr.iErrorCode = UNKNOWN;
		stcErr.sFunction =
				"clPostHarvestSkiddingMort::SetupTimeSinceHarvestGrid";
		throw(stcErr);
	}
}
...
int clPostHarvestSkiddingMort::GetTimeSinceHarvest(clTree *p_oTree) {
	...
	int iSp = p_oTree->GetSpecies(), iTp = p_oTree->GetType(), iTime;
	...
	if (!mp_oTimeSinceHarvestGrid)
		...
		mp_oTimeSinceHarvestGrid->GetValueAtPoint(fTreeX, fTreeY, m_iTimeCode,
				&iTime);
		return iTime;
