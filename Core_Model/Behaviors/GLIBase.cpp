...
void clGLIBase::AddTreeToGliFishEye(const float &fTargetX,
	...
	try {
		...
		p_oNeighbor->GetValue(p_oPop->GetHeightCode(iSpecies, iType),
				&fNeighHeight);
...
void clGLIBase::GLIFisheyeNoCanopyIntersect(const float &fTargetX,
		const float &fTargetY, const float &fNeighX, const float &fNeighY,
		const float &fNeighCanrad, const float &fNeighHeight,
	...
	try {
		...
		if (m_fSinMinSunAng
				> fHeightDiff
						/ sqrt(
								(fHeightDiff * fHeightDiff)
										+ (fDistToNearEdge * fDistToNearEdge)))
			return;
		...
		while (1) {
			...
			fC = fYDist * fYDist + fXDist * fXDist
					- fNeighCanrad * fNeighCanrad;
			fC = fB * fB - 4.0 * fA * fC;
			...
			if (fC >= 0.0) {
				...
				iAltStart = (int) (fCrownBaseDiffChunkConv
						/ sqrt(
								(double) (fSquareOfFarDist
										+ fCrownBaseDiffSquared)));
				if (iAltStart < m_iMinAngRow)
					iAltStart = m_iMinAngRow;

				iAltEnd =
						(int) (fHeightDiffChunkConv
								/ sqrt(
										(double) (fSquareOfNearDist
												+ fHeightDiffSquared)));
				if (iAltEnd >= m_iNumAltAng)
					iAltEnd = m_iNumAltAng - 1;
			...
			else if (iAziChunkIncrementer == 1)
				return;
...
void clGLIBase::GLIFisheyeCanopyIntersect(const float &fTargetX,
	...
	try {
		...
		fCCoeff = (fXDist * fXDist) + (fYDist * fYDist)
				- (fNeighCanrad * fNeighCanrad);
		...
		if (fCrownBase <= fTargetHeight) { //neighbor blocks whole sky
			...
			for (i = 0; i < iHalfAzi; i++) {
				...
				for (j = 0; j < 2; j++) {
					...
					fDistToFarEdge = (fXSolution * fXSolution)
							+ (fYSolution * fYSolution);
					iAltStart =
							(int) (floor(
									((fHeightDiff * m_iNumAltAng)
											/ sqrt(
													(fDistToFarEdge)
															+ (fHeightDiff
																	* fHeightDiff)))));
