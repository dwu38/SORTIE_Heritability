...
void clStorm::SetStormProbability(int iReturnInterval, float fValue) {
	if (iReturnInterval < 0 || iReturnInterval >= m_iNumSeverityClasses) {
		...
		std::stringstream s;
		s << "Bad return interval argument to SetStormProbability: "
				<< iReturnInterval;
		stcErr.sMoreInfo = s.str();
		throw(stcErr);
	}
	...
	try {
		...
		if (stochastic == m_iStochasticity) {
			...
			if (lognormal == iTemp) {
				...
				std::stringstream s;
				s
						<< "Unrecognized value for damage pattern probability distribution function: "
						<< iTemp;
				stcErr.sMoreInfo = s.str();
				stcErr.iErrorCode = BAD_DATA;
				throw(stcErr);
			...
			if (normal == m_iDistribution || lognormal == m_iDistribution) {
				FillSingleValue(p_oDocElement, "st_standardDeviation",
						&m_fStdDev, true);
			}
		}
		...
		if (fabs(m_fSineD) > VERY_SMALL_VALUE
				&& fabs(m_fSSTPeriod) < VERY_SMALL_VALUE) {
			modelErr stcErr;
			stcErr.sFunction = "clStorm::ReadParFile";
		...
		FillSingleValue(p_oDocElement, "st_stmTrendSlopeM", &m_fTrendSlopeM,
				true);

		//Trend function intercept (i)
		FillSingleValue(p_oDocElement, "st_stmTrendInterceptI",
				&m_fTrendInterceptI, true);
		...
		for (i = 0; i < m_iNumScheduledStorms; i++) {
			if (mp_stormsList[i].fMin > mp_stormsList[i].fMax) {
				...
				stcErr.sFunction = "clStorm::ReadParFile";
				stcErr.sMoreInfo =
						"For scheduled storms, minimum cannot be greater than maximum.";
				stcErr.iErrorCode = BAD_DATA;
				throw(stcErr);
			}
			if (mp_stormsList[i].fMin > 1.0 || mp_stormsList[i].fMin < 0.0) {
				...
				stcErr.sFunction = "clStorm::ReadParFile";
				stcErr.sMoreInfo =
						"For scheduled storms, minimum must be between 0 and 1.";
				stcErr.iErrorCode = BAD_DATA;
				throw(stcErr);
			}
			if (mp_stormsList[i].fMax > 1.0 || mp_stormsList[i].fMax < 0.0) {
				...
				stcErr.sFunction = "clStorm::ReadParFile";
				stcErr.sMoreInfo =
						"For scheduled storms, maximum must be between 0 and 1.";
				stcErr.iErrorCode = BAD_DATA;
				throw(stcErr);
			}
		}
...
void clStorm::DoGridSetup() {
	try {
		...
		if (mapped == m_iSusceptibility) {
			mp_oSusceptibilityMap = mp_oSimManager->GetGridObject(
					"Storm Susceptibility");
			if ( NULL == mp_oSusceptibilityMap) {
				modelErr stcErr;
				stcErr.sFunction = "clStorm::DoGridSetup";
				stcErr.sMoreInfo =
						"A grid map for the grid \"Storm Susceptibility\" is required for this file.";
				stcErr.iErrorCode = BAD_DATA;
				throw(stcErr);
			}
			...
			//doesn't exist
			m_iSusceptIndexCode = mp_oSusceptibilityMap->GetFloatDataCode(
					"index");
			if (-1 == m_iSusceptIndexCode) {
				modelErr stcErr;
				stcErr.sFunction = "clStorm::DoGridSetup";
				stcErr.sMoreInfo =
						"The grid map for the grid \"Storm Susceptibility\" is missing the data member\"index\".";
				stcErr.iErrorCode = BAD_DATA;
				throw(stcErr);
			}
		}
		...
		if ( NULL != mp_oStormGrid) {
			...
			if ( NULL != mp_oSusceptibilityMap) {
				if (mp_oSusceptibilityMap->GetNumberXCells()
						!= mp_oStormGrid->GetNumberXCells()
						|| mp_oSusceptibilityMap->GetNumberYCells()
								!= mp_oStormGrid->GetNumberYCells()) {
					modelErr stcErr;
					stcErr.sFunction = "clStorm::DoGridSetup";
					stcErr.sMoreInfo =
							"The grid cell resolutions for \"Storm Susceptibility\" and \"Storm Damage\" must match.";
					stcErr.iErrorCode = BAD_DATA;
					throw(stcErr);
				}
			}

			m_i1DmgIndexCode = mp_oStormGrid->GetPackageFloatDataCode(
					"1dmg_index");
			...
			if (-1 == m_i1DmgIndexCode) {
				...
				stcErr.sFunction = "clStorm::DoGridSetup";
				stcErr.sMoreInfo =
						"The grid map for the grid \"Storm Damage\" is missing the data member\"1dmg_index\".";
				stcErr.iErrorCode = BAD_DATA;
				throw(stcErr);
			}
			if (-1 == m_iDmgIndexCode) {
				...
				stcErr.sFunction = "clStorm::DoGridSetup";
				stcErr.sMoreInfo =
						"The grid map for the grid \"Storm Damage\" is missing the data member\"dmg_index\".";
				stcErr.iErrorCode = BAD_DATA;
				throw(stcErr);
			}
			if (-1 == m_iStormTimeCode) {
				...
				stcErr.sFunction = "clStorm::DoGridSetup";
				stcErr.sMoreInfo =
						"The grid map for the grid \"Storm Damage\" is missing the data member\"stormtime\".";
				stcErr.iErrorCode = BAD_DATA;
				throw(stcErr);
			}
		} else {
			...
			m_i1DmgIndexCode = mp_oStormGrid->RegisterPackageFloat(
					"1dmg_index");
...
void clStorm::Action() {
	try {
		float fNumberYearsPerTimestep =
				mp_oSimManager->GetNumberOfYearsPerTimestep(),
				fCurrentTimestep = mp_oSimManager->GetCurrentTimestep(), fX, //for calculating effects of cyclicity
		...
		if (m_iNumScheduledStorms > 0) {
			for (i = 0; i < m_iNumScheduledStorms; i++) {
				if (mp_stormsList[i].iTS == iTS) {
					...
					fMeanSeverity = ((mp_stormsList[i].fMax
							- mp_stormsList[i].fMin) * clModelMath::GetRand())
							+ mp_stormsList[i].fMin;
		...
		if (fabs(m_fSineD) > VERY_SMALL_VALUE) {
			fX = 4 * ((fCurrentTimestep * fNumberYearsPerTimestep))
					/ m_fSSTPeriod;
		...
		for (i = 0; i < m_iNumSeverityClasses; i++) {
			for (j = 0; j < fNumberYearsPerTimestep; j++) {
				if (clModelMath::GetRand()
						<= mp_fStormProbabilities[i] * fCyclicity) {
					//Get the storm's mean severity
					fMeanSeverity = ((float) i / 10)
							+ (clModelMath::GetRand() / 10);
					ApplyDamage(fMeanSeverity);
					bStorms = true;
				}
			}
		...
		if (uniform == m_iSusceptibility) {
			...
			for (iX = 0; iX < iNumXCells; iX++) {
				for (iY = 0; iY < iNumYCells; iY++) {
					...
					mp_oSusceptibilityMap->GetValueOfCell(iX, iY,
							m_iSusceptIndexCode, &fSusceptibility);
...
void clStorm::AdjustTimeSinceLastStormCounter(bool bStormThisTimestep) {
	float fCounter, fNumYearsPerTimestep =
			mp_oSimManager->GetNumberOfYearsPerTimestep();
	int iX, iY, iNumXCells = mp_oStormGrid->GetNumberXCells(), iNumYCells =
			mp_oStormGrid->GetNumberYCells();
...
void clStorm::DoSeverityAverages() {
	...
	float fAvg, fVal;
	int iX, iY, iNumXCells = mp_oStormGrid->GetNumberXCells(), iNumYCells =
			mp_oStormGrid->GetNumberYCells(), iPkgCount;
