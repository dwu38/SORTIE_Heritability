/*
 * ClimateSeedlingHeightGrowth.cpp
 *
 *  Created on: Mar 14, 2018
 *      Author: ucmuser
 */
#include "ClimateSeedlingHeightGrowth.h"
...
// Constructor
/////////////////////////////////////////////////////////////////////////////*/
clClimateSeedlingHeightGrowth::clClimateSeedlingHeightGrowth( clSimManager * p_oSimManager ) :
     ...
     clGrowthBase( p_oSimManager )
{
  ...
  {
  mp_fInter = NULL;
	mp_fXSm = NULL; //added seedling size 0
  mp_fSm = NULL;
  mp_fMd = NULL;
  mp_fLg = NULL;
  mp_fJMxC = NULL;
  ...
  // mp_oGrid = NULL;
    ...
    m_sNameString = "ClimateSeedlingHeightGrowthshell";
    m_sXMLRoot = "ClimateSeedlingHeightGrowth";
  }
  catch ( modelErr & err )
  {
    throw( err );
  ...
  catch ( ... )
  {
    ...
    stcErr.sFunction = "clClimateSeedlingHeightGrowth::clClimateSeedlingHeightGrowth" ;
    throw( stcErr );
  }
}
...
// Destructor
/////////////////////////////////////////////////////////////////////////////*/
clClimateSeedlingHeightGrowth::~clClimateSeedlingHeightGrowth()
{
  delete[] mp_fInter;
	delete[] mp_fXSm; //added seedling size 0
  delete[] mp_fSm;
  delete[] mp_fMd;
  delete[] mp_fLg;
  delete[] mp_fJMxC;
...
// DoShellSetup()
/////////////////////////////////////////////////////////////////////////////*/
void clClimateSeedlingHeightGrowth::DoShellSetup( DOMDocument * p_oDoc )
{
  ...
  try
  {
	...
	mp_fXSm = new float[m_iNumBehaviorSpecies]; //added seedling size 0
    ...
    FillSpeciesSpecificValue( p_oElement, "gr_SeedlingsHeightInter",
        "gr_shiVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true );
    //Transfer values to our permanent array
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
    	mp_fInter[mp_iIndexes[p_fTempValues[i].code]] = p_fTempValues[i].val;


	//Xtra Small seedlings
	FillSpeciesSpecificValue(p_oElement, "gr_SeedlingsHeightXSm",
		"gr_shxsmVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true ); //added seedling size 0
	//Transfer values to our permanent array
	for (i = 0; i < m_iNumBehaviorSpecies; i++)
		mp_fXSm[mp_iIndexes[p_fTempValues[i].code]] = p_fTempValues[i].val;


    // Small seedlings parameter
    FillSpeciesSpecificValue( p_oElement, "gr_SeedlingsHeightSm",
        "gr_shsmVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true );
    //Transfer values to our permanent array
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
    ...
    FillSpeciesSpecificValue( p_oElement, "gr_SeedlingsHeightMd",
        "gr_shmdVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true );
    //Transfer values to our permanent array
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
    ...
    FillSpeciesSpecificValue( p_oElement, "gr_SeedlingsHeightLg",
        "gr_shlgVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true );
    //Transfer values to our permanent array
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
    ...
    FillSpeciesSpecificValue( p_oElement, "gr_SeedlingsHeightJMxC",
        "gr_shjmxcVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true );
    //Transfer values to our permanent array
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
    ...
    FillSpeciesSpecificValue( p_oElement, "gr_SeedlingsHeightPC",
        "gr_shpcVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true );
    //Transfer values to our permanent array
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
    ...
    FillSpeciesSpecificValue( p_oElement, "gr_SeedlingsHeightBA",
        "gr_shbaVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true );
    //Transfer values to our permanent array
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
  ...
  catch ( ... )
  {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    stcErr.sFunction = "clClimateSeedlingHeightGrowth::DoShellSetup" ;
    throw( stcErr );
  }
}
...
// SetNameData()
/////////////////////////////////////////////////////////////////////////////*/
void clClimateSeedlingHeightGrowth::SetNameData(std::string sNameString) {
 try {

   //Check the string passed and set the flags accordingly
   if (sNameString.compare("ClimateSeedlingHeightGrowth") == 0) {
     m_iGrowthMethod = diameter_auto;
   } else if (sNameString.compare("ClimateSeedlingHeightGrowth diam only") == 0) {
     ...
     stcErr.sFunction = "clClimateSeedlingHeightGrowth::SetNameData";
...
// GetBAT
////////////////////////////////////////////////////////////////////////////
float clClimateSeedlingHeightGrowth::GetBAT(float &fX, float &fY, clTreePopulation *p_oPop)
{
...
/////////////////////////////////////////////////////////////////////////////*/
float clClimateSeedlingHeightGrowth::CalcDiameterGrowthValue(clTree *p_oTree, clTreePopulation *p_oPop, float fHeightGrowth) {

  clPlot *p_oPlot = mp_oSimManager->GetPlotObject();
  float fX, fY,fDiam,result,
  ...
  if (fDiam < 0.01){
	  fDBHEffect = mp_fXSm[iSp]; //added seedling size 0
  }
  else if (fDiam < 0.25){
  ...
  fTransProb = 1 / ( 1 + exp ( - (mp_fInter[iSp] + fDBHEffect +
  ...
  if (clModelMath::GetRand() >= fTransProb)
  {
	  ...
	  if (fDiam < 0.01){
		  result = clModelMath::GetRand() * (0.01-fDiam); //added seedling size 0
	  }
	  else if (fDiam < 0.25){
  ...
  else
  {
	  ...
	  if (fDiam < 0.01){
	  		  // growth between 0.01-diam and 0.25-diam
	  		  result = clModelMath::GetRand() * (0.25 - 0.01) + 0.01-fDiam; //added seedling size 0
	  }
	  else if (fDiam < 0.25){
