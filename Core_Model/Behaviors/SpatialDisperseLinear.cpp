...
//---------------------------------------------------------------------------
#include "SpatialDisperseLinear.h"
//---------------------------------------------------------------------------
...
// Constructor
///////////////////////////////////////////////////////////////////////////////
clSpatialDispersalLinear::clSpatialDispersalLinear(clSimManager *p_oSimManager) :
		clWorkerBase(p_oSimManager), clBehaviorBase(p_oSimManager), clDisperseBase(
				p_oSimManager) {
	try {
		m_sNameString = "spatialdisperselinear";
		m_iNumFunctions = 3;
		m_iNumCovers = 2;
		...
		stcErr.sFunction = "clSpatialDispersalLinear::clSpatialDispersalLinear";
		throw(stcErr);
	}
}
...
// Destructor
///////////////////////////////////////////////////////////////////////////////
clSpatialDispersalLinear::~clSpatialDispersalLinear() {
...
// DeclareArrays
///////////////////////////////////////////////////////////////////////////////
void clSpatialDispersalLinear::DeclareArrays() {
	int i, j, k;
...
// DoShellSetup
///////////////////////////////////////////////////////////////////////////////
void clSpatialDispersalLinear::DoShellSetup(xercesc::DOMDocument *p_oDoc) {
	try {
		...
		for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
			if (clTreePopulation::adult == mp_whatSpeciesTypeCombos[i].iType)
				bAdults = true;
			else if (clTreePopulation::sapling
					== mp_whatSpeciesTypeCombos[i].iType)
				bSaplings = true;
			else if (clTreePopulation::stump
					== mp_whatSpeciesTypeCombos[i].iType)
				m_bStumps = true;
		}
		...
		modelErr stcErr;
		stcErr.iErrorCode = UNKNOWN;
		stcErr.sFunction = "clSpatialDispersalLinear::DoShellSetup";
		throw(stcErr);
	}
}
...
// PopulateUsedTable()
///////////////////////////////////////////////////////////////////////////////
void clSpatialDispersalLinear::PopulateUsedTable(clTreePopulation *p_oPop) {
	...
	for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
		...
		if (mp_whatSpeciesTypeCombos[i].iType != clTreePopulation::sapling
				...
				&& mp_whatSpeciesTypeCombos[i].iType
						!= clTreePopulation::stump) {
			modelErr stcErr;
			stcErr.sFunction = "clSpatialDisperseLinear::PopulateUsedTable";
			stcErr.sMoreInfo =
...
///////////////////////////////////////////////////////////////////////////////
void clSpatialDispersalLinear::GetParameterFileData(
	...
	try {
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			if (p_iTemp[i].val < 0 || p_iTemp[i].val >= m_iNumFunctions) {
				...
				stcErr.iErrorCode = BAD_DATA;
				stcErr.sFunction =
						"clSpatialDispersalLinear::GetParameterFileData";
				std::stringstream s;
				s << "Invalid disperse function code " << p_iTemp[i].val;
				stcErr.sMoreInfo = s.str();
				throw(stcErr);
			} else {
				mp_iWhatFunction[canopy][mp_iIndexes[mp_iWhatSpecies[i]]] =
		...
		if (iNumFunctionSpecies > 0) {
			...
			for (i = 0; i < iNumFunctionSpecies; i++)
				mp_fDispersalX0[weibull][canopy][mp_iIndexes[p_fTemp[i].code]] =
						p_fTemp[i].val;
			...
			//Transfer to permanent array
			for (i = 0; i < iNumFunctionSpecies; i++) {
				mp_fThetaXb[weibull][canopy][mp_iIndexes[p_fTemp[i].code]] =
						p_fTemp[i].val;
				if (p_fTemp[i].val >= 50) {
					modelErr stcErr;
					stcErr.iErrorCode = BAD_DATA;
			...
			//Beta
			FillSpeciesSpecificValue(p_oElement, "di_weibullCanopyBeta",
					"di_wcbVal", p_fTemp, iNumFunctionSpecies, p_oPop, true);
			//Transfer to permanent array
			for (i = 0; i < iNumFunctionSpecies; i++) {
				mp_fBeta[weibull][canopy][mp_iIndexes[p_fTemp[i].code]] =
						p_fTemp[i].val;
				if (p_fTemp[i].val > 25) {
					modelErr stcErr;
					stcErr.iErrorCode = BAD_DATA;
			...
			//STR
			FillSpeciesSpecificValue(p_oElement, "di_weibullCanopySTR",
					"di_wcsVal", p_fTemp, iNumFunctionSpecies, p_oPop, true);
			//Transfer to permanent array
			for (i = 0; i < iNumFunctionSpecies; i++)
				mp_fStr[weibull][canopy][mp_iIndexes[p_fTemp[i].code]] =
						p_fTemp[i].val;
		...
		if (iNumFunctionSpecies > 0) {
			...
			//Transfer to permanent array
			for (i = 0; i < iNumFunctionSpecies; i++)
				mp_fDispersalX0[lognormal][canopy][mp_iIndexes[p_fTemp[i].code]] =
						p_fTemp[i].val;
			...
			//Transfer to permanent array
			for (i = 0; i < iNumFunctionSpecies; i++)
				mp_fThetaXb[lognormal][canopy][mp_iIndexes[p_fTemp[i].code]] =
						p_fTemp[i].val;

			//Beta
			FillSpeciesSpecificValue(p_oElement, "di_lognormalCanopyBeta",
					"di_lcbVal", p_fTemp, iNumFunctionSpecies, p_oPop, true);
			//Transfer to permanent array
			for (i = 0; i < iNumFunctionSpecies; i++) {
				mp_fBeta[lognormal][canopy][mp_iIndexes[p_fTemp[i].code]] =
						p_fTemp[i].val;
				if (p_fTemp[i].val > 25) {
					modelErr stcErr;
					stcErr.iErrorCode = BAD_DATA;
			...
			//STR
			FillSpeciesSpecificValue(p_oElement, "di_lognormalCanopySTR",
					"di_lcsVal", p_fTemp, iNumFunctionSpecies, p_oPop, true);
			//Transfer to permanent array
			for (i = 0; i < iNumFunctionSpecies; i++)
				mp_fStr[lognormal][canopy][mp_iIndexes[p_fTemp[i].code]] =
						p_fTemp[i].val;
		} //end of if (iNumFunctionSpecies > 0)

		//**********************************
		//2Dt canopy parameters
		//**********************************
		//Start by getting a list of species that use 2Dt for canopy - we'll
		//only require parameters for those species
		//First count 'em
		iNumFunctionSpecies = 0;
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			if (twodt == mp_iWhatFunction[canopy][i])
				iNumFunctionSpecies++;
		...
		if (iNumFunctionSpecies > 0) {
			...
			//Transfer to permanent array
			for (i = 0; i < iNumFunctionSpecies; i++)
				mp_fDispersalX0[twodt][canopy][mp_iIndexes[p_fTemp[i].code]] =
						p_fTemp[i].val;
			...
			//Transfer to permanent array
			for (i = 0; i < iNumFunctionSpecies; i++)
				mp_fThetaXb[twodt][canopy][mp_iIndexes[p_fTemp[i].code]] =
						p_fTemp[i].val;

			//Beta
			FillSpeciesSpecificValue(p_oElement, "di_twodtCanopyBeta",
					"di_tcbVal", p_fTemp, iNumFunctionSpecies, p_oPop, true);
			//Transfer to permanent array
			for (i = 0; i < iNumFunctionSpecies; i++) {
				mp_fBeta[twodt][canopy][mp_iIndexes[p_fTemp[i].code]] =
						p_fTemp[i].val;
				/*        if (p_fTemp[i].val > 25) {
				 modelErr stcErr;
				 stcErr.iErrorCode = BAD_DATA;
			...
			//STR
			FillSpeciesSpecificValue(p_oElement, "di_twodtCanopySTR",
					"di_tcsVal", p_fTemp, iNumFunctionSpecies, p_oPop, true);
			//Transfer to permanent array
			for (i = 0; i < iNumFunctionSpecies; i++)
				mp_fStr[twodt][canopy][mp_iIndexes[p_fTemp[i].code]] =
						p_fTemp[i].val;
		} //end of if (iNumFunctionSpecies > 0)
		...
		if (m_bIsGap) {
			...
			//Maximum gap density
			FillSingleValue(p_oElement, "di_maxGapDensity", &m_iMaxGapDensity,
					true);

			//Load gap values
			//Gap functions
			FillSpeciesSpecificValue(p_oElement, "di_gapFunction", "di_gfVal",
					p_iTemp, m_iNumBehaviorSpecies, p_oPop, true);
			...
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
				if (p_iTemp[i].val < 0 || p_iTemp[i].val >= m_iNumFunctions) {
					...
					mp_iWhatFunction[gap][mp_iIndexes[mp_iWhatSpecies[i]]] =
							(function) p_iTemp[i].val;
			...
			if (iNumFunctionSpecies > 0) {
				...
				//Dispersal
				FillSpeciesSpecificValue(p_oElement, "di_weibullGapDispersal",
						"di_wgdVal", p_fTemp, iNumFunctionSpecies, p_oPop,
						true);
				//Transfer to permanent array
				for (i = 0; i < iNumFunctionSpecies; i++)
					mp_fDispersalX0[weibull][gap][mp_iIndexes[p_fTemp[i].code]] =
							p_fTemp[i].val;

				//Theta
				FillSpeciesSpecificValue(p_oElement, "di_weibullGapTheta",
						"di_wgtVal", p_fTemp, iNumFunctionSpecies, p_oPop,
						true);
				//Transfer to permanent array
				for (i = 0; i < iNumFunctionSpecies; i++) {
					mp_fThetaXb[weibull][gap][mp_iIndexes[p_fTemp[i].code]] =
							p_fTemp[i].val;
					if (p_fTemp[i].val >= 50) {
						modelErr stcErr;
						stcErr.iErrorCode = BAD_DATA;
				...
				//mp_fBeta
				FillSpeciesSpecificValue(p_oElement, "di_weibullGapBeta",
						"di_wgbVal", p_fTemp, iNumFunctionSpecies, p_oPop,
						true);
				//Transfer to permanent array
				for (i = 0; i < iNumFunctionSpecies; i++) {
					mp_fBeta[weibull][gap][mp_iIndexes[p_fTemp[i].code]] =
							p_fTemp[i].val;
					if (p_fTemp[i].val > 25) {
						modelErr stcErr;
						stcErr.iErrorCode = BAD_DATA;
				...
				//mp_fStr
				FillSpeciesSpecificValue(p_oElement, "di_weibullGapSTR",
						"di_wgsVal", p_fTemp, iNumFunctionSpecies, p_oPop,
						true);
				//Transfer to permanent array
				for (i = 0; i < iNumFunctionSpecies; i++)
					mp_fStr[weibull][gap][mp_iIndexes[p_fTemp[i].code]] =
							p_fTemp[i].val;
			} //end of if (iNumFunctionSpecies > 0) {
			...
			if (iNumFunctionSpecies > 0) {
				...
				//X0
				FillSpeciesSpecificValue(p_oElement, "di_lognormalGapX0",
						"di_lgx0Val", p_fTemp, iNumFunctionSpecies, p_oPop,
						true);
				//Transfer to permanent array
				for (i = 0; i < iNumFunctionSpecies; i++)
					mp_fDispersalX0[lognormal][gap][mp_iIndexes[p_fTemp[i].code]] =
							p_fTemp[i].val;

				//Xb
				FillSpeciesSpecificValue(p_oElement, "di_lognormalGapXb",
						"di_lgxbVal", p_fTemp, iNumFunctionSpecies, p_oPop,
						true);
				//Transfer to permanent array
				for (i = 0; i < iNumFunctionSpecies; i++)
					mp_fThetaXb[lognormal][gap][mp_iIndexes[p_fTemp[i].code]] =
							p_fTemp[i].val;

				//Beta
				FillSpeciesSpecificValue(p_oElement, "di_lognormalGapBeta",
						"di_lgbVal", p_fTemp, iNumFunctionSpecies, p_oPop,
						true);
				//Transfer to permanent array
				for (i = 0; i < iNumFunctionSpecies; i++) {
					mp_fBeta[lognormal][gap][mp_iIndexes[p_fTemp[i].code]] =
							p_fTemp[i].val;
					if (p_fTemp[i].val > 25) {
						modelErr stcErr;
						stcErr.iErrorCode = BAD_DATA;
				...
				//STR
				FillSpeciesSpecificValue(p_oElement, "di_lognormalGapSTR",
						"di_lgsVal", p_fTemp, iNumFunctionSpecies, p_oPop,
						true);
				//Transfer to permanent array
				for (i = 0; i < iNumFunctionSpecies; i++)
					mp_fStr[lognormal][gap][mp_iIndexes[p_fTemp[i].code]] =
							p_fTemp[i].val;
			}
			...
			//2dt gap parameters
			//**********************************
			//Start by getting a list of species that use 2dt for gap - we'll only
			//require parameters for those species
			//First count 'em
			iNumFunctionSpecies = 0;
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
				if (twodt == mp_iWhatFunction[gap][i])
					iNumFunctionSpecies++;
			...
			if (iNumFunctionSpecies > 0) {
				...
				//X0
				FillSpeciesSpecificValue(p_oElement, "di_twodtGapX0",
						"di_tgx0Val", p_fTemp, iNumFunctionSpecies, p_oPop,
						true);
				//Transfer to permanent array
				for (i = 0; i < iNumFunctionSpecies; i++)
					mp_fDispersalX0[twodt][gap][mp_iIndexes[p_fTemp[i].code]] =
							p_fTemp[i].val;

				//Xb
				FillSpeciesSpecificValue(p_oElement, "di_twodtGapXb",
						"di_tgxbVal", p_fTemp, iNumFunctionSpecies, p_oPop,
						true);
				//Transfer to permanent array
				for (i = 0; i < iNumFunctionSpecies; i++)
					mp_fThetaXb[twodt][gap][mp_iIndexes[p_fTemp[i].code]] =
							p_fTemp[i].val;

				//Beta
				FillSpeciesSpecificValue(p_oElement, "di_twodtGapBeta",
						"di_tgbVal", p_fTemp, iNumFunctionSpecies, p_oPop,
						true);
				//Transfer to permanent array
				for (i = 0; i < iNumFunctionSpecies; i++) {
					mp_fBeta[twodt][gap][mp_iIndexes[p_fTemp[i].code]] =
							p_fTemp[i].val;
					/*
					 if (p_fTemp[i].val > 25) {
					 modelErr stcErr;
				...
				//STR
				FillSpeciesSpecificValue(p_oElement, "di_twodtGapSTR",
						"di_tgsVal", p_fTemp, iNumFunctionSpecies, p_oPop,
						true);
				//Transfer to permanent array
				for (i = 0; i < iNumFunctionSpecies; i++)
					mp_fStr[twodt][gap][mp_iIndexes[p_fTemp[i].code]] =
							p_fTemp[i].val;
		...
		if (m_bStumps) {
			...
			for (i = 0; i < m_iNumSpeciesTypeCombos; i++)
				if (clTreePopulation::stump
						== mp_whatSpeciesTypeCombos[i].iType) {
					p_fTemp[iNumFunctionSpecies].code =
							mp_whatSpeciesTypeCombos[i].iSpecies;
					iNumFunctionSpecies++;
				}

			//Stump STR values
			FillSpeciesSpecificValue(p_oElement, "di_suckerSTR", "di_ssVal",
			...
			//Stump Beta values
			FillSpeciesSpecificValue(p_oElement, "di_suckerBeta", "di_sbVal",
					p_fTemp, iNumFunctionSpecies, p_oPop, true);
		...
		modelErr stcErr;
		stcErr.iErrorCode = UNKNOWN;
		stcErr.sFunction = "clSpatialDispersalLinear::GetParameterFileData";
		throw(stcErr);
	}
}
...
// CalcFecAndFunctions
///////////////////////////////////////////////////////////////////////////////
void clSpatialDispersalLinear::CalcFecAndFunctions() {
	...
	if (m_bIsGap) {
		for (i = 0; i < m_iNumFunctions; i++) {
			...
			for (j = 0; j < m_iNumCovers; j++) {
				...
				for (k = 0; k < m_iNumBehaviorSpecies; k++) {
					if (i
							== mp_iWhatFunction[j][mp_iIndexes[mp_iWhatSpecies[k]]]) {
						...
						CalculateProbabilityDistribution(mp_fCumProb[i][j][k],
								m_iMaxDistance, mp_iWhatSpecies[k], iFunction,
								iCover);
					}
				}
			}
		...
		for (i = 0; i < m_iNumFunctions; i++) {
			...
			for (k = 0; k < m_iNumBehaviorSpecies; k++) {
				if (i
						== mp_iWhatFunction[canopy][mp_iIndexes[mp_iWhatSpecies[k]]]) {
...
//               / pow( 30, mp_fBeta[i] [canopy] [mp_iIndexes[mp_iWhatSpecies[k]]] );
					...
					CalculateProbabilityDistribution(mp_fCumProb[i][canopy][k],
							m_iMaxDistance, mp_iWhatSpecies[k], iFunction,
							canopy);
				}
			}
			...
			if (clTreePopulation::stump == mp_whatSpeciesTypeCombos[i].iType)
				mp_fStumpFecundity[mp_iIndexes[mp_whatSpeciesTypeCombos[i].iSpecies]] =
						mp_fStumpStr[mp_iIndexes[mp_whatSpeciesTypeCombos[i].iSpecies]]
								/ pow(30,
										mp_fStumpBeta[mp_iIndexes[mp_whatSpeciesTypeCombos[i].iSpecies]]);
	}
}
...
/////////////////////////////////////////////////////////////////////////////
void clSpatialDispersalLinear::CalculateProbabilityDistribution(
		float *p_fProbArray, int iMaxDistance, int iSpecies, function iFunction,
		cover iCover) {
	try {
		...
		for (iDistance = 0; iDistance < iMaxDistance; iDistance++) {
			fInc = M_PI * (2 * (iDistance + 0.5))
					* CalculateFunctionValue((iDistance + 0.5), iSpecies,
							iFunction, iCover);
		...
		modelErr stcErr;
		stcErr.iErrorCode = UNKNOWN;
		stcErr.sFunction = "clSpatialDisperseLinear::CalculateProbability...";
		throw(stcErr);
	}
}
...
// AddSeeds()
/////////////////////////////////////////////////////////////////////////////
void clSpatialDispersalLinear::AddSeeds() {
	try {
		clTreePopulation *p_oPop =
				(clTreePopulation*) mp_oSimManager->GetPopulationObject(
						"treepopulation");
		...
		modelErr stcErr;
		stcErr.iErrorCode = UNKNOWN;
		stcErr.sFunction = "clSpatialDisperseLinear::AddSeeds";
		throw(stcErr);
	}
}
...
// SpatialDisperse
///////////////////////////////////////////////////////////////////////////////
void clSpatialDispersalLinear::SpatialDisperse(clTree *p_oTree,
	...
	try {
		float fRand, //random number for determining aspects of a seed's placement
				...
				fXCellLength = mp_oSeedGrid->GetLengthXCells(), fYCellLength =
						mp_oSeedGrid->GetLengthYCells(), fCellLength; //shorter of the two above
		...
		function iCanFunc = mp_iWhatFunction[canopy][iSpIndex], iGapFunc =
				mp_iWhatFunction[gap][iSpIndex], iFunction; //variable
		...
		mp_oSeedGrid->GetValueAtPoint(fParentX, fParentY, m_iIsGapCode,
				&bIsParentInGap);
		...
		if (clTreePopulation::stump == iType) {
			...
			fNumSeeds = clModelMath::Round(
					mp_fStumpFecundity[iSpIndex]
							* pow(fDbh, mp_fStumpBeta[iSpIndex]), 0)
			...
			fSurvivalChance = (mp_fStumpStr[iSpIndex]
					* (pow((fDbh / 30), mp_fStumpBeta[iSpIndex])))
					/ (mp_fStr[iGapFunc][gap][iSpIndex]
							* (pow((fDbh / 30),
									mp_fBeta[iGapFunc][gap][iSpIndex])));

		} else { //this is not a stump

			//To calculate the number of seeds for live trees, use the maximum of gap
			//STR and canopy STR.
			if (mp_fStr[iCanFunc][canopy][iSpIndex]
					>= mp_fStr[iGapFunc][gap][iSpIndex]) {
			...
			fSurvivalChance = (mp_fStr[iGapFunc][gap][iSpIndex]
					* (pow((fDbh / 30), mp_fBeta[iGapFunc][gap][iSpIndex])))
					/ (mp_fStr[iCanFunc][canopy][iSpIndex]
							* (pow((fDbh / 30),
									mp_fBeta[iCanFunc][canopy][iSpIndex])));
		}
		...
		for (f = 0; f < fNumSeeds; f++) {
			...
			iDistanceCounter = 0;
			while (fRand
					> mp_fCumProb[iFunction][iCover][iSpIndex][iDistanceCounter])
				iDistanceCounter++;
			...
			fSeedX = p_oPlot->CorrectX(
					p_oPlot->GetUncorrectedX(fParentX, fAngle, fDistance));
			fSeedY = p_oPlot->CorrectY(
					p_oPlot->GetUncorrectedY(fParentY, fAngle, fDistance));

			//Get the gap status for this location
			mp_oSeedGrid->GetValueAtPoint(fSeedX, fSeedY, m_iIsGapCode,
					&bIsSeedInGap);
			...
			if (fSurvivalChance < 1.0) { //lower gap STR
				if (bIsSeedInGap) {
					fRand = clModelMath::GetRand();
					if (fRand <= fSurvivalChance)
						bSurvives = true;
					else
						bSurvives = false;
				} else
					bSurvives = true; //if in canopy - automatic survival
			} else if (fSurvivalChance > 1.0) { //lower canopy STR
				if (!bIsSeedInGap) {
					fRand = clModelMath::GetRand();
					if (fRand <= 1 / fSurvivalChance)
						bSurvives = true;
					else
						bSurvives = false;
				} else
			...
			//adjustments.
			//***************************************
			if (!bIsParentInGap && !bIsSeedInGap)
				goto plantSeed;
			...
			//distance is where the seed lands.
				...
				fRand = clModelMath::GetRand();
				while (fRand
						> mp_fCumProb[iCanFunc][canopy][iSpIndex][iDistanceCounter])
					iDistanceCounter++;
				fPrevBucket =
						(iDistanceCounter == 0) ?
								0.0 :
				...
				if (fNewDistance < fDistance) {
					fSeedX = p_oPlot->CorrectX(
							p_oPlot->GetUncorrectedX(fParentX, fAngle,
									fNewDistance));
					fSeedY = p_oPlot->CorrectY(
							p_oPlot->GetUncorrectedY(fParentY, fAngle,
									fNewDistance));
				}
			} else {
				...
				if (!bIsParentInGap) {
					...
					fRand = clModelMath::GetRand();
					while (fRand
							> mp_fCumProb[iGapFunc][gap][iSpIndex][iDistanceCounter])
						iDistanceCounter++;
					fPrevBucket =
							(iDistanceCounter == 0) ?
									0.0 :
				...
				for (fStepDistance = fCellLength;
						fStepDistance <= fWalkDistance; fStepDistance +=
								fCellLength) {
					//Compute the point location for this step
					fStepX = p_oPlot->CorrectX(
							p_oPlot->GetUncorrectedX(fStartX, fAngle,
									fStepDistance));
					fStepY = p_oPlot->CorrectY(
							p_oPlot->GetUncorrectedY(fStartY, fAngle,
									fStepDistance));

					//Under canopy?  If so, drop the seed here
					mp_oSeedGrid->GetValueAtPoint(fStepX, fStepY, m_iIsGapCode,
							&bIsSeedInGap);
					if (!bIsSeedInGap) {
						fSeedX = fStepX;
						fSeedY = fStepY;
		...
		modelErr stcErr;
		stcErr.iErrorCode = UNKNOWN;
		stcErr.sFunction = "clSpatialDispersalLinear::Disperse";
		throw(stcErr);
	}
}
...
// NonSpatialDisperse
//////////////////////////////////////////////////////////////////////////////
void clSpatialDispersalLinear::NonSpatialDisperse(clTree *p_oTree,
		clTreePopulation *p_oPop, clPlot *p_oPlot, float fDbh) {
	try {
		...
		modelErr stcErr;
		stcErr.iErrorCode = UNKNOWN;
		stcErr.sFunction = "clSpatialDisperseLinear::NonSpatialDisperse";
		throw(stcErr);
	}
}
...
///////////////////////////////////////////////////////////////////////////*/
float clSpatialDispersalLinear::CalculateFunctionValue(float fDistance,
		int iSpecies, function iFunction, cover iCover) {
	if (weibull == iFunction)
		return clModelMath::CalculateWeibullFunction(
				mp_fDispersalX0[iFunction][iCover][mp_iIndexes[iSpecies]],
				mp_fThetaXb[iFunction][iCover][mp_iIndexes[iSpecies]],
				fDistance);
	else {
		if (lognormal == iFunction)
			return clModelMath::CalculateLognormalFunction(
					mp_fDispersalX0[iFunction][iCover][mp_iIndexes[iSpecies]],
					mp_fThetaXb[iFunction][iCover][mp_iIndexes[iSpecies]],
					fDistance);
		else
			return clModelMath::CalculatetwodtFunction(
					mp_fDispersalX0[iFunction][iCover][mp_iIndexes[iSpecies]],
					fDistance);
	}
}
...
// CalculateGapStatus()
///////////////////////////////////////////////////////////////////////////////
void clSpatialDispersalLinear::CalculateGapStatus(clTreePopulation *p_oPop) {
	try {

		if (m_bUpdatedGapStatus)
			return;
		...
		short int iSp, iType, //species and type of a given tree
				iNumXCells = mp_oSeedGrid->GetNumberXCells(), iNumYCells =
						mp_oSeedGrid->GetNumberYCells(), i, j; //loop counters
		...
		sprintf(cQuery, "%s%d%s%d", "type=", clTreePopulation::sapling, ",",
				clTreePopulation::adult);
		...
		modelErr stcErr;
		stcErr.iErrorCode = UNKNOWN;
		stcErr.sFunction = "clSpatialDisperseLinear::CalculateGapStatus";
		throw(stcErr);
	}
}
...
// SetNameData()
////////////////////////////////////////////////////////////////////////////
void clSpatialDispersalLinear::SetNameData(std::string sNameString) {
	try {
		if (sNameString.compare("GapDisperse") == 0) {
			m_bIsGap = true;
			m_sXMLRoot = "GapDisperseLinear";

		} else if (sNameString.compare("NonGapDisperseLinear") == 0) {
			m_bIsGap = false;
			m_sXMLRoot = "NonGapDisperse";
		} else { //error!
			modelErr stcErr;
			stcErr.iErrorCode = BAD_DATA;
			stcErr.sFunction = "clSpatialDispersalLinear::SetNameData";
...
// GetNumberOfSeeds()
////////////////////////////////////////////////////////////////////////////
float clSpatialDispersalLinear::GetNumberOfSeeds(float fDbh, short int iSp,
		int iCover, int iFunc) {
	return clModelMath::RandomRound(
			(mp_fFecundity[iFunc][iCover][mp_iIndexes[iSp]]
					+ fDbh * mp_fBeta[iFunc][iCover][mp_iIndexes[iSp]])
					* m_fNumYearsPerTimestep);
