...
void clStormLight::GetData(xercesc::DOMDocument *p_oDoc) {
	try {
		...
		FillSingleValue(p_oElement, "li_stormLightIntercept", &m_fIntercept,
				true);

		//The max time since damage that a tree can count toward light calculations
		FillSingleValue(p_oElement, "li_stormLightMaxDmgTime", &m_iMaxDmgTime,
				true);

		//The max time since damage that a snag can count toward light calculations
		FillSingleValue(p_oElement, "li_stormLightSnagMaxDmgTime",
				&m_iMaxSnagDmgTime, true);

		//Minimum number of trees for full canopy
		FillSingleValue(p_oElement, "li_stormLightMinFullCanopy",
				&m_fMinCanopyTrees, true);
		//Error if this is negative
		if (m_fMinCanopyTrees < 0) {
			modelErr stcErr;
			stcErr.sFunction = "clStormLight::GetData";
			stcErr.sMoreInfo =
					"Minimum number of trees for full canopy cannot be less than 0.";
			stcErr.iErrorCode = BAD_DATA;
			throw(stcErr);
		}
		...
		if (deterministic_pdf != m_iStochasticity) {
			FillSingleValue(p_oElement, "li_stormLightRandPar",
					&m_fRandParameter, true);
		}
		...
		if (!mp_oLightGrid) {
			...
			mp_oLightGrid = mp_oSimManager->CreateGrid("Storm Light", 0, 1, 0,
					0);
			...
			if (-1 == m_iGridLightCode) {
				...
				stcErr.sFunction = "clStormLight::GetData";
				stcErr.sMoreInfo =
						"\"Storm Light\" grid was incorrectly set up in the parameter file.  Missing float \"Light\".";
				throw(stcErr);
			}
		}
		...
		for (i = 0; i < iNumSpecies; i++) {
			mp_iStmDmgCodes[adult][i] = p_oPop->GetIntDataCode("stm_dmg", i,
					clTreePopulation::adult);
			mp_iStmDmgCodes[snag][i] = p_oPop->GetIntDataCode("stm_dmg", i,
...
void clStormLight::Action() {
	try {
		...
		float fLight, //light level
				fMinAdultHeight = p_oPop->GetMinimumAdultHeight(), fX, fY; //holders for the tree's X and Y location
		int iDamage, //tree damage level
				...
				iNumXCells = mp_oLightGrid->GetNumberXCells(), iNumYCells =
						mp_oLightGrid->GetNumberYCells(), iSp, iTp, iX, iY;
		...
		for (iX = 0; iX < iNumXCells; iX++) {
			for (iY = 0; iY < iNumYCells; iY++) {
				...
				sprintf(cQuery, "%s%f%s%f%s%f%s%f", "distance=", m_fMaxRadius,
						" FROM x=", fX, "y=", fY, "::height=", fMinAdultHeight);
				...
				while (p_oNeighbor != NULL) {
					...
					if (clTreePopulation::seedling != iTp
						...
						if (clTreePopulation::adult == iTp)
							...
							iDamage = mp_iStmDmgCodes[snag][iSp];
						...
						if (-1 != iDamage) {
							...
							if (0 == iDamage && clTreePopulation::snag == iTp) {
								p_oNeighbor->GetValue(
										p_oPop->GetIntDataCode("Age", iSp, iTp),
										&iDamage);
								iDamage += 2000;
							}
							...
							if ((clTreePopulation::snag == iTp && iDamage > 0)
								...
								iDamage =
										iDamage > 2000 ?
												iDamage % 2000 : iDamage % 1000;
								if ((clTreePopulation::adult == iTp
										&& iDamage < m_iMaxDmgTime)
										|| (clTreePopulation::snag == iTp
												&& iDamage < m_iMaxSnagDmgTime))
									//Finally!  increment
									iNumDamagedTrees++;
							}
				...
				if (0 == iNumCanopyTrees) {
					...
					fLight = ((1 - (iNumCanopyTrees / m_fMinCanopyTrees)) * 100)
							+ clModelMath::CalcPointValue(
									(float) iNumDamagedTrees
											/ (float) iNumCanopyTrees, m_fSlope,
									m_fIntercept);
				} else {
				...
				if (normal_pdf == m_iStochasticity) {
					...
					fLight = clModelMath::LognormalRandomDraw(fLight,
							m_fRandParameter);
				}
				...
				mp_oLightGrid->SetValueAtPoint(fX, fY, m_iGridLightCode,
						fLight);
			}
		}
	} catch (modelErr &err) {
