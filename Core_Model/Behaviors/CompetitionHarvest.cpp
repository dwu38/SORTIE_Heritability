...
void clCompetitionHarvest::RegisterTreeDataMembers() {
	...
	for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
		...
		mp_iCOECodes[mp_whatSpeciesTypeCombos[i].iSpecies][mp_whatSpeciesTypeCombos[i].iType
				- clTreePopulation::sapling] = p_oPop->RegisterFloat("COE",
...
void clCompetitionHarvest::ReadParameterFileData(xercesc::DOMDocument *p_oDoc,
	...
	try {
		...
		stringstream sTag;
		float fTemp, fPlotSize = p_oPlot->GetPlotArea(), fTimestep =
		...
		FillSpeciesSpecificValue(p_oElement, "di_compHarvMaxCrowdingRadius",
				"di_chmcrVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Make sure that the max radius of neighbor effects is > 0
		for (i = 0; i < m_iNumSpecies; i++)
			mp_fMaxCrowdingRadius[i] = 0;
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			if (p_fTempValues[i].val <= 0) {
				modelErr stcErr;
				stcErr.sFunction =
						"clCompetitionHarvest::ReadParameterFileData";
				stcErr.iErrorCode = BAD_DATA;
				stcErr.sMoreInfo =
						"All values for max crowding radius must be greater than 0.";
				throw(stcErr);
			}
		...
		for (i = 0; i < m_iNumSpecies; i++)
			mp_fPropToCut[i] = 0;
		FillSpeciesSpecificValue(p_oElement, "di_compHarvProportion",
				"di_chpVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		...
		if (m_bIsSpeciesSpecific) {
			...
			if (fabs(fTemp - 1.0) > 0.0001) {
				...
				stcErr.iErrorCode = BAD_DATA;
				stcErr.sMoreInfo =
						"Proportion to cut for different species must add up to 1.";
				throw(stcErr);
			}
		}
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			sTag << "di_compHarv"
					<< p_oPop->TranslateSpeciesCodeToName(mp_iWhatSpecies[i])
					<< "NeighborLambda";
		...
		if (ba_prop == harvest && (m_fAmtToCut < 0 || m_fAmtToCut > 1)) {
			...
			stcErr.iErrorCode = BAD_DATA;
			stcErr.sMoreInfo =
					"The amount to cut must be a proportion between 0 and 1.";
			throw(stcErr);
		...
		if (ba_amt == harvest || ba_prop == harvest) {
			FillSingleValue(p_oElement, "di_compHarvMinInterval", &m_iInterval,
					true);
			FillSingleValue(p_oElement, "di_compHarvBAThreshold",
					&m_fBAThreshold, true);
		} else
			FillSingleValue(p_oElement, "di_compHarvInterval", &m_iInterval,
					true);
		...
		FillSingleValue(p_oElement, "di_compHarvHarvestedListFile",
				&m_sHarvestListFilename, false);
...
void clCompetitionHarvest::ResetResultsGrid() {
	...
	short int iNumXCells = mp_oResultsGrid->GetNumberXCells(), iNumYCells =
			mp_oResultsGrid->GetNumberYCells(), i, j, iSp; //loop counters
	...
	for (i = 0; i < iNumXCells; i++)
		for (j = 0; j < iNumYCells; j++) {
			...
			for (iSp = 0; iSp < m_iNumSpecies; iSp++) {
				mp_oResultsGrid->SetValueOfCell(i, j, mp_iDenCutCodes[iSp],
						iValue);
				mp_oResultsGrid->SetValueOfCell(i, j, mp_iBaCutCodes[iSp],
						fValue);
			}
		}
}
...
void clCompetitionHarvest::SetupGrids() {
	try {
		...
		mp_oResultsGrid = mp_oSimManager->GetGridObject(
				"Competition Harvest Results");
		if (NULL == mp_oResultsGrid) {

			mp_oResultsGrid = mp_oSimManager->CreateGrid(
					"Competition Harvest Results", m_iNumSpecies, //number of int data members
			...
			for (i = 0; i < m_iNumSpecies; i++) {
				...
				mp_iBaCutCodes[i] = mp_oResultsGrid->RegisterFloat(
						sLabel.str());
				sLabel.str("");
			}
		} else {
			...
			for (i = 0; i < m_iNumSpecies; i++) {
				sLabel << "Cut Density_" << i;
				mp_iDenCutCodes[i] = mp_oResultsGrid->GetFloatDataCode(
						sLabel.str());
				if (-1 == mp_iDenCutCodes[i]) {
					...
					s << "Couldn't find the \"" << sLabel.str()
							<< "\" member of the \"Competition Harvest Results\" grid.";
					stcErr.sMoreInfo = s.str();
					stcErr.iErrorCode = BAD_DATA;
					throw(stcErr);
				}
				...
				sLabel << "Cut Basal Area_" << i;
				mp_iBaCutCodes[i] = mp_oResultsGrid->GetFloatDataCode(
						sLabel.str());
...
void clCompetitionHarvest::Action() {
	try {
		...
		if (m_iTimeSinceLastHarvest < m_iInterval)
		...
		if (interval == harvest) {
			...
			if (m_bIsSpeciesSpecific)
				CutTreesSpeciesSpecific(fBA);
			else
				CutTreesNotSpeciesSpecific(fBA);
...
float clCompetitionHarvest::CalculateOneCOE(clPlot *p_oPlot,
		clTreePopulation *p_oPop, clTree *p_oTree) {
	...
	float fCOE = 0, //competitive effect - the end result of all this math
			...
			fTemp, fNeighX, fNeighY, //holders for the neighbor tree's X and Y
	...
	while (p_oNeighbor) {
		...
		if (clTreePopulation::seedling != iNeighTp
			...
			if (0 != fDistance) {
				...
				p_oNeighbor->GetValue(p_oPop->GetDbhCode(iNeighSp, iNeighTp),
						&fNeighDbh);

				//Add competitive effect on neighbor
				fTemp = pow(
						pow(fTargetDbh / m_fQ, mp_fAlpha[iNeighSp])
								/ pow(fDistance, mp_fBeta[iNeighSp]),
						mp_fD[iNeighSp]);
...
//////////////////////////////////////////////////////////////////////////////
void clCompetitionHarvest::CalculateAllCOEsSpeciesSpecific(
		clTreePopulation *p_oPop) {
	clPlot *p_oPlot = mp_oSimManager->GetPlotObject();
	clTree *p_oTree;
		...
		for (iY = 0; iY < m_iNumY; iY++) {
			...
			while (p_oTree) {
				...
				if ((clTreePopulation::sapling == iTp
						...
						&& mp_iCOECodes[iSp][iTp - clTreePopulation::sapling]
								> -1 && p_bEligible[iSp]) {
					...
					if (clTreePopulation::seedling == iTp) {
						p_oTree->GetValue(p_oPop->GetHeightCode(iSp, iTp),
								&fHeight);
...
//////////////////////////////////////////////////////////////////////////////
void clCompetitionHarvest::CalculateAllCOEsNotSpeciesSpecific(
		clTreePopulation *p_oPop) {
	clPlot *p_oPlot = mp_oSimManager->GetPlotObject();
	clTree *p_oTree;
	float fCOE, fHeight, fDbh;
...
//////////////////////////////////////////////////////////////////////////////
void clCompetitionHarvest::RecalculateCOESpeciesSpecific(
		clTreePopulation *p_oPop, const float &fX, const float &fY) {
	clPlot *p_oPlot = mp_oSimManager->GetPlotObject();
	clTree *p_oTree;
	bool *p_bEligible = new bool[m_iNumSpecies];
	float fCOE, fHeight, fDbh, fCellLength = p_oPop->GetGridCellSize();
...
//////////////////////////////////////////////////////////////////////////////
void clCompetitionHarvest::RecalculateCOENotSpeciesSpecific(
		clTreePopulation *p_oPop, const float &fX, const float &fY) {
	clPlot *p_oPlot = mp_oSimManager->GetPlotObject();
	clTree *p_oTree;
	float fCOE, fHeight, fDbh, fCellLength = p_oPop->GetGridCellSize();
...
void clCompetitionHarvest::CutTreesNotSpeciesSpecific(const float &fPlotBA) {
	...
	float fX, fY, //coordinates of cut trees
			fTemp, fBA, //basal area of trees to cut
			fDbh, fMaxCOE, //for finding the highest COE tree
			fCutSoFar = 0, //how much has been cut so far
			fAmtToCutThisTS; //amount to cut this timestep
	int i, iX,
			iY, //loop counters
			iTemp, iTs = mp_oSimManager->GetCurrentTimestep(), iSp, iTp, iMaxX,
			iMaxY; //for finding the highest COE tree
	...
	while (false == bHarvestDone) {
		...
		if (0 < fMaxCOE) {
			...
			if ((fCutSoFar + fBA) < fAmtToCutThisTS)
				...
				bKillTree = clModelMath::GetRand()
						< (fAmtToCutThisTS - fCutSoFar) / fBA;
			...
			if (bKillTree) {
				...
				mp_oResultsGrid->GetValueAtPoint(fX, fY, mp_iDenCutCodes[iSp],
						&iTemp);
				iTemp += 1;
				mp_oResultsGrid->SetValueAtPoint(fX, fY, mp_iDenCutCodes[iSp],
						iTemp);

				mp_oResultsGrid->GetValueAtPoint(fX, fY, mp_iBaCutCodes[iSp],
						&fTemp);
				fTemp += fBA;
				mp_oResultsGrid->SetValueAtPoint(fX, fY, mp_iBaCutCodes[iSp],
						fTemp);
	...
	while (false == bHarvestDone) {
		...
		if (0 < fMaxCOE) {
			...
			if ((mp_fAlreadyCut[iSp] + fBA) < mp_fTargetToCut[iSp])
				...
				//decide whether to cut it
				bKillTree = clModelMath::GetRand()
						< (mp_fTargetToCut[iSp] - mp_fAlreadyCut[iSp]) / fBA;
