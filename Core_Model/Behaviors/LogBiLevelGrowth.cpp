...
void clLogBiLevelGrowth::DoShellSetup(DOMDocument *p_oDoc) {
	...
	try {
		...
		FillSpeciesSpecificValue(p_oElement, "gr_lognormalBilevLoLiteX0",
				"gr_lbllx0Val", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			...
			mp_fLoLightX0[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;
		}

		//Low-light Xb
		FillSpeciesSpecificValue(p_oElement, "gr_lognormalBilevLoLiteXb",
				"gr_lbllxbVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer values to our permanent array while making sure none of them
		//are 0
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			...
			mp_fLoLightXb[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;
		}

		//Low-light max growth
		FillSpeciesSpecificValue(p_oElement, "gr_lognormalBilevLoLiteMaxGrowth",
				"gr_lbllmgVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer values to our permanent array while making sure none of them
		//are 0
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			...
			mp_fLoLightMaxGrowth[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;
		}
		...
		if (NULL != mp_oStormLight) {
			...
			FillSpeciesSpecificValue(p_oElement,
					"gr_lognormalBilevHiLiteThreshold", "gr_lobhltVal",
			...
			for (i = 0; i < m_iNumBehaviorSpecies; i++) {
				if (p_fTempValues[i].val < 0 || p_fTempValues[i].val > 100) {
					...
					stcErr.sFunction = "clLogBiLevelGrowth::DoShellSetup";
					stcErr.sMoreInfo =
							"All values in high-light growth threshold must be between 0 and 100.";
					throw(stcErr);
				}
				mp_fHiLightThreshold[mp_iIndexes[p_fTempValues[i].code]] =
						p_fTempValues[i].val;
			}

			//High-light X0
			FillSpeciesSpecificValue(p_oElement, "gr_lognormalBilevHiLiteX0",
					"gr_lbhlx0Val", p_fTempValues, m_iNumBehaviorSpecies,
					p_oPop, true);
			//Transfer values to our permanent array while making sure none of them
			//are 0
			for (i = 0; i < m_iNumBehaviorSpecies; i++) {
				...
				mp_fHiLightX0[mp_iIndexes[p_fTempValues[i].code]] =
						p_fTempValues[i].val;
			}

			//High-light Xb
			FillSpeciesSpecificValue(p_oElement, "gr_lognormalBilevHiLiteXb",
					"gr_lbhlxbVal", p_fTempValues, m_iNumBehaviorSpecies,
					p_oPop, true);
			//Transfer values to our permanent array while making sure none of them
			//are 0
			for (i = 0; i < m_iNumBehaviorSpecies; i++) {
				...
				mp_fHiLightXb[mp_iIndexes[p_fTempValues[i].code]] =
						p_fTempValues[i].val;
			}

			//High-light max growth
			FillSpeciesSpecificValue(p_oElement,
					"gr_lognormalBilevHiLiteMaxGrowth", "gr_lbhlmgVal",
					p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
			//Transfer values to our permanent array while making sure none of them
			//are 0
			for (i = 0; i < m_iNumBehaviorSpecies; i++) {
				if (p_fTempValues[i].val < 0) {
					...
					stcErr.sFunction = "clLogBiLevelGrowth::DoShellSetup";
					stcErr.sMoreInfo =
							"Max growth values must be greater than 0.";
					throw(stcErr);
				}
				mp_fHiLightMaxGrowth[mp_iIndexes[p_fTempValues[i].code]] =
						p_fTempValues[i].val;
			}
...
float clLogBiLevelGrowth::CalcHeightGrowthValue(clTree *p_oTree,
	...
	if (fLightLevel < mp_fHiLightThreshold[mp_iIndexes[iSp]]) {
		return m_fYearsPerTimestep * mp_fLoLightMaxGrowth[mp_iIndexes[iSp]]
				* exp(
						-0.5
								* pow(
										log(
												fHeight
														/ mp_fLoLightX0[mp_iIndexes[iSp]])
												/ mp_fLoLightXb[mp_iIndexes[iSp]],
										2));
	}
	return m_fYearsPerTimestep * mp_fHiLightMaxGrowth[mp_iIndexes[iSp]]
			* exp(
					-0.5
							* pow(
									log(
											fHeight
													/ mp_fHiLightX0[mp_iIndexes[iSp]])
											/ mp_fHiLightXb[mp_iIndexes[iSp]],
									2));
}
