...
void clSubstrateDepSeedSurvival::SetNameData(std::string sNameString) {
	try {
		if (sNameString.compare("NoGapSubstrateSeedSurvival") == 0) {
			...
			m_bUsesGap = true;
			m_bUsesMicro = false;
		} else if (sNameString.compare("MicrotopographicSubstrateSeedSurvival")
				== 0) {
			m_bUsesMicro = true;
			m_bUsesGap = false;
		} else { //error!
...
void clSubstrateDepSeedSurvival::GetParameterFileData(
	...
	try {
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fCanGroundScarSoilFav[mp_iIndexes[p_fTemp[i].code]] =
					p_fTemp[i].val;
		...
		//Now transfer the values to the permanent array
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fCanGroundTipUpFav[mp_iIndexes[p_fTemp[i].code]] =
					p_fTemp[i].val;

		//Canopy fresh log favorability
		FillSpeciesSpecificValue(p_oElement, "es_freshLogCanopyFav",
				"es_flcfVal", p_fTemp, m_iNumBehaviorSpecies, p_oPop, true);
		//Now transfer the values to the permanent array
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fCanGroundFreshLogFav[mp_iIndexes[p_fTemp[i].code]] =
					p_fTemp[i].val;
		...
		//Now transfer the values to the permanent array
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fCanGroundDecLogFav[mp_iIndexes[p_fTemp[i].code]] =
					p_fTemp[i].val;
		...
		//Now transfer the values to the permanent array
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fCanGroundForFlMossFav[mp_iIndexes[p_fTemp[i].code]] =
					p_fTemp[i].val;

		//Verify that all values are between 0 and 1
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			if (mp_fCanGroundScarSoilFav[i] < 0
					|| mp_fCanGroundScarSoilFav[i] > 1
					|| mp_fCanGroundTipUpFav[i] < 0
					|| mp_fCanGroundTipUpFav[i] > 1
					|| mp_fCanGroundFreshLogFav[i] < 0
					|| mp_fCanGroundFreshLogFav[i] > 1
					|| mp_fCanGroundDecLogFav[i] < 0
					|| mp_fCanGroundDecLogFav[i] > 1
					|| mp_fCanGroundForFlLitterFav[i] < 0
					|| mp_fCanGroundForFlLitterFav[i] > 1
					|| mp_fCanGroundForFlMossFav[i] < 0
					|| mp_fCanGroundForFlMossFav[i] > 1) {
				modelErr stcErr;
				stcErr.sFunction =
						"clSubstrateDepSeedSurvival::GetParameterFileData";
				stcErr.sMoreInfo =
						"All substrate favorabilities must be between 0 and 1.";
				stcErr.iErrorCode = BAD_DATA;
				throw(stcErr);
			}
		}
		...
		if (m_bUsesGap) {
			...
			//Now transfer the values to the permanent array
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
				mp_fGapMoundScarSoilFav[mp_iIndexes[p_fTemp[i].code]] =
						p_fTemp[i].val;
			...
			//Now transfer the values to the permanent array
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
				mp_fGapMoundTipUpFav[mp_iIndexes[p_fTemp[i].code]] =
						p_fTemp[i].val;

			//Gap fresh log favorability
			FillSpeciesSpecificValue(p_oElement, "es_freshLogGapFav",
					"es_flgfVal", p_fTemp, m_iNumBehaviorSpecies, p_oPop, true);
			//Now transfer the values to the permanent array
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
				mp_fGapMoundFreshLogFav[mp_iIndexes[p_fTemp[i].code]] =
						p_fTemp[i].val;

			//Gap decayed log favorability
			FillSpeciesSpecificValue(p_oElement, "es_decayedLogGapFav",
					"es_dlgfVal", p_fTemp, m_iNumBehaviorSpecies, p_oPop, true);
			//Now transfer the values to the permanent array
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
				mp_fGapMoundDecLogFav[mp_iIndexes[p_fTemp[i].code]] =
						p_fTemp[i].val;

			//Gap forest floor litter favorability
			FillSpeciesSpecificValue(p_oElement, "es_forestFloorLitterGapFav",
					"es_fflgfVal", p_fTemp, m_iNumBehaviorSpecies, p_oPop,
					true);
			//Now transfer the values to the permanent array
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
			...
			FillSpeciesSpecificValue(p_oElement, "es_forestFloorMossGapFav",
					"es_ffmgfVal", p_fTemp, m_iNumBehaviorSpecies, p_oPop,
					true);
			//Now transfer the values to the permanent array
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
				mp_fGapMoundForFlMossFav[mp_iIndexes[p_fTemp[i].code]] =
						p_fTemp[i].val;

			//Verify that all values are between 0 and 1
			for (i = 0; i < m_iNumBehaviorSpecies; i++) {
				if (mp_fGapMoundScarSoilFav[i] < 0
						|| mp_fGapMoundScarSoilFav[i] > 1
						|| mp_fGapMoundTipUpFav[i] < 0
						|| mp_fGapMoundTipUpFav[i] > 1
						|| mp_fGapMoundFreshLogFav[i] < 0
						|| mp_fGapMoundFreshLogFav[i] > 1
						|| mp_fGapMoundDecLogFav[i] < 0
						|| mp_fGapMoundDecLogFav[i] > 1
						|| mp_fGapMoundForFlLitterFav[i] < 0
						|| mp_fGapMoundForFlLitterFav[i] > 1
						|| mp_fGapMoundForFlMossFav[i] < 0
						|| mp_fGapMoundForFlMossFav[i] > 1) {
					modelErr stcErr;
					stcErr.sFunction =
							"clSubstrateDepSeedSurvival::GetParameterFileData";
					stcErr.sMoreInfo =
		...
		if (m_bUsesMicro) {
			...
			FillSingleValue(p_oElement, "es_moundProportion", &m_fMoundProp,
					true);
			...
			FillSpeciesSpecificValue(p_oElement, "es_tipUpMoundFav",
					"es_tumfVal", p_fTemp, m_iNumBehaviorSpecies, p_oPop, true);
			//Now transfer the values to the permanent array
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
				mp_fGapMoundTipUpFav[mp_iIndexes[p_fTemp[i].code]] =
						p_fTemp[i].val;
			...
			FillSpeciesSpecificValue(p_oElement, "es_freshLogMoundFav",
					"es_flmfVal", p_fTemp, m_iNumBehaviorSpecies, p_oPop, true);
			//Now transfer the values to the permanent array
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
				mp_fGapMoundFreshLogFav[mp_iIndexes[p_fTemp[i].code]] =
						p_fTemp[i].val;
			...
			FillSpeciesSpecificValue(p_oElement, "es_forestFloorLitterMoundFav",
					"es_fflmfVal", p_fTemp, m_iNumBehaviorSpecies, p_oPop,
					true);
			//Now transfer the values to the permanent array
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
				mp_fGapMoundForFlLitterFav[mp_iIndexes[p_fTemp[i].code]] =
			...
			FillSpeciesSpecificValue(p_oElement, "es_forestFloorMossMoundFav",
					"es_ffmmfVal", p_fTemp, m_iNumBehaviorSpecies, p_oPop,
					true);
			//Now transfer the values to the permanent array
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
				mp_fGapMoundForFlMossFav[mp_iIndexes[p_fTemp[i].code]] =
...
void clSubstrateDepSeedSurvival::Action() {
	...
	float
			...
			fForestFloorMossFav;
	short int iNumXCells = mp_oSubstrateFavGrid->GetNumberXCells(), iNumYCells =
			mp_oSubstrateFavGrid->GetNumberYCells(), iX, iY, iSp, i, j, k; //loop counters
	...
	if (false == m_bUsesMicro) {
		for (iX = 0; iX < iNumXCells; iX++)
			for (iY = 0; iY < iNumYCells; iY++) {
				...
				if (m_bUsesGap) {
					mp_oSubstrateFavGrid->GetPointOfCell(iX, iY, &fSeedX,
							&fSeedY);
				...
				for (k = 0; k < m_iNumBehaviorSpecies; k++) {
					if (bIsGap) {
						...
						fTipUpFav =
								mp_fGapMoundTipUpFav[mp_iIndexes[mp_iWhatSpecies[k]]];
						fFreshLogFav =
						...
						fTipUpFav =
								mp_fCanGroundTipUpFav[mp_iIndexes[mp_iWhatSpecies[k]]];
						fFreshLogFav =
					...
					mp_oSubstrateGrid->GetValueOfCell(iX, iY, m_iDecayedLogCode,
							&fCover);
	...
	for (iX = 0; iX < iNumXCells; iX++) {
		for (iY = 0; iY < iNumYCells; iY++) {
			...
			for (i = 0; i < m_iNumBehaviorSpecies; i++) {
				...
				mp_oSeedGrid->GetValueOfCell(iX, iY, mp_iSeedGridCode[iSp],
						&fNumSeeds);
				...
				for (j = 0; j < fNumSeeds; j++) {
					...
					fSeedX = p_oPlot->CorrectX(
							fXOrig + (clModelMath::GetRand() * fXCellLength));
					fSeedY = p_oPlot->CorrectY(
							fYOrig + (clModelMath::GetRand() * fYCellLength));
...
void clSubstrateDepSeedSurvival::SetupGrids() {
	try {
		...
		m_iForestFloorLitterCode = mp_oSubstrateGrid->GetFloatDataCode(
				"fflitter");
		...
		if (-1 == m_iScarifiedSoilCode || -1 == m_iForestFloorLitterCode
				|| -1 == m_iTipUpCode || -1 == m_iFreshLogCode
				|| -1 == m_iDecayedLogCode || -1 == m_iForestFloorMossCode) {
			modelErr stcErr;
			stcErr.iErrorCode = BAD_DATA;
			stcErr.sFunction = "clSubstrateDepSeedSurvival::SetUpGrids";
		...
		if (NULL == mp_oSubstrateFavGrid) {
			...
			for (i = 0; i < iNumSpecies; i++) {
				...
				mp_iSubstrFavCode[i] = mp_oSubstrateFavGrid->RegisterFloat(
						sLabel.str());
				sLabel.str("");
			}
		}
		...
		if (NULL == mp_oSeedGrid) {
			...
			stcErr.sFunction = "clSubstrateDepSeedSurvival::SetupGrids";
			stcErr.sMoreInfo =
					"Disperse behaviors are required if establishment is to be used.";
			throw(stcErr);
		}
