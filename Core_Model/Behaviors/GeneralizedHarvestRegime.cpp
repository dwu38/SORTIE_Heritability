...
//////////////////////////////////////////////////////////////////////////////
void clGeneralizedHarvestRegime::ReadHarvestParameterFileData(
	...
	FillSingleValue(p_oElement, "di_genHarvRemoveDist", &m_bUseGammaDist,
			false);
	...
	if (m_bUseGammaDist) {
		...
		FillSingleValue(p_oElement, "di_genHarvLogRemoveAlpha",
				&m_fGammaMeanRemoveA, true);

		// Amount to remove beta
		FillSingleValue(p_oElement, "di_genHarvLogRemoveBeta",
				&m_fGammaMeanRemoveB, true);

		// Amount to remove mu
		FillSingleValue(p_oElement, "di_genHarvLogRemoveMu",
				&m_fGammaMeanRemoveM, true);
		...
		double *p1 = mp_fCutAmtIntensityClasses, *p2 =
		...
		for (i = 0; i < m_iNumUserDefDistSizeClasses; i++) {
			if (mp_fCutAmtIntensityClasses[i] < 0
				...
				stcErr.sFunction =
						"clGeneralizedHarvestRegime::ReadHarvestParameterFileData";
				stcErr.sMoreInfo =
						"All values for harvest intensity classes must be between 0 and 1.";
				throw(stcErr);
			}
			...
			if (mp_fCutAmtIntensityClassProb[i] < 0
				...
				stcErr.sFunction =
						"clGeneralizedHarvestRegime::ReadHarvestParameterFileData";
				stcErr.sMoreInfo =
						"All values for harvest intensity class probabilities must be between 0 and 1.";
				throw(stcErr);
			}
		...
		if (abs(fTemp - 1) > 0.001) {
			...
			stcErr.sFunction =
					"clGeneralizedHarvestRegime::ReadHarvestParameterFileData";
			stcErr.sMoreInfo =
					"Harvest intensity class probabilities must add up to 1.";
			throw(stcErr);
		}

		for (i = 1; i < m_iNumUserDefDistSizeClasses; i++) {
			if (mp_fCutAmtIntensityClasses[i]
					<= mp_fCutAmtIntensityClasses[i - 1]) {
				modelErr stcErr;
				stcErr.iErrorCode = BAD_DATA;
				stcErr.sFunction =
						"clGeneralizedHarvestRegime::ReadHarvestParameterFileData";
				stcErr.sMoreInfo =
						"Harvest intensity classes must be ordered from lowest to highest.";
				throw(stcErr);
			}
		}
		...
		for (i = 1; i < m_iNumUserDefDistSizeClasses; i++) {
			mp_fCutAmtIntensityClassProb[i] += mp_fCutAmtIntensityClassProb[i
					- 1];
		}
	}
	...
	//Allowed range
	FillSingleValue(p_oElement, "di_genHarvAllowedDeviation", &m_fAllowedRange,
			true);

	//Whether to use biomass (true) or basal area (false)
	FillSingleValue(p_oElement, "di_genHarvUseBiomassOrBA", &m_bUseBiomass,
			true);

	//Whether to implement sapling mortality
	FillSingleValue(p_oElement, "di_genHarvDoSaplingMort", &m_bSaplingMortality,
			false);
...
void clGeneralizedHarvestRegime::Action() {
	...
	double fAmtBAToCut,  //target amount of plot BA to cut
			...
			fThisBA, fCutProbSigma, fCutProb, fCorrectionFactor, fBARemoved = 0;
	float fDbh;
	int i, iSp;
	bool bIsHarvested;
	...
	fAmtBAToCut =
			fPercentBAToCut * 0.01 * m_fTotalBA; //convert from % to proportion
#ifdef DEBUG_GEN_HARV
	...
	fCutProbSigma = m_fCutProbA
			+ m_fCutProbB * pow(fPercentBAToCut, m_fCutProbC);
	...
	while (p_oTree) {
		...
		if (clModelMath::GetRand() <= fCutProb) {
			...
			p_oTree->SetValue(mp_iHarvestCode[iSp], false);
  ...
  out << fBARemoved << "\n";
	...
	if (m_bSaplingMortality) {
		...
		fCutProb =
				m_fSapP
						+ ((1 - m_fSapP)
								/ (1 + pow(fPercentBAToCut / m_fSapM, m_fSapN)));
...
void clGeneralizedHarvestRegime::GetDataCodes() {
	...
	if (m_bUseBiomass == false)
		return;

	int i;
	mp_iBiomassCode = new short int[m_iNumSpecies];
	for (i = 0; i < m_iNumSpecies; i++) {
		mp_iBiomassCode[i] = mp_oPop->GetFloatDataCode("Biomass", i,
		...
		if (-1 == mp_iBiomassCode[i]) {
			...
			s << "Type/species combo species=" << i << " type="
					<< clTreePopulation::adult
					<< " does not have Dimension Analysis applied.";
			stcErr.iErrorCode = BAD_DATA;
...
bool clGeneralizedHarvestRegime::CutThisTimestep() {
	...
	float fVal, fLogProb, fPlotArea = p_oPlot->GetPlotArea();
	...
	if (m_bUseBiomass) {
		...
		while (p_oTree) {
			...
			p_oTree->GetValue(
					mp_oPop->GetDbhCode(p_oTree->GetSpecies(),
	...
	//NOT logging
	if (0 == m_fTotalBiomass)
		return false;

	fLogProb = m_fLogProbA
			* exp(-m_fLogProbM * pow(m_fTotalBiomass, m_fLogProbB));

	if (clModelMath::GetRand() <= fLogProb)
		return false;
	else
		return true;
}
...
void clGeneralizedHarvestRegime::RegisterTreeDataMembers() {
	...
	for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
		...
		if (clTreePopulation::adult != mp_whatSpeciesTypeCombos[i].iType) {
			modelErr stcErr;
			stcErr.sFunction =
					"clGeneralizedHarvestRegime::RegisterTreeDataMembers";
...
double clGeneralizedHarvestRegime::GetPercentToCut() {
	double fPercentBAToCutMean, fPercentBAToCut;
	int i;
	...
	if (m_bUseGammaDist) {
		...
		fPercentBAToCutMean = m_fGammaMeanRemoveA
				* exp(
						-m_fGammaMeanRemoveM
								* pow(m_fTotalBiomass, m_fGammaMeanRemoveB));
		if (fPercentBAToCutMean <= 0)
			return 0;

		//Cut it off at 100 draws; after that we'll go with 100
		i = 0;
		do {
			fPercentBAToCut = clModelMath::GammaRandomDraw(fPercentBAToCutMean,
					m_fScale);
			i++;
		} while (fPercentBAToCut > 100 && i < 100);
		if (i == 99 && fPercentBAToCut > 100)
			fPercentBAToCut = 100;
		...
		if (i >= m_iNumUserDefDistSizeClasses)
			i = m_iNumUserDefDistSizeClasses - 1;
		...
		if (i == 0)
			...
			fLowerBound = mp_fCutAmtIntensityClasses[i - 1];
