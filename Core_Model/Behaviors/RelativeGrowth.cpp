...
void clRelativeGrowth::DoShellSetup(DOMDocument *p_oDoc) {
	try {
		...
		if (diameter_auto == m_iGrowthMethod
				|| diameter_only == m_iGrowthMethod) {
		...
		if (diameter_auto == m_iGrowthMethod
			...
			FillSpeciesSpecificValue(p_oElement, "gr_relHeightGrowthHeightExp",
					"gr_rhgheVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
					true);
		}
...
float clRelativeGrowth::CalcDiameterGrowthValue(clTree *p_oTree,
	...
	fAmountDiamIncrease = pow(fDiam, mp_fExp[iSpecies])
...
float clRelativeGrowth::CalcHeightGrowthValue(clTree *p_oTree,
	...
	float fAnnualRelativeGrowth, //amount of annual relative growth
			...
			fAmountHeightIncrease; //amount by which the tree's height will
								   //increase
	short int iSpecies = p_oTree->GetSpecies(), iType = p_oTree->GetType(),
			iLightCode = mp_oGrowthOrg->GetLightCode(iSpecies, iType), iYr;
	...
	for (iYr = 0; iYr < m_fNumberYearsPerTimestep; iYr++) {
		fNewHeight += fAnnualRelativeGrowth
				* pow(fNewHeight, mp_fExp[iSpecies]);
	}
