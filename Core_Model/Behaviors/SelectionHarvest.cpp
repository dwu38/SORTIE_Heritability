...
void clSelectionHarvest::GetData(DOMDocument *p_oDoc) {
	try {
		...
		if (0 == m_iNumAllowedCutRanges) {
			...
			stcErr.sMoreInfo =
					"Invalid number of cut ranges. At least 1 cut range must be specified.";
			throw(stcErr);
		}
		...
		for (i = 0; i < m_iNumAllowedCutRanges; i++) {
			...
			FillSingleValue(p_oElement, "sha_target_BA", &mp_fTargetBA[i],
					true);
			...
			if (mp_fTargetBA[i] < 0) {
				...
				stcErr.sFunction = "clSelectionHarvest::GetData";
				stcErr.sMoreInfo =
						"Invalid Target Basal Area value. Value must be greater than or equal to zero.";
				throw(stcErr);
			}
			...
			if (mp_fHighDBH[i] < 0 || mp_fLowDBH[i] < 0) {
				...
				stcErr.sFunction = "clSelectionHarvest::GetData";
				stcErr.sMoreInfo =
						"Invalid bound on size class. Values must be greater than or equal to zero.";
				throw(stcErr);
			}
			...
			if (mp_fLowDBH[i] >= mp_fHighDBH[i]) {
				...
				stcErr.sFunction = "clSelectionHarvest::GetData";
				stcErr.sMoreInfo =
						"Invalid bound on size class. The lower bound must be smaller than the upper bound.";
				throw(stcErr);
			}
			...
			if (i != 0) {
				if (mp_fLowDBH[i] <= mp_fHighDBH[i - 1]) {
					...
					stcErr.sFunction = "clSelectionHarvest::GetData";
					stcErr.sMoreInfo =
							"Invalid bound on size class. The values must not overlap.";
					throw(stcErr);
				}
			}
		}
...
/////////////////////////////////////////////////////////////////////////////
int clSelectionHarvest::CalculateBasalAreaDifference(
	...
	for (i = 0; i < m_iNumAllowedCutRanges; i++) {
		...
		mp_fLandscapeBasalArea[i] = mp_fLandscapeBasalArea[i]
				- mp_fTempTargetBA[i];
...
void clSelectionHarvest::CreateMasterCutPackage(int iSpecies) {
	...
	for (int i = 0; i < m_iNumAllowedCutRanges; i++) {
		if ((int) mp_fLandscapeBasalArea[i] == 0
				|| (int) mp_fBasalArea[i] == 0) {
			mp_fBasalArea[i] = 0;
		} else {
	...
	if (!mp_oNewPackage) //either there's no packages in grid, or new is first
		...
		//make the new one after the one in mp_oNewPackage
	...
	for (int i = 0; i < iNumRows; i++) {
		...
		for (int j = 0; j < iNumCols; j++) {
			...
			if (!mp_oNewPackage) //either there's no packages in grid, or new is first
				...
				mp_oNewPackage = mp_oCutEventsGrid->CreatePackage(
						mp_oNewPackage);
			...
			if (clTreePopulation::seedling != iTreeType) {
				...
				for (i = 0; i < m_iNumAllowedCutRanges; i++) {
					if (fDbh >= p_fLoDbh[i] && fDbh <= p_fHiDbh[i]) {
						p_fTotalBasalArea[i] += clModelMath::CalculateBasalArea(
								fDbh);
					}
				}
...
//////////////////////////////////////////////////////////////////////////////
void clSelectionHarvest::GetBasalArea(clTreePopulation *p_oTreePop,
		int iSpecies, float *p_fTotalBasalArea, float *p_fLoDbh,
		float *p_fHiDbh) {
	try {
		clTree *p_oTree;                   //tree whose basal area we're getting
		clTreeSearch *p_oTreeList;         //list of all trees in the population
		...
		while (p_oTree != NULL) {
			...
			iTreeType = p_oTree->GetType();
			if (clTreePopulation::seedling != iTreeType
					&& iTreeSpecies == iSpecies) {
				p_oTree->GetValue(mp_oPop->GetDbhCode(iTreeSpecies, iTreeType),
						&fDbh);
				for (i = 0; i < m_iNumAllowedCutRanges; i++) {
					if (fDbh >= p_fLoDbh[i] && fDbh <= p_fHiDbh[i]) {
						mp_fBasalArea[i] += clModelMath::CalculateBasalArea(
								fDbh);
					}
				}
			} //end of if (clTreePopulation::seedling != iTreeType)
...
void clSelectionHarvest::GetHarvestGrids() {
	try {
		...
		m_iMasterTimestepCode = mp_oMasterCutsGrid->GetPackageIntDataCode(
				"timestep");
		...
		m_iAmountTypeCode = mp_oMasterCutsGrid->GetPackageIntDataCode(
				"amttype");

		for (i = 0; i < iNumSpecies; i++) {
			sprintf(cFloatLabel, "%s%d", "species", i);
			mp_iSpeciesCodes[i] = mp_oMasterCutsGrid->GetPackageBoolDataCode(
					cFloatLabel);
		}

		for (i = 0; i < m_iNumAllowedCutRanges; i++) {
			sprintf(cFloatLabel, "%s%d", "rangemin", i);
			mp_iRangeMinCodes[i] = mp_oMasterCutsGrid->GetPackageFloatDataCode(
					cFloatLabel);
			sprintf(cFloatLabel, "%s%d", "rangemax", i);
			mp_iRangeMaxCodes[i] = mp_oMasterCutsGrid->GetPackageFloatDataCode(
					cFloatLabel);
		...
		if (-1 == m_iAmountTypeCode || -1 == m_iMasterIDCode
				|| -1 == m_iMasterTimestepCode || -1 == m_iCutTypeCode) {
			modelErr stcErr;
			stcErr.iErrorCode = BAD_DATA;
			stcErr.sFunction = "clSelectionHarvest::GetHarvestGrids";
		...
		for (i = 0; i < iNumSpecies; i++) {
			if (-1 == mp_iSpeciesCodes[i]) {
				...
				stcErr.sFunction = "clSelectionHarvest::GetHarvestGrids";
				stcErr.sMoreInfo =
						"Unexpected master grid data return code for species.";
				throw(stcErr);
			}
		}
		for (i = 0; i < m_iNumAllowedCutRanges; i++) {
			if (-1 == mp_iRangeMinCodes[i] || -1 == mp_iRangeMaxCodes[i]
					|| -1 == mp_iRangeAmountCodes[i]) {
				modelErr stcErr;
				stcErr.iErrorCode = BAD_DATA;
				stcErr.sFunction = "clSelectionHarvest::GetHarvestGrids";
				stcErr.sMoreInfo =
						"Unexpected master grid data return code for range.";
				throw(stcErr);
			}
		}
		...
		m_iCutTimestepCode = mp_oCutEventsGrid->GetPackageIntDataCode(
				"timestep");
...
void clSelectionHarvest::ValidatePackages() {
	try {
		...
		short int iNumXCells = mp_oCutEventsGrid->GetNumberXCells(),
				iNumYCells = mp_oCutEventsGrid->GetNumberYCells(), i, j;
				...
				while (p_oPackage) {
					...
					while (p_oNextPackage) {
						...
						if (iID == iNextID) {
							...
							stcErr.iErrorCode = BAD_DATA;
							stcErr.sFunction =
									"clSelectionHarvest::ValidatePackages";
							stcErr.sMoreInfo =
									"Harvest cut may not be applied twice to the same grid cell.";
							throw(stcErr);
						}
...
void clSelectionHarvest::GetTimeSinceHarvest() {
	try {
		...
		m_iHarvestTypeCode = mp_oResultsGrid->GetPackageIntDataCode(
				"Harvest Type");
		...
		if (iTimestep == 1) {
			...
			for (i = 0; i < iNumXCells; i++) {
				for (j = 0; j < iNumYCells; j++) {
					...
					mp_oTimeSinceHarvestGrid->SetValueOfCell(i, j, m_iTime,
							m_iInitialAge);
		...
		//End of First timestep administrative stuff
			...
			for (i = 0; i < iNumXCells; i++) {
				for (j = 0; j < iNumYCells; j++) {
					...
					mp_oTimeSinceHarvestGrid->GetValueOfCell(i, j, m_iTime,
							&iTime);
					//Get the harvest type value from the grid cell
					mp_oResultsGrid->GetValueOfCell(i, j, m_iHarvestTypeCode,
							&iHarvestType);
