...
void clFuncResponseSeedPredation::Action() {
	try {
		...
		if (m_bIsLinked) {
			...
			for (iX = 0; iX < iNumXCells; iX++) {
				for (iY = 0; iY < iNumYCells; iY++) {
					mp_oPredatorGrid->GetValueOfCell(iX, iY, m_iNumPredsCode,
							&fNumPredators);
			...
			for (iX = 0; iX < iNumXCells; iX++) {
				for (iY = 0; iY < iNumYCells; iY++) {
					mp_oPredatorGrid->SetValueOfCell(iX, iY, m_iNumPredsCode,
							fNumPredators);
				}
			}
				...
				for (iY = 0; iY < iNumYCells; iY++) {
					...
					for (i = 0; i < m_iNumBehaviorSpecies; i++) {
						mp_oSeedGrid->SetValueOfCell(iX, iY,
								mp_iSeedGridCode[i], mp_fTempSeeds[i]);
					}
					mp_oPredatorGrid->SetValueOfCell(iX, iY, m_iNumPredsCode,
							m_fTempPredators);
				}
...
void clFuncResponseSeedPredation::DoModel() {
	...
	//if outputting - number of overall predators for each timestep
			...
			*p_fPerCapitaOfftake = NULL, *p_fNumSeeds = NULL;
	try {
		using namespace std;
		fstream oOut;
		float
				...
				fAllOfftake, fTotalSeeds, fModelAreaInHa,
		...
		if (m_bIsLinked) {
			...
			fModelAreaInHa = m_fCellArea / M_SQ_PER_HA;
		...
		for (iWeek = 0; iWeek < m_iNumWeeksToModel; iWeek++) {
			...
			for (i = 0; i < m_iNumBehaviorSpecies; i++) {
				...
				fPerCapitaSeedAvail = (p_fNumSeeds[i] / m_fTempPredators)
						* fWeekToDay;
			...
			fInstantaneousRateOfChange =
					mp_fMaxInstantaneousDeclineRate[iSeason]
			...
			m_fTempPredators = m_fTempPredators
					* exp(fInstantaneousRateOfChange);
...
void clFuncResponseSeedPredation::GetParameterFileData(DOMDocument *p_oDoc) {
	...
	try {
		...
		//absense of food
		FillSingleValue(p_oElement, "pr_funcRespMaxInstDeclineRate1",
				&mp_fMaxInstantaneousDeclineRate[0], true);
		FillSingleValue(p_oElement, "pr_funcRespMaxInstDeclineRate2",
				&mp_fMaxInstantaneousDeclineRate[1], true);

		//Population's demographic efficiency
		FillSingleValue(p_oElement, "pr_funcRespDemographicEfficiency1",
				&mp_fDemographicEfficiency[0], true);
		FillSingleValue(p_oElement, "pr_funcRespDemographicEfficiency2",
				&mp_fDemographicEfficiency[1], true);

		//Density-dependent coefficient
		FillSingleValue(p_oElement, "pr_funcRespDensityDependentCoefficient1",
				&mp_fDensityDependentCoefficient[0], true);
		FillSingleValue(p_oElement, "pr_funcRespDensityDependentCoefficient2",
				&mp_fDensityDependentCoefficient[1], true);

		//Maximum intake rate - number of seeds per predator per week
		FillSpeciesSpecificValue(p_oElement, "pr_funcRespMaxIntakeRate",
				"pr_frmirVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		FillSpeciesSpecificValue(p_oElement, "pr_funcRespForagingEfficiency",
				"pr_frfeVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fForagingEfficiency[i] = p_fTempValues[i].val;

		//Number of weeks for which seedfall occurs
		FillSingleValue(p_oElement, "pr_funcRespNumWeeksSeedFall",
				&m_iNumWeeksSeedFall, true);
		...
		if (m_iNumWeeksSeedFall <= 0) {
			...
			stcErr.sFunction =
					"clFuncResponseSeedPredation::GetParameterFileData";
			stcErr.sMoreInfo =
					"Number of weeks of seedfall must be greater than zero.";
			throw(stcErr);
		}

		//Number of weeks to model
		FillSingleValue(p_oElement, "pr_funcRespNumWeeksToModel",
				&m_iNumWeeksToModel, true);
		...
		if (m_iNumWeeksToModel <= 0 || m_iNumWeeksToModel > 52) {
			...
			stcErr.sFunction =
					"clFuncResponseSeedPredation::GetParameterFileData";
			stcErr.sMoreInfo =
					"Number of weeks to model must be between 1 and 52 inclusive.";
			throw(stcErr);
		}

		//Week in which germination starts
		FillSingleValue(p_oElement, "pr_funcRespWeekGerminationStarts",
				&m_iWeekGerminationStarts, true);
		...
		if (m_iWeekGerminationStarts < 0 || m_iWeekGerminationStarts > 52) {
			...
			stcErr.sFunction =
					"clFuncResponseSeedPredation::GetParameterFileData";
			stcErr.sMoreInfo =
					"Week germination starts must be between 0 and 52 inclusive.";
			throw(stcErr);
		}

		//Week in which season 2 starts
		FillSingleValue(p_oElement, "pr_funcRespWeekSeason2Starts",
				&m_iWeekSeason2Starts, true);
		//Throw an error if not between 0 and 52 (inclusive)
		if (m_iWeekSeason2Starts < 0 || m_iWeekSeason2Starts > 52) {
			...
			stcErr.sFunction =
					"clFuncResponseSeedPredation::GetParameterFileData";
			stcErr.sMoreInfo =
					"Week season 2 must be between 0 and 52 inclusive.";
			throw(stcErr);
		}

		//Proportion germinating
		FillSingleValue(p_oElement, "pr_funcRespProportionGerminating",
				&m_fProportionGerminating, true);
		...
		if (m_fProportionGerminating < 0 || m_fProportionGerminating > 1) {
			...
			stcErr.sFunction =
					"clFuncResponseSeedPredation::GetParameterFileData";
			stcErr.sMoreInfo =
					"Proportion germinating must be between 0 and 1 inclusive.";
			throw(stcErr);
		}

		//Output filename, if desired
		FillSingleValue(p_oElement, "pr_funcRespOutputFilename", &m_sOutput,
				false);

		//Whether or not to preserve predator densities
		FillSingleValue(p_oElement, "pr_funcRespPreservePredatorDensities",
				&m_bPreservePredatorDensities, true);

		//Predator initial density in number per square meter
		FillSingleValue(p_oElement, "pr_funcRespPredatorInitialDensity",
				&m_fPredatorInitialDensity, true);
		//Throw an error if not positive
		if (m_fPredatorInitialDensity <= 0) {
			...
			stcErr.sFunction =
					"clFuncResponseSeedPredation::GetParameterFileData";
			stcErr.sMoreInfo =
					"Predator initial density must be greater than zero.";
			throw(stcErr);
		}
		delete[] p_fTempValues;
...
void clFuncResponseSeedPredation::GetData(DOMDocument *p_oDoc) {
	try {
		...
		if (mp_oSeedGrid == NULL) {
			...
			stcErr.sMoreInfo =
					"Disperse behaviors must be used with functional response seed predation.";
		...
		m_fCellArea = mp_oSeedGrid->GetLengthXCells()
		...
		if (mp_oPredatorGrid == NULL) {
			...
			if (mp_oSeedGrid->GetNumberXCells()
					!= mp_oPredatorGrid->GetNumberXCells()
					|| mp_oSeedGrid->GetNumberYCells()
							!= mp_oPredatorGrid->GetNumberYCells()) {
				modelErr stcErr;
				stcErr.iErrorCode = BAD_DATA;
				stcErr.sFunction = "clFuncResponseSeedPredation::GetData";
				stcErr.sMoreInfo =
						"Predator grid cell resolution doesn't match seed grid cell resolution.";
				throw(stcErr);
			}
		}
...
void clFuncResponseSeedPredation::SetNameData(std::string sNameString) {
	...
	if (sNameString.compare("FunctionalResponseSeedPredation") == 0) {
		m_sNameString = "FunctionalResponseSeedPredation";
		m_bIsLinked = false;
	} else if (sNameString.compare("LinkedFunctionalResponseSeedPredation")
			== 0) {
