...
void clDoubleMMRelGrowth::SetNameData(std::string sNameString) {
	try {
		...
		} else if (sNameString.compare("DoubleResourceRelative diam only")
				== 0) {
...
void clDoubleMMRelGrowth::DoShellSetup(DOMDocument *p_oDoc) {
	...
	try {
		...
		FillSpeciesSpecificValue(p_oElement, "gr_diamDoubleMMResourceInfluence",
				"gr_ddmmriVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			if (mp_fSlopeDiamGrowthResponse[mp_iWhatSpecies[i]] != 0.0)
				mp_fSlopeDiamGrowthResponse[mp_iWhatSpecies[i]] = (1
		...
		if (-1 == m_iResourceCode) {
			...
			stcErr.sFunction = "clDoubleMMRelGrowth::DoShellSetup";
			stcErr.sMoreInfo =
					"Grid object \"Resources\" is set up incorrectly.";
			stcErr.iErrorCode = CANT_FIND_OBJECT;
			throw(stcErr);
		}
...
float clDoubleMMRelGrowth::CalcDiameterGrowthValue(clTree *p_oTree,
	...
	fAnnualRelativeGrowth = mp_fAsympDiamGrowth[iSpecies]
			+ (mp_fResourceInfluence[iSpecies] * fResource);
	fAnnualRelativeGrowth = (fAnnualRelativeGrowth * fGli)
			/ ((fAnnualRelativeGrowth / mp_fSlopeDiamGrowthResponse[iSpecies])
					+ fGli);
