...
clLightDepSeedSurvival::clLightDepSeedSurvival(clSimManager *p_oSimManager) :
		clWorkerBase(p_oSimManager), clBehaviorBase(p_oSimManager), clLightBase(
				p_oSimManager), clGLIBase(p_oSimManager) {
	try {
...
/////////////////////////////////////////////////////////////////////////////
void clLightDepSeedSurvival::GetParameterFileData(
	...
	try {
		...
		//Optimum GLI
		FillSpeciesSpecificValue(p_oElement, "es_optimumGLI", "es_ogVal",
		...
		FillSpeciesSpecificValue(p_oElement, "es_lowSlope", "es_lsVal", p_fTemp,
		...
		FillSpeciesSpecificValue(p_oElement, "es_highSlope", "es_hsVal",
				p_fTemp, m_iNumBehaviorSpecies, p_oPop, true);
		//Now transfer the values to the permanent array
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			if (mp_fOptimumGLI[i] < 0 || mp_fOptimumGLI[i] > 100) {
				...
				stcErr.sFunction =
						"clLightDepSeedSurvival::GetParameterFileData";
		...
		if (!m_bUseStormLight) {
			...
			//Minimum sun angle
			FillSingleValue(p_oElement, "li_minSunAngle", &m_fMinSunAngle,
					true);
			//Azimuth of north - not required for backwards compatibility
			FillSingleValue(p_oElement, "li_AziOfNorth", &m_fAzimuthOfNorth,
					false);
			...
			FillSpeciesSpecificValue(p_oElement,
					"li_lightExtinctionCoefficient", "li_lecVal", p_fTempAll,
					p_oPop, true);
			//Transfer to array
			...
			FillSpeciesSpecificValue(p_oElement, "es_lightExtCoeffFullDmg",
					"es_lecfdVal", p_fTempAll, p_oPop, false);
			//Transfer to array
			for (i = 0; i < m_iNumTotalSpecies; i++) {
				mp_fLightExtCoeff[i][2] = p_fTempAll[i];
			}
			...
			FillSpeciesSpecificValue(p_oElement, "es_lightExtCoeffPartDmg",
					"es_lecpdVal", p_fTempAll, p_oPop, false);
			//Transfer to array
			for (i = 0; i < m_iNumTotalSpecies; i++) {
				mp_fLightExtCoeff[i][1] = p_fTempAll[i];
			}
			...
			FillSingleValue(p_oElement, "es_lightHeight", &m_fLightHeight,
					true);
			...
			for (i = 0; i < m_iNumTotalSpecies; i++) {
				for (j = 0; j < 3; j++) {
					if (mp_fLightExtCoeff[i][j] < 0
							|| mp_fLightExtCoeff[i][j] > 1) {
						modelErr stcErr;
						stcErr.iErrorCode = BAD_DATA;
						stcErr.sFunction =
								"clLightDepSeedSurvival::GetParameterFileData";
						stcErr.sMoreInfo =
								"All light extinction coefficients must be between 0 and 1.";
						throw(stcErr);
					}
				}
			}
			...
			if (m_fLightHeight < 0) {
				...
				stcErr.sFunction =
						"clLightDepSeedSurvival::GetParameterFileData";
				stcErr.sMoreInfo =
						"Height of light calculations must be greater than 0.";
				throw(stcErr);
			}
			...
			if (m_iNumAltAng < 1 || m_iNumAziAng < 1) {
				...
				stcErr.sFunction =
						"clLightDepSeedSurvival::GetParameterFileData";
				stcErr.sMoreInfo =
						"Number of altitude and azimuth sky divisions must be at least 1.";
				throw(stcErr);
			...
			if (0 > m_fAzimuthOfNorth || (2.0 * M_PI) < m_fAzimuthOfNorth) {
				...
				stcErr.sFunction =
						"clLightDepSeedSurvival::GetParameterFileData";
				stcErr.sMoreInfo =
						"Azimuth of north must be between 0 and 2PI.";
				throw(stcErr);
			}
...
void clLightDepSeedSurvival::DoLightSetup() {
	...
	try {
		...
		clTreePopulation *p_oPop =
				(clTreePopulation*) mp_oSimManager->GetPopulationObject(
						"treepopulation");
		m_fMaxSearchDistance = (mp_oLightOrg->GetMaxTreeHeight()
				- m_fLightHeight) * m_fRcpTanMinAng
...
// GetGLI()
/////////////////////////////////////////////////////////////////////////////
float clLightDepSeedSurvival::GetGLI(clTreePopulation *p_oPop, const float &fX,
		const float &fY) {
	...
	sprintf(cQuery, "%s%f%s%f%s%f%s%f", "distance=", m_fMaxSearchDistance,
...
void clLightDepSeedSurvival::SetupGrid() {
	...
	for (i = 0; i < m_iNumBehaviorSpecies; i++) {
		...
		if (-1 == mp_iSeedGridCode[i]) {
			...
			stcErr.sFunction = "clLightDepSeedSurvival::SetupGrids";
			stcErr.sMoreInfo =
					"Unexpected grid data return code for Dispersed Seeds.";
			throw(stcErr);
		}
		sLabel.str("");
	}
	...
	if (m_bUseStormLight) {
		...
		if ( NULL == mp_oStormLightGrid) {
			...
			stcErr.sFunction = "clLightDepSeedSurvival::SetupGrids";
			stcErr.sMoreInfo =
					"The Storm Light behavior is required for Storm Light Seed Dependent Survival.";
			throw(stcErr);
		}
		...
		if (-1 == m_iLightCode) {
			...
			stcErr.sFunction = "clLightDepSeedSurvival::SetupGrids";
			stcErr.sMoreInfo =
					"Unexpected grid data return code for Storm Light.";
			throw(stcErr);
		}
	}
}
...
void clLightDepSeedSurvival::GetTreeDataMemberCodes() {
	...
	for (i = 0; i < m_iNumTotalSpecies; i++) {
		...
		mp_iDamageCodes[i][0] = p_oPop->GetIntDataCode("stm_dmg", i,
				clTreePopulation::sapling);
		mp_iDamageCodes[i][1] = p_oPop->GetIntDataCode("stm_dmg", i,
...
float clLightDepSeedSurvival::GetLightFavorability(const int &iSpecies,
	...
	if (fGLI > mp_fOptimumGLI[iSpIndex]) {
		...
		fFavorability = 1
				- (mp_fHighGLISlope[iSpIndex]
						* (fGLI - mp_fOptimumGLI[iSpIndex]));
	} else if (fGLI < mp_fOptimumGLI[iSpIndex]) {
		//GLI is less than optimum - calculate using below-optimum slope
		fFavorability =
				1
						- (mp_fLowGLISlope[iSpIndex]
								* (mp_fOptimumGLI[iSpIndex] - fGLI));
	} else {
			...
			for (iY = 0; iY < iNumYCells; iY++) {
				...
				for (i = 0; i < m_iNumBehaviorSpecies; i++) {
					...
					mp_oSeedGrid->GetValueOfCell(iX, iY,
							mp_iSeedGridCode[mp_iIndexes[iSpecies]],
							&fNumSeeds);
					...
					mp_oSeedGrid->SetValueOfCell(iX, iY,
							mp_iSeedGridCode[mp_iIndexes[iSpecies]], fNewSeeds);
				}
			}
		}
	} catch (modelErr &err) {
...
float clLightDepSeedSurvival::GetLightExtinctionCoefficient(clTree *p_oTree) {
	...
	if (clTreePopulation::snag == iTp
			|| -1
					== mp_iDamageCodes[p_oTree->GetSpecies()][iTp
							- clTreePopulation::sapling]) {
	...
	p_oTree->GetValue(
			mp_iDamageCodes[p_oTree->GetSpecies()][p_oTree->GetType()
					- clTreePopulation::sapling], &iDamage);
...
void clLightDepSeedSurvival::SetNameData(std::string sNameString) {
	try {
		...
		if (sNameString.compare("LightDependentSeedSurvival") == 0) {
			m_bUseStormLight = false;
		} else if (sNameString.compare("StormLightDependentSeedSurvival")
				== 0) {
