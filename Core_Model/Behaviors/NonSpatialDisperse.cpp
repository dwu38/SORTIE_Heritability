...
void clNonSpatialDispersal::DoShellSetup(DOMDocument *p_oDoc) {
	...
	try {
		...
		//Slope of lambda
		FillSpeciesSpecificValue(p_oElement, "di_nonSpatialSlopeOfLambda",
				"di_nssolVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		...
		//Intercept of lambda
		FillSpeciesSpecificValue(p_oElement, "di_nonSpatialInterceptOfLambda",
				"di_nsiolVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		FillSpeciesSpecificValue(p_oElement, "di_minDbhForReproduction",
				"di_mdfrVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fDbhForReproduction[p_fTempValues[i].code] =
					p_fTempValues[i].val;
...
void clNonSpatialDispersal::AddSeeds() {
	...
	try {
		...
		float
				fBasalArea, //basal area in sq. m./ha for a species
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			...
			//Compute lambda
			fLambda = clModelMath::CalcPointValue(fBasalArea,
					mp_fSlopeOfLambda[iSpecies],
					mp_fInterceptOfLambda[iSpecies]);
...
float clNonSpatialDispersal::ComputeBasalArea(clTreePopulation *p_oPop,
	...
	if (p_oPop->GetMinAdultDBH(iSpecies) <= mp_fDbhForReproduction[iSpecies]) {
		sprintf(cQuery, "%s%d%s%d", "type=", clTreePopulation::adult,
				"::species=", iSpecies);
	} else {
		sprintf(cQuery, "%s%d%s%d%s%d", "type=", clTreePopulation::adult, ",",
				clTreePopulation::sapling, "::species=", iSpecies);
	}
	...
	while (p_oTree) {
		...
		p_oTree->GetValue(p_oPop->GetDbhCode(iSpecies, p_oTree->GetType()),
				&fDbh);
