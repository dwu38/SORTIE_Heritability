...
void clPRSemiStochGrowth::DoShellSetup(DOMDocument *p_oDoc) {
	try {
		...
		FillSpeciesSpecificValue(p_oElement, "gr_prStochHiteThreshold",
				"gr_pshtVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer values to our permanent array
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fHeightThreshold[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;

		//"a" for deterministic growth
		FillSpeciesSpecificValue(p_oElement, "gr_prStochDetermA", "gr_psdaVal",
		...
		FillSpeciesSpecificValue(p_oElement, "gr_prStochDetermB", "gr_psdbVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer values to our permanent array
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		FillSpeciesSpecificValue(p_oElement, "gr_prStochMeanDBH", "gr_psmdVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer values to our permanent array
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fMeanDiam[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;

		//Standard deviation for stochastic growth
		FillSpeciesSpecificValue(p_oElement, "gr_prStochStdDev", "gr_pssdVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer values to our permanent array
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fDiamStdDev[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;
...
//////////////////////////////////////////////////////////////////////////////
float clPRSemiStochGrowth::CalcDiameterGrowthValue(clTree *p_oTree,
	...
	if (fHeight > mp_fHeightThreshold[mp_iIndexes[iSpecies]]) {
		do {
			...
			fDiamIncrement = (clModelMath::NormalRandomDraw(
					mp_fDiamStdDev[mp_iIndexes[iSpecies]])
					+ mp_fMeanDiam[mp_iIndexes[iSpecies]]) - fDiam;
		} while (fDiam + fDiamIncrement < 0.1);
	} else {

		//Calculate the amount of diam increase based on the new height
		fHeight *= 100; //transform to cm
		fDiamIncrement = (mp_fA[mp_iIndexes[iSpecies]]
				* exp(-mp_fB[mp_iIndexes[iSpecies]] * fHeight)) - fDiam;
