	...
	try {
		...
		clTreePopulation *p_oPop =
				(clTreePopulation*) mp_oSimManager->GetPopulationObject(
						"treepopulation");
		const char *cCounterLabel = "lf_count", *cZLabel = "z", *cLightLabel =
				"Light";
		float fTemp;
		...
		FillSingleValue(p_oElement, "lf_lightExtinctionCoefficient",
				&m_fLightExtinctionCoefficient, true);
		...
		for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
			mp_iCounterCodes[mp_whatSpeciesTypeCombos[i].iSpecies][mp_whatSpeciesTypeCombos[i].iType] =
					p_oPop->RegisterInt(cCounterLabel,
			...
			mp_iLightCodes[mp_whatSpeciesTypeCombos[i].iSpecies][mp_whatSpeciesTypeCombos[i].iType] =
					p_oPop->GetFloatDataCode(cLightLabel,
							mp_whatSpeciesTypeCombos[i].iSpecies,
							mp_whatSpeciesTypeCombos[i].iType);

			mp_iHeightCodes[mp_whatSpeciesTypeCombos[i].iSpecies][mp_whatSpeciesTypeCombos[i].iType] =
					p_oPop->GetHeightCode(mp_whatSpeciesTypeCombos[i].iSpecies,
							mp_whatSpeciesTypeCombos[i].iType);
...
void clLightFilter::Action() {
	try {
		...
		int
				iCounter, //tree's respite counter value
				iNumYearsPerTimestep =
		...
		while (p_oTree) {
			...
			iFilterHeight = (int) (m_iFilterHeight
					* (0.9995 + (0.001 * clModelMath::GetRand())));

			//If above the filter height, exit out
			if (iEffectiveHeight >= iFilterHeight)
				goto nextTree;
			...
			fPercentTransmission = exp(
					-iPathLength * m_fLightExtinctionCoefficient);
