...
void clRipleysKCalculator::GetParameterFileData(xercesc::DOMDocument *p_oDoc) {
	try {
		...
		FillSingleValue(p_oElement, "an_RipleysKMaxDistance", &m_fMaxDistance,
				true);

		//Get the increment
		FillSingleValue(p_oElement, "an_RipleysKDistanceInc", &m_fIncrement,
				true);
...
void clRipleysKCalculator::SetupGrid() {
	...
	//Create the grid
	mp_oGrid = mp_oSimManager->CreateGrid("Ripley's K", 0,      //number of ints
	...
	try {
		...
		for (i = 0; i <= m_iNumTotalSpecies; i++) {
			if (p_fNumTrees[i] > 0) {
				...
				for (j = 0; j < m_iNumIncs; j++) {
					mp_fKValues[i][j] = (mp_fKValues[i][j] * fPlotArea)
							/ p_fNumTrees[i];
				}
			}
		}
