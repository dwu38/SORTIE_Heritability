/*
 * DisperseParamChange.cpp
 *
 *  Created on: 10 janv. 2018
 *      Author: melaine
 *
 *	(DW edit: added ParamG2 by copying ParamG1 code, not sure if it works)
 *
 *
 */

//---------------------------------------------------------------------------
#include "DisperseParamChange.h"
//---------------------------------------------------------------------------
...
// Constructor
///////////////////////////////////////////////////////////////////////////////
clDisperseParamChange::clDisperseParamChange(clSimManager *p_oSimManager) :
		clWorkerBase(p_oSimManager), clBehaviorBase(p_oSimManager), clDisperseBase(
				p_oSimManager) {
	try {
		m_sNameString = "DisperseParamChange";
		m_sXMLRoot = "DisperseParamChange";
		...
		stcErr.sFunction = "clDisperseParamChange::clDisperseParamChange";
		throw(stcErr);
	}
}
...
// Destructor
///////////////////////////////////////////////////////////////////////////////
clDisperseParamChange::~clDisperseParamChange() {
...
// DoShellSetup
///////////////////////////////////////////////////////////////////////////////
void clDisperseParamChange::DoShellSetup(xercesc::DOMDocument *p_oDoc) {
	try {
		...
		modelErr stcErr;
		stcErr.iErrorCode = UNKNOWN;
		stcErr.sFunction = "clDisperseParamChange::DoShellSetup";
		throw(stcErr);
	}
}
...
// PopulateUsedTable()
///////////////////////////////////////////////////////////////////////////////
void clDisperseParamChange::PopulateUsedTable(clTreePopulation *p_oPop) {
	...
	for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
		...
		if (mp_whatSpeciesTypeCombos[i].iType != clTreePopulation::sapling
			...
			stcErr.sMoreInfo =
					"Dispersal may only be applied to saplings and adults.";
			stcErr.iErrorCode = BAD_DATA;
			throw(stcErr);
		}
		mp_bIsUsed[mp_whatSpeciesTypeCombos[i].iSpecies][mp_whatSpeciesTypeCombos[i].iType] =
				true;
	}
}
...
///////////////////////////////////////////////////////////////////////////////
void clDisperseParamChange::GetParameterFileData(xercesc::DOMDocument *p_oDoc,
		clTreePopulation *p_oPop) {
	...
	try {
		...
		FillSpeciesSpecificValue(p_oElement, "di_minDbhForReproduction",
				"di_mdfrVal", p_fTemp, m_iNumBehaviorSpecies, p_oPop, true);
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			if (p_iTemp[i].val != weibull && p_iTemp[i].val != lognormal
				...
				stcErr.iErrorCode = BAD_DATA;
				stcErr.sFunction =
						"clDisperseParamChange::GetParameterFileData";
				std::stringstream s;
				s << "Invalid disperse function code " << p_iTemp[i].val;
				stcErr.sMoreInfo = s.str();
				throw(stcErr);
			} else {
				mp_iWhatFunction[mp_iIndexes[p_iTemp[i].code]] =
						(function) p_iTemp[i].val;
			}

		//Non-masting beta
		FillSpeciesSpecificValue(p_oElement, "di_spatialBeta", "di_sbVal",
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			...
			if (p_fTemp[i].val > 25) {
				...
				stcErr.sFunction =
						"clDisperseParamChange::GetParameterFileData";
				stcErr.sMoreInfo =
						"One or more beta values is too large and will cause math errors.";
				throw(stcErr);
			}
		}

		//Masting beta
		FillSpeciesSpecificValue(p_oElement, "di_mastingBeta", "di_mbVal",
				p_fTemp, m_iNumBehaviorSpecies, p_oPop, true);
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
		...
		FillSpeciesSpecificValue(p_oElement, "di_spatialSTR", "di_sstrVal",
				p_fTemp, m_iNumBehaviorSpecies, p_oPop, true);
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fStrMean[nonmast][i] = p_fTemp[i].val;

		//Masting mean STR
		FillSpeciesSpecificValue(p_oElement, "di_mastingSTR", "di_mstrVal",
				p_fTemp, m_iNumBehaviorSpecies, p_oPop, true);
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fStrMean[mast][i] = p_fTemp[i].val;

		//Non-masting STR std dev - we might not need it but read it anyway
		FillSpeciesSpecificValue(p_oElement, "di_spatialSTRStdDev",
				"di_sstrsdVal", p_fTemp, m_iNumBehaviorSpecies, p_oPop, true);
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fStrStdDev[nonmast][i] = p_fTemp[i].val;

		//Masting STR std dev - we might not need it but read it anyway
		FillSpeciesSpecificValue(p_oElement, "di_mastingSTRStdDev",
				"di_mstrsdVal", p_fTemp, m_iNumBehaviorSpecies, p_oPop, true);
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			...
			else if (zeroinflated_poisson_pdf == p_iTemp[i].val)
				mp_iWhatPDFForSTR[i] = zeroinflated_poisson_pdf;
			else {
				modelErr stcErr;
				stcErr.iErrorCode = BAD_DATA;
				stcErr.sFunction =
		...
		FillSpeciesSpecificValue(p_oElement, "di_spatialPropParticipating",
				"di_sppVal", p_fTemp, m_iNumBehaviorSpecies, p_oPop, true);
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			if (p_fTemp[i].val < 0 || p_fTemp[i].val > 1) {
				...
				stcErr.sFunction =
						"clDisperseParamChange::GetParameterFileData";
				stcErr.sMoreInfo =
						"Fraction participating must be between 0 and 1.";
				throw(stcErr);
			}
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		FillSpeciesSpecificValue(p_oElement, "di_mastPropParticipating",
				"di_mppVal", p_fTemp, m_iNumBehaviorSpecies, p_oPop, true);
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			if (p_fTemp[i].val < 0 || p_fTemp[i].val > 1) {
				modelErr stcErr;
				stcErr.iErrorCode = BAD_DATA;
		...
		FillSpeciesSpecificValue(p_oElement, "di_mastGroup", "di_mgVal",
				p_iTemp, m_iNumBehaviorSpecies, p_oPop, true);
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_iGroup[i] = p_iTemp[i].val;
		//Whether to draw STR per species or tree
		FillSpeciesSpecificValue(p_oElement, "di_mastDrawPerSpecies",
				"di_mdpsVal", p_iTemp, m_iNumBehaviorSpecies, p_oPop, true);
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			if (1 == p_iTemp[i].val)
		...
		modelErr stcErr;
		stcErr.iErrorCode = UNKNOWN;
		stcErr.sFunction = "clDisperseParamChange::GetParameterFileData";
		throw(stcErr);
	}
}
...
// CalcSeedCDF
///////////////////////////////////////////////////////////////////////////////
void clDisperseParamChange::CalcSeedCDF(xercesc::DOMDocument *p_oDoc,
		clTreePopulation *p_oPop) {
	float *p_fDispersalX0 = new float[m_iNumBehaviorSpecies], *p_fThetaXb =
			new float[m_iNumBehaviorSpecies];
	floatVal *p_fTemp = NULL; //for getting species-specific values
	try {
		clPlot *p_oPlot = mp_oSimManager->GetPlotObject();
		DOMElement *p_oElement = p_oDoc->getDocumentElement();
		float fXPlotLength = p_oPlot->GetXPlotLength(), fYPlotLength =
		...
		//species use weibull, which use lognormal and which use 2dt
		...
		if (iNumFunctionSpecies > 0) {
			...
			FillSpeciesSpecificValue(p_oElement, "di_weibullCanopyDispersal",
					"di_wcdVal", p_fTemp, iNumFunctionSpecies, p_oPop, true);
			//Transfer to array
			for (i = 0; i < iNumFunctionSpecies; i++)
				p_fDispersalX0[mp_iIndexes[p_fTemp[i].code]] = p_fTemp[i].val;

			//Theta
			FillSpeciesSpecificValue(p_oElement, "di_weibullCanopyTheta",
					"di_wctVal", p_fTemp, iNumFunctionSpecies, p_oPop, true);
			//Transfer to array
			for (i = 0; i < iNumFunctionSpecies; i++) {
				...
				if (p_fTemp[i].val >= 50) {
					modelErr stcErr;
					stcErr.iErrorCode = BAD_DATA;
					stcErr.sFunction = "clDisperseParamChange::CalcSeedCDF";
					stcErr.sMoreInfo =
							"One or more weibull theta values is too large and will cause math errors.";
					throw(stcErr);
				}
			}
		...
		if (iNumFunctionSpecies > 0) {
			//X0
			FillSpeciesSpecificValue(p_oElement, "di_lognormalCanopyX0",
					"di_lcx0Val", p_fTemp, iNumFunctionSpecies, p_oPop, true);
			//Transfer to array
			for (i = 0; i < iNumFunctionSpecies; i++)
				p_fDispersalX0[mp_iIndexes[p_fTemp[i].code]] = p_fTemp[i].val;

			//Xb
			FillSpeciesSpecificValue(p_oElement, "di_lognormalCanopyXb",
					"di_lcxbVal", p_fTemp, iNumFunctionSpecies, p_oPop, true);
			//Transfer to array
			for (i = 0; i < iNumFunctionSpecies; i++)
				p_fThetaXb[mp_iIndexes[p_fTemp[i].code]] = p_fTemp[i].val;
		}
		...
		if (iNumFunctionSpecies > 0) {
			//X0
			FillSpeciesSpecificValue(p_oElement, "di_twodtCanopyX0",
					"di_tcx0Val", p_fTemp, iNumFunctionSpecies, p_oPop, true);
			//Transfer to array
			for (i = 0; i < iNumFunctionSpecies; i++)
				p_fDispersalX0[mp_iIndexes[p_fTemp[i].code]] = p_fTemp[i].val;

			//Xb
			FillSpeciesSpecificValue(p_oElement, "di_twodtCanopyXb",
					"di_tcxbVal", p_fTemp, iNumFunctionSpecies, p_oPop, true);
			//Transfer to array
			for (i = 0; i < iNumFunctionSpecies; i++)
				p_fThetaXb[mp_iIndexes[p_fTemp[i].code]] = p_fTemp[i].val;
		}
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			CalculateProbabilityDistribution(mp_fSeedCDF[nonmast][i],
					m_iMaxDistance,
		...
		if (iNumFunctionSpecies > 0) {
			...
			FillSpeciesSpecificValue(p_oElement, "di_weibullMastingDispersal",
					"di_wmdVal", p_fTemp, iNumFunctionSpecies, p_oPop, true);
			//Transfer to array
			for (i = 0; i < iNumFunctionSpecies; i++)
				p_fDispersalX0[mp_iIndexes[p_fTemp[i].code]] = p_fTemp[i].val;

			//Theta
			FillSpeciesSpecificValue(p_oElement, "di_weibullMastingTheta",
					"di_wmtVal", p_fTemp, iNumFunctionSpecies, p_oPop, true);
			//Transfer to array
			for (i = 0; i < iNumFunctionSpecies; i++) {
				p_fThetaXb[mp_iIndexes[p_fTemp[i].code]] = p_fTemp[i].val;
				if (p_fTemp[i].val >= 50) {
		...
		if (iNumFunctionSpecies > 0) {
			//X0
			FillSpeciesSpecificValue(p_oElement, "di_lognormalMastingX0",
					"di_lmx0Val", p_fTemp, iNumFunctionSpecies, p_oPop, true);
			//Transfer to array
			for (i = 0; i < iNumFunctionSpecies; i++)
				p_fDispersalX0[mp_iIndexes[p_fTemp[i].code]] = p_fTemp[i].val;

			//Xb
			FillSpeciesSpecificValue(p_oElement, "di_lognormalMastingXb",
					"di_lmxbVal", p_fTemp, iNumFunctionSpecies, p_oPop, true);
			//Transfer to array
			for (i = 0; i < iNumFunctionSpecies; i++)
				p_fThetaXb[mp_iIndexes[p_fTemp[i].code]] = p_fTemp[i].val;
		}
		...
		if (iNumFunctionSpecies > 0) {
			//X0
			FillSpeciesSpecificValue(p_oElement, "di_twodtMastingX0",
					"di_tmx0Val", p_fTemp, iNumFunctionSpecies, p_oPop, true);
			//Transfer to array
			for (i = 0; i < iNumFunctionSpecies; i++)
				p_fDispersalX0[mp_iIndexes[p_fTemp[i].code]] = p_fTemp[i].val;

			//Xb
			FillSpeciesSpecificValue(p_oElement, "di_twodtMastingXb",
					"di_tmxbVal", p_fTemp, iNumFunctionSpecies, p_oPop, true);
			//Transfer to array
			for (i = 0; i < iNumFunctionSpecies; i++)
				p_fThetaXb[mp_iIndexes[p_fTemp[i].code]] = p_fTemp[i].val;
		}
		...
		//Calculate cumulative probability arrays
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			CalculateProbabilityDistribution(mp_fSeedCDF[mast][i],
					m_iMaxDistance,
					mp_iWhatFunction[mp_iIndexes[mp_iWhatSpecies[i]]],
					p_fDispersalX0[mp_iIndexes[mp_iWhatSpecies[i]]],
					p_fThetaXb[mp_iIndexes[mp_iWhatSpecies[i]]]);
...
/////////////////////////////////////////////////////////////////////////////
void clDisperseParamChange::CalculateProbabilityDistribution(
		float *p_fProbArray, const int &iMaxDistance, const function &iFunction,
		const float &fDispersalX0, const float &fThetaXb) {
	...
	for (iDistance = 0; iDistance < iMaxDistance; iDistance++) {
		if (weibull == iFunction) {
			fInc = M_PI * (1 + 2 * iDistance)
					* clModelMath::CalculateWeibullFunction(fDispersalX0,
							fThetaXb, iDistance + 1);
		} else {
			if (lognormal == iFunction) {
				fInc = M_PI * (1 + 2 * iDistance)
						* clModelMath::CalculateLognormalFunction(fDispersalX0,
								fThetaXb, iDistance + 1);
			} else {
				fInc = M_PI * (1 + 2 * iDistance)
						* clModelMath::CalculatetwodtFunction(fDispersalX0,
								iDistance + 1);
			}
		}
	...
	for (iDistance = 1; iDistance < iMaxDistance; iDistance++) {
		if (p_fProbArray[iDistance] < p_fProbArray[iDistance - 1]) {
			...
			stcErr.sMoreInfo =
					"Probability array does not increase sequentially.";
			throw(stcErr);
		}
	}
}
...
///////////////////////////////////////////////////////////////////////////////
// AddSeeds()
// modified to change MeanParamG1
/////////////////////////////////////////////////////////////////////////////
void clDisperseParamChange::AddSeeds() {
	try {
		clTreePopulation *p_oPop =
				(clTreePopulation*) mp_oSimManager->GetPopulationObject(
						"treepopulation");
		...
		float fDbh, //tree's dbh
				temp, // to store the number of seeds
				iParamG1, //ParamG1 for the specific tree
				iParamG2, //ParamG2 for the specific tree
				iParamM1, //ParamM1 for the specific tree
				iParamM2; //ParamM2 for the specific tree

		float MeanParamG1; //putting this here for now
        float StDevParamG1; //putting this here for now
        float NSHeritParamG1; //putting this here for now
        float SelParamG1; //putting this here for now

		float MeanParamG2; //putting this here for now
        float StDevParamG2; //putting this here for now
        float NSHeritParamG2; //putting this here for now
        float SelParamG2; //putting this here for now

		float MeanParamM1; //putting this here for now
        float StDevParamM1; //putting this here for now
        float NSHeritParamM1; //putting this here for now
        float SelParamM1; //putting this here for now

		float MeanParamM2; //putting this here for now
        float StDevParamM2; //putting this here for now
        float NSHeritParamM2; //putting this here for now
        float SelParamM2; //putting this here for now

		short int iSp, iType, //species and type of a given tree
				iEvent, i;
		int p_NumSpecies = p_oPop->GetNumberOfSpecies();
		//Allocate memory for the species-specific arrays
		//float *p_SumParam; //splitting p_SumParam for individual parameters
		float *p_SumParamG1;
		float *p_SumParamG2;
		float *p_SumParamM1;
		float *p_SumParamM2;
		float *p_SumSeeds;
		//p_SumParam = new float[p_NumSpecies]; //splitting p_SumParam for individual parameters
		p_SumParamG1 = new float[p_NumSpecies];
		p_SumParamG2 = new float[p_NumSpecies];
		p_SumParamM1 = new float[p_NumSpecies];
		p_SumParamM2 = new float[p_NumSpecies];
		p_SumSeeds = new float[p_NumSpecies];
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			...
			if (deterministic_pdf == mp_iWhatPDFForSTR[i]) {
				mp_fFecundity[i] = m_fNumYearsPerTimestep
						* mp_fStrMean[iEvent][i] / pow(30, mp_fBeta[iEvent][i]);
			} else if (mp_bDrawSTRPerSpecies[i]) {
				float fSTR;
				if (normal_pdf == mp_iWhatPDFForSTR[i]) {
					fSTR = mp_fStrMean[iEvent][i]
							+ clModelMath::NormalRandomDraw(
									mp_fStrStdDev[iEvent][i]);
				} else if (lognormal_pdf == mp_iWhatPDFForSTR[i]) {
					fSTR = clModelMath::LognormalRandomDraw(
							mp_fStrMean[iEvent][i], mp_fStrStdDev[iEvent][i]);
				} else {
					fSTR = clModelMath::ZeroInflatedPoissonRandomDraw(
							mp_fStrMean[iEvent][i], mp_fStrStdDev[iEvent][i]);
				}
				mp_fFecundity[i] = m_fNumYearsPerTimestep * fSTR
						/ pow(30, mp_fBeta[iEvent][i]);
			}
		}
		...
		while (p_oTree) {
			...
			if (mp_bIsUsed[iSp][iType]) {
				...
				// and a random number lets it participate,
				//let it reproduce
				if (fDbh >= mp_fDbhForReproduction[mp_iIndexes[iSp]]
						&& clModelMath::GetRand()
								< mp_fFractionParticipating[mp_iEvent[mp_iIndexes[iSp]]][mp_iIndexes[iSp]]) {
					temp = DisperseOneParentSeeds(p_oTree, p_oPop, p_oPlot,
							fDbh);

					// get species ParamG1
					p_oTree->GetValue(p_oPop->GetParamG1Code(iSp, iType),
							&iParamG1);
					// to compute the weighted mean:
					p_SumSeeds[iSp] = p_SumSeeds[iSp] + temp;
					p_SumParamG1[iSp] = p_SumParamG1[iSp] + temp * iParamG1;

					// get species ParamG2
					p_oTree->GetValue(p_oPop->GetParamG2Code(iSp, iType),
							&iParamG2);
					// to compute the weighted mean:
					//p_SumSeeds[iSp] = p_SumSeeds[iSp] + temp;
					p_SumParamG2[iSp] = p_SumParamG2[iSp] + temp * iParamG2;

					// get species ParamM1
					p_oTree->GetValue(p_oPop->GetParamM1Code(iSp, iType),
							&iParamM1);
					// to compute the weighted mean:
		//			p_SumSeeds[iSp] = p_SumSeeds[iSp] + temp;
					p_SumParamM1[iSp] = p_SumParamM1[iSp] + temp * iParamM1;

					// get species ParamM2
					p_oTree->GetValue(p_oPop->GetParamM2Code(iSp, iType),
							&iParamM2);
					// to compute the weighted mean:
		//			p_SumSeeds[iSp] = p_SumSeeds[iSp] + temp;
					p_SumParamM2[iSp] = p_SumParamM2[iSp] + temp * iParamM2;
		...
		// change ParamG1 for each species
		for (iSp = 0; iSp < p_NumSpecies; iSp++) {
			// compute the mean by dividing by the number of seeds
			p_SumParamG1[iSp] = p_SumParamG1[iSp] / p_SumSeeds[iSp];
			p_SumParamG2[iSp] = p_SumParamG2[iSp] / p_SumSeeds[iSp];
			p_SumParamM1[iSp] = p_SumParamM1[iSp] / p_SumSeeds[iSp];
			p_SumParamM2[iSp] = p_SumParamM2[iSp] / p_SumSeeds[iSp];

            MeanParamG1 = p_oPop->GetMeanParamG1(iSp);
            StDevParamG1 = p_oPop->GetStDevParamG1(iSp);

            NSHeritParamG1 = p_oPop->GetNSHeritParamG1(iSp); //narrow-sense heritability of the ParamG1
            SelParamG1 = p_SumParamG1[iSp] - MeanParamG1;

            MeanParamG2 = p_oPop->GetMeanParamG2(iSp);
            StDevParamG2 = p_oPop->GetStDevParamG2(iSp);

            NSHeritParamG2 = p_oPop->GetNSHeritParamG2(iSp); //narrow-sense heritability of the ParamG2
            SelParamG2 = p_SumParamG2[iSp] - MeanParamG2;

            MeanParamM1 = p_oPop->GetMeanParamM1(iSp);
            StDevParamM1 = p_oPop->GetStDevParamM1(iSp);

            NSHeritParamM1 = p_oPop->GetNSHeritParamM1(iSp); //narrow-sense heritability of the ParamM1
            SelParamM1 = p_SumParamM1[iSp] - MeanParamM1;

            MeanParamM2 = p_oPop->GetMeanParamM2(iSp);
            StDevParamM2 = p_oPop->GetStDevParamM2(iSp);

            NSHeritParamM2 = p_oPop->GetNSHeritParamM2(iSp); //narrow-sense heritability of the ParamM2
            SelParamM2 = p_SumParamM2[iSp] - MeanParamM2;

			//change current p_SumParam[iSp] to SeedParamG1 from heritability function
			//p_SumParam[iSp] = HeritabilityG1(p_SumParam[], StDevParamG1, MeanParamG1, &iSp);
            p_SumParamG1[iSp] = clModelMath::NormalRandomDraw(StDevParamG1) + (MeanParamG1 + NSHeritParamG1 * SelParamG1);
            p_SumParamG2[iSp] = clModelMath::NormalRandomDraw(StDevParamG2) + (MeanParamG2 + NSHeritParamG2 * SelParamG2);
            p_SumParamM1[iSp] = clModelMath::NormalRandomDraw(StDevParamM1) + (MeanParamM1 + NSHeritParamM1 * SelParamM1);
            p_SumParamM2[iSp] = clModelMath::NormalRandomDraw(StDevParamM2) + (MeanParamM2 + NSHeritParamM2 * SelParamM2);

			// change the param
			p_oPop->SetMeanParamG1(iSp, p_SumParamG1[iSp]);
			p_oPop->SetMeanParamG2(iSp, p_SumParamG2[iSp]);
			p_oPop->SetMeanParamM1(iSp, p_SumParamM1[iSp]);
			p_oPop->SetMeanParamM2(iSp, p_SumParamM2[iSp]);
...
// modified to give the number of seeds produced
///////////////////////////////////////////////////////////////////////////////
float clDisperseParamChange::DisperseOneParentSeeds(clTree *p_oTree,
		clTreePopulation *p_oPop, clPlot *p_oPlot, float fDbh) {
	try {
		...
		iSpIndex = mp_iIndexes[iSp], iEvent = mp_iEvent[iSpIndex];
		...
		for (f = 0; f < fNumSeeds; f++) {
			...
			fPrevBucket =
					(iDistanceCounter == 0) ?
							0.0 :
							mp_fSeedCDF[iEvent][iSpIndex][iDistanceCounter - 1];

			//Calculate the distance at which the seed will land
			fDistance = (iDistanceCounter
					+ (fRand - fPrevBucket)
							/ (mp_fSeedCDF[iEvent][iSpIndex][iDistanceCounter]
									- fPrevBucket));
			...
			mp_oSeedGrid->GetValueAtPoint(fSeedX, fSeedY, mp_iNumSeedsCode[iSp],
					&fNumGridSeeds);
			fNumGridSeeds++;
			mp_oSeedGrid->SetValueAtPoint(fSeedX, fSeedY, mp_iNumSeedsCode[iSp],
					fNumGridSeeds);
		...
		modelErr stcErr;
		stcErr.iErrorCode = UNKNOWN;
		stcErr.sFunction = "clDisperseParamChange::DisperseOneParentSeeds";
		throw(stcErr);
	}
}
...
// CalcMastCDF
///////////////////////////////////////////////////////////////////////////////
void clDisperseParamChange::CalcMastCDF(xercesc::DOMDocument *p_oDoc) {
	float *p_fA = new float[m_iNumBehaviorSpecies], *p_fB =
			new float[m_iNumBehaviorSpecies];
	floatVal *p_fTemp = NULL; //for getting species-specific values
	try {
		clTreePopulation *p_oPop =
				(clTreePopulation*) mp_oSimManager->GetPopulationObject(
						"treepopulation");
		DOMElement *p_oElement = p_oDoc->getDocumentElement();
		float fNumYearsPerTimestep =
				mp_oSimManager->GetNumberOfYearsPerTimestep(), fProb;
		int iNumberRunTimesteps = mp_oSimManager->GetNumberOfTimesteps(),
				iSpeciesMax, i, j; //loop counter
		...
		//CDF "a" value
		FillSpeciesSpecificValue(p_oElement, "di_mastCDFA", "di_mcdfaVal",
				p_fTemp, m_iNumBehaviorSpecies, p_oPop, true);
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			if (p_fTemp[i].val == 0) {
				modelErr stcErr;
				stcErr.iErrorCode = BAD_DATA;
				stcErr.sFunction = "clDisperseParamChange::CalcMastCDF";
		...
		//CDF "b" value
		FillSpeciesSpecificValue(p_oElement, "di_mastCDFB", "di_mcdfbVal",
				p_fTemp, m_iNumBehaviorSpecies, p_oPop, true);
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			...
			while (iSpeciesMax < iNumberRunTimesteps && fProb < 0.9999) {
				...
				fProb = 1
						/ (1
								+ pow(
										((iSpeciesMax * fNumYearsPerTimestep)
												/ p_fA[i]), p_fB[i]));
			}
		...
		if (m_iMaxTimesteps == 1)
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
				mp_iTimestepsSinceLastMast[i] = 0;
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			...
			for (j = 1; j < m_iMaxTimesteps; j++) {
				mp_fMastCDF[i][j] = 1
						/ (1
								+ pow(((j * fNumYearsPerTimestep) / p_fA[i]),
										p_fB[i]));
...
// FormatQueryString
///////////////////////////////////////////////////////////////////////////////
void clDisperseParamChange::FormatQueryString(clTreePopulation *p_oPop) {
	int i;
	...
	if (bAdults && !bSaplings)
		...
		sprintf(m_cQuery, "%s%d%s%d", "type=", clTreePopulation::sapling, ",",
				clTreePopulation::adult);
}
...
// SetGetSeedsFunctionPointers
///////////////////////////////////////////////////////////////////////////////
void clDisperseParamChange::SetGetSeedsFunctionPointers() {
	...
	for (i = 0; i < m_iNumBehaviorSpecies; i++) {
		if (deterministic_pdf == mp_iWhatPDFForSTR[i]
				|| mp_bDrawSTRPerSpecies[i]) {
			bDeclareFecundity = true;
			mp_GetSeeds[i] = &clDisperseParamChange::GetNumberOfSeedsNoDraw;
		} else if (normal_pdf == mp_iWhatPDFForSTR[i]) {
			mp_GetSeeds[i] = &clDisperseParamChange::GetNumberOfSeedsDrawNormal;
		} else if (lognormal_pdf == mp_iWhatPDFForSTR[i]) {
			mp_GetSeeds[i] =
					&clDisperseParamChange::GetNumberOfSeedsDrawLognormal;
		} else {
			mp_GetSeeds[i] = &clDisperseParamChange::GetNumberOfSeedsDrawZIP;
		}
	}
...
// DecideMast
///////////////////////////////////////////////////////////////////////////////
void clDisperseParamChange::DecideMast() {
	int i, j;
	mastEvent iEvent;
	...
	for (i = 0; i < m_iNumBehaviorSpecies; i++) {
		...
		if (numevents == mp_iEvent[i]) {
			...
			if (clModelMath::GetRand()
					< mp_fMastCDF[i][mp_iTimestepsSinceLastMast[i]])
				iEvent = mast;
			else
				iEvent = nonmast;
...
///////////////////////////////////////////////////////////////////////////////
float clDisperseParamChange::GetNumberOfSeedsNoDraw(const float &fDbh,
		const short int &iSp) {
	return clModelMath::RandomRound(
			mp_fFecundity[mp_iIndexes[iSp]]
					* pow(fDbh,
							mp_fBeta[mp_iEvent[mp_iIndexes[iSp]]][mp_iIndexes[iSp]])
...
///////////////////////////////////////////////////////////////////////////////
float clDisperseParamChange::GetNumberOfSeedsDrawNormal(const float &fDbh,
		const short int &iSp) {
	float fSTR =
...
///////////////////////////////////////////////////////////////////////////////
float clDisperseParamChange::GetNumberOfSeedsDrawLognormal(const float &fDbh,
		const short int &iSp) {
...
//////////////////////////////////////////////////////////////////////////////
// GetNumberOfSeedsDrawZIP
///////////////////////////////////////////////////////////////////////////////
float clDisperseParamChange::GetNumberOfSeedsDrawZIP(const float &fDbh,
		const short int &iSp) {
	float fSTR = clModelMath::ZeroInflatedPoissonRandomDraw(
			mp_fStrMean[mp_iEvent[mp_iIndexes[iSp]]][mp_iIndexes[iSp]],
			mp_fStrStdDev[mp_iEvent[mp_iIndexes[iSp]]][mp_iIndexes[iSp]]);
	return fSTR * m_fNumYearsPerTimestep
			* pow((fDbh / 30),
...
/*/ ////////////////////////////////////////////////////////////////////////////
 WriteCumProbArray
 /////////////////////////////////////////////////////////////////////////////*/
/*void clDisperseParamChange::WriteCumProbArray() { fstream matrices; int iMaxDistance = mp_oDisperseOrg->GetMaxDistance();
