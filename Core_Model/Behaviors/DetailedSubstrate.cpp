...
void clDetailedSubstrate::GetParameterFileData(xercesc::DOMDocument *p_oDoc) {
	try {
		...
		FillSingleValue(p_oElement, "su_logSizeClassBoundary",
				&m_fLogSizeClassBoundary, true);

		//Log species group assignments
		m_iNumLogSpGroupsUsed = 0;

		FillSpeciesSpecificValue(p_oElement, "su_logSpGroupAssignment",
				"su_lsgaVal", p_iTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);

		//Transfer values to our permanent array
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			mp_iLogSpGroupAssignment[p_iTempValues[i].code] =
					p_iTempValues[i].val - 1; //subtract 1 to make array zero-indexed
		...
		FillSingleValue(p_oElement, "su_rootTipupFactor", &m_fRootTipupFactor,
				true);
		...
		for (k = 1; k <= 5; k++) {
			...
			FillSingleValue(p_oElement, sConcatenatedString.str(),
					&mp_fPropFallenTreesLogDecayClass[k - 1], true);
			sConcatenatedString.str("");

			sConcatenatedString << "su_propFallenSnagsLogDecayClass" << k;
			FillSingleValue(p_oElement, sConcatenatedString.str(),
					&mp_fPropFallenSnagsLogDecayClass[k - 1], true);
			sConcatenatedString.str("");
		}

		//Proportion of fallen that uproot
		FillSpeciesSpecificValue(p_oElement, "su_propOfFallUproot",
				"su_pofuVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer values to our permanent array, turning each value into a
		//per-timestep proportion on the way
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fPropOfFallenThatUproot[p_fTempValues[i].code] =
					p_fTempValues[i].val;

		//Proportion of snags that uproot
		FillSpeciesSpecificValue(p_oElement, "su_propOfSnagsUproot",
				"su_posuVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer values to our permanent array
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fPropOfSnagsThatUproot[p_fTempValues[i].code] =
					p_fTempValues[i].val;

		//Scarified soil decay alpha
		FillSingleValue(p_oElement, "su_scarSoilDecayAlpha", &m_fScarifiedSoilA,
				true);
		//Scarified soil decay beta
		FillSingleValue(p_oElement, "su_scarSoilDecayBeta", &m_fScarifiedSoilB,
				true);
		//Tipup decay alpha
		...
		FillSingleValue(p_oElement, "su_maxNumberDecayYears", &m_iMaxDecayYears,
				true);

		//Proportion of forest floor that is moss
		FillSingleValue(p_oElement, "su_mossProportion", &m_fMossProportion,
				true);

		//Directional tree fall?
		FillSingleValue(p_oElement, "su_directionalTreeFall",
				&m_bUseDirectionalTreeFall, true);

		//Initial conditions - scarified soil
		FillSingleValue(p_oElement, "su_initialScarSoil", &m_fInitScarifiedSoil,
				true);
		...
		FillSingleValue(p_oElement, "su_initialSmallLogMeanDiameter",
				&mp_fInitialLogMeanDiameter[0], false);
		FillSingleValue(p_oElement, "su_initialLargeLogMeanDiameter",
				&mp_fInitialLogMeanDiameter[1], false);
		...
		for (i = 0; i < m_iNumLogSpGroupsUsed; i++) { //i is SpGroup (0-2)
			...
			for (j = 0; j <= 1; j++) { //j is index of size class (small=0, large=1)
				...
				for (k = 1; k <= 5; k++) { //k is decay class (1-5)
					sConcatenatedString << "su_initialLogSpGroup" << i + 1
							<< p_sSizes[j] << "DecayClass" << k;
					FillSingleValue(p_oElement, sConcatenatedString.str(),
							&(mp_fInitLog[i][j][k - 1]), true);
					sConcatenatedString.str("");

					sConcatenatedString << "su_logSpGroup" << i + 1
							<< p_sSizes[j] << "DecayClass" << k << "DecayAlpha";
					FillSingleValue(p_oElement, sConcatenatedString.str(),
							&(mp_fLogDecayAlpha[i][j][k - 1]), true);
					sConcatenatedString.str("");

					sConcatenatedString << "su_logSpGroup" << i + 1
							<< p_sSizes[j] << "DecayClass" << k << "DecayBeta";
					FillSingleValue(p_oElement, sConcatenatedString.str(),
							&(mp_fLogDecayBeta[i][j][k - 1]), true);
					sConcatenatedString.str("");
				} //end for k
			} //end for j
		} // end for i
		...
		if (mp_oHarvestGrid) {
			...
			FillSingleValue(p_oElement, "su_partialCutScarSoil",
					&m_fPartCutScarifiedSoil, true);
			//Gap cut conditions - scarified soil
			FillSingleValue(p_oElement, "su_gapCutScarSoil",
					&m_fGapCutScarifiedSoil, true);
			//Clear cut conditions - scarified soil
			FillSingleValue(p_oElement, "su_clearCutScarSoil",
					&m_fClearCutScarifiedSoil, true);

			//Partial cut conditions - tipup
			FillSingleValue(p_oElement, "su_partialCutTipup", &m_fPartCutTipUp,
					true);
			//Gap cut conditions - tipup
			FillSingleValue(p_oElement, "su_gapCutTipup", &m_fGapCutTipUp,
					true);
			//Clear cut conditions - tipup
			FillSingleValue(p_oElement, "su_clearCutTipup", &m_fClearCutTipUp,
					true);
			...
			//try to read in values
			FillSingleValue(p_oElement, "su_partialCutSmallLogMeanDiameter",
					&mp_fPartCutLogMeanDiameter[0], false);
			FillSingleValue(p_oElement, "su_partialCutLargeLogMeanDiameter",
					&mp_fPartCutLogMeanDiameter[1], false);
			//Gap cut conditions - mean diameter by size class
			FillSingleValue(p_oElement, "su_gapCutSmallLogMeanDiameter",
					&mp_fGapCutLogMeanDiameter[0], false);
			FillSingleValue(p_oElement, "su_gapCutLargeLogMeanDiameter",
					&mp_fGapCutLogMeanDiameter[1], false);
			//Clear cut conditions - mean diameter by size class
			FillSingleValue(p_oElement, "su_clearCutSmallLogMeanDiameter",
					&mp_fClearCutLogMeanDiameter[0], false);
			FillSingleValue(p_oElement, "su_clearCutLargeLogMeanDiameter",
					&mp_fClearCutLogMeanDiameter[1], false);
			...
			for (i = 0; i < m_iNumLogSpGroupsUsed; i++) { //i is SpGroup (0-2)
				...
				for (j = 0; j <= 1; j++) { //j is index of size class (small=0, large=1)
					...
					for (k = 1; k <= 5; k++) { //k is decay class (1-5)
						sConcatenatedString << "su_partialCutLogSpGroup"
								<< i + 1 << p_sSizes[j] << "DecayClass" << k;
						FillSingleValue(p_oElement, sConcatenatedString.str(),
								&(mp_fPartCutLog[i][j][k - 1]), true);
						sConcatenatedString.str("");

						sConcatenatedString << "su_gapCutLogSpGroup" << i + 1
								<< p_sSizes[j] << "DecayClass" << k;
						FillSingleValue(p_oElement, sConcatenatedString.str(),
								&(mp_fGapCutLog[i][j][k - 1]), true);
						sConcatenatedString.str("");

						sConcatenatedString << "su_clearCutLogSpGroup" << i + 1
								<< p_sSizes[j] << "DecayClass" << k;
						FillSingleValue(p_oElement, sConcatenatedString.str(),
								&(mp_fClearCutLog[i][j][k - 1]), true);
						sConcatenatedString.str("");
					} //end for k
				} //end for j
			} //end for i
		...
		m_iMaxDecayTimesteps = (int) ceil(
				m_iMaxDecayYears
						/ mp_oSimManager->GetNumberOfYearsPerTimestep()) + 1;

		//Species group assignments
		for (i = 0; i < m_iNumTotalSpecies; i++) {
			if (mp_iLogSpGroupAssignment[i] < 0
					|| mp_iLogSpGroupAssignment[i] > 2) {
				modelErr stcErr;
				stcErr.iErrorCode = BAD_DATA;
				stcErr.sFunction = "clDetailedSubstrate::GetParameterFileData";
				stcErr.sMoreInfo =
						"Species must be assigned a group numbered 1-3.";
				throw(stcErr);
			}
		}
		...
		for (i = 0; i < m_iNumLogSpGroupsUsed; i++) { //i is SpGroup (0-2)
			for (j = 0; j <= 1; j++) { //j is index of size class (small=0, large=1)
				for (k = 1; k <= 5; k++) {  //k is decay class (1-5)
					if (mp_fLogDecayBeta[i][j][k - 1]
							> 30.0 / log(m_iMaxDecayTimesteps))
						bBadDecayValue = true;
					else if (mp_fLogDecayAlpha[i][j][k - 1] > 0.0)
						bBadDecayValue = true;
					else if (mp_fLogDecayAlpha[i][j][k - 1]
							* pow(m_iMaxDecayTimesteps,
									mp_fLogDecayBeta[i][j][k - 1]) < -30.0)
						bBadDecayValue = true;
				}
			}
		}
		...
		if (m_fScarifiedSoilB > 30.0 / log(m_iMaxDecayTimesteps))
			...
			bBadDecayValue = true;
		else if (m_fScarifiedSoilA
				* pow(m_iMaxDecayTimesteps, m_fScarifiedSoilB) < -30.0)
			bBadDecayValue = true;
		...
		if (bBadDecayValue) {
			...
			stcErr.sFunction = "clDetailedSubstrate::GetParameterFileData";
			stcErr.sMoreInfo =
					"Substrate decay parameter(s) outside of valid range.";
			throw(stcErr);
		}
		...
		if (fTemp > 1.0) {
			...
			stcErr.sFunction = "clDetailedSubstrate::GetParameterFileData";
			stcErr.sMoreInfo =
					"Initial conditions substrate proportions must add up to be less than 1";
			throw(stcErr);
		}
		...
		if (mp_fInitialLogMeanDiameter[0] > m_fLogSizeClassBoundary) {
			...
			stcErr.sFunction = "clDetailedSubstrate::GetParameterFileData";
			stcErr.sMoreInfo =
					"su_initialSmallLogMeanDiameter must be less than su_logSizeClassBoundary";
			throw(stcErr);
		}
		if (mp_fInitialLogMeanDiameter[1] < m_fLogSizeClassBoundary) {
			...
			stcErr.sFunction = "clDetailedSubstrate::GetParameterFileData";
			stcErr.sMoreInfo =
					"su_initialLargeLogMeanDiameter must be greater than su_logSizeClassBoundary";
			throw(stcErr);
		}
		...
		if (fabs(fTemp) > 0.0001) {
			...
			stcErr.sFunction = "clDetailedSubstrate::GetParameterFileData";
			stcErr.sMoreInfo =
					"Proportion of fallen live trees in decay classes 1-5 must sum to 1.";
			throw(stcErr);
		}
		...
		if (fabs(fTemp) > 0.0001) {
			...
			stcErr.sFunction = "clDetailedSubstrate::GetParameterFileData";
			stcErr.sMoreInfo =
					"Proportion of fallen snags in decay classes 1-5 must sum to 1.";
			throw(stcErr);
		}
		...
		if (mp_oHarvestGrid) {
			...
			if (fTemp > 1.0) {
				...
				stcErr.sFunction = "clDetailedSubstrate::GetParameterFileData";
				stcErr.sMoreInfo =
						"Partial cut substrate proportions must add up to be less than 1";
				throw(stcErr);
			}
			...
			if (fTemp > 1.0) {
				...
				stcErr.sFunction = "clDetailedSubstrate::GetParameterFileData";
				stcErr.sMoreInfo =
						"Gap cut substrate proportions must add up to be less than 1";
				throw(stcErr);
			}
			...
			if (fTemp > 1.0) {
				...
				stcErr.sFunction = "clDetailedSubstrate::GetParameterFileData";
				stcErr.sMoreInfo =
						"Clear cut substrate proportions must add up to be less than 1";
				throw(stcErr);
			}
			...
			if (mp_fPartCutLogMeanDiameter[0] > m_fLogSizeClassBoundary) {
				...
				stcErr.sFunction = "clDetailedSubstrate::GetParameterFileData";
				stcErr.sMoreInfo =
						"su_partialCutSmallLogMeanDiameter must be less than su_logSizeClassBoundary";
				throw(stcErr);
			}
			if (mp_fPartCutLogMeanDiameter[1] < m_fLogSizeClassBoundary) {
				...
				stcErr.sFunction = "clDetailedSubstrate::GetParameterFileData";
				stcErr.sMoreInfo =
						"su_partialCutLargeLogMeanDiameter must be greater than su_logSizeClassBoundary";
				throw(stcErr);
			}
			...
			if (mp_fGapCutLogMeanDiameter[0] > m_fLogSizeClassBoundary) {
				...
				stcErr.sFunction = "clDetailedSubstrate::GetParameterFileData";
				stcErr.sMoreInfo =
						"su_gapCutSmallLogMeanDiameter must be less than su_logSizeClassBoundary";
				throw(stcErr);
			}
			if (mp_fGapCutLogMeanDiameter[1] < m_fLogSizeClassBoundary) {
				...
				stcErr.sFunction = "clDetailedSubstrate::GetParameterFileData";
				stcErr.sMoreInfo =
						"su_gapCutLargeLogMeanDiameter must be greater than su_logSizeClassBoundary";
				throw(stcErr);
			}
			...
			if (mp_fClearCutLogMeanDiameter[0] > m_fLogSizeClassBoundary) {
				...
				stcErr.sFunction = "clDetailedSubstrate::GetParameterFileData";
				stcErr.sMoreInfo =
						"su_clearCutSmallLogMeanDiameter must be less than su_logSizeClassBoundary";
				throw(stcErr);
			}
			if (mp_fClearCutLogMeanDiameter[1] < m_fLogSizeClassBoundary) {
				...
				stcErr.sFunction = "clDetailedSubstrate::GetParameterFileData";
				stcErr.sMoreInfo =
						"su_clearCutLargeLogMeanDiameter must be greater than su_logSizeClassBoundary";
				throw(stcErr);
			}
...
void clDetailedSubstrate::CalculateDecayProportions() {
	try {
		...
		int iNumYearsPerTS =
				(int) (mp_oSimManager->GetNumberOfYearsPerTimestep());
		...
		for (i = 0; i < m_iNumLogSpGroupsUsed; i++) { //i is SpGroup (0-2)
			...
			for (j = 0; j <= 1; j++) { //j is index of size class (small=0, large=1)
				...
				for (k = 1; k <= 5; k++) {   //k is decay class (1-5)
					mp_fLogDecayProp[i][j][k - 1] =
							new float*[m_iMaxDecayTimesteps];
					for (t = 0; t < m_iMaxDecayTimesteps; t++) {
						mp_fLogDecayProp[i][j][k - 1][t] =
								new float[iNumYearsPerTS];
					}
				}
			}
		}
					...
					for (k = 1; k <= 5; k++) { //k is decay class (1-5)
						for (u = 0; u < iNumYearsPerTS; u++) { //iterate years within a timestep
							...
							if (0 == mp_fLogDecayBeta[i][j][k - 1])
								...
								fTemp = pow((t * fNumYearsPerTS + u),
										mp_fLogDecayBeta[i][j][k - 1]);
							mp_fLogDecayProp[i][j][k - 1][t][u] = exp(
									mp_fLogDecayAlpha[i][j][k - 1] * fTemp);
						} //end for i, j, k, u
					}
				}
			}
	...
	try {
		...
		for (i = 0; i < iNumLogSpGroupsUsed; i++) {
			...
			for (j = 0; j <= 1; j++) {
				...
				for (k = 1; k <= 5; k++) {
					mp_iLogCalcsCode[i][j][k - 1] =
							new int[m_iMaxDecayTimesteps];
				} //end for k
			} //end for j
		} //end for i
		...
		if ( NULL == mp_oSubstrateGrid) {
			...
			m_iPkgScarSoilCode = mp_oSubstrateGrid->RegisterPackageFloat(
					"scarsoil");
			m_iPkgTipupCode = mp_oSubstrateGrid->RegisterPackageFloat("tipup");

			m_iTotalLogCode = mp_oSubstrateGrid->RegisterFloat("totallog");
			m_iTotalLogVolCode = mp_oSubstrateGrid->RegisterFloat(
					"totallogvol");

			for (i = 0; i < iNumLogSpGroupsUsed; i++) {
				for (j = 0; j <= 1; j++) {
					for (k = 1; k <= 5; k++) {
						sLabel << "loggroup" << i + 1 << p_cSizes[j] << "decay"
								<< k;
						mp_iLogCodes[i][j][k - 1] =
								mp_oSubstrateGrid->RegisterFloat(sLabel.str());
						sLabel.str("");
						sLabel << "vloggroup" << i + 1 << p_cSizes[j]
								<< "decay%d" << k;
						mp_iLogVolCodes[i][j][k - 1] =
								mp_oSubstrateGrid->RegisterFloat(sLabel.str());
						sLabel.str("");
					} //end for k
				} //end for j
		...
		} else //The substrate grid was created from a map file. Validate it.
		{
			...
			if ((fXCellLength > 0
					&& mp_oSubstrateGrid->GetLengthXCells() != fXCellLength)
					|| (fYCellLength > 0
				...
				stcErr.sMoreInfo =
						"Grid map values for grid \"DetailedSubstrate\" contain cell lengths which do not match the allowed values.";
				throw(stcErr);
			}
			...
			m_iTotalLogVolCode = mp_oSubstrateGrid->GetFloatDataCode(
					"totallogvol");
				...
				for (j = 0; j <= 1; j++) {
					for (k = 1; k <= 5; k++) {
						sLabel << "loggroup" << i + 1 << p_cSizes[j] << "decay"
								<< k;
						mp_iLogCodes[i][j][k - 1] =
								mp_oSubstrateGrid->GetFloatDataCode(
										sLabel.str());
						sLabel.str("");
						if (mp_iLogCodes[i][j][k - 1] == (-1))
							iNoLogCode = -1;
						sLabel << "vloggroup" << i + 1 << p_cSizes[j] << "decay"
								<< k;
						mp_iLogVolCodes[i][j][k - 1] =
								mp_oSubstrateGrid->GetFloatDataCode(
										sLabel.str());
			...
			//this point
			if (-1 == m_iScarSoilCode || -1 == m_iFFMossCode
					|| -1 == m_iFFLitterCode || -1 == m_iTipupCode
					|| -1 == iNoLogCode || -1 == m_iTotalLogCode
					|| -1 == m_iTotalLogVolCode) {
				modelErr stcErr;
				stcErr.iErrorCode = BAD_DATA;
				stcErr.sFunction = "clDetailedSubstrate::SetupSubstrateGrid";
				stcErr.sMoreInfo =
						"Grid map setup values for grid \"DetailedSubstrate\" did not contain all required grid data members.";
				throw(stcErr);
			}
			...
			if (-1 == m_iPkgAgeCode) {
				...
				m_iPkgScarSoilCode = mp_oSubstrateGrid->RegisterPackageFloat(
						"scarsoil");
				m_iPkgTipupCode = mp_oSubstrateGrid->RegisterPackageFloat(
						"tipup");
			...
			} else //age code is present
			{
				...
				m_iPkgScarSoilCode = mp_oSubstrateGrid->GetPackageFloatDataCode(
						"scarsoil");
				m_iPkgTipupCode = mp_oSubstrateGrid->GetPackageFloatDataCode(
						"tipup");
				...
				if (-1 == m_iPkgScarSoilCode || -1 == m_iPkgTipupCode) {
					...
					stcErr.iErrorCode = BAD_DATA;
					stcErr.sFunction =
							"clDetailedSubstrate::SetupSubstrateGrid";
					stcErr.sMoreInfo =
							"Grid map setup values for grid \"DetailedSubstrate\" did not contain all required grid data members.";
					throw(stcErr);
		...
		if ( NULL == mp_oCalcGrid) {
			...
			for (i = 0; i < iNumLogSpGroupsUsed; i++) {
				for (j = 0; j <= 1; j++) {
					for (k = 1; k <= 5; k++) {
						sLabel << "newloggroup" << i + 1 << p_cSizes[j]
								<< "decay" << k;
						mp_iNewLogCalcsCode[i][j][k - 1] =
								mp_oCalcGrid->RegisterFloat(sLabel.str());
						sLabel.str("");

						for (t = 0; t < m_iMaxDecayTimesteps; t++) {
							sLabel << "loggroup" << i + 1 << p_cSizes[j]
									<< "decay" << k << "_" << t;
							mp_iLogCalcsCode[i][j][k - 1][t] =
									mp_oCalcGrid->RegisterFloat(sLabel.str());
							sLabel.str("");
		...
		else //Calc grid was created.
		{
			...
			//Make sure the cell lengths are right
			if ((mp_oCalcGrid->GetLengthXCells()
					!= mp_oSubstrateGrid->GetLengthXCells())
					|| (mp_oCalcGrid->GetLengthYCells()
							!= mp_oSubstrateGrid->GetLengthYCells())) {
				//Throw an eror
				modelErr stcErr;
				stcErr.iErrorCode = BAD_DATA;
				stcErr.sFunction = "clDetailedSubstrate::SetupSubstrateGrid";
				stcErr.sMoreInfo =
						"Grid map values for grid \"detailedsubstratecalcs\" contain cell lengths which do not match the allowed values.";
				throw(stcErr);
			}
			...
			if (-1 == m_iNewTipupCalcsCode) {
				...
				stcErr.sFunction = "clDetailedSubstrate::SetupSubstrateGrid";
				stcErr.sMoreInfo =
						"Grid map setup values for grid \"substratecalcs\" did not contain all required grid data members.";
				throw(stcErr);
			}
				...
				for (j = 0; j <= 1; j++) {
					...
					for (k = 1; k <= 5; k++) {
						...
						sLabel << "newloggroup" << i + 1 << p_cSizes[j]
								<< "decay" << k;
						mp_iNewLogCalcsCode[i][j][k - 1] =
								mp_oCalcGrid->GetFloatDataCode(sLabel.str());
						sLabel.str("");
						...
						if (-1 == mp_iNewLogCalcsCode[i][j][k - 1]) {
							...
							stcErr.sFunction =
									"clDetailedSubstrate::SetupSubstrateGrid";
							stcErr.sMoreInfo =
									"Grid map setup values for grid \"detailedsubstratecalcs\" did not contain all required grid data members.";
							throw(stcErr);
						}
		...
		m_fGridCellArea = mp_oSubstrateGrid->GetLengthXCells()
				* mp_oSubstrateGrid->GetLengthYCells();
		...
		//**************************************
		if (mp_oHarvestGrid) {
			m_iHarvestTypeCode = mp_oHarvestGrid->GetIntDataCode(
					"Harvest Type");
		...
		//Register data members
		m_iOrigScarSoilCode = mp_oOriginalSubstrateGrid->RegisterFloat(
				"scarsoil");
		m_iOrigFFMossCode = mp_oOriginalSubstrateGrid->RegisterFloat("ffmoss");
		m_iOrigFFLitterCode = mp_oOriginalSubstrateGrid->RegisterFloat(
				"fflitter");
		m_iOrigTipupCode = mp_oOriginalSubstrateGrid->RegisterFloat("tipup");
		m_iOrigFreshLogCode = mp_oOriginalSubstrateGrid->RegisterFloat(
				"freshlog");
...
void clDetailedSubstrate::SetInitialSubstrate() {
	try {
		...
		int iNumXCells = mp_oSubstrateGrid->GetNumberXCells(), iNumYCells =
				mp_oSubstrateGrid->GetNumberYCells();
		int iX, iY, i, j, k; //loop counters
		float
				fInitScarSoil, //initial amounts, either from param file or map
				fInitTipup, p_fInitLog[m_iNumLogSpGroupsUsed][2][5],
		...
		for (iX = 0; iX < iNumXCells; iX++) {
			for (iY = 0; iY < iNumYCells; iY++) {
				if (m_bParFileHasSubstrateMap) {
					...
					mp_oSubstrateGrid->GetValueOfCell(iX, iY, m_iScarSoilCode,
							&fInitScarSoil);
					mp_oSubstrateGrid->GetValueOfCell(iX, iY, m_iTipupCode,
							&fInitTipup);

					for (i = 0; i < m_iNumLogSpGroupsUsed; i++) //i is SpGroup (0-2)
						for (j = 0; j <= 1; j++) //j is index of size class (small=0, large=1)
							for (k = 1; k <= 5; k++)    //k is decay class (1-5)
								mp_oSubstrateGrid->GetValueOfCell(iX, iY,
										mp_iLogCodes[i][j][k - 1],
										&(p_fInitLog[i][j][k - 1]));
					...
					mp_oSubstrateGrid->SetValueOfCell(iX, iY, m_iScarSoilCode,
							fInitScarSoil);
					mp_oSubstrateGrid->SetValueOfCell(iX, iY, m_iTipupCode,
							fInitTipup);

					for (i = 0; i < m_iNumLogSpGroupsUsed; i++) { //i is SpGroup (0-2)
						for (j = 0; j <= 1; j++) { //j is index of size class (small=0, large=1)
							for (k = 1; k <= 5; k++) {  //k is decay class (1-5)
								mp_oSubstrateGrid->SetValueOfCell(iX, iY,
										mp_iLogCodes[i][j][k - 1],
										p_fInitLog[i][j][k - 1]);
								//Calc vol equivalent
								fTemp = (1.0 / 3.0) * p_fInitLog[i][j][k - 1]
										* mp_fInitialLogMeanDiameter[j] / 2.0
										* M_PI * 10000.0 * 0.01;
								mp_oSubstrateGrid->SetValueOfCell(iX, iY,
										mp_iLogVolCodes[i][j][k - 1], fTemp);
							}
						}
					}

					mp_oSubstrateGrid->SetValueOfCell(iX, iY, m_iFFMossCode,
							fForestFloor * m_fMossProportion);
					mp_oSubstrateGrid->SetValueOfCell(iX, iY, m_iFFLitterCode,
							fForestFloor * (1 - m_fMossProportion));
				...
				for (i = 0; i < m_iNumLogSpGroupsUsed; i++) { //i is SpGroup (0-2)
					for (j = 0; j <= 1; j++) { //j is index of size class (small=0, large=1)
						for (k = 1; k <= 5; k++) {   //k is decay class (1-5)
							...
							mp_oCalcGrid->SetValueOfCell(iX, iY,
									mp_iLogCalcsCode[i][j][k - 1][0],
									p_fInitLog[i][j][k - 1]);

							//Calculate total log cover and volume
							mp_oSubstrateGrid->GetValueOfCell(iX, iY,
									mp_iLogCodes[i][j][k - 1], &fTemp);
							fAreaTotal += fTemp;
							mp_oSubstrateGrid->GetValueOfCell(iX, iY,
									mp_iLogVolCodes[i][j][k - 1], &fTemp);
							fVolTotal += fTemp;
				...
				mp_oSubstrateGrid->SetValueOfCell(iX, iY, m_iTotalLogCode,
						fAreaTotal);
				mp_oSubstrateGrid->SetValueOfCell(iX, iY, m_iTotalLogVolCode,
						fVolTotal);
...
void clDetailedSubstrate::GetCodes() {
	try {
		...
		std::string
				sDeadLabel = "dead", //labels used to tell what type of dead tree we have.
				sFallLabel = "Fall", sNewBreakLabel = "NewBreakHeight",
		...
		for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
			...
			if (mp_whatSpeciesTypeCombos[i].iType
					== clTreePopulation::seedling) {
				modelErr stcErr;
				stcErr.sFunction = "clDetailedSubstrate::GetCodes";
				stcErr.sMoreInfo =
						"Detailed substrate cannot be applied to seedlings.";
				stcErr.iErrorCode = BAD_DATA;
				throw(stcErr);
			}

			mp_iFallCodes[mp_whatSpeciesTypeCombos[i].iSpecies] =
					p_oPop->GetBoolDataCode(sFallLabel,
							mp_whatSpeciesTypeCombos[i].iSpecies,
							clTreePopulation::adult);

			mp_iOldBreakCodes[mp_whatSpeciesTypeCombos[i].iSpecies] =
					p_oPop->GetFloatDataCode(sOldBreakLabel,
							mp_whatSpeciesTypeCombos[i].iSpecies,
							clTreePopulation::snag);

			mp_iNewBreakCodes[mp_whatSpeciesTypeCombos[i].iSpecies][mp_whatSpeciesTypeCombos[i].iType] =
					p_oPop->GetFloatDataCode(sNewBreakLabel,
							mp_whatSpeciesTypeCombos[i].iSpecies,
							mp_whatSpeciesTypeCombos[i].iType);

			mp_iDeadCodes[mp_whatSpeciesTypeCombos[i].iSpecies][mp_whatSpeciesTypeCombos[i].iType] =
					p_oPop->GetIntDataCode(sDeadLabel,
							mp_whatSpeciesTypeCombos[i].iSpecies,
							mp_whatSpeciesTypeCombos[i].iType);

			mp_bSubstrateApplies[mp_whatSpeciesTypeCombos[i].iSpecies][mp_whatSpeciesTypeCombos[i].iType] =
					true;
	...
	try {
		...
		for (iX = 0; iX < iNumXCells; iX++)
			for (iY = 0; iY < iNumYCells; iY++) {
				...
				mp_oHarvestGrid->GetValueOfCell(iX, iY, m_iHarvestTypeCode,
						&iCutType);
					...
					for (j = 0; j <= 1; j++) { //j is index of size class (small=0, large=1)
						for (k = 1; k <= 5; k++) {  //k is decay class (1-5)
							if (iCutType == clDisturbance::partial)
								p_fNewLog[i][j][k - 1] = mp_fPartCutLog[i][j][k
										- 1];
							else if (iCutType == clDisturbance::gap)
								p_fNewLog[i][j][k - 1] = mp_fGapCutLog[i][j][k
										- 1];
							else
								//clearcut
								p_fNewLog[i][j][k - 1] = mp_fClearCutLog[i][j][k
										- 1];
				...
				while (p_oPackage) {
					...
					p_oPackage->SetValue(m_iPkgScarSoilCode,
							fTemp * (1 - fHarvestSubstrateAdded));

					p_oPackage->GetValue(m_iPkgTipupCode, &fTemp);
					p_oPackage->SetValue(m_iPkgTipupCode,
							fTemp * (1 - fHarvestSubstrateAdded));
				...
				if (!bSubstrateAdded) {
					...
					p_oNextToLastPackage =
							mp_oSubstrateGrid->GetFirstPackageOfCell(iX, iY);
					if (!p_oNextToLastPackage) {
						//there aren't any packages already, so the new one goes first
						p_oPackage = mp_oSubstrateGrid->CreatePackageOfCell(iX,
								iY);
					} else {
						while (p_oNextToLastPackage) {
							...
							if (!p_oPackage) {
								...
								p_oPackage = mp_oSubstrateGrid->CreatePackage(
										p_oNextToLastPackage);
								//Break the loop
				...
				for (t = 0; t < m_iMaxDecayTimesteps; t++)
					for (i = 0; i < m_iNumLogSpGroupsUsed; i++) //i is SpGroup (0-2)
						for (j = 0; j <= 1; j++) //j is index of size class (small=0, large=1)
							for (k = 1; k <= 5; k++) { //k is decay class (1-5)
								mp_oCalcGrid->GetValueOfCell(iX, iY,
										mp_iLogCalcsCode[i][j][k - 1][t],
										&fTemp);
								mp_oCalcGrid->SetValueOfCell(iX, iY,
										mp_iLogCalcsCode[i][j][k - 1][t],
				...
				for (i = 0; i < m_iNumLogSpGroupsUsed; i++) //i is SpGroup (0-2)
					for (j = 0; j <= 1; j++) //j is index of size class (small=0, large=1)
						for (k = 1; k <= 5; k++) { //k is decay class (1-5)
							//area
							mp_oCalcGrid->GetValueOfCell(iX, iY,
									mp_iLogCalcsCode[i][j][k - 1][0], &fTemp);
							...
							fTemp += (1.0 / 3.0) * p_fNewLog[i][j][k - 1]
									* p_fNewLogMeanDiameter[j] / 2.0 * M_PI
									* 10000.0 * 0.01;
							mp_oSubstrateGrid->SetValueOfCell(iX, iY,
									mp_iLogVolCodes[i][j][k - 1], fTemp);
						}
...
void clDetailedSubstrate::CalcMainGridTotals() {
	...
	try {
		...
		int iNumXCells = mp_oSubstrateGrid->GetNumberXCells(), iNumYCells =
				mp_oSubstrateGrid->GetNumberYCells();
		float fTemp, fTotalLogArea, fTotalVolume, fDecay123LogArea,
				fDecay45LogArea; //holders for grid values
		...
		for (iX = 0; iX < iNumXCells; iX++)
			for (iY = 0; iY < iNumYCells; iY++) {
				...
				mp_oOriginalSubstrateGrid->SetValueOfCell(iX, iY,
						m_iOrigScarSoilCode, fScarSoil);
				mp_oOriginalSubstrateGrid->SetValueOfCell(iX, iY,
						m_iOrigTipupCode, fTipup);
				mp_oOriginalSubstrateGrid->SetValueOfCell(iX, iY,
						m_iOrigFreshLogCode, fDecay123LogArea);
				mp_oOriginalSubstrateGrid->SetValueOfCell(iX, iY,
						m_iOrigDecLogCode, fDecay45LogArea);
				mp_oOriginalSubstrateGrid->SetValueOfCell(iX, iY,
						m_iOrigFFMossCode, fMoss);
				mp_oOriginalSubstrateGrid->SetValueOfCell(iX, iY,
						m_iOrigFFLitterCode, fLitter);
...
void clDetailedSubstrate::DecaySubstrate() {
	try {
		...
		int
				iAge, //package age
				iNumXCells = mp_oSubstrateGrid->GetNumberXCells(), iNumYCells =
						mp_oSubstrateGrid->GetNumberYCells(), iX, iY, i, j, k,
				t, u, v; //various loop counters
				...
				for (i = 0; i < m_iNumLogSpGroupsUsed; i++) //i is SpGroup (0-2)
					for (j = 0; j <= 1; j++) { //j is index of size class (small=0, large=1)
						...
						for (t = 0; t < m_iMaxDecayTimesteps; t++) //t is timestep number
							for (k = 1; k <= 5; k++) { //k is decay class (1-5)
								if (mp_fLogDecayProp[i][j][k - 1][t][0]
										< 0.00001) { //avoid dividing by zero
									p_fInitialAmountHolder[k - 1][t] = 0.0;
								} else {
									mp_oCalcGrid->GetValueOfCell(iX, iY,
											mp_iLogCalcsCode[i][j][k - 1][t],
											&fTemp);
									p_fInitialAmountHolder[k - 1][t] =
											fTemp
													/ mp_fLogDecayProp[i][j][k
															- 1][t][0];
								}
							} //end for t, k
							...
							for (t = 0; t < m_iMaxDecayTimesteps; t++) { //t is timestep number
								for (k = 4; k >= 1; k--) { //k is decay class (1-4, 5 just leaves)
									...
									if (u < (iNumYearsPerTS - 1)) //amount remaining is in next year within same timestep
										...
										//exceeded max decay timesteps; amount remaining is 0 and we dump everything that's left
										p_fNewAmountHolder[k][0] +=
												p_fInitialAmountHolder[k - 1][t]
														* mp_fLogDecayProp[i][j][k
																- 1][t][u];
						...
						for (k = 1; k <= 5; k++) { //k is decay class
							...
							if (k > 1) { //nothing advanced to decay class 1
								...
								if (iNumYearsPerTS % 2 == 0) //even number of years per timestep, divide middle year in half
									fTemp +=
											0.5
													* p_fNewAmountHolder[k - 1][iNumYearsPerTS
															/ 2];
								else
									//odd number of years per timestep, the middle year all goes to age 0
									fTemp +=
											p_fNewAmountHolder[k - 1][iNumYearsPerTS
													/ 2];
								...
								for (u = (iNumYearsPerTS / 2 + 1);
										u < iNumYearsPerTS; u++)
									fTemp += p_fNewAmountHolder[k - 1][u];

								mp_oCalcGrid->SetValueOfCell(iX, iY,
										mp_iLogCalcsCode[i][j][k - 1][1],
										fTemp);
							...
							//over the timestep to zero.
								...
								fTemp = 0.0;
								mp_oCalcGrid->SetValueOfCell(iX, iY,
										mp_iLogCalcsCode[i][j][0][0], fTemp);
				...
				if (p_oPackage) {
					while (p_oPackage) {
						...
						if (iAge >= m_iMaxDecayTimesteps) {
							...
							//Scarified soil
							p_oPackage->GetValue(m_iPkgScarSoilCode, &fTemp);
							fTemp *= mp_fScarSoilDecayProp[iAge]
									/ mp_fScarSoilDecayProp[iAge - 1];
							p_oPackage->SetValue(m_iPkgScarSoilCode, fTemp);

							//Tipup
							p_oPackage->GetValue(m_iPkgTipupCode, &fTemp);
							fTemp *= mp_fTipupDecayProp[iAge]
									/ mp_fTipupDecayProp[iAge - 1];
							p_oPackage->SetValue(m_iPkgTipupCode, fTemp);
					...
					for (j = 0; j <= 1; j++) { //j is index of size class (small=0, large=1)
						...
						for (k = 1; k <= 5; k++) { //k is decay class (1-5)
							...
							if (fOldAreaAllDecayClasses < 0.000001)
								...
								fNewVolByDecayClass = fOldVolAllDecayClasses
										* fNewAreaByDecayClass
										/ fOldAreaAllDecayClasses;
	...
	try {
		...
		for (iX = 0; iX < iNumXCells; iX++)
			for (iY = 0; iY < iNumYCells; iY++) {
				...
				mp_oCalcGrid->SetValueOfCell(iX, iY, m_iNewTipupCalcsCode,
						(float) 0.0);
			} //end iY
		...
		while (p_oTree) {
			...
			if (iTp == clTreePopulation::adult) {
				...
				p_oTree->GetValue(mp_iNewBreakCodes[iSp][iTp],
						&fNewBreakHeight);
			...
			if (iTp != clTreePopulation::snag && notdead == iDead)
				//tree is still alive
				goto nextTree;
			if (iTp == clTreePopulation::adult && iDead > notdead && !bFall
					&& fNewBreakHeight < 0.0)
			...
			if (iTp == clTreePopulation::adult && !bFall
					&& fNewBreakHeight > 0.0)
					//tree broke along bole, top is added as log substrate
							{
				bTopOnly = true;
			} else if (iTp == clTreePopulation::snag && notdead == iDead
					&& fNewBreakHeight > 0.0)
				//tree is a snag that has broken this timestep
				bTopOnly = true;
			else if (iTp == clTreePopulation::snag && iDead > notdead
					&& fOldBreakHeight > 0.0)
			...
			while (fRandom > 0.0) {
				...
				if (iTp != clTreePopulation::snag)
					fRandom -=
							mp_fPropFallenTreesLogDecayClass[iDecayClass - 1];
				else
					fRandom -=
							mp_fPropFallenSnagsLogDecayClass[iDecayClass - 1];
			}
			...
			for (i = 0; i < iNumTrunkChunks; i++) {
							...
							fRadiusHeightRatio
									* (pow(fLowerChunkHeight, 2)
											- pow(
													...
													2));
					fTrunkChunkVol =
							(1.0 / 3.0) * M_PI * pow(fRadiusHeightRatio, 2)
									* (pow(fLowerChunkHeight, 3)
											- pow(
													fExcludedTopHeight
															+ (iNumTrunkChunks
																	- 1)
																	* fTrunkInterval,
													3))
									/ (m_fGridCellArea / 10000.0);

				} else {

					//this is not the bottommost piece and it's height is 0.5 m
					//(slightly different formulas used)
					fLowerChunkHeight = fExcludedTopHeight
							+ (iNumTrunkChunks - i) * fTrunkInterval;
				...
				fChunkDiam = 2 * 100 * fRadiusHeightRatio
						* (fLowerChunkHeight - 0.5 * fTrunkInterval);
				...
				mp_oCalcGrid->GetCellOfPoint(
						p_oPlot->CorrectX(
								p_oPlot->GetUncorrectedX(fX, fAngle, fStep)),
						p_oPlot->CorrectY(
								p_oPlot->GetUncorrectedY(fY, fAngle, fStep)),
						&iStepX, &iStepY);

				//Get the amount of new log currently in this tree's grid cell from the calculation grid
				mp_oCalcGrid->GetValueOfCell(iStepX, iStepY,
						mp_iNewLogCalcsCode[mp_iLogSpGroupAssignment[iSp]][iDiamClass][iDecayClass
								- 1], &fSubstrate);
				//Drop a trunk chunk in this cell
				fSubstrate += fTrunkChunkArea;
				mp_oCalcGrid->SetValueOfCell(iStepX, iStepY,
						mp_iNewLogCalcsCode[mp_iLogSpGroupAssignment[iSp]][iDiamClass][iDecayClass
								- 1], fSubstrate);

				//same for volume
				mp_oSubstrateGrid->GetValueOfCell(iStepX, iStepY,
						mp_iLogVolCodes[mp_iLogSpGroupAssignment[iSp]][iDiamClass][iDecayClass
								- 1], &fSubstrate);
				fSubstrate += fTrunkChunkVol;
				mp_oSubstrateGrid->SetValueOfCell(iStepX, iStepY,
						mp_iLogVolCodes[mp_iLogSpGroupAssignment[iSp]][iDiamClass][iDecayClass
								- 1], fSubstrate);
			...
			if ((iTp == clTreePopulation::adult
					&& fRandom <= mp_fPropOfFallenThatUproot[iSp])
					|| (iTp == clTreePopulation::snag
							&& fRandom <= mp_fPropOfSnagsThatUproot[iSp])) {
				//Yes, it uprooted - add the new tipup exposed to this grid's new
				//tipup variable
				mp_oCalcGrid->GetValueAtPoint(fX, fY, m_iNewTipupCalcsCode,
						&fSubstrate);
				fSubstrate += M_PI * pow(fRadius * m_fRootTipupFactor, 2);
				mp_oCalcGrid->SetValueAtPoint(fX, fY, m_iNewTipupCalcsCode,
						fSubstrate);
	...
	try {
		...
		for (iX = 0; iX < iNumXCells; iX++)
			for (iY = 0; iY < iNumYCells; iY++) {
				...
				mp_oCalcGrid->GetValueOfCell(iX, iY, m_iNewTipupCalcsCode,
						&fNewTipup);
				fTotalNew += fNewTipup;
				...
				for (i = 0; i < m_iNumLogSpGroupsUsed; i++) //i is SpGroup (0-2)
					for (j = 0; j <= 1; j++) //j is index of size class (small=0, large=1)
						for (k = 1; k <= 5; k++) {  //k is decay class (1-5)
							...
							mp_oCalcGrid->GetValueOfCell(iX, iY,
									mp_iNewLogCalcsCode[i][j][k - 1], &fTemp1);
							fTotalNew += fTemp1;
				...
				for (i = 0; i < m_iNumLogSpGroupsUsed; i++) //i is SpGroup (0-2)
					for (j = 0; j <= 1; j++) //j is index of size class (small=0, large=1)
						for (k = 1; k <= 5; k++) {  //k is decay class (1-5)
							...
							mp_oCalcGrid->GetValueOfCell(iX, iY,
									mp_iLogCalcsCode[i][j][k - 1][0], &fTemp1);
							mp_oCalcGrid->GetValueOfCell(iX, iY,
									mp_iNewLogCalcsCode[i][j][k - 1], &fTemp2);
