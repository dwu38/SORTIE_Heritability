...
void clStormKiller::GetDeadCodes(clTreePopulation *p_oPop) {
	...
	for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
		...
		if (-1
			...
			stcErr.sMoreInfo =
					"You must use tree mortality behaviors along with the storm killer.";
			throw(stcErr);
		}
	}
}
...
void clStormKiller::GetStmDmgCodes() {
	...
	for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
		if (clTreePopulation::seedling == mp_whatSpeciesTypeCombos[i].iType) {
			...
			stcErr.sMoreInfo =
					"The storm killer behavior cannot be applied to seedlings.";
			throw(stcErr);
		}

		mp_iStmDmgCodes[mp_whatSpeciesTypeCombos[i].iType][mp_whatSpeciesTypeCombos[i].iSpecies] =
				p_oPop->GetIntDataCode("stm_dmg",
						mp_whatSpeciesTypeCombos[i].iSpecies,
						mp_whatSpeciesTypeCombos[i].iType);
		...
		if (-1
			...
			stcErr.sFunction = "clStormKiller::GetStmDmgCodes";
			stcErr.sMoreInfo =
					"You must use the storm damage applier behavior along with the storm killer.";
			throw(stcErr);
		}
	}
}
...
void clStormKiller::GetParameterFileData(xercesc::DOMDocument *p_oDoc) {
	...
	try {
		...
		FillSpeciesSpecificValue(p_oElement, "st_minDBH", "st_mdVal",
		...
		FillSpeciesSpecificValue(p_oElement, "st_stmMedDmgSurvProbA",
				"st_smdspaVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		FillSpeciesSpecificValue(p_oElement, "st_stmMedDmgSurvProbB",
				"st_smdspbVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		FillSpeciesSpecificValue(p_oElement, "st_stmFullDmgSurvProbA",
				"st_sfdspaVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		FillSpeciesSpecificValue(p_oElement, "st_stmFullDmgSurvProbB",
				"st_sfdspbVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		FillSpeciesSpecificValue(p_oElement, "st_stmPropTipUpFullDmg",
				"st_sptufdVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			if (p_fTempValues[i].val < 0 || p_fTempValues[i].val > 1) {
				...
				stcErr.sMoreInfo =
						"All values for proportion of fully-damaged trees that tip-up must be between 0 and 1.";
				throw(stcErr);
			}
		...
		FillSingleValue(p_oElement, "st_numYearsStormSnags", &m_iSnagYears,
				true);
		if (m_iSnagYears < 0) {
			modelErr stcErr;
			stcErr.iErrorCode = BAD_DATA;
			stcErr.sFunction = "clStormKiller::GetParameterFileData";
			stcErr.sMoreInfo =
					"The number of years snags last parameter cannot be less than 0.";
			throw(stcErr);
		}
	...
	try {
		...
		while (p_oTree) {
			...
			if (clTreePopulation::snag == iTp) {
				...
				iSnagYears =
						iTreeDamage > 2000 ?
								iTreeDamage % 2000 : iTreeDamage % 1000;
			...
			if (iTreeDamage < 2000) {
				//Medium damage
				fSurvivalProb = exp(
						mp_fStmDmgMedA[iSp] + mp_fStmDmgMedB[iSp] * fDbh)
						/ (1
								+ exp(
										mp_fStmDmgMedA[iSp]
												+ mp_fStmDmgMedB[iSp] * fDbh));
			} else {
				//Heavy damage
				fSurvivalProb =
						exp(
								mp_fStmDmgHeavyA[iSp]
										+ mp_fStmDmgHeavyB[iSp] * fDbh)
								/ (1
										+ exp(
												mp_fStmDmgHeavyA[iSp]
														+ mp_fStmDmgHeavyB[iSp]
																* fDbh));
			}
			...
			if (fSurvivalProb < clModelMath::GetRand()) {
				...
				//snags, create one
				if (-1 != mp_iStmDmgCodes[clTreePopulation::snag][iSp]
					...
					if (iTreeDamage >= 2000) {
						if (clModelMath::GetRand() <= mp_fPropTipUp[iSp]) {
							//Yes - "kill" the snag
							p_oTree->SetValue(
									p_oPop->GetIntDataCode("dead", iSp,
											clTreePopulation::snag), storm);
						}
					...
					p_oTree->SetValue(
							mp_iStmDmgCodes[clTreePopulation::snag][iSp],
							iTreeDamage);
					goto nextTree;
				} else {
					//No snags - just kill this tree
					p_oTree->SetValue(p_oPop->GetIntDataCode("dead", iSp, iTp),
							storm);
					goto nextTree;
				}
			}
