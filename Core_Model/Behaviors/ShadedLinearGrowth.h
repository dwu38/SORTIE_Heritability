...
// ShadedLinearGrowth
//---------------------------------------------------------------------------
#if !defined(ShadedLinearGrowth_H)
#define ShadedLinearGrowth_H

#include "GrowthBase.h"

/**
 * Increments growth according to a linear equation with exponential reduction
 * for shade.  This can be used to create a growth increment with no automatic
 * height adjustment, a growth increment with automatic height adjustment, or a
 * height increment.
 *
 * The equation used in this behavior is:
 * <center>Y = (a + b * diam)*(GLI/100)<sup>c</sup></center>
 * where
 * <ul>
 ...
 * (n * diameter growth / X).
 *
 * The name string is "shadedlineargrowthshell".  In the parameter file:  For
 * diameter growth with no automatic height adjustment, call "ShadedLinearGrowth
 * diam only".  For diameter growth with automatic height adjustment, call
 * "ShadedLinearGrowth".  For height growth, call "ShadedLinearGrowth height".
 *
 ...
 */
class clShadedLinearGrowth: virtual public clGrowthBase {
...
public:
	 ...
	 */
	clShadedLinearGrowth(clSimManager *p_oSimManager);
	 ...
	 * the shaded linear growth equation.  The function value is calculated for
	 * each year of the timestep by using the original diameter plus
	 * fDiameterGrowth divided by the years per timestep times the loop counter for
	 * the year being compounded.  The final increment is the sum of all the
	 ...
	 * using the shaded linear growth equation.  The function value is calculated
	 * for each year of the timestep by repeatedly incrementing a copy of the
	 * diameter (the tree's actual diameter remains untouched, as it is supposed
	 * to).  The final increment is the sum of all the intermediate increments.
	 *
	 * @param p_oTree Tree for which to calculate growth.
	 * @param p_oPop Tree population object, just in case it's needed.
...
protected:
	 ...
	 * The meaning of what is returned depends on the type of growth the behavior
	 * is doing.
	 * @param iSpecies Tree species.
	 * @param fGLI The GLI value.
	 * @param fDiam The diameter for which to calculate this increment.
	 * @return The value of the shaded linear growth function.  Units depend on the
