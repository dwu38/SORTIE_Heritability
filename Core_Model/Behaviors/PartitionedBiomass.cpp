...
void clPartitionedBiomass::GetAppliesTo() {
	...
	for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
		if (clTreePopulation::seedling == mp_whatSpeciesTypeCombos[i].iType) {
			...
			stcErr.sMoreInfo =
					"Partitioned Biomass behavior cannot be applied to seedlings.";
			stcErr.iErrorCode = BAD_DATA;
			throw(stcErr);
		}
		mp_bAppliesTo[mp_whatSpeciesTypeCombos[i].iSpecies][mp_whatSpeciesTypeCombos[i].iType
				- clTreePopulation::sapling] = true;
	}
}
...
void clPartitionedBiomass::GetTreeCodes(clTreePopulation *p_oPop) {
	...
	for (i = 0; i < m_iNumTotalSpecies; i++) {
		...
		for (j = 0; j < iNumTypes; j++) {
			mp_iXCodes[i][j] = p_oPop->GetXCode(i,
					j + clTreePopulation::sapling);
			mp_iYCodes[i][j] = p_oPop->GetYCode(i,
					j + clTreePopulation::sapling);
		}
	}
	...
	if (m_bIsDbh) {
		...
		for (i = 0; i < m_iNumTotalSpecies; i++) {
			...
			for (j = 0; j < iNumTypes; j++) {
				mp_iDbhCodes[i][j] = p_oPop->GetDbhCode(i,
						j + clTreePopulation::sapling);
			}
		}
	} else {
		...
		for (i = 0; i < m_iNumTotalSpecies; i++) {
			...
			for (j = 0; j < iNumTypes; j++) {
				mp_iHeightCodes[i][j] = p_oPop->GetHeightCode(i,
						j + clTreePopulation::sapling);
			}
		}
	}
...
void clPartitionedBiomass::SetupGrid() {
	...
	if (NULL == mp_oGrid) {
		...
		mp_oGrid = mp_oSimManager->CreateGrid("Partitioned Biomass", 0, //number of ints
...
void clPartitionedBiomass::Action() {
	try {
		...
		int iNumXCells = mp_oGrid->GetNumberXCells(), iNumYCells =
				mp_oGrid->GetNumberYCells(), iX, iY, iSp, iTp;
		...
		if (m_bIsDbh) {
			while (p_oTree) {
				...
				if (mp_bAppliesTo[iSp][iTp]) {
					...
					if (fValue > 0) {
						mp_oGrid->GetValueAtPoint(fX, fY, mp_iLeafCodes[iSp],
								&fGridValue);
						fGridValue += fValue;
						mp_oGrid->SetValueAtPoint(fX, fY, mp_iLeafCodes[iSp],
								fGridValue);
					}
					...
					if (fValue > 0) {
						mp_oGrid->GetValueAtPoint(fX, fY, mp_iBranchCodes[iSp],
								&fGridValue);
						fGridValue += fValue;
						mp_oGrid->SetValueAtPoint(fX, fY, mp_iBranchCodes[iSp],
								fGridValue);
					}
					...
					if (fValue > 0) {
						mp_oGrid->GetValueAtPoint(fX, fY, mp_iBoleCodes[iSp],
								&fGridValue);
						fGridValue += fValue;
						mp_oGrid->SetValueAtPoint(fX, fY, mp_iBoleCodes[iSp],
								fGridValue);
					}
				}
			...
			while (p_oTree) {
				...
				if (mp_bAppliesTo[iSp][iTp]) {
					...
					fValue = (mp_fHeightLeafA[iSp] * fHeight)
							+ mp_fHeightLeafB[iSp];
					if (fValue > 0) {
						mp_oGrid->GetValueAtPoint(fX, fY, mp_iHLeafCodes[iSp],
								&fGridValue);
						fGridValue += fValue;
						mp_oGrid->SetValueAtPoint(fX, fY, mp_iHLeafCodes[iSp],
								fGridValue);
					}
					...
					fValue = (mp_fHeightBoleA[iSp] * fHeight)
							+ mp_fHeightBoleB[iSp];
					if (fValue > 0) {
						mp_oGrid->GetValueAtPoint(fX, fY, mp_iHBoleCodes[iSp],
								&fGridValue);
						fGridValue += fValue;
						mp_oGrid->SetValueAtPoint(fX, fY, mp_iHBoleCodes[iSp],
								fGridValue);
					}
				}
