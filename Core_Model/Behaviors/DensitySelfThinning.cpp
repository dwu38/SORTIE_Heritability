...
void clDensitySelfThinning::DoShellSetup(xercesc::DOMDocument *p_oDoc) {
	...
	try {
		...
		if (m_fNumberYearsPerTimestep != 1) {
			...
			stcErr.sMoreInfo =
					"This behavior can only be used with a one year timestep";
			throw(stcErr);
		}
		...
		FillSpeciesSpecificValue(p_oElement, "mo_selfThinRadius", "mo_strVal",
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			mp_fSelfThinRadius[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;
			if (mp_fSelfThinRadius[mp_iIndexes[p_fTempValues[i].code]] <= 0) {
				...
				stcErr.sFunction = "clDensitySelfThinning::DoShellSetup";
				stcErr.sMoreInfo =
						"The Density Self Thinning parameter SelfThinRadius must be a positive number.";
				throw(stcErr);
			}
		}
		//Minimum Density for Mortality
		FillSpeciesSpecificValue(p_oElement, "mo_minDensityForMort",
				"mo_mdfmVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets and check that they are positive
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			mp_fMinDensityForMort[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;
			if (mp_fMinDensityForMort[mp_iIndexes[p_fTempValues[i].code]] < 0) {
				...
				stcErr.sFunction = "clDensitySelfThinning::DoShellSetup";
				stcErr.sMoreInfo =
						"The Density Self Thinning parameter MinDensityForMort must be greater than or equal to 0.";
				throw(stcErr);
			}
		}
		...
		FillSpeciesSpecificValue(p_oElement, "mo_selfThinAsymptote",
				"mo_staVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets and check that they are positive
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			mp_fSelfThinAsymptote[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;
			if (mp_fSelfThinAsymptote[mp_iIndexes[p_fTempValues[i].code]]
					<= 0) {
				modelErr stcErr;
				stcErr.iErrorCode = BAD_DATA;
				stcErr.sFunction = "clDensitySelfThinning::DoShellSetup";
				stcErr.sMoreInfo =
						"The Density Self Thinning parameter SelfThinAsymptote must be a positive number.";
				throw(stcErr);
			}
		}
		...
		FillSpeciesSpecificValue(p_oElement, "mo_selfThinDiamEffect",
				"mo_stdieVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			mp_fSelfThinDiamEffect[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;
		}

		//Self Thinning Density Effect
		FillSpeciesSpecificValue(p_oElement, "mo_selfThinDensityEffect",
				"mo_stdeeVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets and check that they are positive
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			mp_fSelfThinDensityEffect[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;
			if (mp_fSelfThinDensityEffect[mp_iIndexes[p_fTempValues[i].code]]
					<= 0) {
				modelErr stcErr;
				stcErr.iErrorCode = BAD_DATA;
				stcErr.sFunction = "clDensitySelfThinning::DoShellSetup";
				stcErr.sMoreInfo =
						"The Density Self Thinning parameter SelfThinDensityEffect must be a positive number.";
				throw(stcErr);
			}
		}
...
void clDensitySelfThinning::CalculateNeighborhoodTreeCountAndMeanDiam10(
		clTree *p_oTree, float *p_fTreeCount, float *p_fMeanDiam10) {
	try {
		clTreePopulation *p_oPop =
				(clTreePopulation*) mp_oSimManager->GetPopulationObject(
						"treepopulation");
		...
		sQuery << "distance=" << mp_fSelfThinRadius[mp_iIndexes[iTargetSpecies]]
				<< "FROM x=" << fTargetX << "y=" << fTargetY << "::height="
				<< 0.0;
		...
		while (p_oNeighbor) {
			if (p_oNeighbor != p_oTree) {
				...
				if (iNeighType == clTreePopulation::seedling
						|| iNeighType == clTreePopulation::sapling) {
					iNeighborCount++;
					iNeighSpecies = p_oNeighbor->GetSpecies();

					//Get the neighbor's Diam10
					p_oNeighbor->GetValue(
							p_oPop->GetDiam10Code(iNeighSpecies, iNeighType),
							&fDiam10);
		...
		*p_fMeanDiam10 = fSumNeighDiam10 / static_cast<float>(iNeighborCount);
		...
		stcErr.iErrorCode = UNKNOWN;
		stcErr.sFunction =
				"clDensitySelfThinning::CalculateNeighborhoodTreeCountAndMeanDiam10";
		throw(stcErr);
	}
}
...
// DoMort()
////////////////////////////////////////////////////////////////////////////
deadCode clDensitySelfThinning::DoMort(clTree *p_oTree, const float &fDbh,
	...
	CalculateNeighborhoodTreeCountAndMeanDiam10(p_oTree, &fTreeCount,
			&fMeanDiam10);

	//Calculate density of trees in the neighborhood (trees/ha)
	fDensity = (fTreeCount * 10000.0)
			/ (3.1415926 * mp_fSelfThinRadius[mp_iIndexes[iSpecies]]
					* mp_fSelfThinRadius[mp_iIndexes[iSpecies]]);
	...
	if (fDensity <= mp_fMinDensityForMort[mp_iIndexes[iSpecies]])
		...
		fDeathProb = ((mp_fSelfThinAsymptote[mp_iIndexes[iSpecies]]
				+ (mp_fSelfThinDiamEffect[mp_iIndexes[iSpecies]] * fMeanDiam10))
				* fDensity)
				/ (((mp_fSelfThinAsymptote[mp_iIndexes[iSpecies]]
						+ (mp_fSelfThinDiamEffect[mp_iIndexes[iSpecies]]
								* fMeanDiam10))
						/ mp_fSelfThinDensityEffect[mp_iIndexes[iSpecies]])
						+ fDensity);
