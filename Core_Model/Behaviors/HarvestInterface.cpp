...
void clHarvestInterface::SetNameData(std::string sNameString) {
	...
	while (std::string::npos != iStart) {
		...
		iStart++;
		mp_sNewTreeDataMembers[iValNum] = sNameString.substr(iStart,
				(iEnd - iStart));
...
void clHarvestInterface::RegisterTreeDataMembers() {
	...
	for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
		mp_bAppliesToTrees[mp_whatSpeciesTypeCombos[i].iSpecies][mp_whatSpeciesTypeCombos[i].iType] =
				true;
	}

	if (0 == m_iNewTreeFloats)
		return;
...
void clHarvestInterface::GetData(xercesc::DOMDocument *p_oDoc) {
	try {
		using namespace std;
		DOMNodeList *p_oColumns, *p_oAllColumns;
		...
		int iNumColumns, //number of extra columns added by user to input file
				...
				iCode, iParamLine, //line on the batch parameters file, if applicable
				iNumTypes = p_oPop->GetNumberOfTypes();
		...
		FillSingleValue(p_oElement, "hi_harvestableTreesFile", &m_sInputFile,
				true);

		// Path and filename of the file of the trees to cut that is created by the
		// harvest executable
		FillSingleValue(p_oElement, "hi_treesToHarvestFile", &m_sTreesToCutFile,
				true);
		...
		if (m_iNewTreeFloats > 0) {
			FillSingleValue(p_oElement, "hi_treesToUpdateFile",
					&m_sTreesToUpdateFile, false);
		}

		// Argument string to pass to the user executable - optional
		FillSingleValue(p_oElement, "hi_executableArguments", &m_sArguments,
				false);
		...
		p_oAllColumns = p_oElement->getElementsByTagName(
				XMLString::transcode("hi_dataMembers"));
		if (0 < p_oAllColumns->getLength()) {
			p_oElement = (DOMElement*) p_oAllColumns->item(0);

			p_oColumns = p_oElement->getElementsByTagName(
					XMLString::transcode("hi_dataMember"));
				...
				for (i = 0; i < iNumColumns; i++) {
					cData =
							xercesc::XMLString::transcode(
									p_oColumns->item(i)->getChildNodes()->item(
											0)->getNodeValue());
				...
				for (iSp = 0; iSp < m_iNumSpecies; iSp++) {
					for (iTp = 0; iTp < iNumTypes; iTp++) {
						if (mp_bAppliesToTrees[iSp][iTp]) {
							...
							//Is it a float?
							if (p_oPop->GetFloatDataCode(mp_sFileColumns[i],
									iSp, iTp) > -1) {
								iCode = 0;
								if (notassigned == mp_iMemberType[i]) {
									...
									stcErr.sMoreInfo = s.str();
									stcErr.sFunction =
											"clHarvestInterface::GetData";
									throw(stcErr);
								}
							}

							//Is it an int?
							if (p_oPop->GetIntDataCode(mp_sFileColumns[i], iSp,
									iTp) > -1) {
								iCode = 0;
								if (notassigned == mp_iMemberType[i]) {
							...
							//Is it a bool?
							if (p_oPop->GetBoolDataCode(mp_sFileColumns[i], iSp,
									iTp) > -1) {
								iCode = 0;
								if (notassigned == mp_iMemberType[i]) {
							...
							//Is it a char?
							if (p_oPop->GetStringDataCode(mp_sFileColumns[i],
									iSp, iTp) > -1) {
								iCode = 0;
								if (notassigned == mp_iMemberType[i]) {
		...
		for (i = 0; i < m_iNumFileColumns; i++) {
			...
			for (j = 0; j < m_iNewTreeFloats; j++) {
				if (0
						== mp_sFileColumns[i].compare(
								mp_sNewTreeDataMembers[j])) {
		...
		if (0 < mp_oSimManager->GetBatchNumber()) {
			...
			FillSingleValue(p_oElement, "hi_batchParamsFile", &m_sBatchFileIn,
					false);
			...
			if (0 < m_sBatchFileIn.length()) {
				...
				FillSingleValue(p_oElement, "hi_batchSingleRunParamsFile",
						&m_sBatchParamsOut, true);
				...
				if (iNumLines != iParamLine) {
					...
					stcErr.sFunction = "clHarvestInterface::GetData";
					std::stringstream s;
					s << "Couldn't find parameters for batch run #"
							<< iParamLine;
					stcErr.sMoreInfo = s.str();
				...
				parametersOut.open(m_sBatchParamsOut.c_str(),
						ios::trunc | ios::out);
...
void clHarvestInterface::Action() {
	...
	try {
		...
		long int i, j;
		int iSp, iTp, iTemp, iCount, iVal, iNumLines, iNumBaseColumns = 6;
		bool bVal;
			...
			if (mp_bAppliesToTrees[iSp][iTp]) {
				...
				for (i = iNumBaseColumns; i < m_iNumFileColumns; i++) {
					...
					if (isfloat == mp_iMemberType[i]) {
						p_oTree->GetValue(
								p_oPop->GetFloatDataCode(mp_sFileColumns[i],
										iSp, iTp), &fVal);
						harvest << fVal;
					} else if (isint == mp_iMemberType[i]) {
						p_oTree->GetValue(
								p_oPop->GetIntDataCode(mp_sFileColumns[i], iSp,
										iTp), &iVal);
						harvest << iVal;
					} else if (isbool == mp_iMemberType[i]) {
						p_oTree->GetValue(
								p_oPop->GetBoolDataCode(mp_sFileColumns[i], iSp,
										iTp), &bVal);
						harvest << bVal;
					} else {
						p_oTree->GetValue(
				...
				for (i = 0; i < iNumLines; i++) {
					if (iSp == p_iSp[i] && iTp == p_iTp[i]
						...
						mp_oResultsGrid->SetValueAtPoint(fX, fY,
								m_iHarvestTypeCode, clDisturbance::partial);
						mp_oResultsGrid->GetValueAtPoint(fX, fY,
								mp_iDenCutCodes[0][iSp], &iTemp);
						iTemp += 1;
						mp_oResultsGrid->SetValueAtPoint(fX, fY,
								mp_iDenCutCodes[0][iSp], iTemp);
						...
						if (iTp != clTreePopulation::seedling)
							...
							//tree is a seedling and has no BA
							fThisBA = 0.0;

						mp_oResultsGrid->GetValueAtPoint(fX, fY,
								mp_iBaCutCodes[0][iSp], &fTemp);
						fTemp += fThisBA;
						mp_oResultsGrid->SetValueAtPoint(fX, fY,
								mp_iBaCutCodes[0][iSp], fTemp);
		...
		//Update trees
		//**********************************
		if (m_sTreesToUpdateFile.length() == 0)
			return;
...
void clHarvestInterface::ResetResultsGrid() {
	try {
		...
		short int iNumXCells = mp_oResultsGrid->GetNumberXCells(), iNumYCells =
				...
				p_oPop->GetNumberOfSpecies(), i, j, iSp, iRange; //loop counters
