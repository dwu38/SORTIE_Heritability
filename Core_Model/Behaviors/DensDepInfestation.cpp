...
clDensDepInfestation::clDensDepInfestation(clSimManager *p_oSimManager) :
	...
	try {
		//Set the namestring
		m_sNameString = "DensDepInfestation";
		m_sXMLRoot = "DensDepInfestation";
		...
		mp_iDensDepResistanceStatusCodes = NULL;
		mp_fMinDBH = NULL;
		mp_fCohortDBH = NULL;
		...
		stcErr.sFunction = "clDensDepInfestation::clDensDepInfestation";
		throw(stcErr);
	}
}
...
clDensDepInfestation::~clDensDepInfestation() {
	...
	if (mp_iDensDepResistanceStatusCodes) {
		int i;
		for (i = 0; i < m_iTotalNumSpecies; i++)
			delete[] mp_iDensDepResistanceStatusCodes[i];
	}
	delete[] mp_iDensDepResistanceStatusCodes;

	delete[] mp_fProbResistant;
	delete[] mp_fProbConditionallySusceptible;
	delete[] mp_fMinDBH;
...
void clDensDepInfestation::ReadParFile(xercesc::DOMDocument *p_oDoc,
	...
	try {
		...
		FillSingleValue(p_oElement, "di_densDepInfMaxInfestation", &m_fMax,
				true);
		//Make sure all values are between 0 and 1
		if (m_fMax < 0 || m_fMax > 1) {
			modelErr stcErr;
			stcErr.sFunction = "clDensDepInfestation::ReadParFile";
		...
		// a
		FillSingleValue(p_oElement, "di_densDepInfA", &m_fA, true);

		// bx
		FillSingleValue(p_oElement, "di_densDepInfBx", &m_fBx, true);

		// by
		FillSingleValue(p_oElement, "di_densDepInfBy", &m_fBy, true);

		//Minimum DBH to infest
		FillSpeciesSpecificValue(p_oElement, "di_densDepInfMinDBH",
				"di_ddimdVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			...
			if (mp_fMinDBH[p_fTempValues[i].code] < 0) {
				...
				stcErr.sFunction = "clDensDepInfestation::ReadParFile";
				stcErr.sMoreInfo =
						"Minimum DBH for insect infestation cannot be negative.";
				stcErr.iErrorCode = BAD_DATA;
				throw(stcErr);
			}
		}
		...
		//Cohort cutoff DBH
		FillSpeciesSpecificValue(p_oElement, "di_densDepInfCohortDBH",
				"di_ddicdVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets and make sure all values are
		//greater than 0
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			...
			if (mp_fCohortDBH[p_fTempValues[i].code] < 0) {
				...
				stcErr.sFunction = "clDensDepInfestation::ReadParFile";
				stcErr.sMoreInfo =
						"Minimum cohort DBH for insect infestation cannot be negative.";
				stcErr.iErrorCode = BAD_DATA;
				throw(stcErr);
			}
		}
		...
		FillSpeciesSpecificValue(p_oElement, "di_densDepInfPropResistant",
				"di_ddiprVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets and make sure all values are
		//greater than 0
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			...
			if (mp_fProbResistant[p_fTempValues[i].code] < 0
				...
				stcErr.sFunction = "clDensDepInfestation::ReadParFile";
				stcErr.sMoreInfo =
						"Proportion of trees that are resistant must be between 0 and 1.";
				stcErr.iErrorCode = BAD_DATA;
				throw(stcErr);
			}
		}
		...
		FillSpeciesSpecificValue(p_oElement, "di_densDepInfPropCondSusceptible",
				"di_ddipcsVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets and make sure all values are
		//greater than 0
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			mp_fProbConditionallySusceptible[p_fTempValues[i].code] =
					p_fTempValues[i].val;
			if (mp_fProbConditionallySusceptible[p_fTempValues[i].code] < 0
				...
				stcErr.sFunction = "clDensDepInfestation::ReadParFile";
				stcErr.sMoreInfo =
						"Proportion of trees that are conditionally susceptible  must be between 0 and 1.";
				stcErr.iErrorCode = BAD_DATA;
				throw(stcErr);
			}
		}
		...
		FillSingleValue(p_oElement, "di_densDepInfStartYear", &m_iFirstTimestep,
				true);
		...
		FillSingleValue(p_oElement, "di_densDepInfEndYear", &m_iLastTimestep,
				false);
		//Transform to timestep
...
void clDensDepInfestation::Action() {
	...
	int iSp, iTp, iInf, iStatus, iNumYrsTimestep =
		...
		if (-1 < mp_iYearsInfestedCodes[iSp][iTp]) {
			...
			if (fDbh >= mp_fMinDBH[iSp]) {
				...
				if (iInf > 0) {
					...
					p_oTree->GetValue(
							mp_iDensDepResistanceStatusCodes[iSp][iTp],
							&iStatus);
					...
					if (iStatus > resistant) {
						...
						if (fDbh < mp_fCohortDBH[iSp]) {
							if (clModelMath::GetRand() <= m_fSmallCohortProb) {
								iInf = iNumYrsTimestep;
								p_oTree->SetValue(
										mp_iYearsInfestedCodes[iSp][iTp], iInf);
							}
						} else {
...
void clDensDepInfestation::FormatQueryString(clTreePopulation *p_oPop) {
	...
	for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
		if (clTreePopulation::sapling == mp_whatSpeciesTypeCombos[i].iType) {
			bSapling = true;
		} else if (clTreePopulation::adult
...
void clDensDepInfestation::RegisterTreeDataMembers() {
	...
	mp_iDensDepResistanceStatusCodes = new short int*[m_iTotalNumSpecies];
	for (i = 0; i < m_iTotalNumSpecies; i++) {
		mp_iYearsInfestedCodes[i] = new short int[iNumTypes];
		mp_iDensDepResistanceStatusCodes[i] = new short int[iNumTypes];
		for (j = 0; j < iNumTypes; j++) {
			mp_iYearsInfestedCodes[i][j] = -1;
			mp_iDensDepResistanceStatusCodes[i][j] = -1;
		}
	}
	...
	for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
		...
		if (clTreePopulation::seedling == mp_whatSpeciesTypeCombos[i].iType) {
			modelErr stcErr;
			stcErr.sFunction = "clDensDepInfestation::RegisterTreeDataMembers";
		...
		mp_iYearsInfestedCodes[mp_whatSpeciesTypeCombos[i].iSpecies][mp_whatSpeciesTypeCombos[i].iType] =
				p_oPop->RegisterInt("YearsInfested",
						mp_whatSpeciesTypeCombos[i].iSpecies,
						mp_whatSpeciesTypeCombos[i].iType);
		mp_iDensDepResistanceStatusCodes[mp_whatSpeciesTypeCombos[i].iSpecies][mp_whatSpeciesTypeCombos[i].iType] =
				p_oPop->RegisterInt("DensDepResistanceStatus",
						mp_whatSpeciesTypeCombos[i].iSpecies,
						mp_whatSpeciesTypeCombos[i].iType);
	}
}
...
void clDensDepInfestation::TreeInventory() {
	...
	while (p_oTree) {
		...
		//Is this one of our pool trees? Use the DensDepResistanceStatus code to check
		if (mp_iDensDepResistanceStatusCodes[iSp][iTp] > -1) {
			...
			p_oTree->GetValue(mp_iDensDepResistanceStatusCodes[iSp][iTp],
					&iStatus);
			...
			if (iStatus == 0) {
				...
				if (clModelMath::GetRand() <= mp_fProbResistant[iSp]) {
					iStatus = resistant;
				} else if (clModelMath::GetRand()
						<= mp_fProbConditionallySusceptible[iSp]) {
					iStatus = cond_susceptible;
				} else {
					iStatus = susceptible;
				}
				p_oTree->SetValue(mp_iDensDepResistanceStatusCodes[iSp][iTp],
						iStatus);
...
void clDensDepInfestation::InfestInitialConditionsTrees() {
	...
	float fB, fDbh, fTargetRate, fRand;
	int i, iSp, iTp, iInf, iStatus, iNumYrsTimestep =
			(int) mp_oSimManager->GetNumberOfYearsPerTimestep();
				...
				if (iStatus > resistant) {
					...
					if (fDbh < mp_fCohortDBH[iSp]) {
						if (clModelMath::GetRand() <= m_fSmallCohortProb) {
							...
							p_oTree->SetValue(mp_iYearsInfestedCodes[iSp][iTp],
									iInf);
						}
					} else {
						if (clModelMath::GetRand() <= m_fLargeCohortProb) {
...
// Action()
/////////////////////////////////////////////////////////////////////////////
void clDensDepInfestation::DetermineCohortInfestationProbability(
		double fTargetRate) {
	//Figure out how many total trees must be infested
	long iNumToInfest = fTargetRate
			* (m_iNumSmallCohortPoolTrees + m_iNumLargeCohortPoolTrees);

	if (m_iNumLargeCohortPoolTrees == 0 && m_iNumSmallCohortPoolTrees == 0)
		return;

	if (iNumToInfest
			<= (m_iNumSmallCohortInfestedTrees + m_iNumLargeCohortInfestedTrees)) {
	...
	iNumToInfest -= (m_iNumSmallCohortInfestedTrees
			+ m_iNumLargeCohortInfestedTrees);

	//Case: we don't have any trees of one cohort or the other
	if (m_iNumLargeCohortPoolTrees == 0) {
		m_fLargeCohortProb = 0;
		m_fSmallCohortProb = (double) iNumToInfest
				/ (double) (m_iNumSmallInfestibleTrees
						- m_iNumSmallCohortInfestedTrees);
		return;
	}
	...
	if (m_iNumSmallCohortPoolTrees == 0) {
		m_fSmallCohortProb = 0;
		m_fLargeCohortProb = (double) iNumToInfest
				/ (double) (m_iNumLargeInfestibleTrees
						- m_iNumLargeCohortInfestedTrees);
		return;
	}
	...
	if (iNumToInfest
			< (m_iNumLargeInfestibleTrees - m_iNumLargeCohortInfestedTrees)) {
		...
		long iLargeToInfest = m_iNumLargeInfestibleTrees
				- m_iNumLargeCohortInfestedTrees;
		m_fSmallCohortProb = (double) (iNumToInfest - iLargeToInfest)
				/ (double) (m_iNumSmallInfestibleTrees
						- m_iNumSmallCohortInfestedTrees);
	}
