...
void clResourceMortality::ReadParameterFileData(xercesc::DOMDocument *p_oDoc) {
	...
	//Scaling factor (rho)
	FillSpeciesSpecificValue(p_oElement, "mo_resMortScalingFactor",
			"mo_rmsfVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
	//Transfer to the appropriate array buckets
	for (i = 0; i < m_iNumBehaviorSpecies; i++)
		mp_fRho[mp_iIndexes[p_fTempValues[i].code]] = p_fTempValues[i].val;

	//Function mode (mu)
	FillSpeciesSpecificValue(p_oElement, "mo_resMortMode", "mo_rmmVal",
	...
	FillSpeciesSpecificValue(p_oElement, "mo_resMortGrowthIncSurv",
			"mo_rmgisVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
	//Transfer to the appropriate array buckets
	for (i = 0; i < m_iNumBehaviorSpecies; i++)
		mp_fDelta[mp_iIndexes[p_fTempValues[i].code]] = p_fTempValues[i].val;

	//Low growth function shape (sigma)
	FillSpeciesSpecificValue(p_oElement, "mo_resMortLoGrowthShape",
			"mo_rmlgsVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
	//Transfer to the appropriate array buckets
	for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		for (i = 0; i < m_iNumTotalSpecies; i++)
			for (j = 0; j < iTotalTypes; j++)
				if (mp_bUsesThisMortality[i][j]) {
					mp_iGrowthCodes[mp_iIndexes[i]][j] =
							p_oPop->GetFloatDataCode(cLabel, i, j);
					...
					if (-1 == mp_iGrowthCodes[mp_iIndexes[i]][j]) {
						modelErr stcErr;
						stcErr.sFunction =
								"clResourceMortality::GetTreeDataMemberCodes";
						std::stringstream s;
...
// DoMort()
////////////////////////////////////////////////////////////////////////////
deadCode clResourceMortality::DoMort(clTree *p_oTree, const float &fDbh,
	...
	fSurvProb =
			(mp_fRho[mp_iIndexes[iSpecies]]
					* exp(
							-pow(fResource - mp_fMu[mp_iIndexes[iSpecies]], 2)
									/ (2
											* (pow(
													mp_fDelta[mp_iIndexes[iSpecies]]
															* fGrowth
															+ mp_fSigma[mp_iIndexes[iSpecies]],
													2)))));
