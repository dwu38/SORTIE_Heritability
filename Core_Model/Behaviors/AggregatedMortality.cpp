...
void clAggregatedMortality::DoShellSetup(xercesc::DOMDocument *p_oDoc) {
	try {
		...
		m_fMortalityEpisodeProbability = fNumYearsPerTimestep
...
void clAggregatedMortality::ReadParFile(xercesc::DOMDocument *p_oDoc) {
	...
	FillSingleValue(p_oElement, "mo_aggReturnInterval",
			&m_fMortalityEpisodeProbability, true);
	...
	FillSingleValue(p_oElement, "mo_aggPropTreesToKill", &m_fMortalityProb,
			true);
	...
	FillSingleValue(p_oElement, "mo_aggNumTreesToClump", &m_iNumTreesToClump,
			true);
	...
	if (m_iNumTreesToClump < 1) {
		...
		stcErr.sFunction = "clAggregatedMortality::ReadParFile";
		stcErr.sMoreInfo =
				"Number of trees to aggregate must be greater than 0.";
		stcErr.iErrorCode = BAD_DATA;
		throw(stcErr);
	}
	...
	FillSingleValue(p_oElement, "mo_aggClumpSizeDeterministic",
			&m_bClumpSizeDeterministic, true);

	//Clumping parameter for negative binomial distribution, if required
	if (!m_bClumpSizeDeterministic)
		FillSingleValue(p_oElement, "mo_aggClumpingParameter",
				&m_fClumpingParameter, true);
}
...
void clAggregatedMortality::PreMortCalcs(clTreePopulation *p_oPop) {
	...
	if (clModelMath::GetRand() > m_fMortalityEpisodeProbability)
		return;
	...
	while (p_oTree) {
		if (IsValid(p_oTree) && clModelMath::GetRand() < m_fMortalityProb) {
			...
			if (!m_bClumpSizeDeterministic) {
				...
				iClumpSize = clModelMath::NegBinomialRandomDraw(
						(float) m_iNumTreesToClump, m_fClumpingParameter);
			}
...
// KillClump()
////////////////////////////////////////////////////////////////////////////
void clAggregatedMortality::KillClump(clTreePopulation *p_oPop, clTree *p_oTree,
		int iNumToKill) {
	...
	int iNumTrees = 0, iSp, iTp, iTemp, j, iDead = natural;
	...
	do {
		...
		sQuery << "distance=" << fDistance << "FROM x=" << fTargetX << "y="
				<< fTargetY << "::height=" << fHeight;
	...
	while (p_oTree) {
		if (IsValid(p_oTree)) {
			...
			if (fDistance >= p_fDistances[iTemp]) {
				...
				//a space
				iTemp++;
				for (j = iTemp; (j > 0) && (p_fDistances[j - 1] > fDistance);
						j--) {
	...
	for (j = 0; j < iNumToKill; j++)
		p_oClumpTrees[j]->SetValue(
				mp_iDeadCodes[p_oClumpTrees[j]->GetSpecies()][p_oClumpTrees[j]->GetType()],
				iDead);
...
bool clAggregatedMortality::IsValid(clTree *p_oTree) {
	int iSp = p_oTree->GetSpecies(), iTp = p_oTree->GetType(), iIsDead;
...
// DoMort()
////////////////////////////////////////////////////////////////////////////
deadCode clAggregatedMortality::DoMort(clTree *p_oTree, const float &fDbh,
		const short int &iSpecies) {
	int iIsDead;
	p_oTree->GetValue(mp_iDeadCodes[p_oTree->GetSpecies()][p_oTree->GetType()],
			&iIsDead);
