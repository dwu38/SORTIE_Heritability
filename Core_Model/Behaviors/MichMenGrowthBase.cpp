...
void clMichMenBase::GetParameterFileData(DOMDocument *p_oDoc) {
	try {
		...
		if (mp_fSlopeDiamGrowthResponse) {
			FillSpeciesSpecificValue(p_oElement, "gr_slopeGrowthResponse",
					"gr_sgrVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
					true);

			//Transfer to the array
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
				mp_fSlopeDiamGrowthResponse[p_fTempValues[i].code] =
						p_fTempValues[i].val;
		}
		...
		if (mp_fSlopeHeightGrowthResponse) {
			FillSpeciesSpecificValue(p_oElement, "gr_slopeHeightGrowthResponse",
					"gr_shgrVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
					true);

			//Transfer to the array
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
				mp_fSlopeHeightGrowthResponse[p_fTempValues[i].code] =
						p_fTempValues[i].val;
		}

		//Asymptotic diameter growth
		if (mp_fAsympDiamGrowth) {
			FillSpeciesSpecificValue(p_oElement, "gr_asympDiameterGrowth",
					"gr_adgVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
					true);

			//Transfer to the array
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
				mp_fAsympDiamGrowth[p_fTempValues[i].code] =
						p_fTempValues[i].val;
		}

		//Asymptotic height growth
		if (mp_fAsympHeightGrowth) {
			FillSpeciesSpecificValue(p_oElement, "gr_asympHeightGrowth",
					"gr_ahgVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
					true);

			//Transfer to the array
			for (i = 0; i < m_iNumBehaviorSpecies; i++)
				mp_fAsympHeightGrowth[p_fTempValues[i].code] =
						p_fTempValues[i].val;
		}
		...
		if (mp_fAdultConstRadInc) {
			FillSpeciesSpecificValue(p_oElement, "gr_adultConstRadialInc",
					"gr_acriVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
					true);

			for (i = 0; i < m_iNumBehaviorSpecies; i++)
				mp_fAdultConstRadInc[p_fTempValues[i].code] =
						p_fTempValues[i].val;
		...
		if (mp_fAdultConstBAInc) {
			...
			FillSpeciesSpecificValue(p_oElement, "gr_adultConstAreaInc",
					"gr_acaiVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
					true);

			for (i = 0; i < m_iNumBehaviorSpecies; i++)
				mp_fAdultConstBAInc[p_fTempValues[i].code] =
						p_fTempValues[i].val;
		}
...
float clMichMenBase::GetGrowthMemberValue(clTree *p_oTree,
	...
	int iSp = p_oTree->GetSpecies(), iTp = p_oTree->GetType(), iGLICode =
			mp_oGrowthOrg->GetLightCode(iSp, iTp);
