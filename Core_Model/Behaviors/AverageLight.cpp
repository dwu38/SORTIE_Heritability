...
void clAverageLight::DoShellSetup(xercesc::DOMDocument *p_oDoc) {
	try {
		...
		if (!mp_oGLIMapGrid) {
			...
			stcErr.sMoreInfo =
					"GLI Map behavior must be used with the Average Light behavior.";
			throw(stcErr);
		}
		...
		if (!mp_oAvgLightGrid) {
			...
			mp_oAvgLightGrid = mp_oSimManager->CreateGrid("Average Light", 0, 1,
					0, 0);
			...
			if (-1 == m_iAvgLightCode) {
				...
				stcErr.sFunction = "clAverageLight::DoShellSetup";
				stcErr.sMoreInfo =
						"Average grid set up incorrectly. Can't find data member \"GLI\".";
				throw(stcErr);
			}
		}
	} catch (modelErr &err) {
...
void clAverageLight::CalculateAverage(clTreePopulation *p_oPop) {
	try {
		float fStartX, fStartY, fEndX, fEndY, fLengthXCells =
				mp_oAvgLightGrid->GetLengthXCells(), fLengthYCells =
				mp_oAvgLightGrid->GetLengthYCells(), fPlotLengthX =
				p_oPop->GetXPlotLength(), fPlotLengthY =
				p_oPop->GetYPlotLength(), fVal;
		int iNumXCells = mp_oAvgLightGrid->GetNumberXCells(), iNumYCells =
				mp_oAvgLightGrid->GetNumberYCells(), iX, iY; //loop counter

		//Calculate each cell's light level
		for (iX = 0; iX < iNumXCells; iX++) {
			fStartX = iX * fLengthXCells;
			fEndX = ((iX + 1) * fLengthXCells) - VERY_SMALL_VALUE;
			if (fEndX >= fPlotLengthX)
				fEndX = fPlotLengthX - VERY_SMALL_VALUE;

			for (iY = 0; iY < iNumYCells; iY++) {
				fStartY = iY * fLengthYCells;
				fEndY = ((iY + 1) * fLengthYCells) - VERY_SMALL_VALUE;
				if (fEndY >= fPlotLengthY)
					fEndY = fPlotLengthY - VERY_SMALL_VALUE;

				fVal = mp_oGLIMapGrid->GetAverageFloatValue(fStartX, fStartY,
						fEndX, fEndY, m_iMapLightCode);
