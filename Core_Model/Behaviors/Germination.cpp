...
void clGermination::GetData(DOMDocument *p_oDoc) {
	try {
		...
		FillSpeciesSpecificValue(p_oElement, "ge_proportionGerminating",
				"ge_pgVal", p_fTemp, m_iNumBehaviorSpecies, p_oPop, true);
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			if (mp_fProportionGerminating[mp_iWhatSpecies[i]] < 0
					|| mp_fProportionGerminating[mp_iWhatSpecies[i]] > 1) {
				modelErr stcErr;
				stcErr.sFunction = "clGermination::GetData";
				stcErr.sMoreInfo =
						"All values in proportion germinating must be between 0 and 1.";
		...
		if (mp_oSeedGrid == NULL) {
			...
			stcErr.sFunction = "clGermination::GetData";
			stcErr.sMoreInfo =
					"Disperse behaviors must be used with germination.";
		...
		for (iY = 0; iY < iNumYCells; iY++) {
			...
			for (i = 0; i < m_iNumBehaviorSpecies; i++) {
				...
				fNumSeeds =
						p_oMath->RandomRound(
								fNumSeeds
										* mp_fProportionGerminating[mp_iWhatSpecies[i]]);
