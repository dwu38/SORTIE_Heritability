...
//---------------------------------------------------------------------------
#include <JulMaxPrecipMortality.h>
...
//#include "NCI/WateritationEffectBase.h"
...
// Constructor
//////////////////////////////////////////////////////////////////////////////
clJulMaxPrecipMortality::clJulMaxPrecipMortality(clSimManager *p_oSimManager) :
	...
	try {
		...
		m_sXMLRoot = "JulMaxPrecipMortality";
		...
		stcErr.sFunction = "clJulMaxPrecipMortality::clJulMaxPrecipMortality";
		throw(stcErr);
	}
}
...
// Destructor
////////////////////////////////////////////////////////////////////////////
clJulMaxPrecipMortality::~clJulMaxPrecipMortality() {
...
// GetTreeMemberCodes()
////////////////////////////////////////////////////////////////////////////
void clJulMaxPrecipMortality::GetTreeMemberCodes() {
	...
	for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
		...
		if (-1
				== mp_iDeadCodes[mp_whatSpeciesTypeCombos[i].iSpecies][mp_whatSpeciesTypeCombos[i].iType]) {
			modelErr stcErr;
			stcErr.sFunction = "clJulMaxPrecipMortality::GetTreeMemberCodes";
...
// DoShellSetup()
////////////////////////////////////////////////////////////////////////////
void clJulMaxPrecipMortality::DoShellSetup(xercesc::DOMDocument *p_oDoc) {
	clTreePopulation *p_oPop =
			(clTreePopulation*) mp_oSimManager->GetPopulationObject(
					"treepopulation");
	...
	for (i = 0; i < m_iNumSpeciesTypeCombos; i++)
		if (clTreePopulation::sapling != mp_whatSpeciesTypeCombos[i].iType
			...
			modelErr stcErr;
			stcErr.iErrorCode = BAD_DATA;
			stcErr.sFunction = "clJulMaxPrecipMortality::DoShellSetup";
			stcErr.sMoreInfo =
	...
	FillSpeciesSpecificValue(p_oElement, "mo_JulMaxPrecipInter",
			"mo_JulMaxPrecipInterVal", p_fTempValues, m_iNumBehaviorSpecies,
			p_oPop, true);
	for (i = 0; i < m_iNumBehaviorSpecies; i++)
	...
	FillSpeciesSpecificValue(p_oElement, "mo_JulMaxPrecipDiam",
			"mo_JulMaxPrecipDiamVal", p_fTempValues, m_iNumBehaviorSpecies,
			p_oPop, true);
	for (i = 0; i < m_iNumBehaviorSpecies; i++)
	...
	FillSpeciesSpecificValue(p_oElement, "mo_JulMaxPrecipDiam2",
			"mo_JulMaxPrecipDiam2Val", p_fTempValues, m_iNumBehaviorSpecies,
			p_oPop, true);
	for (i = 0; i < m_iNumBehaviorSpecies; i++)
	...
	FillSpeciesSpecificValue(p_oElement, "mo_JulMaxPrecipNCI",
			"mo_JulMaxPrecipNCIVal", p_fTempValues, m_iNumBehaviorSpecies,
			p_oPop, true);
	for (i = 0; i < m_iNumBehaviorSpecies; i++)
	...
	FillSpeciesSpecificValue(p_oElement, "mo_JulMaxPrecipJulMax",
			"mo_JulMaxPrecipJulMaxVal", p_fTempValues, m_iNumBehaviorSpecies,
			p_oPop, true);
	for (i = 0; i < m_iNumBehaviorSpecies; i++)
	...
	FillSpeciesSpecificValue(p_oElement, "mo_JulMaxPrecipPrecip",
			"mo_JulMaxPrecipPrecipVal", p_fTempValues, m_iNumBehaviorSpecies,
			p_oPop, true);
	for (i = 0; i < m_iNumBehaviorSpecies; i++)
...
// DoMort
////////////////////////////////////////////////////////////////////////////
deadCode clJulMaxPrecipMortality::DoMort(clTree *p_oTree, const float &fDbh,
...
// PreMortCalcs
////////////////////////////////////////////////////////////////////////////
void clJulMaxPrecipMortality::PreMortCalcs(clTreePopulation *p_oPop) {
	try {
    ...
    out << "Timestep\tSpecies\tDBH\tNCI\tSurv Prob\tDead?\n";
		...
		float
				...
				fPlotTemp = p_oPlot->GetJulMaxTemp(), //July maximum temperature
				fPlotWater = p_oPlot->GetMeanAnnualPrecip(); //Current plot water deficit
		...
		while (p_oTree) {
			...
			if (mp_bUsesThisMortality[iSpecies][iType]) {
				...
				if (iDead <= natural) {
					...
					fSurvivalProb = exp(
							mp_fInter[iSpecies] + mp_fDiam[iSpecies] * fDbh
									+ mp_fDiam2[iSpecies] * fDbh * fDbh
									+ mp_fNCI[iSpecies] * nci.fNCI1
									+ mp_fJulMax[iSpecies] * fPlotTemp
									+ mp_fPrecip[iSpecies] * fPlotWater);
					fSurvivalProb = 1 / (1 + fSurvivalProb);
					//Get annual survival
					fSurvivalProb = pow(fSurvivalProb,
							1 / m_fMaxSurvivalPeriod);
    ...
    out.close();
		...
		modelErr stcErr;
		stcErr.iErrorCode = UNKNOWN;
		stcErr.sFunction = "clJulMaxPrecipMortality::PreMortGrowth";
		throw(stcErr);
	}
}
