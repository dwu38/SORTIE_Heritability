...
void clPRStormBiLevelGrowth::DoShellSetup(DOMDocument *p_oDoc) {
	...
	try {
		...
		if (NULL == mp_oStorm) {
			...
			stcErr.sMoreInfo =
					"The storm behavior must be used with PR Storm Bi-Level Growth.";
			throw(stcErr);
		}
		...
		if (-1 == m_iStormtimeCode) {
			...
			stcErr.sFunction = "clPRStormBiLevelGrowth::DoShellSetup";
			stcErr.sMoreInfo =
					"The \"stormtime\" was not found in the \"Storm Damage\" grid.";
			throw(stcErr);
		}
		...
		if (NULL == mp_oStormLight) {
			...
			stcErr.sFunction = "clPRStormBiLevelGrowth::DoShellSetup";
			stcErr.sMoreInfo =
					"The storm light behavior must be used with PR Storm Bi-Level Growth.";
			throw(stcErr);
		}
		...
		if (-1 == m_iLightCode) {
			...
			stcErr.sFunction = "clPRStormBiLevelGrowth::DoShellSetup";
			stcErr.sMoreInfo =
					"The \"Light\" was not found in the \"Storm Light\" grid.";
			throw(stcErr);
		}
		...
		FillSpeciesSpecificValue(p_oElement,
				"gr_prBilevStmGrwthLoLiteIntercept", "gr_pbsglliVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer values to our permanent array
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fLoLightIntercept[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;

		//Low-light growth slope
		FillSpeciesSpecificValue(p_oElement, "gr_prBilevStmGrwthLoLiteSlope",
				"gr_pbsgllsVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer values to our permanent array
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fLoLightSlope[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;

		//High-light threshold
		FillSpeciesSpecificValue(p_oElement,
				"gr_prBilevStmGrwthHiLiteThreshold", "gr_pbsghltVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		...
		FillSpeciesSpecificValue(p_oElement, "gr_prBilevStmGrwthHiLiteA",
				"gr_pbsghlaVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		...
		FillSpeciesSpecificValue(p_oElement, "gr_prBilevStmGrwthHiLiteB",
				"gr_pbsghlbVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer values to our permanent array
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fHiLightB[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;
...
float clPRStormBiLevelGrowth::CalcDiameterGrowthValue(clTree *p_oTree,
	...
	if (fLightLevel < mp_fHiLightThreshold[mp_iIndexes[iSp]]) {
		fGrowth = m_fYearsPerTimestep
				* clModelMath::CalcPointValue(fDiam,
						mp_fLoLightSlope[mp_iIndexes[iSp]],
						mp_fLoLightIntercept[mp_iIndexes[iSp]]);
		if (fDiam + fGrowth < MINDIAM)
			fGrowth = fDiam - MINDIAM;
		return fGrowth;
	}
	...
	if (fHeight > fHeightGrowth)
		fHeight = fHeightGrowth - 0.01;
