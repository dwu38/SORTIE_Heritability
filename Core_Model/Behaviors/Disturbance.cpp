...
void clDisturbance::ReadHarvestParameterFileData(xercesc::DOMDocument *p_oDoc) {
	...
	int
			iNumHarvestEvents, //total number of cut events to extract
			iX,
			iY, //X and Y grid values for package
			iNumberTimesteps = mp_oSimManager->GetNumberOfTimesteps(),
			iNumSpecies = mp_oPop->GetNumberOfSpecies(), iTimestep, //timestep of harvest cut event
	...
	for (i = 0; i < iNumHarvestEvents; i++) {
		...
		for (j = 0; j < iNumChildren; j++) {
			...
			if (-1 == iTemp) { //throw an error
				...
				stcErr.iErrorCode = BAD_DATA;
				stcErr.sFunction =
						"clDisturbance::ReadHarvestParameterFileData";
		...
		for (j = 0; j < iNumChildren; j++) {
			...
			if (fTemp < 0) {
				...
				stcErr.sFunction =
						"clDisturbance::ReadHarvestParameterFileData";
				stcErr.sMoreInfo =
						"Harvest cut range value cannot be negative.";
				throw(stcErr);
			}
		}
		...
		FillSpeciesSpecificValue(p_oElement, "ha_percentSeedlingsDie",
				"ha_psdVal", p_fSeedlingValues, iNumSpecies, mp_oPop, false);
		for (j = 0; j < iNumSpecies; j++) {
		...
		if (iNumChildren > 0 && iCutType == percentDen) {
			...
			stcErr.sFunction = "clDisturbance::ReadHarvestParameterFileData";
			stcErr.sMoreInfo =
					"Cut priorities cannot be used with percent density cuts.";
			throw(stcErr);
		}
		for (j = 0; j < iNumChildren; j++) {
			p_oNode = p_oCutSpecificsList->item(j);
...
void clDisturbance::ReadMortEpParameterFileData(xercesc::DOMDocument *p_oDoc) {
	...
	floatVal *p_fSeedlingValues; //for getting species-specific values
	int
			iNumDisturbanceEvents, //total number of cut events to extract
			iX,
			iY, //X and Y grid values for package
			iNumberTimesteps = mp_oSimManager->GetNumberOfTimesteps(),
			iNumSpecies = mp_oPop->GetNumberOfSpecies(), iTimestep, //timestep of mortality episode
			iMinIndex, //index of the smallest value on a list
			iNumChildren, //number of sub-cut-events
			iTemp; //for extracting integer data from parameter file
	...
	for (i = 0; i < iNumDisturbanceEvents; i++) {
		...
		for (j = 0; j < iNumChildren; j++) {
			...
			if (fTemp < 0) {
				...
				stcErr.sFunction = "clDisturbance::ReadMortEpParameterFileData";
				stcErr.sMoreInfo =
						"Mortality episode cut range value cannot be negative.";
				throw(stcErr);
			}
		}
		...
		for (j = 0; j < iNumSpecies; j++)
			p_fSeedlingValues[j].val = 0;
		FillSpeciesSpecificValue(p_oElement, "ds_percentSeedlingsDie",
				"ds_psdVal", p_fSeedlingValues, iNumSpecies, mp_oPop, false);
		for (j = 0; j < iNumSpecies; j++) {
			p_oNewPackage->SetValue(mp_iSeedlingCodes[j],
					(float) (p_fSeedlingValues[j].val / 100.0));
		}
...
void clDisturbance::ValidatePackages() {
	try {
		...
		short int iNumXCells = mp_oCutEventsGrid->GetNumberXCells(),
				iNumYCells = mp_oCutEventsGrid->GetNumberYCells(), i, j, k;
			...
			for (k = 0; k < m_iNumAllowedCutRanges; k++) {
				...
				if (fLowDbh != 0 && fHiDbh != 0 && fAmount != 0) {
					...
					if (partial != iCutType && k > 0) {
						...
						stcErr.sMoreInfo =
								"Cut ranges may not be defined for harvest gap or clear cuts.";
						throw(stcErr);
					}
					...
					if (fHiDbh <= fLowDbh) {
						...
						stcErr.sFunction = "clDisturbance::ValidatePackages";
						stcErr.sMoreInfo =
								"The low dbh range value in a harvest cut must be smaller than the high.";
						throw(stcErr);
					}
					...
					if (fLowDbh < 0 || fHiDbh < 0 || fAmount < 0) {
						...
						stcErr.sFunction = "clDisturbance::ValidatePackages";
						stcErr.sMoreInfo =
								"No value in a harvest cut range may be negative.";
						throw(stcErr);
					}
					...
					if (fPrevHiDbh > fLowDbh) {
						...
						stcErr.sFunction = "clDisturbance::ValidatePackages";
						stcErr.sMoreInfo =
								"Harvest cut ranges may not overlap.";
						throw(stcErr);
					}
					...
					if ((percentBA == iAmountType || percentDen == iAmountType)
						...
						stcErr.sFunction = "clDisturbance::ValidatePackages";
						stcErr.sMoreInfo =
								"Harvest cut percentage amounts must be less than 100.";
						throw(stcErr);
					}
			...
			for (i = 0; i < iNumSpecies; i++) {
				...
				if (fAmount < 0 || fAmount > 1) {
					...
					stcErr.sFunction =
							"clDisturbance::ReadHarvestParameterFileData";
					stcErr.sMoreInfo =
							"Seedling death rate must be between 0 and 100.";
					throw(stcErr);
				}
			}
				...
				while (p_oPackage) {
					...
					while (p_oNextPackage) {
						...
						if (iID == iNextID) {
							...
							stcErr.iErrorCode = BAD_DATA;
							stcErr.sFunction =
									"clDisturbance::ValidatePackages";
							stcErr.sMoreInfo =
									"Disturbance event may not be applied twice to the same grid cell.";
							throw(stcErr);
						}
		...
		for (i = 0; i < iNumXCells; i++)
			for (j = 0; j < iNumYCells; j++) {
				...
				if (m_bIsHarvest) {
					...
					mp_oResultsGrid->SetValueOfCell(i, j, m_iHarvestTypeCode,
							iValue);
				}
				...
				for (iSp = 0; iSp < iNumSpecies; iSp++) {
					mp_oResultsGrid->SetValueOfCell(i, j,
							mp_iSeedlingCutCodes[iSp], iValue);
					for (iRange = 0; iRange < m_iNumAllowedCutRanges;
							iRange++) {
						mp_oResultsGrid->SetValueOfCell(i, j,
								mp_iDenCutCodes[iRange][iSp], iValue);
						mp_oResultsGrid->SetValueOfCell(i, j,
								mp_iBaCutCodes[iRange][iSp], fValue);
					}
				}
			}
	...
	try {
		...
		if (m_bIsHarvest) {
			...
			mp_oMasterCutsGrid->ChangePackageDataStructure(
					...
					(3 * m_iNumAllowedCutRanges) + iNumSpecies
							+ (2 * m_iNumAllowedPriorities), //float data members
					m_iNumAllowedPriorities, //char data members
					iNumSpecies); //bool data members

			//Register the data members
			m_iMasterTimestepCode = mp_oMasterCutsGrid->RegisterPackageInt(
					"timestep");
			m_iMasterIDCode = mp_oMasterCutsGrid->RegisterPackageInt("id");
			m_iCutTypeCode = mp_oMasterCutsGrid->RegisterPackageInt("cuttype");
			m_iAmountTypeCode = mp_oMasterCutsGrid->RegisterPackageInt(
					"amttype");

			for (i = 0; i < m_iNumAllowedCutRanges; i++) {
				sLabel << "rangemin" << i;
				mp_iRangeMinCodes[i] = mp_oMasterCutsGrid->RegisterPackageFloat(
						sLabel.str());
				sLabel.str("");
				sLabel << "rangemax" << i;
				mp_iRangeMaxCodes[i] = mp_oMasterCutsGrid->RegisterPackageFloat(
						sLabel.str());
				sLabel.str("");
				sLabel << "rangeamt" << i;
				mp_iRangeAmountCodes[i] =
						mp_oMasterCutsGrid->RegisterPackageFloat(sLabel.str());
				sLabel.str("");
			}

			for (i = 0; i < iNumSpecies; i++) {
				sLabel << "species" << i;
				mp_iSpeciesCodes[i] = mp_oMasterCutsGrid->RegisterPackageBool(
						sLabel.str());
				sLabel.str("");
				sLabel << "seedling" << i;
				mp_iSeedlingCodes[i] = mp_oMasterCutsGrid->RegisterPackageFloat(
						sLabel.str());
				sLabel.str("");
			}
			...
			for (i = 0; i < m_iNumAllowedPriorities; i++) {
				...
				mp_iPriorityNameCodes[i] =
						mp_oMasterCutsGrid->RegisterPackageString(sLabel.str());
				sLabel.str("");
				sLabel << "prioritytype" << i;
				mp_iPriorityTypeCodes[i] =
						mp_oMasterCutsGrid->RegisterPackageInt(sLabel.str());
				sLabel.str("");
				sLabel << "prioritymin" << i;
				mp_iPriorityMinCodes[i] =
						mp_oMasterCutsGrid->RegisterPackageFloat(sLabel.str());
				sLabel.str("");
				sLabel << "prioritymax" << i;
				mp_iPriorityMaxCodes[i] =
						mp_oMasterCutsGrid->RegisterPackageFloat(sLabel.str());
				sLabel.str("");
			}
			...
			//Register the data members
			m_iCutTimestepCode = mp_oCutEventsGrid->RegisterPackageInt(
					"timestep");
			...
			for (i = 0; i < m_iNumAllowedCutRanges; i++)
				for (j = 0; j < iNumSpecies; j++) {
					...
					mp_iDenCutCodes[i][j] = mp_oResultsGrid->RegisterInt(
							sLabel.str());
					sLabel.str("");
					sLabel << "Cut Basal Area_" << i << "_" << j;
					mp_iBaCutCodes[i][j] = mp_oResultsGrid->RegisterFloat(
							sLabel.str());
					sLabel.str("");
				}
			...
			for (i = 0; i < iNumSpecies; i++) {
				...
				mp_iSeedlingCutCodes[i] = mp_oResultsGrid->RegisterInt(
						sLabel.str());
				sLabel.str("");
			}
		} else {
			...
			mp_oMasterCutsGrid->ChangePackageDataStructure(3, //number of package int data members
					...
					0, //number of package char data members
					iNumSpecies); //number of package bool data members
			...
			//*************************************
			mp_oCutEventsGrid = mp_oSimManager->CreateGrid(
					"mortepisodecutevents", //grid name
					0, //number of int data members
					0, //number of float data members
					0, //number of char data members
					0, //number of bool data members
			...
			//*************************************
			mp_oResultsGrid = mp_oSimManager->CreateGrid(
					"Mortality Episode Results", //grid name
...
void clDisturbance::Action() {
	try {
		...
		while (p_oMasterPackage) {
			...
			//Get this package's timestep
			p_oMasterPackage->GetValue(m_iMasterTimestepCode,
					&iPackageTimestep);
...
void clDisturbance::CutTrees(clPackage *p_oMasterPackage) {
	...
	try {
		...
		float fPriorityMin, fPriorityMax, fTemp; //for doing grid values
		int iNumXCells = mp_oCutEventsGrid->GetNumberXCells(), iNumYCells =
				mp_oCutEventsGrid->GetNumberYCells(), iNumSpecies =
				mp_oPop->GetNumberOfSpecies(), iAmountType, //in what terms the cut is expressed
				iCutType, //type of cut to be performed
				iPriorityType, iNumCellsInCutArea; //number of cells in cut area
		short int iSp, i, j;

		//#define test;
#ifdef test
    float fTemp2;
    fstream harv( "HarvestTrees.xls", ios::out | ios::app );
    harv << "Timestep: " << mp_oSimManager->GetCurrentTimestep() << "\n" << "X\tY\tSpecies\tDbh\n";
		...
		iNumCellsInCutArea = AssembleCutArea(p_oMasterPackage, iNumXCells,
				iNumYCells, p_cutArea, p_fLoDbh, p_fHiDbh, p_fAmountToRemove,
				p_bSpeciesCut);
		...
		if (absDen == iAmountType) //Round to nearest whole number
			for (i = 0; i < m_iNumAllowedCutRanges; i++)
				p_fAmountToRemove[i] = clModelMath::Round(p_fAmountToRemove[i],
						0);
		...
		for (iSp = 0; iSp < iNumSpecies; iSp++)
			if (p_bSpeciesCut[iSp]) {
				...
				if (percentBA == iAmountType) {
					...
					GetBasalArea(p_cutArea, iSp, p_fTotalBasalArea, p_fLoDbh,
							p_fHiDbh);
				}
				...
				for (j = 0; j < m_iNumAllowedPriorities; j++) {
					if (mp_iPriorityNameCodes[j] > -1) {
						p_oMasterPackage->GetValue(mp_iPriorityNameCodes[j],
								&sPriorityName);
						if (sPriorityName.length() > 0) {
							p_oMasterPackage->GetValue(mp_iPriorityTypeCodes[j],
									&iPriorityType);
							p_oMasterPackage->GetValue(mp_iPriorityMinCodes[j],
									&fPriorityMin);
							p_oMasterPackage->GetValue(mp_iPriorityMaxCodes[j],
									&fPriorityMax);

							if (percentBA == iAmountType)
								CutSpeciesPercentBAPriority(iSp, p_cutArea,
										p_fLoDbh, p_fHiDbh, p_fAmountToRemove,
										p_fAmountRemoved, p_fTotalBasalArea,
										sPriorityName, iPriorityType,
										fPriorityMin, fPriorityMax);
							else if (absDen == iAmountType)
								CutSpeciesAbsDenPriority(iSp, p_cutArea,
										p_fLoDbh, p_fHiDbh, p_fAmountToRemove,
										p_fAmountRemoved, sPriorityName,
										iPriorityType, fPriorityMin,
										fPriorityMax);
							else if (absBA == iAmountType)
								CutSpeciesAbsBAPriority(iSp, p_cutArea,
										p_fLoDbh, p_fHiDbh, p_fAmountToRemove,
										p_fAmountRemoved, sPriorityName,
										iPriorityType, fPriorityMin,
										fPriorityMax);
				...
				if (percentDen == iAmountType)
					CutSpeciesPercentDen(iSp, p_cutArea, p_fLoDbh, p_fHiDbh,
							p_fAmountToRemove, p_fAmountRemoved);
				else if (percentBA == iAmountType)
					CutSpeciesPercentBA(iSp, p_cutArea, p_fLoDbh, p_fHiDbh,
							p_fAmountToRemove, p_fAmountRemoved,
							p_fTotalBasalArea);
				else if (absDen == iAmountType)
					CutSpeciesAbsDen(iSp, p_cutArea, p_fLoDbh, p_fHiDbh,
							p_fAmountToRemove, p_fAmountRemoved);
				else
					// absBA == iAmountType)
					CutSpeciesAbsBA(iSp, p_cutArea, p_fLoDbh, p_fHiDbh,
							p_fAmountToRemove, p_fAmountRemoved);

			} //end of if (p_bSpeciesCut[iSp])

		KillSeedlings(p_cutArea, p_fKillProb);
...
// CutSpeciesPercentBA()
//////////////////////////////////////////////////////////////////////////////
void clDisturbance::CutSpeciesPercentBA(int iSp, stcGridList *&p_cutArea,
	...
	float fDbh, fThisBA; //basal area of a single tree;
	int i;
	...
	for (i = 0; i < m_iNumAllowedCutRanges; i++)
		bTreeIsDead = bTreeIsDead
				&& p_fAmountRemoved[i] >= p_fAdjustedToRemove[i];
	if (bTreeIsDead)
		return;

	//Now remove the trees
	p_oTree = GetTallestTreeInCutArea(p_cutArea, iSp);
	while (p_oTree) {
		bTreeIsDead = false;
		fThisBA = 0;
		...
		for (i = 0; i < m_iNumAllowedCutRanges; i++) {
			if (fDbh >= p_fLoDbh[i] && fDbh <= p_fHiDbh[i]) {
				...
				if (p_fAmountRemoved[i] < p_fAdjustedToRemove[i]) {
					...
					//Don't go over the limit - maybe a smaller tree will get us
					//closer
					if (p_fAmountRemoved[i] <= p_fAdjustedToRemove[i])
						bTreeIsDead = true;
					else
		...
		p_oNextTree = GetNextTreeInCutArea(p_cutArea, iSp);
		if (bTreeIsDead) {
...
// CutSpeciesAbsBA()
//////////////////////////////////////////////////////////////////////////////
void clDisturbance::CutSpeciesAbsBA(int iSp, stcGridList *&p_cutArea,
		float *p_fLoDbh, float *p_fHiDbh, float *p_fAmountToRemove,
	...
	for (i = 0; i < m_iNumAllowedCutRanges; i++)
		bTreeIsDead = bTreeIsDead
				&& p_fAmountRemoved[i] >= p_fAmountToRemove[i];
	if (bTreeIsDead)
		return;
...
//////////////////////////////////////////////////////////////////////////////
void clDisturbance::CutSpeciesPercentDen(int iSp, stcGridList *&p_cutArea,
		float *p_fLoDbh, float *p_fHiDbh, float *p_fAmountToRemove,
		float *p_fAmountRemoved) {

	clTree *p_oTree, //tree we're working with
			*p_oNextTree; //placeholder for getting the next tree
	float fDbh, fRand; //random number
...
// CutSpeciesAbsDen()
//////////////////////////////////////////////////////////////////////////////
void clDisturbance::CutSpeciesAbsDen(int iSp, stcGridList *&p_cutArea,
		float *p_fLoDbh, float *p_fHiDbh, float *p_fAmountToRemove,
		float *p_fAmountRemoved) {
	clTree *p_oTree, //tree we're working with
			*p_oNextTree; //placeholder for getting the next tree
...
void clDisturbance::CutSpeciesPercentBAPriority(int iSp,
		stcGridList *&p_cutArea, float *p_fLoDbh, float *p_fHiDbh,
		float *p_fAmountToRemove, float *p_fAmountRemoved,
		float *p_fTotalBasalArea, std::string sPriorityName, int &iPriorityType,
		float &fPriorityMin, float &fPriorityMax) {

	clTree *p_oTree, //tree we're working with
			*p_oNextTree; //placeholder for getting the next tree
	float *p_fAdjustedToRemove = new float[m_iNumAllowedCutRanges];
	float fDbh, fTemp, fThisBA; //basal area of a single tree;
	int i, iTemp, iCode;
		...
		for (i = 0; i < m_iNumAllowedCutRanges; i++) {
			if (fDbh >= p_fLoDbh[i] && fDbh <= p_fHiDbh[i]) {
				...
				if (floatType == iPriorityType) {
					iCode = mp_oPop->GetFloatDataCode(sPriorityName, iSp,
							p_oTree->GetType());
					if (iCode > -1) {
					...
					iCode = mp_oPop->GetIntDataCode(sPriorityName, iSp,
							p_oTree->GetType());
					if (iCode > -1) {
					...
					iCode = mp_oPop->GetBoolDataCode(sPriorityName, iSp,
							p_oTree->GetType());
					if (iCode > -1) {
						p_oTree->GetValue(iCode, &bTemp);
						if ((bTemp && fPriorityMin == 1)
								|| (!bTemp && fPriorityMin == 0))
							bTreeIsPriority = true;
					}
				}
...
//////////////////////////////////////////////////////////////////////////////
void clDisturbance::CutSpeciesAbsBAPriority(int iSp, stcGridList *&p_cutArea,
		float *p_fLoDbh, float *p_fHiDbh, float *p_fAmountToRemove,
...
void clDisturbance::AddTreeStatsToResults(clTree *p_oTree, float *p_fLoDbh,
	...
	int i, iSp = p_oTree->GetSpecies(), iTemp;
	...
	for (i = 0; i < m_iNumAllowedCutRanges; i++)
		if (fDbh >= p_fLoDbh[i] && fDbh <= p_fHiDbh[i]) {
			mp_oResultsGrid->GetValueAtPoint(fX, fY, mp_iDenCutCodes[i][iSp],
					&iTemp);
			iTemp += 1;
			mp_oResultsGrid->SetValueAtPoint(fX, fY, mp_iDenCutCodes[i][iSp],
					iTemp);

			if (0 == fThisBA)
				fThisBA = clModelMath::CalculateBasalArea(fDbh);

			mp_oResultsGrid->GetValueAtPoint(fX, fY, mp_iBaCutCodes[i][iSp],
					&fTemp);
			fTemp += fThisBA;
			mp_oResultsGrid->SetValueAtPoint(fX, fY, mp_iBaCutCodes[i][iSp],
					fTemp);
...
int clDisturbance::AssembleCutArea(clPackage *p_oMasterPackage,
	...
	int
			...
			iNumCellsInCutArea = 0; //number of cells in the cut area
		...
		for (j = 0; j < iNumYCells; j++) {
			...
			while (p_oMasterPackage) {
				...
				if (p_cutArea != NULL) {
					...
					p_cellRecord->iTreeX = (int) floor(
							(i * m_fDistXCellLen) / m_fPopCellLen);
					p_cellRecord->iTreeY = (int) floor(
							(j * m_fDistYCellLen) / m_fPopCellLen);
					p_cellRecord->p_oTree = NULL;
					p_cellRecord->fDbh = 0;
					p_cellRecord->next = p_cutArea;
					p_cutArea = p_cellRecord;
				} else {
					//Provision for first cut in linked list
					p_cutArea = new stcGridList;
					p_cutArea->iDistX = i;
					p_cutArea->iDistY = j;
					p_cutArea->iTreeX = (int) floor(
							(i * m_fDistXCellLen) / m_fPopCellLen);
					p_cutArea->iTreeY = (int) floor(
							(j * m_fDistYCellLen) / m_fPopCellLen);
					p_cutArea->next = NULL;
					p_cutArea->p_oTree = NULL;
					p_cutArea->fDbh = 0;
				}
...
void clDisturbance::GetBasalArea(stcGridList *p_cutArea,
	...
	stcGridList *p_cellRecord = NULL; //grid cell
	...
	while (p_cellRecord) {
		...
		while (p_oTree) {
			...
			if (iSpecies == iTreeSpecies
					&& IsTreeInCell(p_oTree, p_cellRecord->iDistX,
							p_cellRecord->iDistY)) {
				iTreeType = p_oTree->GetType();
				if (clTreePopulation::seedling != iTreeType
						&& clTreePopulation::snag != iTreeType) {
					p_oTree->GetValue(
							mp_oPop->GetDbhCode(iTreeSpecies, iTreeType),
							&fDbh);
					for (i = 0; i < m_iNumAllowedCutRanges; i++)
						if (fDbh >= p_fLoDbh[i] && fDbh <= p_fHiDbh[i])
							p_fTotalBasalArea[i] +=
									clModelMath::CalculateBasalArea(fDbh);
				}
...
clTree* clDisturbance::GetTallestTreeInCutArea(stcGridList *&p_cutArea,
	...
	stcGridList *p_sortedCutArea = NULL, //where to put the sorted list
	...
	while (p_cellRecord) {
		...
		while (p_oTree) {
			if (p_oTree->GetSpecies() == iSpecies) {
				...
				iDeadCode = mp_oPop->GetIntDataCode("dead",
	...
	//the linked list
	p_cutArea = p_sortedCutArea;
	return (p_cutArea->p_oTree);
}
...
clTree* clDisturbance::GetNextTreeInCutArea(stcGridList *&p_cutArea,
	...
	//of that species
	p_oTree = p_cutArea->p_oTree->GetShorter();
	while (p_oTree) {
		if (p_oTree->GetSpecies() == iSpecies) {
			//See if this tree already died in another SORTIE disturbance
			...
			if (iTemp == notdead && p_oTree->GetType() != clTreePopulation::snag
					&& IsTreeInCell(p_oTree, p_cutArea->iDistX,
							p_cutArea->iDistY)) {
				if (clTreePopulation::seedling == p_oTree->GetType()) {
					//No more trees to bother with here - null the pointer and break
					//the loop
	...
	if (p_cutArea->next && fDbh < p_cutArea->next->fDbh) { //yep - needs to move
		p_sorter = p_cutArea->next;
		p_cutArea = p_sorter;
		while (p_sorter) {
			//Does our record come last?
			if (NULL == p_sorter->next) {
...
// IsTreeInCell()
//////////////////////////////////////////////////////////////////////////////
bool clDisturbance::IsTreeInCell(clTree *p_oTree, int iX, int iY) {
	float fX, fY;
	...
	if ((int) floor(fX / m_fDistXCellLen) == iX
			&& (int) floor(fY / m_fDistYCellLen) == iY) {
		return true;
	} else {
		return false;
	}
...
// KillSeedlings()
//////////////////////////////////////////////////////////////////////////////
void clDisturbance::KillSeedlings(stcGridList *p_cutArea, float *p_fKillProb) {

	clTree *p_oTree, *p_oNextTree;
	stcGridList *p_cellRecord = NULL; //grid cell
	float fMaxSeedlingHeight = 0, fHeight = 0;
	int iNumSpecies = mp_oPop->GetNumberOfSpecies(), iSp, iTp, iTemp;
	...
	for (iSp = 0; iSp < iNumSpecies; iSp++) {
		...
		fMaxSeedlingHeight =
				fHeight > fMaxSeedlingHeight ? fHeight : fMaxSeedlingHeight;
	}
	...
	while (p_cellRecord) {
		...
		while (p_oTree) {
			...
			if (clTreePopulation::seedling == iTp && p_fKillProb[iSp] > 0
					&& IsTreeInCell(p_oTree, p_cellRecord->iDistX,
							p_cellRecord->iDistY)) {

				if (clModelMath::GetRand() <= p_fKillProb[iSp]) {
					mp_oPop->KillTree(p_oTree, m_iReasonCode);
					mp_oResultsGrid->GetValueOfCell(p_cellRecord->iDistX,
							p_cellRecord->iDistY, mp_iSeedlingCutCodes[iSp],
							&iTemp);
					iTemp++;
					mp_oResultsGrid->SetValueOfCell(p_cellRecord->iDistX,
							p_cellRecord->iDistY, mp_iSeedlingCutCodes[iSp],
							iTemp);
				}
