...
void clNCIMasterQuadratGrowth::DoShellSetup(xercesc::DOMDocument *p_oDoc) {
	...
	FillSpeciesSpecificValue(p_oElement, "gr_nciMaxPotentialGrowth",
			"gr_nmpgVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
	...
	for (i = 0; i < m_iNumBehaviorSpecies; i++) {
		...
		if (mp_fMaxPotentialValue[mp_iWhatSpecies[i]] <= 0) {
			...
			err.sMoreInfo =
					"All values for max potential growth must be greater than 0.";
			throw(err);
		}
	}
...
float clNCIMasterQuadratGrowth::CalcDiameterGrowthValue(clTree *p_oTree,
	...
	p_oTree->GetValue(
			p_oPop->GetXCode(p_oTree->GetSpecies(), p_oTree->GetType()), &fX);
	p_oTree->GetValue(
			p_oPop->GetYCode(p_oTree->GetSpecies(), p_oTree->GetType()), &fY);
	mp_oGrid->GetValueAtPoint(fX, fY,
			mp_iGridGrowthCodes[p_oTree->GetSpecies()], &fGrowth);
...
void clNCIMasterQuadratGrowth::PreGrowthCalcs(clTreePopulation *p_oPop) {
	float *p_fTempEffect, *p_fPrecipEffect, *p_fNEffect;
	try {

		clPlot *p_oPlot = mp_oSimManager->GetPlotObject();
		clNCITermBase::ncivals nci;
		float fCrowdingEffect, //tree's crowding effect
				fX, fY, fGrowth, fInfectionEffect, //tree's infection effect
				fNumberYearsPerTimestep =
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			p_fPrecipEffect[mp_iWhatSpecies[i]] =
					mp_oPrecipEffect->CalculatePrecipitationEffect(p_oPlot,
							mp_iWhatSpecies[i]);
			p_fTempEffect[mp_iWhatSpecies[i]] =
					mp_oTempEffect->CalculateTemperatureEffect(p_oPlot,
							mp_iWhatSpecies[i]);
			p_fNEffect[mp_iWhatSpecies[i]] =
					mp_oNEffect->CalculateNitrogenEffect(p_oPlot,
							mp_iWhatSpecies[i]);
		}
		...
		for (iX = 0; iX < iNumXCells; iX++) {
			for (iY = 0; iY < iNumYCells; iY++) {
				...
				for (iSp = 0; iSp < m_iNumBehaviorSpecies; iSp++) {
					...
					//Get NCI
					nci = mp_oNCITerm->CalculateNCITerm(NULL, p_oPop, p_oPlot,
							fX, fY, iSpecies);

					//Get the infection effect
					fInfectionEffect =
							mp_oInfectionEffect->CalculateInfectionEffect(NULL);

					//Get the crowding effect
					fCrowdingEffect =
							mp_oCrowdingEffect->CalculateCrowdingEffect(NULL, 0,
									nci, iSpecies);

					//Calculate actual growth in cm/yr
					fGrowth = mp_fMaxPotentialValue[iSpecies] * fCrowdingEffect
							* p_fPrecipEffect[iSpecies]
							* p_fTempEffect[iSpecies] * p_fNEffect[iSpecies]
							* fInfectionEffect;
					...
					mp_oGrid->SetValueOfCell(iX, iY,
							mp_iGridGrowthCodes[iSpecies], fGrowth);
				}
			}
		}
...
// DeterministicAdjust()
/////////////////////////////////////////////////////////////////////////////
float clNCIMasterQuadratGrowth::DeterministicAdjust(float fNumber,
		int iSpecies) {
	return fNumber;
}
...
void clNCIMasterQuadratGrowth::SetupGrid(clTreePopulation *p_oPop) {
	...
	if (NULL == mp_oGrid) {
		...
		mp_oGrid = mp_oSimManager->CreateGrid("NCI Quadrat Growth", 0, //number of ints
