...
void clInsectInfestationMortality::DoShellSetup(xercesc::DOMDocument *p_oDoc) {
	...
	try {
		...
		//Intercept
		FillSpeciesSpecificValue(p_oElement, "mo_insectMortIntercept",
				"mo_imiVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			if (0 > p_fTempValues[i].val || 1 < p_fTempValues[i].val) {
				...
				stcErr.sMoreInfo =
						"All values for mortality intercept must be between 0 and 1.";
				throw(stcErr);
			}
		...
		FillSpeciesSpecificValue(p_oElement, "mo_insectMortMaxRate",
				"mo_immrVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer values to our permanent array, making sure all values are
		//between 0 and 1
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			if (0 > p_fTempValues[i].val || 1 < p_fTempValues[i].val) {
				...
				stcErr.sFunction = "clInsectInfestationMortality::DoShellSetup";
				stcErr.sMoreInfo =
						"All values for max mortality must be between 0 and 1.";
				throw(stcErr);
			}
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			if (0 == p_fTempValues[i].val) {
				...
				stcErr.sFunction = "clInsectInfestationMortality::DoShellSetup";
				stcErr.sMoreInfo =
						"Values for insect mortality X0 cannot be 0.";
				throw(stcErr);
			}
		...
		for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
			mp_iDataCodes[mp_whatSpeciesTypeCombos[i].iSpecies][mp_whatSpeciesTypeCombos[i].iType] =
		...
		for (i = 0; i < m_iNumSpecies; i++) {
			...
			for (j = 0; j < m_iMaxMortTime; j++)
				mp_fMortProbs[i][j] = 0;
		}

		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			for (j = 1; j < m_iMaxMortTime; j++) {
				mp_fMortProbs[mp_iWhatSpecies[i]][j] =
						mp_fIntercept[mp_iWhatSpecies[i]]
								+ ((mp_fMax[mp_iWhatSpecies[i]]
										- mp_fIntercept[mp_iWhatSpecies[i]])
										/ (1
												+ pow(
														j
																/ mp_fX0[mp_iWhatSpecies[i]],
														mp_fXb[mp_iWhatSpecies[i]])));
			}
		}
	} catch (modelErr &err) {
		delete[] p_fTempValues;
	...
	if (iYrsInfested < m_iMaxMortTime) {
		...
		fProb = mp_fIntercept[iSpecies]
				+ ((mp_fMax[iSpecies] - mp_fIntercept[iSpecies])
						/ (1
								+ pow(iYrsInfested / mp_fX0[iSpecies],
										mp_fXb[iSpecies])));
	}
