...
// ReadParameterFile()
////////////////////////////////////////////////////////////////////////////
void clClimateCompDepNeighborhoodSurvival::ReadParameterFile(
	...
	try {
		...
		//A
		FillSpeciesSpecificValue(p_oElement, "mo_climCompDepNeighA",
				"mo_ccdnaVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		...
		//B
		FillSpeciesSpecificValue(p_oElement, "mo_climCompDepNeighB",
				"mo_ccdnbVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		//M
		FillSpeciesSpecificValue(p_oElement, "mo_climCompDepNeighM",
				"mo_ccdnmVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		//N
		FillSpeciesSpecificValue(p_oElement, "mo_climCompDepNeighN",
				"mo_ccdnnVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		//C
		FillSpeciesSpecificValue(p_oElement, "mo_climCompDepNeighC",
				"mo_ccdncVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		//D
		FillSpeciesSpecificValue(p_oElement, "mo_climCompDepNeighD",
				"mo_ccdndVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		//Max crowding radius
		FillSingleValue(p_oElement, "mo_climCompDepNeighRadius", &m_fRadius,
				true);
		...
		stcErr.sFunction =
				"clClimateCompDepNeighborhoodSurvival::ReadParameterFile";
		throw(stcErr);
	}
}
...
// DoShellSetup()
////////////////////////////////////////////////////////////////////////////
void clClimateCompDepNeighborhoodSurvival::DoShellSetup(
...
// DoMort
////////////////////////////////////////////////////////////////////////////
deadCode clClimateCompDepNeighborhoodSurvival::DoMort(clTree *p_oTree,
		const float &fDbh, const short int &iSpecies) {
	clTreePopulation *p_oPop =
			(clTreePopulation*) mp_oSimManager->GetPopulationObject(
					"treepopulation");
	float fX, fY, fNumberYearsPerTimestep =
			mp_oSimManager->GetNumberOfYearsPerTimestep(), fSurvivalProb, //Tree's annual survival probability
			fBAT; //Neighborhood basal area of adult trees
	short int iType = p_oTree->GetType(), iX, iY;
	bool bIsDead;
	...
	mp_oGrid->GetValueAtPoint(fX, fY, mp_iGridSurvivalCodes[iSpecies],
			&fSurvivalProb);
	if (-1 == fSurvivalProb) {
		...
		mp_oGrid->SetValueOfCell(iX, iY, mp_iGridSurvivalCodes[iSpecies],
				fSurvivalProb);
	}
...
// GetBAT
////////////////////////////////////////////////////////////////////////////
float clClimateCompDepNeighborhoodSurvival::GetBAT(float &fX, float &fY,
		clTreePopulation *p_oPop) {
...
// PreMortCalcs
////////////////////////////////////////////////////////////////////////////
void clClimateCompDepNeighborhoodSurvival::PreMortCalcs(
		clTreePopulation *p_oPop) {
	clPlot *p_oPlot = mp_oSimManager->GetPlotObject();
	float fPlotTemp = p_oPlot->GetMeanAnnualTemp(), //Current plot temperature
	fWD = p_oPlot->GetWaterDeficit(), fValue;
	short int i, ind, iX, iY, iNumTotalSpecies = p_oPop->GetNumberOfSpecies(),
			iNumXCells = mp_oGrid->GetNumberXCells(), iNumYCells =
					mp_oGrid->GetNumberYCells();

	for (i = 0; i < m_iNumBehaviorSpecies; i++) {
		ind = mp_iWhatSpecies[i];
		mp_fTempFunction[ind] = exp(
				-0.5 * pow((fPlotTemp - mp_fM[ind]) / mp_fN[ind], 2));
		mp_fWDFunction[ind] = exp(
				-0.5 * pow((fWD - mp_fC[ind]) / mp_fD[ind], 2));
	}
	...
	for (iX = 0; iX < iNumXCells; iX++)
		for (iY = 0; iY < iNumYCells; iY++) {
			...
			for (i = 0; i < iNumTotalSpecies; i++)
				mp_oGrid->SetValueOfCell(iX, iY, mp_iGridSurvivalCodes[i],
						fValue);
		}
	...
	if (NULL == mp_oGrid) {
		...
		for (i = 0; i < iNumTotalSpecies; i++) {
			...
			if (-1 == mp_iGridSurvivalCodes[i]) {
				modelErr stcErr;
				stcErr.sFunction =
						"clClimateCompDepNeighborhoodSurvival::SetupGrid";
				std::stringstream s;
			...
			if (-1 == m_iBATCode) {
				...
				stcErr.sFunction =
						"clClimateCompDepNeighborhoodSurvival::SetupGrid";
				stcErr.sMoreInfo =
						"Couldn't find the \"BAT\" member of the \"Climate Comp Dep Neighborhood Survival\" grid.";
				stcErr.iErrorCode = BAD_DATA;
				throw(stcErr);
			}
		}
