...
clGrowthOrg::~clGrowthOrg() {
	...
	if (mp_iSizeClassCodes)
		for (i = 0; i < m_iTotalSpecies; i++)
			delete[] mp_iSizeClassCodes[i];

	delete[] mp_fMaxTreeHeight;
	mp_fMaxTreeHeight = NULL;

	delete[] mp_oDiameterGrowthTable;
	mp_oDiameterGrowthTable = NULL;
	delete[] mp_oHeightGrowthTable;
	mp_oHeightGrowthTable = NULL;
	...
	mp_iHeightCodes = NULL;
	delete[] mp_iSizeClassCodes;
	mp_iSizeClassCodes = NULL;
	delete[] mp_oShellList;
	mp_oShellList = NULL;
}
...
void clGrowthOrg::DoSetup(clSimManager *p_oSimManager,
	...
	try {
		...
		//Get the return codes for the "size class" data member variable for each
		//species/type combo that has a shell pointer in the functions table
		GetSizeClassVariableCodes();
		...
		for (i = 0; i < iNumBehaviors; i++) {
			...
			if (std::string::npos != sBehaviorName.find(sShellMarker)) {
				...
				if (clGrowthBase::diameter_auto == iGrowthType
						|| clGrowthBase::diameter_only == iGrowthType) {
					...
					for (j = 0; j < iNumCombos; j++) {
						...
						if (mp_oDiameterGrowthTable[combo.iSpecies][combo.iType]
								!= NULL) {
							modelErr stcErr;
							stcErr.sFunction =
									"clGrowthOrg::PopulateGrowthTables";
							std::stringstream s;
							s << "Type/species combo species=" << combo.iSpecies
									<< " type=" << combo.iType
									<< " has more than one growth object assigned.";
							stcErr.sMoreInfo = s.str();
							stcErr.iErrorCode = DATA_READ_ONLY;
							throw(stcErr);
						} else
							mp_oDiameterGrowthTable[combo.iSpecies][combo.iType] =
									p_oGrowthShell;
					}
					...
					for (j = 0; j < iNumCombos; j++) {
						...
						//Make sure another growth shell hasn't already claimed this combo -
						//if it has, throw an error.  If not, assign this pointer
						if (mp_oHeightGrowthTable[combo.iSpecies][combo.iType]
								!= NULL) {
							modelErr stcErr;
							stcErr.sFunction =
									"clGrowthOrg::PopulateGrowthTables";
							...
							mp_oHeightGrowthTable[combo.iSpecies][combo.iType] =
									p_oGrowthShell;
			...
			for (j = 0; j < m_iTotalTypes; j++) {
				...
				if (( NULL == mp_oDiameterGrowthTable[i][j]
						...
						|| ( NULL != mp_oDiameterGrowthTable[i][j]
								&& clGrowthBase::diameter_only
										== mp_oDiameterGrowthTable[i][j]->GetGrowthMethod()
								&& NULL == mp_oHeightGrowthTable[i][j])) {
...
void clGrowthOrg::GetLightVariableCodes() {
	try {
		...
		for (i = 0; i < m_iTotalSpecies; i++)
			for (j = 0; j < m_iTotalTypes; j++)
				if ( NULL != mp_oDiameterGrowthTable[i][j]) {
					mp_iLightCodes[i][j] = mp_oPop->GetFloatDataCode(cLabel, i,
							j);
				}
	} catch (modelErr &err) {
		throw(err);
			...
			for (j = 0; j < m_iTotalTypes; j++)
				if ( NULL != mp_oHeightGrowthTable[i][j]) {
					...
					if (-1 == mp_iHeightCodes[i][j]) {
						modelErr stcErr;
						stcErr.sFunction =
								"clGrowthOrg::GetHeightVariableCodes";
						std::stringstream s;
						s << "Type/species combo species=" << i << " type=" << j
...
// GetSizeClassVariableCodes()
//////////////////////////////////////////////////////////////////////////////
void clGrowthOrg::GetSizeClassVariableCodes() {
	try {
		short int i, j; //loop counters

		//Declare and initialize the return codes array
		mp_iSizeClassCodes = new short int*[m_iTotalSpecies];
		for (i = 0; i < m_iTotalSpecies; i++) {
			mp_iSizeClassCodes[i] = new short int[m_iTotalTypes];
			for (j = 0; j < m_iTotalTypes; j++)
				mp_iSizeClassCodes[i][j] = -1;
		}
		...
		for (i = 0; i < m_iTotalSpecies; i++)
			for (j = 0; j < m_iTotalTypes; j++)
				if ( NULL != mp_oHeightGrowthTable[i][j]) {
					mp_iSizeClassCodes[i][j] = mp_oPop->GetSizeClassCode(i, j);

					//If the return code is -1, throw an error
					if (-1 == mp_iSizeClassCodes[i][j]) {
						modelErr stcErr;
						stcErr.sFunction =
								"clGrowthOrg::GetSizeClassVariableCodes";
						std::stringstream s;
						s << "Type/species combo species=" << i << " type=" << j
								<< " does not have size class as expected.";
						stcErr.sMoreInfo = s.str();
						stcErr.iErrorCode = BAD_DATA;
						throw(stcErr);
					}
		...
		modelErr stcErr;
		stcErr.iErrorCode = UNKNOWN;
		stcErr.sFunction = "clGrowthOrg::GetSizeClassVariableCodes";
		throw(stcErr);
	}
}
...
void clGrowthOrg::DoGrowthAssignments() {
	try {
		...
		int iDead, iSizeClassVal; //size class value to be assigned to the tree
		...
		while (p_oTree) {
			...
			if ( NULL != mp_oDiameterGrowthTable[iSp][p_oTree->GetType()]) {
				...
				iDeadCode = mp_oPop->GetIntDataCode("dead", iSp,
						p_oTree->GetType());
				if (-1 != iDeadCode) {
				...
				iGrowthType =
						mp_oDiameterGrowthTable[iSp][p_oTree->GetType()]->GetGrowthMethod();
				...
				if (clGrowthBase::diameter_only == iGrowthType
					...
					if (mp_oHeightGrowthTable[iSp][p_oTree->GetType()]->m_sNameString
							== "seedlingsheightgrowthshell") {
						// the growth increment is an integer because it uses size class instead of actual amount of growth
						iSizeClassVal =
								mp_oHeightGrowthTable[iSp][p_oTree->GetType()]->CalcSizeClassGrowthValue(
										p_oTree, mp_oPop, 0);
						if (iSizeClassVal == 0)
							fHeightGrowthVal = 0.01;
						else if (iSizeClassVal == 1)
							fHeightGrowthVal = 0.1;
						else if (iSizeClassVal == 2)
							fHeightGrowthVal = 0.3;
						else if (iSizeClassVal == 3)
							fHeightGrowthVal = 0.95;
						else if (iSizeClassVal > 3)
							// changed to be high enought to become a sapling
							fHeightGrowthVal = 1.41;
						//fHeightGrowthVal = 0;
						//fDiamGrowthVal = 0;
						fDiamGrowthVal =
								mp_oDiameterGrowthTable[iSp][p_oTree->GetType()]->CalcDiameterGrowthValue(
										p_oTree, mp_oPop, fHeightGrowthVal);
					} else {
						fHeightGrowthVal =
								mp_oHeightGrowthTable[iSp][p_oTree->GetType()]->CalcHeightGrowthValue(
										p_oTree, mp_oPop, 0);
						fDiamGrowthVal =
								mp_oDiameterGrowthTable[iSp][p_oTree->GetType()]->CalcDiameterGrowthValue(
										p_oTree, mp_oPop, fHeightGrowthVal);
					...
					if (clGrowthBase::diameter_only == iGrowthType) {
						fHeightGrowthVal =
								mp_oHeightGrowthTable[iSp][p_oTree->GetType()]->CalcHeightGrowthValue(
										p_oTree, mp_oPop, fDiamGrowthVal);
					}
				}

				//Get the new "Growth" value, if appropriate
				fGrowthMemberVal =
						mp_oDiameterGrowthTable[iSp][p_oTree->GetType()]->GetGrowthMemberValue(
								p_oTree, fDiamGrowthVal);

				//Add the values to the tree.  Delay tree population updates until all trees are done
				p_oTree->GetValue(mp_iDiamCodes[iSp][p_oTree->GetType()],
						&fOldDiam);
				fDiamGrowthVal += fOldDiam;

				// check in case it's a seedling and it is using the size class growth
				...
				if (clGrowthBase::diameter_auto == iGrowthType) {
					...
					p_oTree->SetValue(mp_iDiamCodes[iSp][p_oTree->GetType()],
							fDiamGrowthVal, false, true);
					...
					if (p_oTree->GetType() == clTreePopulation::seedling) {
						if (mp_oHeightGrowthTable[iSp][p_oTree->GetType()]->m_sNameString
							...
							iSizeClassVal =
									mp_oHeightGrowthTable[iSp][p_oTree->GetType()]->CalcSizeClassGrowthValue(
											p_oTree, mp_oPop, 0);
							p_oTree->SetValue(
									mp_iSizeClassCodes[iSp][p_oTree->GetType()],
									iSizeClassVal, false, false);
							...
							if (iSizeClassVal == 0)
								...
								fHeightGrowthVal = 1.41;
							p_oTree->SetValue(
									mp_iHeightCodes[iSp][p_oTree->GetType()],
									fHeightGrowthVal, false, false);
						}
					} else {
						p_oTree->GetValue(
								mp_iHeightCodes[iSp][p_oTree->GetType()],
						...
						if (fHeightGrowthVal > mp_fMaxTreeHeight[iSp])
							fHeightGrowthVal = mp_fMaxTreeHeight[iSp];
						p_oTree->SetValue(
								mp_iHeightCodes[iSp][p_oTree->GetType()],
								fHeightGrowthVal, false, false);
					}
				...
				if (mp_iGrowthCodes[iSp][p_oTree->GetType()] > -1) {
					p_oTree->SetValue(mp_iGrowthCodes[iSp][p_oTree->GetType()],
							fGrowthMemberVal);
				}
			}
...
void clGrowthOrg::DoTreeDataMemberRegistrations(clSimManager *p_oSimManager,
	...
	try {
		char cLabel[] = "Growth";
		short int i, j, //loop counters
				iTotalSpecies = p_oPop->GetNumberOfSpecies(), iTotalTypes =
						p_oPop->GetNumberOfTypes();
