...
void clStochasticGapGrowth::GetData(xercesc::DOMDocument *p_oDoc) {
	...
	try {
		...
		if (!mp_oGapLight) {
			...
			stcErr.sMoreInfo =
					"You must use the \"Gap Light\" behavior with \"Stochastic Gap Growth\".";
			throw(stcErr);
		}
		...
		if (-1 == m_iIsGapCode) {
			...
			stcErr.sMoreInfo =
					"Gap Light grid was incorrectly set up.  Missing bool \"Is Gap\".";
			throw(stcErr);
		}
		...
		for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
			mp_bAppliedTo[mp_whatSpeciesTypeCombos[i].iSpecies][mp_whatSpeciesTypeCombos[i].iType] =
					true;
		}
...
void clStochasticGapGrowth::Action() {
	try {
		...
		float fX, fY;
		int iNumXCells = mp_oGrowthCounter->GetNumberXCells(), iNumYCells =
				mp_oGrowthCounter->GetNumberYCells(), iSp, iTp, iX, iY, iVal,
				iZero = 0, iTarget, iDeadCode, //dead code for a tree
				iIsDead;
		bool bIsGap;
		...
		for (iX = 0; iX < iNumXCells; iX++) {
			for (iY = 0; iY < iNumYCells; iY++) {
				mp_oGrowthCounter->SetValueOfCell(iX, iY, m_iCurrentCode,
						iZero);
		...
		for (iSp = 0; iSp < m_iNumSpecies; iSp++) {
			p_fAdultDiam10[iSp] = p_oAllom->ConvertDbhToDiam10(
					p_oPop->GetMinAdultDBH(iSp), iSp);
		}
		...
		while (p_oTree) {
			...
			if (mp_bAppliedTo[iSp][iTp]) {
				...
				if (bIsGap && notdead == iIsDead) {
					...
					mp_oGrowthCounter->GetValueAtPoint(fX, fY, m_iCurrentCode,
							&iVal);
					iVal++;
					mp_oGrowthCounter->SetValueAtPoint(fX, fY, m_iCurrentCode,
							iVal);
				}
			}
			p_oTree = p_oTrees->NextTree();
		...
		for (iX = 0; iX < iNumXCells; iX++) {
			for (iY = 0; iY < iNumYCells; iY++) {
				mp_oGrowthCounter->GetValueOfCell(iX, iY, m_iCurrentCode,
						&iVal);
				if (iVal > 0) {
					iVal = clModelMath::RandomRound(
							clModelMath::GetRand() * iVal);
					if (iVal == 0)
						iVal = 1;
					mp_oGrowthCounter->SetValueOfCell(iX, iY, m_iCurrentCode,
							iZero);
					mp_oGrowthCounter->SetValueOfCell(iX, iY, m_iTargetCode,
							iVal);
				}
			}
		}
		...
		while (p_oTree) {
			...
			if (mp_bAppliedTo[iSp][iTp]) {
				...
				mp_oGrowthCounter->GetValueAtPoint(fX, fY, m_iTargetCode,
						&iTarget);
				...
				if (iTarget > 0) {
					...
					if (iTarget != iVal) {
						...
						mp_oGrowthCounter->SetValueAtPoint(fX, fY,
								m_iCurrentCode, iVal);
					} else {
						...
						if (clTreePopulation::seedling == iTp) {
							p_oTree->SetValue(p_oPop->GetDiam10Code(iSp, iTp),
									p_fAdultDiam10[iSp], false, true);
						}
						//Otherwise set DBH
						else {
							p_oTree->SetValue(p_oPop->GetDbhCode(iSp, iTp),
									p_oPop->GetMinAdultDBH(iSp), false, true);
						}
