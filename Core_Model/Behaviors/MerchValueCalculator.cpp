...
void clMerchValueCalculator::GetParameterFileData(xercesc::DOMDocument *p_oDoc,
	...
	try {
		...
		FillSpeciesSpecificValue(p_oElement, "an_merchValuePricePer1KFeet",
				"an_mvpp1kfVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		...
		//Form classes
		FillSpeciesSpecificValue(p_oElement, "an_merchValueFormClasses",
				"an_mvfcVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets, and
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			...
			if (78 == iFormClass || 79 == iFormClass || 80 == iFormClass
					|| 81 == iFormClass || 84 == iFormClass || 85 == iFormClass)
				mp_fFormClass[p_fTempValues[i].code] = iFormClass;
			else {
				modelErr stcErr;
				stcErr.sFunction =
						"clMerchValueCalculator::GetParameterFileData";
				std::stringstream s;
...
void clMerchValueCalculator::SetupGrid() {
	...
	mp_oValueGrid = mp_oSimManager->CreateGrid("Merchantable Timber Value", 0, //number of ints
...
float clMerchValueCalculator::GetTreeValue(const float &fDBH,
	...
	int iNumLogs = GetNumLogs(fDBH, iSpecies) - 1, iDBHIndex, iTemp;
	...
	try {
		...
		while (p_oTree) {
			...
			if (-1 != mp_iMerchValCodes[iSp][iTp - clTreePopulation::sapling]) {
				...
				//Set its value
				p_oTree->SetValue(
						mp_iMerchValCodes[iSp][iTp - clTreePopulation::sapling],
						fValue);
...
void clMerchValueCalculator::RegisterTreeDataMembers() {
	...
	for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
		...
		if (clTreePopulation::adult != mp_whatSpeciesTypeCombos[i].iType
			...
			modelErr stcErr;
			stcErr.sFunction =
					"clMerchValueCalculator::RegisterTreeDataMembers";
			stcErr.sMoreInfo =
		...
		mp_iMerchValCodes[mp_whatSpeciesTypeCombos[i].iSpecies][mp_whatSpeciesTypeCombos[i].iType
				- clTreePopulation::sapling] = p_oPop->RegisterFloat(
				"Merch Val", mp_whatSpeciesTypeCombos[i].iSpecies,
				mp_whatSpeciesTypeCombos[i].iType);
	}
}
