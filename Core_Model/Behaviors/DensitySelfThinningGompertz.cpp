...
void clDensitySelfThinningGompertz::DoShellSetup(xercesc::DOMDocument *p_oDoc) {
	...
	try {
		...
		FillSpeciesSpecificValue(p_oElement, "mo_gompertzSelfThinningMinHeight",
				"mo_gstmhVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets and check that they are positive
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			mp_fMinHeight[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;
			if (mp_fMinHeight[mp_iIndexes[p_fTempValues[i].code]] < 0) {
				modelErr stcErr;
				stcErr.iErrorCode = BAD_DATA;
				stcErr.sFunction =
						"clDensitySelfThinningGompertz::DoShellSetup";
				stcErr.sMoreInfo =
						"The Gompertz Density Self Thinning parameter minimum height must be a positive number.";
				throw(stcErr);
			}
		}
		//G
		FillSpeciesSpecificValue(p_oElement, "mo_gompertzSelfThinningG",
				"mo_gstgVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		FillSpeciesSpecificValue(p_oElement, "mo_gompertzSelfThinningH",
				"mo_gsthVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		FillSpeciesSpecificValue(p_oElement, "mo_gompertzSelfThinningI",
				"mo_gstiVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fI[mp_iIndexes[p_fTempValues[i].code]] = p_fTempValues[i].val;

		FillSingleValue(p_oElement, "mo_gompertzSelfThinningRadius", &m_fRadius,
				true);
		if (m_fRadius <= 0) {
			modelErr stcErr;
			stcErr.iErrorCode = BAD_DATA;
			...
			stcErr.sMoreInfo =
					"The Gompertz Density Self Thinning parameter search radius must be a positive number.";
			throw(stcErr);
		}
...
float clDensitySelfThinningGompertz::CalculateNeighborhoodTreeCount(
		clTree *p_oTree) {
	clTreePopulation *p_oPop =
			(clTreePopulation*) mp_oSimManager->GetPopulationObject(
					"treepopulation");
	...
	float fNeighCount = 0, fTargetX, fTargetY; //holders for the target tree's X and Y location
	...
	sQuery << "distance=" << m_fRadius << "FROM x=" << fTargetX << "y="
			<< fTargetY << "::height="
