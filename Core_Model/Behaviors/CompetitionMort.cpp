	...
	try {
		...
		for (i = 0; i < m_iNumSpeciesTypeCombos; i++)
			if (clTreePopulation::seedling
				...
				stcErr.sMoreInfo =
						"This behavior cannot be applied to seedlings";
				throw(stcErr);
			}
		...
		FillSpeciesSpecificValue(p_oElement, "mo_CompMort", "mo_cmVal",
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			mp_fCompMort[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;
			...
			if (mp_fCompMort[mp_iIndexes[p_fTempValues[i].code]] <= 0) {
				...
				stcErr.sFunction = "clCompetitionMort::DoShellSetup";
				stcErr.sMoreInfo =
						"The Competition mortality parameter mp_fCompMort must be a positive number.";
				throw(stcErr);
			}
		}

		//Competition-dependent mortality probability function parameters
		FillSpeciesSpecificValue(p_oElement, "mo_CompMortMax", "mo_cmmVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer to the appropriate array buckets and check that they are positive
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			mp_fCompMortMax[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;
			if (mp_fCompMortMax[mp_iIndexes[p_fTempValues[i].code]] <= 0) {
				...
				stcErr.sFunction = "clCompetitionMort::DoShellSetup";
				stcErr.sMoreInfo =
						"The Competition mortality parameter mp_fCompMortMax must be a positive number.";
				throw(stcErr);
			}
		}
		...
		FillSpeciesSpecificValue(p_oElement, "gr_nciMaxPotentialGrowth",
				"gr_nmpgVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fMaxPotentialGrowth[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;
		...
		for (i = 0; i < m_iNumTotalSpecies; i++)
			for (j = 0; j < iTotalTypes; j++)
				if (mp_bUsesThisMortality[i][j]) {
					mp_iGrowthCodes[i][j] = p_oPop->GetFloatDataCode(sLabel, i,
							j);
					...
					if (-1 == mp_iGrowthCodes[i][j]) {
						modelErr stcErr;
						stcErr.sFunction =
								"clCompetitionMort::GetGrowthVariableCodes";
						std::stringstream s;
...
// DoMort()
////////////////////////////////////////////////////////////////////////////
deadCode clCompetitionMort::DoMort(clTree *p_oTree, const float &fDbh,
	...
	fPotentialGrowth =
			mp_fMaxPotentialGrowth[mp_iIndexes[iSpecies]]
					* exp(
							-0.5
									* pow(
											(log(
													mp_fX0[mp_iIndexes[iSpecies]]
															/ fDbhFromBeginningOfTimestep)
													/ mp_fXb[mp_iIndexes[iSpecies]]),
											2));
	...
	if (fRelativeIncrement >= mp_fCompMortMax[mp_iIndexes[iSpecies]])
		...
		fDeathProb = pow(mp_fCompMort[mp_iIndexes[iSpecies]],
				(fRelativeIncrement / mp_fCompMortMax[mp_iIndexes[iSpecies]]));
