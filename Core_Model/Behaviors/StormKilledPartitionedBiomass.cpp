...
void clStormKilledPartitionedBiomass::SetNameData(std::string sNameString) {
	try {
		...
		if (sNameString.compare("StormKilledPartitionedDBHBiomass") == 0) {
			m_bIsDbh = true;
		} else if (sNameString.compare("StormKilledPartitionedHeightBiomass")
				== 0) {
			m_bIsDbh = false;
		} else {
			modelErr stcErr;
...
/////////////////////////////////////////////////////////////////////////////
void clStormKilledPartitionedBiomass::GetParameterFileData(
	...
	if (m_bIsDbh) {
		...
		FillSpeciesSpecificValue(p_oElement, "an_partBioDbhLeafA",
				"an_pbdlaVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				false);
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			mp_fDBHLeafA[p_fTempValues[i].code] = p_fTempValues[i].val;
		}
		FillSpeciesSpecificValue(p_oElement, "an_partBioDbhLeafB",
				"an_pbdlbVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				false);
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			mp_fDBHLeafB[p_fTempValues[i].code] = p_fTempValues[i].val;
		}
		FillSpeciesSpecificValue(p_oElement, "an_partBioDbhBranchA",
				"an_pbdbraVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				false);
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			mp_fDBHBranchA[p_fTempValues[i].code] = p_fTempValues[i].val;
		}
		FillSpeciesSpecificValue(p_oElement, "an_partBioDbhBranchB",
				"an_pbdbrbVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				false);
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			mp_fDBHBranchB[p_fTempValues[i].code] = p_fTempValues[i].val;
		}
		FillSpeciesSpecificValue(p_oElement, "an_partBioDbhBoleA",
				"an_pbdboaVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				false);
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			mp_fDBHBoleA[p_fTempValues[i].code] = p_fTempValues[i].val;
		}
		FillSpeciesSpecificValue(p_oElement, "an_partBioDbhBoleB",
				"an_pbdbobVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				false);
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
		...
		FillSpeciesSpecificValue(p_oElement, "an_partBioHeightLeafA",
				"an_pbhlaVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				false);
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			mp_fHeightLeafA[p_fTempValues[i].code] = p_fTempValues[i].val;
		}
		FillSpeciesSpecificValue(p_oElement, "an_partBioHeightLeafB",
				"an_pbhlbVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				false);
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			mp_fHeightLeafB[p_fTempValues[i].code] = p_fTempValues[i].val;
		}
		FillSpeciesSpecificValue(p_oElement, "an_partBioHeightBoleA",
				"an_pbhboaVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				false);
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			mp_fHeightBoleA[p_fTempValues[i].code] = p_fTempValues[i].val;
		}
		FillSpeciesSpecificValue(p_oElement, "an_partBioHeightBoleB",
				"an_pbhbobVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				false);
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
	...
	try {
		...
		for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
			if (mp_whatSpeciesTypeCombos[i].iType != clTreePopulation::snag) {
				...
				stcErr.iErrorCode = BAD_DATA;
				stcErr.sMoreInfo =
						"Storm Killed Partitioned Biomass can only be applied to snags.";
...
void clStormKilledPartitionedBiomass::GetTreeCodes(clTreePopulation *p_oPop) {
	...
	for (i = 0; i < m_iNumBehaviorSpecies; i++) {
		mp_iXCodes[mp_iWhatSpecies[i]] = p_oPop->GetXCode(mp_iWhatSpecies[i],
				clTreePopulation::snag);
		mp_iYCodes[mp_iWhatSpecies[i]] = p_oPop->GetYCode(mp_iWhatSpecies[i],
				clTreePopulation::snag);
		mp_iStmDmgCodes[mp_iWhatSpecies[i]] = p_oPop->GetIntDataCode("stm_dmg",
				mp_iWhatSpecies[i], clTreePopulation::snag);
		if (-1 == mp_iStmDmgCodes[mp_iWhatSpecies[i]]) {
			modelErr stcErr;
			stcErr.iErrorCode = BAD_DATA;
			stcErr.sMoreInfo =
					"All snags to which the Storm Killed Partitioned Biomass behavior are applied must also have the Storm Damage Applier applied.";
	...
	if (m_bIsDbh) {
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			mp_iDbhCodes[mp_iWhatSpecies[i]] = p_oPop->GetDbhCode(
					mp_iWhatSpecies[i], clTreePopulation::snag);
		}
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			mp_iHeightCodes[mp_iWhatSpecies[i]] = p_oPop->GetHeightCode(
					mp_iWhatSpecies[i], clTreePopulation::snag);
		}
	}
}
...
void clStormKilledPartitionedBiomass::SetupGrid(clTreePopulation *p_oPop) {
	...
	short int iNumTotalSpecies = p_oPop->GetNumberOfSpecies(), i; //loop counter
	...
	mp_oGrid = mp_oSimManager->GetGridObject(
			"Storm Killed Partitioned Biomass");

	if (NULL == mp_oGrid) {

		//Create the grid with five float data members for each species
		mp_oGrid = mp_oSimManager->CreateGrid(
				"Storm Killed Partitioned Biomass", 0,          //number of ints
...
void clStormKilledPartitionedBiomass::Action() {
	try {
		...
		float fGridValue, //value in a grid
				fDbh, fHeight, fX, fY, fValue; //tree's value
		int iNumXCells = mp_oGrid->GetNumberXCells(), iNumYCells =
				mp_oGrid->GetNumberYCells(), iNumTotalSpecies =
				p_oPop->GetNumberOfSpecies(), iStmDmg, iX, iY, iSp;
		...
		if (m_bIsDbh) {
			for (iX = 0; iX < iNumXCells; iX++) {
				for (iY = 0; iY < iNumYCells; iY++) {
					for (iSp = 0; iSp < iNumTotalSpecies; iSp++) {
						mp_oGrid->SetValueOfCell(iX, iY, mp_iLeafCodes[iSp],
								fValue);
						mp_oGrid->SetValueOfCell(iX, iY, mp_iBranchCodes[iSp],
								fValue);
						mp_oGrid->SetValueOfCell(iX, iY, mp_iBoleCodes[iSp],
								fValue);
					}
				}
			}
			...
			for (iX = 0; iX < iNumXCells; iX++) {
				for (iY = 0; iY < iNumYCells; iY++) {
					for (iSp = 0; iSp < iNumTotalSpecies; iSp++) {
						mp_oGrid->SetValueOfCell(iX, iY, mp_iHLeafCodes[iSp],
								fValue);
						mp_oGrid->SetValueOfCell(iX, iY, mp_iHBoleCodes[iSp],
								fValue);
					}
				}
			}
		...
		if (m_bIsDbh) {
			while (p_oTree) {
				...
				if (-1 < mp_iStmDmgCodes[iSp]) {
					...
					if (1000 == iStmDmg || 2000 == iStmDmg) {
						...
						if (fValue > 0) {
							mp_oGrid->GetValueAtPoint(fX, fY,
									mp_iLeafCodes[iSp], &fGridValue);
							fGridValue += fValue;
							mp_oGrid->SetValueAtPoint(fX, fY,
									mp_iLeafCodes[iSp], fGridValue);
						}

						//Calculate this tree's branch biomass and add it to the total
						fValue = (mp_fDBHBranchA[iSp] * fDbh)
								+ mp_fDBHBranchB[iSp];
						if (fValue > 0) {
							mp_oGrid->GetValueAtPoint(fX, fY,
									mp_iBranchCodes[iSp], &fGridValue);
							fGridValue += fValue;
							mp_oGrid->SetValueAtPoint(fX, fY,
									mp_iBranchCodes[iSp], fGridValue);
						}
			...
			for (iX = 0; iX < iNumXCells; iX++) {
				for (iY = 0; iY < iNumYCells; iY++) {
					for (iSp = 0; iSp < iNumTotalSpecies; iSp++) {
						mp_oGrid->GetValueOfCell(iX, iY, mp_iLeafCodes[iSp],
								&fValue);
						fValue *= CONVERT_KG_TO_MG;
						mp_oGrid->SetValueOfCell(iX, iY, mp_iLeafCodes[iSp],
								fValue);
						mp_oGrid->GetValueOfCell(iX, iY, mp_iBranchCodes[iSp],
								&fValue);
						fValue *= CONVERT_KG_TO_MG;
						mp_oGrid->SetValueOfCell(iX, iY, mp_iBranchCodes[iSp],
								fValue);
						mp_oGrid->GetValueOfCell(iX, iY, mp_iBoleCodes[iSp],
								&fValue);
						fValue *= CONVERT_KG_TO_MG;
						mp_oGrid->SetValueOfCell(iX, iY, mp_iBoleCodes[iSp],
								fValue);
			...
			while (p_oTree) {
				...
				if (-1 < mp_iStmDmgCodes[iSp]) {
					...
					if (1000 == iStmDmg || 2000 == iStmDmg) {
						...
						//Calculate this tree's leaf biomass and add it to the total
						fValue = (mp_fHeightLeafA[iSp] * fHeight)
								+ mp_fHeightLeafB[iSp];
						if (fValue > 0) {
							mp_oGrid->GetValueAtPoint(fX, fY,
									mp_iHLeafCodes[iSp], &fGridValue);
							fGridValue += fValue;
							mp_oGrid->SetValueAtPoint(fX, fY,
									mp_iHLeafCodes[iSp], fGridValue);
						}
						...
						//Calculate this tree's bole biomass and add it to the total
						fValue = (mp_fHeightBoleA[iSp] * fHeight)
								+ mp_fHeightBoleB[iSp];
						if (fValue > 0) {
							mp_oGrid->GetValueAtPoint(fX, fY,
									mp_iHBoleCodes[iSp], &fGridValue);
							fGridValue += fValue;
							mp_oGrid->SetValueAtPoint(fX, fY,
									mp_iHBoleCodes[iSp], fGridValue);
						}
			...
			for (iX = 0; iX < iNumXCells; iX++) {
				for (iY = 0; iY < iNumYCells; iY++) {
					for (iSp = 0; iSp < iNumTotalSpecies; iSp++) {
						mp_oGrid->GetValueOfCell(iX, iY, mp_iHLeafCodes[iSp],
								&fValue);
						fValue *= CONVERT_KG_TO_MG;
						mp_oGrid->SetValueOfCell(iX, iY, mp_iHLeafCodes[iSp],
								fValue);
						mp_oGrid->GetValueOfCell(iX, iY, mp_iHBoleCodes[iSp],
								&fValue);
						fValue *= CONVERT_KG_TO_MG;
						mp_oGrid->SetValueOfCell(iX, iY, mp_iHBoleCodes[iSp],
								fValue);
...
// FormatQueryString()
////////////////////////////////////////////////////////////////////////////
void clStormKilledPartitionedBiomass::FormatQueryString(
		clTreePopulation *p_oPop) {
