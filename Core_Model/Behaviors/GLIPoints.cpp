...
void clGLIPoints::GetData(xercesc::DOMDocument *p_oDoc) {
	try {
		...
		oOut << "Points file for parameter file "
				<< mp_oSimManager->GetParFilename() << "\n";
		oOut.close();
	} catch (modelErr &err) {
		throw(err);
	} catch (modelMsg &msg) {
...
void clGLIPoints::SetUpBrightnessArray() {
	...
	if ( NULL != p_oTemp) {
		...
		if (p_oGli->m_iNumAltAng == m_iNumAltAng
				...
				&& fabs(p_oGli->m_fAzimuthOfNorth - m_fAzimuthOfNorth)
						< 0.001) {
...
void clGLIPoints::ReadParameterFileData(xercesc::DOMDocument *p_oDoc) {
	...
	DOMElement *p_oElement = GetParentParametersElement(p_oDoc), *p_oPoint;
	XMLCh *sVal;
	char *cData;
	float fPlotLenX = p_oPlot->GetXPlotLength(), fPlotLenY =
	...
	if (std::string::npos == m_sFileName.find(TEXT_FILE_EXT))
		m_sFileName += TEXT_FILE_EXT;
	...
	for (j = 0; j < m_iNumPoints; j++) {
		...
		cData = XMLString::transcode(
				p_oPoint->getAttributeNode(sVal)->getNodeValue());
		...
		if (mp_pointsList[j].fX >= fPlotLenX || mp_pointsList[j].fX < 0
				|| mp_pointsList[j].fY >= fPlotLenY
				|| mp_pointsList[j].fY < 0) {
			modelErr stcErr;
			stcErr.sFunction = "clGLIPoints::ReadParameterFileData";
			std::stringstream s;
	...
	if (0 >= m_iNumAltAng) {
		...
		stcErr.sFunction = "clGLIPoints::ReadParameterFileData";
		stcErr.sMoreInfo =
				"The number of sky altitude divisions must be greater than 0.";
		throw(stcErr);
	}
	...
	if (0 >= m_iNumAziAng) {
		...
		stcErr.sFunction = "clGLIPoints::ReadParameterFileData";
		stcErr.sMoreInfo =
				"The number of sky azimuth divisions must be greater than 0.";
		throw(stcErr);
	}
...
void clGLIPoints::Action() {
	...
	try {
		...
		for (iPoint = 0; iPoint < m_iNumPoints; iPoint++) {
			...
			fMaxSearchRad = (mp_oLightOrg->GetMaxTreeHeight()
					- mp_pointsList[iPoint].fHeight) * m_fRcpTanMinAng
					+ p_oAllom->GetMaxCrownRadius();

			//Get a list of all trees that are within the search radius and taller than
			//the fish-eye photo height
			sprintf(cQuery, "%s%f%s%f%s%f%s%f", "distance=", fMaxSearchRad,
					" FROM x=", mp_pointsList[iPoint].fX, "y=",
					mp_pointsList[iPoint].fY, "::height=",
					mp_pointsList[iPoint].fHeight);
			...
			while (p_oNeighbor != NULL) {
				...
				if (clTreePopulation::seedling != p_oNeighbor->GetType())
					...
					AddTreeToGliFishEye(mp_pointsList[iPoint].fX,
							mp_pointsList[iPoint].fY,
							mp_pointsList[iPoint].fHeight, p_oNeighbor, p_oPlot,
							p_oPop, p_oAllom);
			...
			//Write the point
			out << mp_pointsList[iPoint].fX << "\t" << mp_pointsList[iPoint].fY
					<< "\t" << mp_pointsList[iPoint].fHeight << "\t" << fGli
					<< "\n";
