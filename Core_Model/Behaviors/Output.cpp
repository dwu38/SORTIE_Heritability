...
void clOutput::GetData(DOMDocument *p_oDoc) {
	...
	if (m_sFileRoot.length() > TARBALL_FILE_EXT.length()) {
		...
		if (m_sFileRoot.substr(m_sFileRoot.length() - TARBALL_FILE_EXT.length()).compare(
				TARBALL_FILE_EXT) == 0) {
...
void clOutput::ExtractLiveTreeInfo(DOMDocument *p_oDoc) {
	...
	try {
		...
		for (i = 0; i < iNumSettings; i++) {
			...
			cData = XMLString::transcode(
					p_oTreeInfo->getAttributeNode(sVal)->getNodeValue());
			XMLString::release(&sVal);
			...
			if (-1 == iSpecies) { //unrecognized species - throw error
				modelErr stcErr;
				stcErr.iErrorCode = BAD_DATA;
				stcErr.sFunction = "clOutput::ExtractLiveTreeInfo";
			...
			//Frequency data
			FillSingleValue(p_oTreeInfo, "ou_saveFreq",
					(int*) &mp_treeSettings[iSpecies][iType].iSaveFreq, true);
			...
			if (0 != p_oDataMemberList->getLength()) {
				mp_treeSettings[iSpecies][iType].iNumInts =
						p_oDataMemberList->getLength();
				//Declare the int array
				mp_treeSettings[iSpecies][iType].p_iIntCodes =
						new short int[mp_treeSettings[iSpecies][iType].iNumInts];
				for (j = 0; j < mp_treeSettings[iSpecies][iType].iNumInts;
						j++) {
					p_oDocNode = p_oDataMemberList->item(j);
					p_oDataMember = (DOMElement*) p_oDocNode;
					cData = XMLString::transcode(
							p_oDataMember->getFirstChild()->getNodeValue());
					iDataMemberCode = p_oPop->GetIntDataCode(cData, iSpecies,
							iType);
					if (-1 == iDataMemberCode) { //unrecognized data code - throw error
						modelErr stcErr;
						stcErr.iErrorCode = BAD_DATA;
						stcErr.sFunction = "clOutput::ExtractLiveTreeInfo";
						stcErr.sMoreInfo =
								"Unrecognized int data member label - ";
						stcErr.sMoreInfo += cData;
						throw(stcErr);
					}
					delete[] cData;
					cData = NULL;
					mp_treeSettings[iSpecies][iType].p_iIntCodes[j] =
							iDataMemberCode;
				}
			...
			XMLString::release(&sVal);
			if (0 != p_oDataMemberList->getLength()) {
				mp_treeSettings[iSpecies][iType].iNumFloats =
						p_oDataMemberList->getLength();
				//Declare the float array
				mp_treeSettings[iSpecies][iType].p_iFloatCodes =
						new short int[mp_treeSettings[iSpecies][iType].iNumFloats];
				for (j = 0; j < mp_treeSettings[iSpecies][iType].iNumFloats;
						j++) {
					p_oDocNode = p_oDataMemberList->item(j);
					p_oDataMember = (DOMElement*) p_oDocNode;
					cData = XMLString::transcode(
							p_oDataMember->getFirstChild()->getNodeValue());
					iDataMemberCode = p_oPop->GetFloatDataCode(cData, iSpecies,
							iType);
					if (-1 == iDataMemberCode) { //unrecognized data code - throw error
						modelErr stcErr;
						stcErr.iErrorCode = BAD_DATA;
						stcErr.sFunction = "clOutput::ExtractLiveTreeInfo";
						stcErr.sMoreInfo =
								"Unrecognized float data member label - ";
						stcErr.sMoreInfo += cData;
						throw(stcErr);
					}
					delete[] cData;
					cData = NULL;
					mp_treeSettings[iSpecies][iType].p_iFloatCodes[j] =
							iDataMemberCode;
				}
			} //end of if (0 != p_oDataMemberList->getLength())
			  //****************Chars*******************
			...
			XMLString::release(&sVal);
			if (0 != p_oDataMemberList->getLength()) {
				mp_treeSettings[iSpecies][iType].iNumStrings =
						p_oDataMemberList->getLength();
				//Declare the int array
				mp_treeSettings[iSpecies][iType].p_iStringCodes =
						new short int[mp_treeSettings[iSpecies][iType].iNumStrings];
				for (j = 0; j < mp_treeSettings[iSpecies][iType].iNumStrings;
						j++) {
					p_oDocNode = p_oDataMemberList->item(j);
					p_oDataMember = (DOMElement*) p_oDocNode;
					cData = XMLString::transcode(
							p_oDataMember->getFirstChild()->getNodeValue());
					iDataMemberCode = p_oPop->GetStringDataCode(cData, iSpecies,
							iType);
					if (-1 == iDataMemberCode) { //unrecognized data code - throw error
						modelErr stcErr;
						stcErr.iErrorCode = BAD_DATA;
						stcErr.sFunction = "clOutput::ExtractLiveTreeInfo";
						stcErr.sMoreInfo =
								"Unrecognized char data member label - ";
						stcErr.sMoreInfo += cData;
						throw(stcErr);
					}
					delete[] cData;
					cData = NULL;
					mp_treeSettings[iSpecies][iType].p_iStringCodes[j] =
							iDataMemberCode;
				}
			} //end of if (0 != p_oDataMemberList->getLength())
			  //****************Bools*******************
			...
			XMLString::release(&sVal);
			if (0 != p_oDataMemberList->getLength()) {
				mp_treeSettings[iSpecies][iType].iNumBools =
						p_oDataMemberList->getLength();
				//Declare the int array
				mp_treeSettings[iSpecies][iType].p_iBoolCodes =
						new short int[mp_treeSettings[iSpecies][iType].iNumBools];
				for (j = 0; j < mp_treeSettings[iSpecies][iType].iNumBools;
						j++) {
					p_oDocNode = p_oDataMemberList->item(j);
					p_oDataMember = (DOMElement*) p_oDocNode;
					cData = XMLString::transcode(
							p_oDataMember->getFirstChild()->getNodeValue());
					iDataMemberCode = p_oPop->GetBoolDataCode(cData, iSpecies,
							iType);
					if (-1 == iDataMemberCode) { //unrecognized data code - throw error
						modelErr stcErr;
						stcErr.iErrorCode = BAD_DATA;
						stcErr.sFunction = "clOutput::ExtractLiveTreeInfo";
						stcErr.sMoreInfo =
								"Unrecognized bool data member label - ";
						stcErr.sMoreInfo += cData;
						throw(stcErr);
					}
					delete[] cData;
					cData = NULL;
					mp_treeSettings[iSpecies][iType].p_iBoolCodes[j] =
							iDataMemberCode;
				}
			} //end of if (0 != p_oDataMemberList->getLength())
...
void clOutput::ExtractDeadTreeInfo(DOMDocument *p_oDoc) {
	...
	try {
		...
		short int iNumSettings, //how many types we have output data for in the file
				iSpecies, iType, //species number
				iDeadReason, iDataMemberCode, //data member code for a label
				i, j, k; //loop counters
		...
		for (i = 0; i < m_iNumSpecies; i++) {
			...
			for (j = 0; j < m_iNumTypes; j++) {
				mp_deadTreeSettings[i][j] = new stcTreeOutputInfo[remove_tree
						+ 1];
				for (k = 0; k <= remove_tree; k++) {
		...
		for (i = 0; i < iNumSettings; i++) {
			...
			if (strcmp("harvest", cData) == 0)
				...
				stcErr.iErrorCode = BAD_DATA;
				stcErr.sMoreInfo =
						"Unrecognized or unsupported dead reason code - ";
				stcErr.sMoreInfo += cData;
				throw(stcErr);
			}
			delete[] cData;
			...
			//Frequency data
			FillSingleValue(p_oTreeInfo, "ou_saveFreq",
					(int*) &mp_deadTreeSettings[iSpecies][iType][iDeadReason].iSaveFreq,
					true);

			//Make sure that the frequency is not negative - if so, throw error
			if (mp_deadTreeSettings[iSpecies][iType][iDeadReason].iSaveFreq
					< 0) {
				modelErr stcErr;
				stcErr.iErrorCode = BAD_DATA;
				stcErr.sFunction = "clOutput::ExtractDeadTreeInfo";
			...
			XMLString::release(&sVal);
			if (0 != p_oDataMemberList->getLength()) {
				mp_deadTreeSettings[iSpecies][iType][iDeadReason].iNumInts =
						p_oDataMemberList->getLength();
				//Declare the int array
				mp_deadTreeSettings[iSpecies][iType][iDeadReason].p_iIntCodes =
						new short int[mp_deadTreeSettings[iSpecies][iType][iDeadReason].iNumInts];
				for (j = 0;
						j
								< mp_deadTreeSettings[iSpecies][iType][iDeadReason].iNumInts;
						j++) {
					...
					delete[] cData;
					cData = NULL;
					mp_deadTreeSettings[iSpecies][iType][iDeadReason].p_iIntCodes[j] =
							iDataMemberCode;
				}
			} //end of if (0 != p_oDataMemberList->getLength())
			  //****************Floats*******************
			...
			XMLString::release(&sVal);
			if (0 != p_oDataMemberList->getLength()) {
				mp_deadTreeSettings[iSpecies][iType][iDeadReason].iNumFloats =
						p_oDataMemberList->getLength();
				//Declare the float array
				mp_deadTreeSettings[iSpecies][iType][iDeadReason].p_iFloatCodes =
						new short int[mp_deadTreeSettings[iSpecies][iType][iDeadReason].iNumFloats];
				for (j = 0;
						j
								< mp_deadTreeSettings[iSpecies][iType][iDeadReason].iNumFloats;
						j++) {
					...
					delete[] cData;
					cData = NULL;
					mp_deadTreeSettings[iSpecies][iType][iDeadReason].p_iFloatCodes[j] =
							iDataMemberCode;
				}
			} //end of if (0 != p_oDataMemberList->getLength())
			  //****************Chars*******************
			...
			XMLString::release(&sVal);
			if (0 != p_oDataMemberList->getLength()) {
				mp_deadTreeSettings[iSpecies][iType][iDeadReason].iNumStrings =
						p_oDataMemberList->getLength();
				//Declare the int array
				mp_deadTreeSettings[iSpecies][iType][iDeadReason].p_iStringCodes =
						new short int[mp_deadTreeSettings[iSpecies][iType][iDeadReason].iNumStrings];
				for (j = 0;
						j
								< mp_deadTreeSettings[iSpecies][iType][iDeadReason].iNumStrings;
						j++) {
					...
					delete[] cData;
					cData = NULL;
					mp_deadTreeSettings[iSpecies][iType][iDeadReason].p_iStringCodes[j] =
							iDataMemberCode;
				}
			} //end of if (0 != p_oDataMemberList->getLength())
			  //****************Bools*******************
			...
			XMLString::release(&sVal);
			if (0 != p_oDataMemberList->getLength()) {
				mp_deadTreeSettings[iSpecies][iType][iDeadReason].iNumBools =
						p_oDataMemberList->getLength();
				//Declare the int array
				mp_deadTreeSettings[iSpecies][iType][iDeadReason].p_iBoolCodes =
						new short int[mp_deadTreeSettings[iSpecies][iType][iDeadReason].iNumBools];
				for (j = 0;
						j
								< mp_deadTreeSettings[iSpecies][iType][iDeadReason].iNumBools;
						j++) {
					...
					delete[] cData;
					cData = NULL;
					mp_deadTreeSettings[iSpecies][iType][iDeadReason].p_iBoolCodes[j] =
							iDataMemberCode;
				}
			} //end of if (0 != p_oDataMemberList->getLength())
		} //end of for (i = 0; i < m_iNumTypeToSave; i++)
...
void clOutput::ExtractGridInfo(DOMDocument *p_oDoc) {
	...
	try {
		...
		if (0 == iNumGrids)
			return; //if nothing found exit without error
		...
		for (i = 0; i < m_iNumGridsToSave; i++) {
			...
			cData = XMLString::transcode(
					p_oGrid->getAttributeNode(sVal)->getNodeValue());
			XMLString::release(&sVal);
			mp_gridSettings[i].p_oGridPointer = mp_oSimManager->GetGridObject(
					cData);
			...
			//Get frequency
			FillSingleValue(p_oGrid, "ou_saveFreq",
					(int*) &mp_gridSettings[i].iSaveFreq, true);
			...
			if (0 != p_oDataMemberList->getLength()) {
				...
				//Declare the int array
				mp_gridSettings[i].p_iIntCodes =
						new short int[mp_gridSettings[i].iNumInts];
				for (j = 0; j < mp_gridSettings[i].iNumInts; j++) {
					...
					cData = XMLString::transcode(
							p_oDataMember->getFirstChild()->getNodeValue());
					iDataMemberCode =
							mp_gridSettings[i].p_oGridPointer->GetIntDataCode(
									cData);
					if (-1 == iDataMemberCode) { //unrecognized data code - throw error
						modelErr stcErr;
						stcErr.iErrorCode = BAD_DATA;
			...
			if (0 != p_oDataMemberList->getLength()) {
				...
				//Declare the float array
				mp_gridSettings[i].p_iFloatCodes =
						new short int[mp_gridSettings[i].iNumFloats];
				for (j = 0; j < mp_gridSettings[i].iNumFloats; j++) {
					...
					cData = XMLString::transcode(
							p_oDataMember->getFirstChild()->getNodeValue());
					iDataMemberCode =
							mp_gridSettings[i].p_oGridPointer->GetFloatDataCode(
									cData);
					if (-1 == iDataMemberCode) { //unrecognized data code - throw error
						modelErr stcErr;
						stcErr.iErrorCode = BAD_DATA;
			...
			if (0 != p_oDataMemberList->getLength()) {
				...
				mp_gridSettings[i].p_iStringCodes =
						new short int[mp_gridSettings[i].iNumStrings];
				for (j = 0; j < mp_gridSettings[i].iNumStrings; j++) {
					...
					cData = XMLString::transcode(
							p_oDataMember->getFirstChild()->getNodeValue());
					iDataMemberCode =
							mp_gridSettings[i].p_oGridPointer->GetStringDataCode(
									cData);
					if (-1 == iDataMemberCode) { //unrecognized data code - throw error
						modelErr stcErr;
						stcErr.iErrorCode = BAD_DATA;
			...
			if (0 != p_oDataMemberList->getLength()) {
				...
				mp_gridSettings[i].p_iBoolCodes =
						new short int[mp_gridSettings[i].iNumBools];
				for (j = 0; j < mp_gridSettings[i].iNumBools; j++) {
					...
					cData = XMLString::transcode(
							p_oDataMember->getFirstChild()->getNodeValue());
					iDataMemberCode =
							mp_gridSettings[i].p_oGridPointer->GetBoolDataCode(
									cData);
					if (-1 == iDataMemberCode) { //unrecognized data code - throw error
						modelErr stcErr;
						stcErr.iErrorCode = BAD_DATA;
			...
			XMLString::release(&sVal);
			if (0 != p_oDataMemberList->getLength()) {
				mp_gridSettings[i].iNumPackageInts =
						p_oDataMemberList->getLength();
				//Declare the int array
				mp_gridSettings[i].p_iPackageIntCodes =
						new short int[mp_gridSettings[i].iNumPackageInts];
				for (j = 0; j < mp_gridSettings[i].iNumPackageInts; j++) {
					...
					cData = XMLString::transcode(
							p_oDataMember->getFirstChild()->getNodeValue());
					iDataMemberCode =
							mp_gridSettings[i].p_oGridPointer->GetPackageIntDataCode(
									cData);
					if (-1 == iDataMemberCode) { //unrecognized data code - throw error
						modelErr stcErr;
						stcErr.iErrorCode = BAD_DATA;
						stcErr.sFunction = "clOutput::ExtractGridInfo";
						stcErr.sMoreInfo =
								"Unrecognized package int data member label - ";
						stcErr.sMoreInfo += cData;
						throw(stcErr);
					}
			...
			XMLString::release(&sVal);
			if (0 != p_oDataMemberList->getLength()) {
				mp_gridSettings[i].iNumPackageFloats =
						p_oDataMemberList->getLength();
				//Declare the float array
				mp_gridSettings[i].p_iPackageFloatCodes =
						new short int[mp_gridSettings[i].iNumPackageFloats];
				for (j = 0; j < mp_gridSettings[i].iNumPackageFloats; j++) {
					...
					cData = XMLString::transcode(
							p_oDataMember->getFirstChild()->getNodeValue());
					iDataMemberCode =
							mp_gridSettings[i].p_oGridPointer->GetPackageFloatDataCode(
									cData);
					if (-1 == iDataMemberCode) { //unrecognized data code - throw error
						modelErr stcErr;
						stcErr.iErrorCode = BAD_DATA;
						stcErr.sFunction = "clOutput::ExtractGridInfo";
						stcErr.sMoreInfo =
								"Unrecognized package float data member label - ";
						stcErr.sMoreInfo += cData;
						throw(stcErr);
					}
					mp_gridSettings[i].p_iPackageFloatCodes[j] =
							iDataMemberCode;
					delete[] cData;
					cData = NULL;
				}
			...
			XMLString::release(&sVal);
			if (0 != p_oDataMemberList->getLength()) {
				mp_gridSettings[i].iNumPackageStrings =
						p_oDataMemberList->getLength();
				//Declare the char array
				mp_gridSettings[i].p_iPackageStringCodes =
						new short int[mp_gridSettings[i].iNumPackageStrings];
				for (j = 0; j < mp_gridSettings[i].iNumPackageStrings; j++) {
					...
					cData = XMLString::transcode(
							p_oDataMember->getFirstChild()->getNodeValue());
					iDataMemberCode =
							mp_gridSettings[i].p_oGridPointer->GetPackageStringDataCode(
									cData);
					if (-1 == iDataMemberCode) { //unrecognized data code - throw error
						modelErr stcErr;
						stcErr.iErrorCode = BAD_DATA;
						stcErr.sFunction = "clOutput::ExtractGridInfo";
						stcErr.sMoreInfo =
								"Unrecognized package char data member label - ";
						stcErr.sMoreInfo += cData;
						throw(stcErr);
					}
					mp_gridSettings[i].p_iPackageStringCodes[j] =
							iDataMemberCode;
					delete[] cData;
					cData = NULL;
				}
			...
			XMLString::release(&sVal);
			if (0 != p_oDataMemberList->getLength()) {
				mp_gridSettings[i].iNumPackageBools =
						p_oDataMemberList->getLength();
				//Declare the bool array
				mp_gridSettings[i].p_iPackageBoolCodes =
						new short int[mp_gridSettings[i].iNumPackageBools];
				for (j = 0; j < mp_gridSettings[i].iNumPackageBools; j++) {
					...
					cData = XMLString::transcode(
							p_oDataMember->getFirstChild()->getNodeValue());
					iDataMemberCode =
							mp_gridSettings[i].p_oGridPointer->GetPackageBoolDataCode(
									cData);
					if (-1 == iDataMemberCode) { //unrecognized data code - throw error
						modelErr stcErr;
						stcErr.iErrorCode = BAD_DATA;
						stcErr.sFunction = "clOutput::ExtractGridInfo";
						stcErr.sMoreInfo =
								"Unrecognized package bool data member label - ";
						stcErr.sMoreInfo += cData;
						throw(stcErr);
					}
...
void clOutput::ExtractSubplotInfo(xercesc::DOMDocument *p_oDoc) {
	try {
		...
		if (fTemp > 0) {
			...
			m_iNumXCells = (int) ceil(
					mp_oSimManager->GetPlotObject()->GetXPlotLength()
							/ m_fXCellLength);
		}
		fTemp = -1;
		...
		if (fTemp > 0) {
			...
			m_iNumYCells = (int) ceil(
					mp_oSimManager->GetPlotObject()->GetYPlotLength()
							/ m_fYCellLength);
		}
		...
		for (i = 0; i < m_iNumSubplotsToSave; i++) {
			...
			//Get subplot name
			FillSingleValue(p_oSubplot, "ou_subplotName",
					&mp_subplots[i].sSubplotName, true);

			//Get grid coords
			p_oGridCellList = p_oSubplot->getElementsByTagName(
					XMLString::transcode("pointSet"));
			...
			for (j = 0; j < iNumCells; j++) {
				...
				cData = XMLString::transcode(
						p_oElement->getAttributeNode(sVal)->getNodeValue());
				iX = atoi(cData);
				XMLString::release(&sVal);
				delete[] cData;
				cData = NULL;
				...
				if (iX >= m_iNumXCells || iX < 0 || iY >= m_iNumYCells
						|| iY < 0) {
					modelErr stcErr;
					stcErr.sFunction = "clOutput::ExtractSubplotInfo";
					std::stringstream s;
...
void clOutput::WriteDetailedOutputHeader() {
	...
	string *p_sFilename = NULL, sTemp, sPath = mp_oSimManager->GetAppPath();
	...
	int iNumRead, //for testing for end-of-file
			iStrLen, iNumOutFiles = m_iNumSubplotsToSave + 1, iPos, i;
	...
	try {
		...
		AssembleFileCode(detailed_output, DETAILED_OUTPUT_FILE_VERSION,
				cFileCode);

		//Write the detailed output opening tag
		sprintf(cBuf, "%s%s%s",
				"<?xml version=\"1.0\" encoding=\"UTF-8\"?><rundata fileCode=\"",
				cFileCode, "\">");
		...
		while (iNumRead == BUFFER_SIZE - 1) { //this is our end-of-file test
		...
		for (i = 1; i < iNumOutFiles; i++) {
			...
			AddFileToNewTarball(mp_subplots[i - 1].sSubplotTarball,
					p_sFilename[i], sPath);
...
void clOutput::Action() {
	try {
		...
		string *p_sTimestepFilename, sPath = mp_oSimManager->GetAppPath();
		//Get the filename of the detailed output timestep file
		int iTimestep = mp_oSimManager->GetCurrentTimestep(), iNumOutputFiles =
				m_iNumSubplotsToSave + 1, i;
		...
		for (i = 1; i < iNumOutputFiles; i++) {
			...
			AddFileToTarball(mp_subplots[i - 1].sSubplotTarball,
					p_sTimestepFilename[i], sPath);
...
void clOutput::WriteTreeData(string *p_sFilename) {
	clTreePopulation *p_oTrees =
			(clTreePopulation*) mp_oSimManager->GetPopulationObject(
					"treepopulation"); //pointer to tree population
	clGhostTreePopulation *p_oGhosts =
			(clGhostTreePopulation*) mp_oSimManager->GetPopulationObject(
					"GhostTreePopulation");
	...
	iX, iY, //tree grid cell
			iNumOutFiles = m_iNumSubplotsToSave + 1, iSp, iTp, iBuf, iCd, j; //loop counters
	bool bTemp;
			...
			for (iTp = 0; iTp < m_iNumTypes; iTp++) {
				if (mp_treeSettings[iSp][iTp].iSaveFreq > -1) {
					...
					if (0 == iTimestep || iNumTimesteps == iTimestep
						...
						mp_treeSettings[iSp][iTp].bSaveThisTimeStep = false;
				}
					...
					if (mp_deadTreeSettings[iSp][iTp][iCd].iSaveFreq > -1) {
						...
						if (0 == iTimestep || iNumTimesteps == iTimestep
								|| (mp_deadTreeSettings[iSp][iTp][iCd].iSaveFreq
										== mp_deadTreeSettings[iSp][iTp][iCd].iSumTimestep)) {
							mp_deadTreeSettings[iSp][iTp][iCd].bSaveThisTimeStep =
									true;
							bTemp = true;
							//Reset the counter
				...
				for (iTp = 0; iTp < m_iNumTypes; iTp++) {
					if (mp_masterTreeSettings[iSp][iTp].bSaveThisTimeStep) {
						...
						sprintf(cTemp, "%s%s%s%d%s", "<tm_treeSettings sp=\"",
								...
								"\" tp=\"", iTp, "\">");
						AddToBuffer(p_cBuf[iBuf], cTemp, p_out[iBuf],
								BUFFER_SIZE);
						...
						if (mp_masterTreeSettings[iSp][iTp].iNumInts > 0) {
							...
							AddToBuffer(p_cBuf[iBuf], cTemp, p_out[iBuf],
									BUFFER_SIZE);
							for (j = 0;
									j < mp_masterTreeSettings[iSp][iTp].iNumInts;
									j++) {
								sprintf(cTemp, "%s%s%s%d%s",
										"<tm_intCode label=\"",
										p_oTrees->GetIntDataLabel(
												mp_masterTreeSettings[iSp][iTp].p_iIntCodes[j],
												iSp, iTp).c_str(), "\">",
										mp_masterTreeSettings[iSp][iTp].p_iIntCodes[j],
										"</tm_intCode>");
								AddToBuffer(p_cBuf[iBuf], cTemp, p_out[iBuf],
						...
						if (mp_masterTreeSettings[iSp][iTp].iNumFloats > 0) { //floats
							...
							for (j = 0;
									j
											< mp_masterTreeSettings[iSp][iTp].iNumFloats;
									j++) {
								sprintf(cTemp, "%s%s%s%d%s",
										"<tm_floatCode label=\"",
										p_oTrees->GetFloatDataLabel(
												mp_masterTreeSettings[iSp][iTp].p_iFloatCodes[j],
												iSp, iTp).c_str(), "\">",
										mp_masterTreeSettings[iSp][iTp].p_iFloatCodes[j],
										"</tm_floatCode>");
								AddToBuffer(p_cBuf[iBuf], cTemp, p_out[iBuf],
						...
						if (mp_masterTreeSettings[iSp][iTp].iNumStrings > 0) { //chars
							...
							for (j = 0;
									j
											< mp_masterTreeSettings[iSp][iTp].iNumStrings;
									j++) {
								sprintf(cTemp, "%s%s%s%d%s",
										"<tm_charCode label=\"",
										p_oTrees->GetStringDataLabel(
												mp_masterTreeSettings[iSp][iTp].p_iStringCodes[j],
												iSp, iTp).c_str(), "\">",
										mp_masterTreeSettings[iSp][iTp].p_iStringCodes[j],
										"</tm_charCode>");
								AddToBuffer(p_cBuf[iBuf], cTemp, p_out[iBuf],
						...
						if (mp_masterTreeSettings[iSp][iTp].iNumBools > 0) { //bools
							...
							for (j = 0;
									j
											< mp_masterTreeSettings[iSp][iTp].iNumBools;
									j++) {
								sprintf(cTemp, "%s%s%s%d%s",
										"<tm_boolCode label=\"",
										p_oTrees->GetBoolDataLabel(
												mp_masterTreeSettings[iSp][iTp].p_iBoolCodes[j],
												iSp, iTp).c_str(), "\">",
										mp_masterTreeSettings[iSp][iTp].p_iBoolCodes[j],
										"</tm_boolCode>");
								AddToBuffer(p_cBuf[iBuf], cTemp, p_out[iBuf],
				...
				if (mp_treeSettings[iSp][iTp].bSaveThisTimeStep) {
					...
					for (iBuf = 1; iBuf < iNumOutFiles; iBuf++) {
						if (mp_subplots[iBuf - 1].p_bUseCell[iX][iY]) {
							WriteTree(p_oTree, p_cBuf[iBuf], cTemp,
									p_out[iBuf]);
						}
					}
				}
...
void clOutput::WriteTimestepHeader(string sFilename) {
	try {
		...
		//Timestep
		sprintf(cTemp, "%s%d%s", "<rt_timestep>",
				mp_oSimManager->GetCurrentTimestep(), "</rt_timestep>");
		fwrite(cTemp, strlen(cTemp), 1, out);
...
void clOutput::WriteGridData(string sFilename) {
	try {
		...
		for (i = 0; i < m_iNumGridsToSave; i++) {
			...
			if (0 == iTimestep || iNumTimesteps == iTimestep
					|| (mp_gridSettings[i].iSaveFreq
							== mp_gridSettings[i].iSumTimestep)) {
				//Reset counter
				mp_gridSettings[i].iSumTimestep = 0;

				//Figure out if there are any package data members to save
				if (mp_gridSettings[i].iNumPackageInts > 0
						|| mp_gridSettings[i].iNumPackageFloats > 0
						|| mp_gridSettings[i].iNumPackageStrings > 0
						|| mp_gridSettings[i].iNumPackageBools > 0)
					bSavePackages = true;
				...
				//Open tag for grid
				sprintf(cTemp, "%s%s%s", "<grid gridName=\"",
						mp_gridSettings[i].p_oGridPointer->GetName().c_str(),
						"\">");
				...
				//Ints
				iNumDataMembers =
						mp_gridSettings[i].p_oGridPointer->GetNumberIntDataMembers();
				if (iNumDataMembers > 0) {
					strcpy(cTemp, "<ma_intCodes>");
					AddToBuffer(cBuf, cTemp, out, BUFFER_SIZE);

					for (j = 0; j < iNumDataMembers; j++) {
						sprintf(cTemp, "%s%s%s%d%s", "<ma_intCode label=\"",
								mp_gridSettings[i].p_oGridPointer->GetIntDataLabel(
										j).c_str(), "\">", j, "</ma_intCode>");
						AddToBuffer(cBuf, cTemp, out, BUFFER_SIZE);
					}
				...
				//floats
				iNumDataMembers =
						mp_gridSettings[i].p_oGridPointer->GetNumberFloatDataMembers();
				if (iNumDataMembers > 0) {
					strcpy(cTemp, "<ma_floatCodes>");
					AddToBuffer(cBuf, cTemp, out, BUFFER_SIZE);

					for (j = 0; j < iNumDataMembers; j++) {
						sprintf(cTemp, "%s%s%s%d%s", "<ma_floatCode label=\"",
								mp_gridSettings[i].p_oGridPointer->GetFloatDataLabel(
										j).c_str(), "\">", j,
								"</ma_floatCode>");
						AddToBuffer(cBuf, cTemp, out, BUFFER_SIZE);
					}
				...
				//chars
				iNumDataMembers =
						mp_gridSettings[i].p_oGridPointer->GetNumberStringDataMembers();
				if (iNumDataMembers > 0) {
					strcpy(cTemp, "<ma_charCodes>");
					AddToBuffer(cBuf, cTemp, out, BUFFER_SIZE);

					for (j = 0; j < iNumDataMembers; j++) {
						sprintf(cTemp, "%s%s%s%d%s", "<ma_charCode label=\"",
								mp_gridSettings[i].p_oGridPointer->GetStringDataLabel(
										j).c_str(), "\">", j, "</ma_charCode>");
						AddToBuffer(cBuf, cTemp, out, BUFFER_SIZE);
					}
				...
				//bools
				iNumDataMembers =
						mp_gridSettings[i].p_oGridPointer->GetNumberBoolDataMembers();
				if (iNumDataMembers > 0) {
					strcpy(cTemp, "<ma_boolCodes>");
					AddToBuffer(cBuf, cTemp, out, BUFFER_SIZE);

					for (j = 0; j < iNumDataMembers; j++) {
						sprintf(cTemp, "%s%s%s%d%s", "<ma_boolCode label=\"",
								mp_gridSettings[i].p_oGridPointer->GetBoolDataLabel(
										j).c_str(), "\">", j, "</ma_boolCode>");
						AddToBuffer(cBuf, cTemp, out, BUFFER_SIZE);
					}
				...
				if (mp_gridSettings[i].p_oGridPointer->GetPackageDataChanged()) {
					//Ints
					iNumDataMembers =
							mp_gridSettings[i].p_oGridPointer->GetNumberIntPackageDataMembers();
					if (iNumDataMembers > 0) {
						...
						for (j = 0; j < iNumDataMembers; j++) {
							sprintf(cTemp, "%s%s%s%d%s", "<ma_intCode label=\"",
									mp_gridSettings[i].p_oGridPointer->GetPackageIntDataLabel(
											j).c_str(), "\">", j,
									"</ma_intCode>");
							AddToBuffer(cBuf, cTemp, out, BUFFER_SIZE);
						}
					...
					//Floats
					iNumDataMembers =
							mp_gridSettings[i].p_oGridPointer->GetNumberFloatPackageDataMembers();
					if (iNumDataMembers > 0) {
						...
						for (j = 0; j < iNumDataMembers; j++) {
							sprintf(cTemp, "%s%s%s%d%s",
									"<ma_floatCode label=\"",
									mp_gridSettings[i].p_oGridPointer->GetPackageFloatDataLabel(
											j).c_str(), "\">", j,
									"</ma_floatCode>");
							AddToBuffer(cBuf, cTemp, out, BUFFER_SIZE);
						}
					...
					//Strings
					iNumDataMembers =
							mp_gridSettings[i].p_oGridPointer->GetNumberStringPackageDataMembers();
					if (iNumDataMembers > 0) {
						...
						for (j = 0; j < iNumDataMembers; j++) {
							sprintf(cTemp, "%s%s%s%d%s",
									"<ma_charCode label=\"",
									mp_gridSettings[i].p_oGridPointer->GetPackageStringDataLabel(
											j).c_str(), "\">", j,
									"</ma_charCode>");
							AddToBuffer(cBuf, cTemp, out, BUFFER_SIZE);
						}
					...
					//Bools
					iNumDataMembers =
							mp_gridSettings[i].p_oGridPointer->GetNumberBoolPackageDataMembers();
					if (iNumDataMembers > 0) {
						...
						for (j = 0; j < iNumDataMembers; j++) {
							sprintf(cTemp, "%s%s%s%d%s",
									"<ma_boolCode label=\"",
									mp_gridSettings[i].p_oGridPointer->GetPackageBoolDataLabel(
											j).c_str(), "\">", j,
									"</ma_boolCode>");
							AddToBuffer(cBuf, cTemp, out, BUFFER_SIZE);
						}
				...
				//Plot size values
				sprintf(cTemp, "%s%g%s%g%s", "<ma_plotLenX>", fPlotLenX,
						"</ma_plotLenX><ma_plotLenY>", fPlotLenY,
						"</ma_plotLenY>");
				AddToBuffer(cBuf, cTemp, out, BUFFER_SIZE);

				//Length of grids
				sprintf(cTemp, "%s%g%s%g%s", "<ma_lengthXCells>",
						mp_gridSettings[i].p_oGridPointer->GetLengthXCells(),
						"</ma_lengthXCells><ma_lengthYCells>",
						mp_gridSettings[i].p_oGridPointer->GetLengthYCells(),
						"</ma_lengthYCells>");
				AddToBuffer(cBuf, cTemp, out, BUFFER_SIZE);
				...
				if (mp_gridSettings[i].iNumInts == 0
						&& mp_gridSettings[i].iNumFloats == 0
						&& mp_gridSettings[i].iNumStrings == 0
						&& mp_gridSettings[i].iNumBools == 0
						&& mp_gridSettings[i].iNumPackageInts == 0
				...
				iNumXCells =
						mp_gridSettings[i].p_oGridPointer->GetNumberXCells();
				iNumYCells =
						mp_gridSettings[i].p_oGridPointer->GetNumberYCells();
				for (iX = 0; iX < iNumXCells; iX++)
					for (iY = 0; iY < iNumYCells; iY++) {

						//opening grid tag
						sprintf(cTemp, "%s%d%s%d%s", "<ma_v x=\"", iX,
								"\" y=\"", iY, "\">");
						AddToBuffer(cBuf, cTemp, out, BUFFER_SIZE);
						...
						for (j = 0; j < mp_gridSettings[i].iNumInts; j++) {
							...
							mp_gridSettings[i].p_oGridPointer->GetValueOfCell(
									iX, iY, mp_gridSettings[i].p_iIntCodes[j],
									&iTemp);

							sprintf(cTemp, "%s%d%s%d%s", "<int c=\"",
									mp_gridSettings[i].p_iIntCodes[j], "\">",
									iTemp, "</int>");
						...
						for (j = 0; j < mp_gridSettings[i].iNumFloats; j++) {
							...
							mp_gridSettings[i].p_oGridPointer->GetValueOfCell(
									iX, iY, mp_gridSettings[i].p_iFloatCodes[j],
									&fTemp);

							sprintf(cTemp, "%s%d%s%g%s", "<fl c=\"",
									mp_gridSettings[i].p_iFloatCodes[j], "\">",
									fTemp, "</fl>");
						...
						for (j = 0; j < mp_gridSettings[i].iNumStrings; j++) {
							...
							mp_gridSettings[i].p_oGridPointer->GetValueOfCell(
									iX, iY,
									mp_gridSettings[i].p_iStringCodes[j],
									&sTemp);

							if (sTemp.length() > 0) {
								sprintf(cTemp, "%s%d%s%s%s", "<ch c=\"",
										mp_gridSettings[i].p_iStringCodes[j],
										"\">", sTemp.c_str(), "</ch>");
						...
						for (j = 0; j < mp_gridSettings[i].iNumBools; j++) {
							...
							mp_gridSettings[i].p_oGridPointer->GetValueOfCell(
									iX, iY, mp_gridSettings[i].p_iBoolCodes[j],
									&bTemp);

							if (true == bTemp)
								sprintf(cTemp, "%s%d%s", "<bl c=\"",
										mp_gridSettings[i].p_iBoolCodes[j],
										"\">true</bl>");
						...
						if (bSavePackages) {
							p_oPackage =
									mp_gridSettings[i].p_oGridPointer->GetFirstPackageOfCell(
											iX, iY);
							while (p_oPackage) {
								...
								for (j = 0;
										j < mp_gridSettings[i].iNumPackageInts;
										j++) {

									p_oPackage->GetValue(
											mp_gridSettings[i].p_iPackageIntCodes[j],
											&iTemp);

									sprintf(cTemp, "%s%d%s%d%s", "<pint c=\"",
											mp_gridSettings[i].p_iPackageIntCodes[j],
											"\">", iTemp, "</pint>");
								...
								for (j = 0;
										j < mp_gridSettings[i].iNumPackageFloats;
										j++) {
								...
								for (j = 0;
									...
									if (sTemp.length() > 0) {
										...
										sprintf(cTemp, "%s%d%s%s%s",
												"<pch c=\"",
												mp_gridSettings[i].p_iPackageStringCodes[j],
												"\">", sTemp.c_str(), "</pch>");

										AddToBuffer(cBuf, cTemp, out,
												BUFFER_SIZE);
									}
								}
								//Write bool values for this package
								for (j = 0;
										j < mp_gridSettings[i].iNumPackageBools;
										j++) {

									p_oPackage->GetValue(
											mp_gridSettings[i].p_iPackageBoolCodes[j],
											&bTemp);

									if (true == bTemp)
										sprintf(cTemp, "%s%d%s", "<pbl c=\"",
												mp_gridSettings[i].p_iPackageBoolCodes[j],
												"\">true</pbl>");
									else
										sprintf(cTemp, "%s%d%s", "<pbl c=\"",
...
string clOutput::GetTimestepFilename(int iTimestep, int iSubplot) {
	...
	if (-1 == iSubplot) {
		...
		return m_sFileRoot + "_" + mp_subplots[iSubplot].sSubplotName + "_"
				+ cTS + DETAILED_OUTPUT_FILE_EXT;
	} else {
		//Error
		modelErr stcErr;
		stcErr.iErrorCode = BAD_DATA;
	...
	for (iSp = 0; iSp < m_iNumSpecies; iSp++) {
		...
		for (iTp = 0; iTp < m_iNumTypes; iTp++) {
			...
			for (i = 0; i < mp_treeSettings[iSp][iTp].iNumInts; i++) {
				iMaxCode =
						(iMaxCode < mp_treeSettings[iSp][iTp].p_iIntCodes[i]) ?
								mp_treeSettings[iSp][iTp].p_iIntCodes[i] :
								iMaxCode;
			}
			for (iCd = 0; iCd <= remove_tree; iCd++) {
				for (i = 0; i < mp_deadTreeSettings[iSp][iTp][iCd].iNumInts;
						i++) {
					iMaxCode =
							(iMaxCode
									< mp_deadTreeSettings[iSp][iTp][iCd].p_iIntCodes[i]) ?
									mp_deadTreeSettings[iSp][iTp][iCd].p_iIntCodes[i] :
									iMaxCode;
				}
			}
			...
			if (iMaxCode > -1) {
				...
				for (iCd = 0; iCd <= remove_tree; iCd++) {
					for (i = 0; i < mp_deadTreeSettings[iSp][iTp][iCd].iNumInts;
							i++) {
						p_bUsed[mp_deadTreeSettings[iSp][iTp][iCd].p_iIntCodes[i]] =
								true;
					}
				}
				...
				mp_masterTreeSettings[iSp][iTp].p_iIntCodes =
						new short int[iNumCodes];
				iNumCodes = 0;
				for (i = 0; i <= iMaxCode; i++)
					if (p_bUsed[i]) {
						mp_masterTreeSettings[iSp][iTp].p_iIntCodes[iNumCodes] =
								i;
						iNumCodes++;
					}
				delete[] p_bUsed;
			...
			for (i = 0; i < mp_treeSettings[iSp][iTp].iNumFloats; i++) {
				iMaxCode =
						(iMaxCode < mp_treeSettings[iSp][iTp].p_iFloatCodes[i]) ?
								mp_treeSettings[iSp][iTp].p_iFloatCodes[i] :
								iMaxCode;
			}
			for (iCd = 0; iCd <= remove_tree; iCd++) {
				for (i = 0; i < mp_deadTreeSettings[iSp][iTp][iCd].iNumFloats;
						i++) {
					iMaxCode =
							(iMaxCode
									< mp_deadTreeSettings[iSp][iTp][iCd].p_iFloatCodes[i]) ?
									mp_deadTreeSettings[iSp][iTp][iCd].p_iFloatCodes[i] :
									iMaxCode;
				}
			}
			...
			if (iMaxCode > -1) {
				...
				for (iCd = 0; iCd <= remove_tree; iCd++) {
					for (i = 0;
							i < mp_deadTreeSettings[iSp][iTp][iCd].iNumFloats;
							i++) {
						p_bUsed[mp_deadTreeSettings[iSp][iTp][iCd].p_iFloatCodes[i]] =
								true;
					}
				}
				...
				mp_masterTreeSettings[iSp][iTp].p_iFloatCodes =
						new short int[iNumCodes];
				iNumCodes = 0;
				for (i = 0; i <= iMaxCode; i++)
					if (p_bUsed[i]) {
						mp_masterTreeSettings[iSp][iTp].p_iFloatCodes[iNumCodes] =
								i;
						iNumCodes++;
					}
				delete[] p_bUsed;
			...
			for (i = 0; i < mp_treeSettings[iSp][iTp].iNumStrings; i++) {
				iMaxCode =
						(iMaxCode < mp_treeSettings[iSp][iTp].p_iStringCodes[i]) ?
								mp_treeSettings[iSp][iTp].p_iStringCodes[i] :
								iMaxCode;
			}
			for (iCd = 0; iCd <= remove_tree; iCd++) {
				for (i = 0; i < mp_deadTreeSettings[iSp][iTp][iCd].iNumStrings;
						i++) {
					iMaxCode =
							(iMaxCode
									< mp_deadTreeSettings[iSp][iTp][iCd].p_iStringCodes[i]) ?
									mp_deadTreeSettings[iSp][iTp][iCd].p_iStringCodes[i] :
									iMaxCode;
				}
			}
			...
			if (iMaxCode > -1) {
				...
				for (iCd = 0; iCd <= remove_tree; iCd++) {
					for (i = 0;
							i < mp_deadTreeSettings[iSp][iTp][iCd].iNumStrings;
							i++) {
						p_bUsed[mp_deadTreeSettings[iSp][iTp][iCd].p_iStringCodes[i]] =
								true;
					}
				}
				...
				mp_masterTreeSettings[iSp][iTp].p_iStringCodes =
						new short int[iNumCodes];
				iNumCodes = 0;
				for (i = 0; i <= iMaxCode; i++)
					if (p_bUsed[i]) {
						mp_masterTreeSettings[iSp][iTp].p_iStringCodes[iNumCodes] =
								i;
						iNumCodes++;
					}
				delete[] p_bUsed;
			...
			for (i = 0; i < mp_treeSettings[iSp][iTp].iNumBools; i++) {
				iMaxCode =
						(iMaxCode < mp_treeSettings[iSp][iTp].p_iBoolCodes[i]) ?
								mp_treeSettings[iSp][iTp].p_iBoolCodes[i] :
								iMaxCode;
			}
			for (iCd = 0; iCd <= remove_tree; iCd++) {
				for (i = 0; i < mp_deadTreeSettings[iSp][iTp][iCd].iNumBools;
						i++) {
					iMaxCode =
							(iMaxCode
									< mp_deadTreeSettings[iSp][iTp][iCd].p_iBoolCodes[i]) ?
									mp_deadTreeSettings[iSp][iTp][iCd].p_iBoolCodes[i] :
									iMaxCode;
				}
			}
			...
			if (iMaxCode > -1) {
				...
				for (iCd = 0; iCd <= remove_tree; iCd++) {
					for (i = 0;
							i < mp_deadTreeSettings[iSp][iTp][iCd].iNumBools;
							i++) {
						p_bUsed[mp_deadTreeSettings[iSp][iTp][iCd].p_iBoolCodes[i]] =
								true;
					}
				}
				...
				mp_masterTreeSettings[iSp][iTp].p_iBoolCodes =
						new short int[iNumCodes];
				iNumCodes = 0;
				for (i = 0; i <= iMaxCode; i++)
					if (p_bUsed[i]) {
						mp_masterTreeSettings[iSp][iTp].p_iBoolCodes[iNumCodes] =
								i;
						iNumCodes++;
					}
				delete[] p_bUsed;
...
void clOutput::WriteTree(clTree *p_oTree, char *cBuf, char *cTemp, FILE *oOut) {
	...
	int iTemp, j, iSp = p_oTree->GetSpecies(), iTp = p_oTree->GetType();
	...
	for (j = 0; j < mp_treeSettings[iSp][iTp].iNumInts; j++) {
		...
		sprintf(cTemp, "%s%d%s%d%s", "<int c=\"",
				mp_treeSettings[iSp][iTp].p_iIntCodes[j], "\">", iTemp,
				"</int>");
		AddToBuffer(cBuf, cTemp, oOut, BUFFER_SIZE);
	}
	...
	for (j = 0; j < mp_treeSettings[iSp][iTp].iNumFloats; j++) {
		...
		sprintf(cTemp, "%s%d%s%g%s", "<fl c=\"",
				mp_treeSettings[iSp][iTp].p_iFloatCodes[j], "\">", fTemp,
				"</fl>");
		AddToBuffer(cBuf, cTemp, oOut, BUFFER_SIZE);
	}
	//Write char values for this tree
	for (j = 0; j < mp_treeSettings[iSp][iTp].iNumStrings; j++) {
		p_oTree->GetValue(mp_treeSettings[iSp][iTp].p_iStringCodes[j],
				&sTempTreeVal);
		if (sTempTreeVal.length() != 0) {
			sprintf(cTemp, "%s%d%s%s%s", "<ch c=\"",
					mp_treeSettings[iSp][iTp].p_iStringCodes[j], "\">",
	...
	for (j = 0; j < mp_treeSettings[iSp][iTp].iNumBools; j++) {
		...
		if (true == bTemp)
			sprintf(cTemp, "%s%d%s", "<bl c=\"",
					mp_treeSettings[iSp][iTp].p_iBoolCodes[j], "\">true</bl>");
		else
			sprintf(cTemp, "%s%d%s", "<bl c=\"",
					mp_treeSettings[iSp][iTp].p_iBoolCodes[j], "\">false</bl>");
...
// WriteGhost()
/////////////////////////////////////////////////////////////////////////////
void clOutput::WriteGhost(clDeadTree *p_oTree, char *cBuf, char *cTemp,
		FILE *oOut) {

	std::string sTempTreeVal;
	float fTemp;
	bool bTemp; //for getting bool values
	int iTemp, j, iSp = p_oTree->GetSpecies(), iTp = p_oTree->GetType(), iRs =
			p_oTree->GetDeadReasonCode();
	...
	for (j = 0; j < mp_deadTreeSettings[iSp][iTp][iRs].iNumInts; j++) {
		p_oTree->GetValue(mp_deadTreeSettings[iSp][iTp][iRs].p_iIntCodes[j],
				&iTemp);
		sprintf(cTemp, "%s%d%s%d%s", "<gint c=\"",
				mp_deadTreeSettings[iSp][iTp][iRs].p_iIntCodes[j], "\">", iTemp,
				"</gint>");
		AddToBuffer(cBuf, cTemp, oOut, BUFFER_SIZE);
	}
	//Write float values for this tree
	for (j = 0; j < mp_deadTreeSettings[iSp][iTp][iRs].iNumFloats; j++) {
		p_oTree->GetValue(mp_deadTreeSettings[iSp][iTp][iRs].p_iFloatCodes[j],
				&fTemp);

		sprintf(cTemp, "%s%d%s%g%s", "<gfl c=\"",
				mp_deadTreeSettings[iSp][iTp][iRs].p_iFloatCodes[j], "\">",
				fTemp, "</gfl>");
		AddToBuffer(cBuf, cTemp, oOut, BUFFER_SIZE);
	}
	//Write char values for this tree
	for (j = 0; j < mp_deadTreeSettings[iSp][iTp][iRs].iNumStrings; j++) {
		p_oTree->GetValue(mp_deadTreeSettings[iSp][iTp][iRs].p_iStringCodes[j],
				&sTempTreeVal);
		if (sTempTreeVal.length() != 0) {
			sprintf(cTemp, "%s%d%s%s%s", "<gch c=\"",
					mp_deadTreeSettings[iSp][iTp][iRs].p_iStringCodes[j], "\">",
	...
	for (j = 0; j < mp_deadTreeSettings[iSp][iTp][iRs].iNumBools; j++) {
		p_oTree->GetValue(mp_deadTreeSettings[iSp][iTp][iRs].p_iBoolCodes[j],
				&bTemp);
		if (true == bTemp)
			sprintf(cTemp, "%s%d%s", "<gbl c=\"",
					mp_deadTreeSettings[iSp][iTp][iRs].p_iBoolCodes[j],
					"\">true</gbl>");
		else
			sprintf(cTemp, "%s%d%s", "<gbl c=\"",
					mp_deadTreeSettings[iSp][iTp][iRs].p_iBoolCodes[j],
