...
void clFoliarChemistry::GetParameterFileData(xercesc::DOMDocument *p_oDoc,
	...
	FillSpeciesSpecificValue(p_oElement, "an_foliarChemWeightA", "an_fcwaVal",
			p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
	for (i = 0; i < m_iNumBehaviorSpecies; i++) {
		mp_fA[p_fTempValues[i].code] = p_fTempValues[i].val;
	}
	FillSpeciesSpecificValue(p_oElement, "an_foliarChemWeightB", "an_fcwbVal",
			p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
	for (i = 0; i < m_iNumBehaviorSpecies; i++) {
		mp_fB[p_fTempValues[i].code] = p_fTempValues[i].val;
	}
	FillSpeciesSpecificValue(p_oElement, "an_foliarChemN", "an_fcnVal",
			p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
	for (i = 0; i < m_iNumBehaviorSpecies; i++) {
		mp_fN[p_fTempValues[i].code] = p_fTempValues[i].val;
	}
	FillSpeciesSpecificValue(p_oElement, "an_foliarChemP", "an_fcpVal",
			p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
	for (i = 0; i < m_iNumBehaviorSpecies; i++) {
		mp_fP[p_fTempValues[i].code] = p_fTempValues[i].val;
	}
	FillSpeciesSpecificValue(p_oElement, "an_foliarChemLignin", "an_fclVal",
			p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
	for (i = 0; i < m_iNumBehaviorSpecies; i++) {
		mp_fLignin[p_fTempValues[i].code] = p_fTempValues[i].val;
	}
	FillSpeciesSpecificValue(p_oElement, "an_foliarChemFiber", "an_fcfVal",
			p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
	for (i = 0; i < m_iNumBehaviorSpecies; i++) {
		mp_fFiber[p_fTempValues[i].code] = p_fTempValues[i].val;
	}
	FillSpeciesSpecificValue(p_oElement, "an_foliarChemCellulose", "an_fccVal",
			p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
	for (i = 0; i < m_iNumBehaviorSpecies; i++) {
		mp_fCellulose[p_fTempValues[i].code] = p_fTempValues[i].val;
	}
	FillSpeciesSpecificValue(p_oElement, "an_foliarChemTannins", "an_fctVal",
			p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
	for (i = 0; i < m_iNumBehaviorSpecies; i++) {
		mp_fTannins[p_fTempValues[i].code] = p_fTempValues[i].val;
	}
	FillSpeciesSpecificValue(p_oElement, "an_foliarChemPhenolics", "an_fcphVal",
			p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
	for (i = 0; i < m_iNumBehaviorSpecies; i++) {
		mp_fPhenolics[p_fTempValues[i].code] = p_fTempValues[i].val;
	}
	FillSpeciesSpecificValue(p_oElement, "an_foliarChemSLA", "an_fcsVal",
			p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
	for (i = 0; i < m_iNumBehaviorSpecies; i++) {
...
void clFoliarChemistry::SetupGrid() {
	...
	if (NULL == mp_oGrid) {
		...
		mp_oGrid = mp_oSimManager->CreateGrid("Foliar Chemistry", 0, //number of ints
...
void clFoliarChemistry::Action() {
	try {
		...
		float fGridValue, //value in a grid
				fDbh, fX, fY, fFoliarWeight, fValue; //tree's value
		...
		for (iX = 0; iX < iNumXCells; iX++) {
			for (iY = 0; iY < iNumYCells; iY++) {
				for (iSp = 0; iSp < m_iNumTotalSpecies; iSp++) {
					...
					mp_oGrid->SetValueOfCell(iX, iY, mp_iLigninCodes[iSp],
							fValue);
					mp_oGrid->SetValueOfCell(iX, iY, mp_iFiberCodes[iSp],
							fValue);
					mp_oGrid->SetValueOfCell(iX, iY, mp_iCelluloseCodes[iSp],
							fValue);
					mp_oGrid->SetValueOfCell(iX, iY, mp_iTanninsCodes[iSp],
							fValue);
					mp_oGrid->SetValueOfCell(iX, iY, mp_iPhenolicsCodes[iSp],
							fValue);
				}
			}
		}
		...
		while (p_oTree) {
			...
			if (mp_bAppliesTo[iSp][iTp]) {
				...
				if (fValue > 0) {
					mp_oGrid->GetValueAtPoint(fX, fY, mp_iNCodes[iSp],
							&fGridValue);
					fGridValue += fValue;
					mp_oGrid->SetValueAtPoint(fX, fY, mp_iNCodes[iSp],
							fGridValue);
				}
				...
				if (fValue > 0) {
					mp_oGrid->GetValueAtPoint(fX, fY, mp_iPCodes[iSp],
							&fGridValue);
					fGridValue += fValue;
					mp_oGrid->SetValueAtPoint(fX, fY, mp_iPCodes[iSp],
							fGridValue);
				}
				...
				if (fValue > 0) {
					mp_oGrid->GetValueAtPoint(fX, fY, mp_iLigninCodes[iSp],
							&fGridValue);
					fGridValue += fValue;
					mp_oGrid->SetValueAtPoint(fX, fY, mp_iLigninCodes[iSp],
							fGridValue);
				}
				...
				if (fValue > 0) {
					mp_oGrid->GetValueAtPoint(fX, fY, mp_iFiberCodes[iSp],
							&fGridValue);
					fGridValue += fValue;
					mp_oGrid->SetValueAtPoint(fX, fY, mp_iFiberCodes[iSp],
							fGridValue);
				}
				...
				if (fValue > 0) {
					mp_oGrid->GetValueAtPoint(fX, fY, mp_iCelluloseCodes[iSp],
							&fGridValue);
					fGridValue += fValue;
					mp_oGrid->SetValueAtPoint(fX, fY, mp_iCelluloseCodes[iSp],
							fGridValue);
				}
				...
				if (fValue > 0) {
					mp_oGrid->GetValueAtPoint(fX, fY, mp_iTanninsCodes[iSp],
							&fGridValue);
					fGridValue += fValue;
					mp_oGrid->SetValueAtPoint(fX, fY, mp_iTanninsCodes[iSp],
							fGridValue);
				}
				...
				if (fValue > 0) {
					mp_oGrid->GetValueAtPoint(fX, fY, mp_iPhenolicsCodes[iSp],
							&fGridValue);
					fGridValue += fValue;
					mp_oGrid->SetValueAtPoint(fX, fY, mp_iPhenolicsCodes[iSp],
							fGridValue);
				}
				...
				if (fValue > 0) {
					mp_oGrid->GetValueAtPoint(fX, fY, mp_iSLACodes[iSp],
							&fGridValue);
					fGridValue += fValue;
					mp_oGrid->SetValueAtPoint(fX, fY, mp_iSLACodes[iSp],
							fGridValue);
				}
			}
