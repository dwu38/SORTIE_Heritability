...
void clSubstrate::GetParameterFileData(xercesc::DOMDocument *p_oDoc) {
	try {
		...
		FillSpeciesSpecificValue(p_oElement, "su_propOfDeadFall", "su_podfVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer values to our permanent array
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fPropOfDeadThatFall[p_fTempValues[i].code] =
					p_fTempValues[i].val;
		...
		FillSingleValue(p_oElement, "su_freshLogDecayAlpha", &m_fFreshLogA,
				true);
		//Fresh log decay beta
		FillSingleValue(p_oElement, "su_freshLogDecayBeta", &m_fFreshLogB,
				true);
		//Decayed log decay alpha
		FillSingleValue(p_oElement, "su_decayedLogDecayAlpha", &m_fDecayedLogA,
				true);
		//Decayed log decay beta
		FillSingleValue(p_oElement, "su_decayedLogDecayBeta", &m_fDecayedLogB,
				true);
		...
		FillSingleValue(p_oElement, "su_initialFreshLog", &m_fInitFreshLog,
				true);
		//Initial conditions - decayed logs
		FillSingleValue(p_oElement, "su_initialDecayedLog", &m_fInitDecayedLog,
				true);
		...
		if (mp_oHarvestGrid) {
			...
			FillSingleValue(p_oElement, "su_partialCutFreshLog",
					&m_fPartCutFreshLog, true);
			//Partial cut conditions - decayed logs
			FillSingleValue(p_oElement, "su_partialCutDecayedLog",
					&m_fPartCutDecayedLog, true);
			...
			FillSingleValue(p_oElement, "su_gapCutFreshLog", &m_fGapCutFreshLog,
					true);
			//Gap cut conditions - decayed logs
			FillSingleValue(p_oElement, "su_gapCutDecayedLog",
					&m_fGapCutDecayedLog, true);
			...
			FillSingleValue(p_oElement, "su_clearCutFreshLog",
					&m_fClearCutFreshLog, true);
			//Clear cut conditions - decayed logs
			FillSingleValue(p_oElement, "su_clearCutDecayedLog",
					&m_fClearCutDecayedLog, true);
		}
		...
		fTemp = 1
				- (m_fInitScarifiedSoil + m_fInitTipUp + m_fInitFreshLog
						+ m_fInitDecayedLog);
		if (fTemp < 0) {
			modelErr stcErr;
		...
		if (mp_oHarvestGrid) {
			...
			fTemp = 1
					- (m_fPartCutScarifiedSoil + m_fPartCutTipUp
							+ m_fPartCutFreshLog + m_fPartCutDecayedLog);
			if (fTemp < 0) {
				modelErr stcErr;
				stcErr.iErrorCode = BAD_DATA;
				stcErr.sFunction = "clSubstrate::GetParameterFileData";
			...
			fTemp = 1
					- (m_fGapCutScarifiedSoil + m_fGapCutTipUp
							+ m_fGapCutFreshLog + m_fGapCutDecayedLog);
			if (fTemp < 0) {
				modelErr stcErr;
				stcErr.iErrorCode = BAD_DATA;
				stcErr.sFunction = "clSubstrate::GetParameterFileData";
			...
			fTemp = 1
					- (m_fClearCutScarifiedSoil + m_fClearCutTipUp
							+ m_fClearCutFreshLog + m_fClearCutDecayedLog);
			if (fTemp < 0) {
				modelErr stcErr;
				stcErr.iErrorCode = BAD_DATA;
				stcErr.sFunction = "clSubstrate::GetParameterFileData";
...
void clSubstrate::SetupSubstrateGrids() {
	try {
		...
		if ( NULL == mp_oSubstrateGrid) {
			...
			m_iPkgFreshLogCode = mp_oSubstrateGrid->RegisterPackageFloat(
					"freshlog");
			...
			if ((fXCellLength > 0
				...
				stcErr.sMoreInfo =
						"Grid map values for grid \"Substrate\" contain cell lengths which do not match the allowed values.";
				throw(stcErr);
			}
			...
			if (-1 == m_iScarSoilCode || -1 == m_iFFMossCode
					...
					|| -1 == m_iFreshLogCode || -1 == m_iDecLogCode) {

				modelErr stcErr;
				stcErr.iErrorCode = BAD_DATA;
				stcErr.sFunction = "clSubstrate::SetupSubstrateGrid";
				stcErr.sMoreInfo =
						"Grid map setup values for grid \"Substrate\" did not contain all required grid data members.";
				throw(stcErr);
			}
			...
			if (-1 == m_iPkgAgeCode) {
				...
				m_iPkgTipupCode = mp_oSubstrateGrid->GetPackageFloatDataCode(
						"tipup");
				m_iPkgFreshLogCode = mp_oSubstrateGrid->GetPackageFloatDataCode(
						"freshlog");

				//Throw an error if any are missing
				if (-1 == m_iPkgScarSoilCode || -1 == m_iPkgTipupCode
						|| -1 == m_iPkgFreshLogCode) {
		...
		if ( NULL == mp_oCalcGrid) {
			...
			mp_oCalcGrid = mp_oSimManager->CreateGrid("substratecalcs", 0, //number of ints
			...
			for (i = 0; i < m_iMaxDecayTimesteps; i++) {
				sLabel << "freshlog_" << i;
				mp_iFLogCalcsCode[i] = mp_oCalcGrid->RegisterFloat(
						sLabel.str());
				sLabel.str("");
				sLabel << "declog_" << i;
				mp_iDecLogCalcsCode[i] = mp_oCalcGrid->RegisterFloat(
						sLabel.str());
				sLabel.str("");
			}
		} else {
			...
			if ((mp_oCalcGrid->GetLengthXCells()
				...
				stcErr.sFunction = "clSubstrate::SetupSubstrateGrid";
				stcErr.sMoreInfo =
						"Grid map values for grid \"substratecalcs\" contain cell lengths which do not match the allowed values.";
				throw(stcErr);
			}
			...
			for (i = 0; i < m_iMaxDecayTimesteps; i++) {
				sLabel << "freshlog_" << i;
				mp_iFLogCalcsCode[i] = mp_oCalcGrid->GetFloatDataCode(
						sLabel.str());
				sLabel.str("");
				...
				sLabel << "declog_" << i;
				mp_iDecLogCalcsCode[i] = mp_oCalcGrid->GetFloatDataCode(
						sLabel.str());
				sLabel.str("");
...
void clSubstrate::PopulateInitialConditions() {
	try {
		...
		float fInitForestFloor = 1
				- (m_fInitScarifiedSoil + m_fInitTipUp + m_fInitFreshLog
						+ m_fInitDecayedLog);
		short int iNumXCells = mp_oSubstrateGrid->GetNumberXCells(),
				iNumYCells = mp_oSubstrateGrid->GetNumberYCells(), i, j; //loop counters

		//Go through each grid cell and give it the proper initial conditions
		for (i = 0; i < iNumXCells; i++)
			for (j = 0; j < iNumYCells; j++) {

				mp_oSubstrateGrid->SetValueOfCell(i, j, m_iScarSoilCode,
						m_fInitScarifiedSoil);
				mp_oSubstrateGrid->SetValueOfCell(i, j, m_iFFMossCode,
						fInitForestFloor * m_fMossProportion);
				mp_oSubstrateGrid->SetValueOfCell(i, j, m_iFFLitterCode,
						fInitForestFloor * (1 - m_fMossProportion));
				mp_oSubstrateGrid->SetValueOfCell(i, j, m_iTipupCode,
						m_fInitTipUp);
				mp_oSubstrateGrid->SetValueOfCell(i, j, m_iFreshLogCode,
						m_fInitFreshLog);
				mp_oSubstrateGrid->SetValueOfCell(i, j, m_iDecLogCode,
						m_fInitDecayedLog);
...
void clSubstrate::HarvestSubstrate() {
	try {
		...
		short int iNumXCells = mp_oHarvestGrid->GetNumberXCells(), iNumYCells =
				mp_oHarvestGrid->GetNumberYCells(), iX, iY, i; //loop counters
		...
		for (iX = 0; iX < iNumXCells; iX++)
			for (iY = 0; iY < iNumYCells; iY++) {
				...
				while (p_oPackage) {
					...
					p_oPackage = mp_oSubstrateGrid->GetFirstPackageOfCell(iX,
							iY);
				}
				...
				for (i = 1; i < m_iMaxDecayTimesteps; i++) {
					mp_oCalcGrid->SetValueOfCell(iX, iY, mp_iFLogCalcsCode[i],
							fNewFreshLog);
				}
				...
				fNewDecayedLog = 0;
				for (i = 1; i < m_iMaxDecayTimesteps; i++) {
					mp_oCalcGrid->SetValueOfCell(iX, iY, mp_iDecLogCalcsCode[i],
							fNewDecayedLog);
				}

				//Get the new substrate amounts depending on the cut type
				if (clDisturbance::partial == iCutType) { //partial cut
				...
				} else if (clDisturbance::gap == iCutType) { //gap cut
				...
				mp_oSubstrateGrid->SetValueOfCell(iX, iY, m_iDecLogCode,
						fNewDecayedLog);
...
void clSubstrate::AddHarvestAndInitialNewSubstrate() {
	try {
		...
		int iCutType; //the harvest cut type that occurred in a grid cell
		short int iNumXCells = mp_oSubstrateGrid->GetNumberXCells(),
				iNumYCells = mp_oSubstrateGrid->GetNumberYCells(), iX, iY; //loop counters
		...
		if (mp_oSimManager->GetCurrentTimestep() > 1) {
			...
			for (iX = 0; iX < iNumXCells; iX++)
				for (iY = 0; iY < iNumYCells; iY++) {
					...
					mp_oCalcGrid->GetValueOfCell(iX, iY, mp_iFLogCalcsCode[0],
							&fTemp);
					mp_oCalcGrid->SetValueOfCell(iX, iY, mp_iFLogCalcsCode[0],
							fNewFreshLog + fTemp);

					mp_oCalcGrid->GetValueOfCell(iX, iY, mp_iDecLogCalcsCode[0],
							&fTemp);
					mp_oCalcGrid->SetValueOfCell(iX, iY, mp_iDecLogCalcsCode[0],
							fNewDecayedLog + fTemp);

					mp_oCalcGrid->GetValueOfCell(iX, iY, m_iTipupCalcsCode,
							&fTemp);
					mp_oCalcGrid->SetValueOfCell(iX, iY, m_iTipupCalcsCode,
							fNewTipup + fTemp);
			...
			for (iX = 0; iX < iNumXCells; iX++)
				for (iY = 0; iY < iNumYCells; iY++) {
					...
					iCutType = -1;
					if (mp_oHarvestGrid) {
						mp_oHarvestGrid->GetValueOfCell(iX, iY,
					...
					} else if (clDisturbance::partial == iCutType) { //partial cut
						fNewTipup = m_fPartCutTipUp;
						fNewFreshLog = m_fPartCutFreshLog;
						fNewDecayedLog = m_fPartCutDecayedLog;
					} else if (clDisturbance::gap == iCutType) { //gap cut
	...
	try {
		...
		while (p_oTree) {
			...
			if (!m_bUseDirectionalTreeFall) {
				mp_oCalcGrid->GetValueAtPoint(fX, fY, mp_iFLogCalcsCode[0],
						&fSubstrate);
				fSubstrate += fTreeLogArea;
				mp_oCalcGrid->SetValueAtPoint(fX, fY, mp_iFLogCalcsCode[0],
						fSubstrate);
			} else {
				...
				for (i = 0; i < iNumTrunkChunks; i++) {
					...
					mp_oCalcGrid->GetCellOfPoint(
							p_oPlot->CorrectX(
									p_oPlot->GetUncorrectedX(fX, fAngle,
											fStep)),
							p_oPlot->CorrectY(
									p_oPlot->GetUncorrectedY(fY, fAngle,
											fStep)), &iStepX, &iStepY);
					...
					mp_oCalcGrid->GetValueOfCell(iStepX, iStepY,
							mp_iFLogCalcsCode[0], &fSubstrate);
			...
			if (iTp == clTreePopulation::adult) {
				...
				if (fRandom <= mp_fPropOfFallenThatUproot[iSp]) {
					...
					//tipup variable
					mp_oCalcGrid->GetValueAtPoint(fX, fY, m_iTipupCalcsCode,
							&fSubstrate);
					fSubstrate += M_PI * pow(fRadius * m_fRootTipupFactor, 2);
					mp_oCalcGrid->SetValueAtPoint(fX, fY, m_iTipupCalcsCode,
							fSubstrate);
				}
	...
	try {
		...
		for (i = 0; i < iNumXCells; i++)
			for (j = 0; j < iNumYCells; j++) {
				...
				mp_oCalcGrid->GetValueOfCell(i, j, mp_iFLogCalcsCode[0],
						&fNewFreshLog);
				mp_oCalcGrid->GetValueOfCell(i, j, m_iTipupCalcsCode,
						&fNewTipup);
				...
				mp_oCalcGrid->SetValueOfCell(i, j, mp_iFLogCalcsCode[0],
						fNewFreshLog);
				mp_oCalcGrid->SetValueOfCell(i, j, m_iTipupCalcsCode,
						fNewTipup);
				...
				if (!bSubstrateAdded) {
					p_oNextToLastPackage =
							mp_oSubstrateGrid->GetFirstPackageOfCell(i, j);
					...
					while (p_oNextToLastPackage) {
						...
						if (!p_oPackage) {
							...
							p_oPackage->SetValue(m_iPkgFreshLogCode,
									fNewFreshLog);
							p_oPackage->SetValue(m_iPkgTipupCode, fNewTipup);
...
void clSubstrate::DecaySubstrate() {
	try {
		...
		int iAge, //package age
				iTemp;
		short int iNumXCells = mp_oSubstrateGrid->GetNumberXCells(),
				iNumYCells = mp_oSubstrateGrid->GetNumberYCells(), iX, iY, k; //loop counters
			...
			for (iY = 0; iY < iNumYCells; iY++) {
				...
				if (iTemp == -1 && 1 != mp_oSimManager->GetCurrentTimestep()) {
					...
					for (k = 1; k < m_iMaxDecayTimesteps; k++) {
						mp_oCalcGrid->GetValueOfCell(iX, iY,
								mp_iFLogCalcsCode[k], &fTemp);
						fDecLogs += fTemp
								* (mp_fFLogDecayProp[k - 1]
										- mp_fFLogDecayProp[k]);
					}
					...
					mp_oSubstrateGrid->GetValueOfCell(iX, iY, m_iDecLogCode,
							&fDecLogs);
				}
				...
				if (p_oPackage) {
					...
					mp_oSubstrateGrid->SetValueOfCell(iX, iY, m_iTipupCode,
							fTipup);
					mp_oSubstrateGrid->SetValueOfCell(iX, iY, m_iFreshLogCode,
							fFreshLogs);
				} else {
					...
					mp_oSubstrateGrid->GetValueOfCell(iX, iY, m_iTipupCode,
							&fTipup);
					mp_oSubstrateGrid->GetValueOfCell(iX, iY, m_iFreshLogCode,
							&fFreshLogs);
				}
				...
				if (fTotalSubstrate <= 1) {
					...
					mp_oSubstrateGrid->SetValueOfCell(iX, iY, m_iFFMossCode,
							(1 - fTotalSubstrate) * m_fMossProportion);
					mp_oSubstrateGrid->SetValueOfCell(iX, iY, m_iFFLitterCode,
							(1 - fTotalSubstrate) * (1 - m_fMossProportion));
					...
					fTotalSubstrate = fScarSoil + fTipup + fFreshLogs
							+ fDecLogs;
					mp_oSubstrateGrid->SetValueOfCell(iX, iY, m_iScarSoilCode,
							fScarSoil);
					mp_oSubstrateGrid->SetValueOfCell(iX, iY, m_iTipupCode,
...
void clSubstrate::UpdateSubstrateAges() {
	...
	float fVal;
	int iNumXCells = mp_oCalcGrid->GetNumberXCells(), iNumYCells =
			mp_oCalcGrid->GetNumberYCells(), iX, iY, i;

	for (iX = 0; iX < iNumXCells; iX++) {
		for (iY = 0; iY < iNumYCells; iY++) {
			for (i = m_iMaxDecayTimesteps - 2; i >= 0; i--) {
				mp_oCalcGrid->GetValueOfCell(iX, iY, mp_iFLogCalcsCode[i],
						&fVal);
				mp_oCalcGrid->SetValueOfCell(iX, iY, mp_iFLogCalcsCode[i + 1],
						fVal);
				mp_oCalcGrid->GetValueOfCell(iX, iY, mp_iDecLogCalcsCode[i],
						&fVal);
				mp_oCalcGrid->SetValueOfCell(iX, iY, mp_iDecLogCalcsCode[i + 1],
						fVal);
			}
...
void clSubstrate::CalculateDecayProportions() {
	float fNumYearsPerTS = mp_oSimManager->GetNumberOfYearsPerTimestep(), fTemp;
	int i;
	...
	for (i = 0; i < m_iMaxDecayTimesteps + 1; i++) {
		...
		if (0 == m_fFreshLogB)
			...
			fTemp = pow((i * fNumYearsPerTS), m_fFreshLogB);
		mp_fFLogDecayProp[i] = exp(m_fFreshLogA * fTemp);

		if (0 == m_fScarifiedSoilB)
			fTemp = 1;
		else
			fTemp = pow((i * fNumYearsPerTS), m_fScarifiedSoilB);
		mp_fScarSoilDecayProp[i] = exp(m_fScarifiedSoilA * fTemp);

		if (0 == m_fTipUpB)
			fTemp = 1;
		else
			fTemp = pow((i * fNumYearsPerTS), m_fTipUpB);
		mp_fTipupDecayProp[i] = exp(m_fTipUpA * fTemp);

		if (0 == m_fDecayedLogB)
			fTemp = 1;
		else
			fTemp = pow((i * fNumYearsPerTS), m_fDecayedLogB);
