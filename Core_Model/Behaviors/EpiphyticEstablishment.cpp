...
void clEpiphyticEstablishment::Action()
{
  try
  {
    ...
    struct newSeedling {
      float fX, fY, fHeight, fParamG1, fParamG2, fParamM1, fParamM2;
      ...
      if ( -1 != mp_iDeadCodes[iSp][iTp] )
      {
        ...
        if (iDead > notdead) {
          ...
          if (clModelMath::GetRand() <= fSeedlingProb) {
            ...
            temp->fParamG1 = clModelMath::NormalRandomDraw(StDevParamG1) + MeanParamG1;
            MeanParamG2 = p_oPop->GetMeanParamG2(iSp);
            StDevParamG2 = p_oPop->GetStDevParamG2(iSp);
            temp->fParamG2 = clModelMath::NormalRandomDraw(StDevParamG2) + MeanParamG2;
            MeanParamM1 = p_oPop->GetMeanParamM1(iSp);
            StDevParamM1 = p_oPop->GetStDevParamM1(iSp);
            temp->fParamM1 = clModelMath::NormalRandomDraw(StDevParamM1) + MeanParamM1;
            MeanParamM2 = p_oPop->GetMeanParamM2(iSp);
            StDevParamM2 = p_oPop->GetStDevParamM2(iSp);
            temp->fParamM2 = clModelMath::NormalRandomDraw(StDevParamM2) + MeanParamM2;
    ...
    while (currentSeedling != NULL) {
      p_oSeedling = p_oPop->CreateTree(currentSeedling->fX, currentSeedling->fY,
						 ...
						 currentSeedling->fParamG1, currentSeedling->fParamG2,
						 currentSeedling->fParamM1, currentSeedling->fParamM2);
