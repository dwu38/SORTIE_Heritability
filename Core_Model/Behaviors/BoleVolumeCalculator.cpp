...
/////////////////////////////////////////////////////////////////////////////
void clBoleVolumeCalculator::GetParameterFileData(
	...
	try {
		...
		//b0
		FillSpeciesSpecificValue(p_oElement, "an_boleB0", "an_bb0Val",
		...
		//b1
		FillSpeciesSpecificValue(p_oElement, "an_boleB1", "an_bb1Val",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		//b2
		FillSpeciesSpecificValue(p_oElement, "an_boleB2", "an_bb2Val",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		//b3
		FillSpeciesSpecificValue(p_oElement, "an_boleB3", "an_bb3Val",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		//b4
		FillSpeciesSpecificValue(p_oElement, "an_boleB4", "an_bb4Val",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		//b5
		FillSpeciesSpecificValue(p_oElement, "an_boleB5", "an_bb5Val",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
		...
		//Form classes
		FillSpeciesSpecificValue(p_oElement, "an_boleFormClasses", "an_bfcVal",
				p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
		//Transfer to the appropriate array buckets
		for (i = 0; i < m_iNumBehaviorSpecies; i++)
			mp_fFormClass[mp_iIndexes[p_fTempValues[i].code]] =
					p_fTempValues[i].val;
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			if (mp_fFormClass[i] < 60 || mp_fFormClass[i] > 100) {
				modelErr stcErr;
				stcErr.sFunction =
						"clBoleVolumeCalculator::GetParameterFileData";
				std::stringstream s;
		...
		while (p_oTree) {
			...
			if (-1
				...
				if (fDBH < MIN_DBH) {
					...
					fVolume =
							mp_fB0[mp_iIndexes[iSp]]
									+ (mp_fB1[mp_iIndexes[iSp]]
											* pow(fDBH,
													mp_fB2[mp_iIndexes[iSp]]))
									+ (mp_fB3[mp_iIndexes[iSp]]
											* pow(fDBH,
													mp_fB4[mp_iIndexes[iSp]])
											* pow(fTreeHeight,
													mp_fB5[mp_iIndexes[iSp]]));
					p_oTree->SetValue(
							mp_iVolumeCodes[mp_iIndexes[iSp]][iTp
									- clTreePopulation::sapling], fVolume);
				}
...
void clBoleVolumeCalculator::RegisterTreeDataMembers() {
	...
	for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
		...
		if (clTreePopulation::adult != mp_whatSpeciesTypeCombos[i].iType
			...
			modelErr stcErr;
			stcErr.sFunction =
					"clBoleVolumeCalculator::RegisterTreeDataMembers";
			stcErr.sMoreInfo =
		...
		mp_iVolumeCodes[mp_iIndexes[mp_whatSpeciesTypeCombos[i].iSpecies]][mp_whatSpeciesTypeCombos[i].iType
				- clTreePopulation::sapling] = p_oPop->RegisterFloat("Bole Vol",
