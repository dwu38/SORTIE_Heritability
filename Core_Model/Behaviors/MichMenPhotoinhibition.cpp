...
void clMichMenPhotoinhibition::DoShellSetup(DOMDocument *p_oDoc) {
	...
	try {
		...
		FillSpeciesSpecificValue(p_oElement, "gr_mmPhotGrowthAlpha",
				"gr_mmpgaVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		...
		FillSpeciesSpecificValue(p_oElement, "gr_mmPhotGrowthBeta",
				"gr_mmpgbVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		...
		//D
		FillSpeciesSpecificValue(p_oElement, "gr_mmPhotGrowthD", "gr_mmpgdVal",
		...
		FillSpeciesSpecificValue(p_oElement, "gr_mmPhotGrowthPhi",
				"gr_mmpgpVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer values to our permanent array
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
...
float clMichMenPhotoinhibition::CalcHeightGrowthValue(clTree *p_oTree,
	...
	fAnnualRelGrowth = (mp_fAlpha[ind]
			/ (1 + (mp_fAlpha[ind] / (mp_fBeta[ind] * fGli))))
			- mp_fD[ind] * fGli;
