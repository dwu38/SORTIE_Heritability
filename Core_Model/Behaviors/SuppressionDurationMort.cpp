	...
	try {
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			if (0 == p_fTempValues[i].val) {
				...
				stcErr.sFunction = "clSuppressionDurationMort::DoShellSetup";
				stcErr.sMoreInfo =
						"Values for suppression duration mortality X0 cannot be 0.";
				throw(stcErr);
			}
			...
			for (j = 1; j < m_iMaxMortTime; j++) {
				mp_fMortProbs[mp_iWhatSpecies[i]][j] =
						mp_fMax[mp_iWhatSpecies[i]]
								/ (1
										+ pow(j / mp_fX0[mp_iWhatSpecies[i]],
...
// DoMort()
///////////////////////////////////////////////////////////////////////////
deadCode clSuppressionDurationMort::DoMort(clTree *p_oTree, const float &fDiam,
