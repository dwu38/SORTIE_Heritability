...
void clQuadratGLILight::DoShellSetup(xercesc::DOMDocument *p_oDoc) {
	try {
		...
		FillSingleValue(p_oElement, "li_quadratLightHeight", &m_fLightHeight,
				true);
		...
		clTreePopulation *p_oPop =
				(clTreePopulation*) mp_oSimManager->GetPopulationObject(
						"treepopulation");
		m_fMaxSearchRad = (mp_oLightOrg->GetMaxTreeHeight() - m_fLightHeight)
				* m_fRcpTanMinAng
		...
		if (!mp_oQuadrats) {
			...
			if (-1 == m_iGridGliCode) {
				...
				stcErr.sFunction = "clQuadratGLILight::DoShellSetup";
				stcErr.sMoreInfo =
						"Quadrat GLI grid was incorrectly set up in the parameter file.  Missing float \"GLI\".";
				throw(stcErr);
			}
		}
		...
		for (iYCell = 0; iYCell < iNumYCells; iYCell++) {
			...
			if (fGli < 0) {
				...
				sprintf(cQuery, "%s%f%s%f%s%f%s%f", "distance=",
						m_fMaxSearchRad, " FROM x=", fX, "y=", fY, "::height=",
						m_fLightHeight);
				p_oShaders = p_oPop->Find(cQuery);
