...
void clBrowsedStochasticMortality::DoShellSetup(xercesc::DOMDocument *p_oDoc) {
	...
	try {
		...
		FillSpeciesSpecificValue(p_oElement, "mo_browsedRandomMortality",
				"mo_brmVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		...
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			...
			mp_fBrowsedMortProb[p_fTempValues[i].code] = 1
		...
		FillSpeciesSpecificValue(p_oElement, "mo_stochasticMortRate",
				"mo_smrVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop,
				true);
		//Transfer values to our permanent array, making sure all values are
		//between 0 and 1 and translating to a timestep probability
		for (i = 0; i < m_iNumBehaviorSpecies; i++) {
			...
			mp_fUnbrowsedMortProb[p_fTempValues[i].code] = 1
					- pow(1 - p_fTempValues[i].val, fYearsPerTimestep);
		}
...
// DoMort()
///////////////////////////////////////////////////////////////////////////
deadCode clBrowsedStochasticMortality::DoMort(clTree *p_oTree,
