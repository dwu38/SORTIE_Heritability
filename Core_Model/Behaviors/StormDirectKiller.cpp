...
void clStormDirectKiller::GetParameterFileData(xercesc::DOMDocument *p_oDoc,
	...
	//"a"
	FillSpeciesSpecificValue(p_oElement, "st_stormDirectKillerA", "st_sdkaVal",
	...
	//"b"
	FillSpeciesSpecificValue(p_oElement, "st_stormDirectKillerB", "st_sdkbVal",
			p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
	//Transfer to the appropriate array buckets
	for (i = 0; i < m_iNumBehaviorSpecies; i++)
			...
			while (p_oPkg) {
				...
				if (fStormSeverity > 0) {
					...
					if (clModelMath::GetRand() <= fMortProb) {
						p_oTree->SetValue(
								p_oPop->GetIntDataCode("dead", iSp, iTp),
								storm);
						goto nextTree;
...
void clStormDirectKiller::GetGridInfo() {
	...
	if ( NULL == mp_oStormDamageGrid) {
		...
		stcErr.sMoreInfo =
				"The storm behavior must be used with the storm behavior.";
		throw(stcErr);
	}
