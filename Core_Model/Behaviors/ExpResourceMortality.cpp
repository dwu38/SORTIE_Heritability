...
////////////////////////////////////////////////////////////////////////////
void clExpResourceMortality::ReadParameterFileData(
	...
	FillSpeciesSpecificValue(p_oElement, "mo_expResMortA", "mo_ermaVal",
	...
	FillSpeciesSpecificValue(p_oElement, "mo_expResMortB", "mo_ermbVal",
			p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
	//Transfer to the appropriate array buckets
	for (i = 0; i < m_iNumBehaviorSpecies; i++)
	...
	FillSpeciesSpecificValue(p_oElement, "mo_expResMortC", "mo_ermcVal",
			p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
	//Transfer to the appropriate array buckets
	for (i = 0; i < m_iNumBehaviorSpecies; i++)
	...
	FillSpeciesSpecificValue(p_oElement, "mo_expResMortD", "mo_ermdVal",
			p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true);
	//Transfer to the appropriate array buckets
	for (i = 0; i < m_iNumBehaviorSpecies; i++)
			...
			for (j = 0; j < iTotalTypes; j++)
				if (mp_bUsesThisMortality[i][j]) {
					...
					if (-1 == mp_iGrowthCodes[mp_iIndexes[i]][j]) {
						modelErr stcErr;
						stcErr.sFunction =
								"clExpResourceMortality::GetTreeDataMemberCodes";
						std::stringstream s;
...
// DoMort()
////////////////////////////////////////////////////////////////////////////
deadCode clExpResourceMortality::DoMort(clTree *p_oTree, const float &fDbh,
	...
	fMortProb = (mp_fD[mp_iIndexes[iSpecies]]
			+ (mp_fA[mp_iIndexes[iSpecies]] * fResource))
			* exp(
					-(mp_fB[mp_iIndexes[iSpecies]]
							+ (mp_fC[mp_iIndexes[iSpecies]] * fResource))
							* fGrowth);
