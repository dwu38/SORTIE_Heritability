...
int clModelMath::RandomRound(float fNumber) {
	...
	return (GetRand() <= fFractionalPart ?
			(int) (fIntegerPart + 1) : (int) fIntegerPart);
}
...
/////////////////////////////////////////////////////////////////////////////
// CalculatetwodtFunction()
/////////////////////////////////////////////////////////////////////////////
float clModelMath::CalculatetwodtFunction(float fX0, float fDistance) {
	//Error if fX0 is zero
	if (0 >= fX0 || 0 > fDistance) {
		modelErr stcErr;
		stcErr.iErrorCode = BAD_DATA;
		stcErr.sFunction = "clModelMath::CalculatetwodtFunction";
		stcErr.sMoreInfo = "X0 and distance must be greater than zero.";
		throw(stcErr);
	}
	if (fDistance == 0)
	...
	//prevent underflow
	float fTemp = 1 / (M_PI * fX0 * pow((1 + (pow(fDistance, 2) / fX0)), 2));
	if (fTemp < -50)
		return 0;
	else
		return exp(fTemp);
...
/////////////////////////////////////////////////////////////////////////////
// ZeroInflatedPoissonRandomDraw()
/////////////////////////////////////////////////////////////////////////////
int clModelMath::ZeroInflatedPoissonRandomDraw(float fLambda, float fp) {
	float fRand = GetRand();
	if (fRand < fp)
		return 0;
	else
...
int clModelMath::NegBinomialRandomDraw(float fMean, float fClumping) {
	...
	while (fRand > fCDF) {
		...
		fPDF = ((iCount + fClumping - 1) / iCount)
				* (fMean / (fClumping + fMean)) * fPDF;
		fCDF = fCDF + fPDF;
	}
