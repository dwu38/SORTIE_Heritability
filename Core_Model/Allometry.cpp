...
void clAllometry::GetData(DOMDocument *p_oDoc, clTreePopulation *p_oPop) {
	...
	FillSpeciesSpecificValue(p_oElement, "tr_interceptDiam10ToDBH",
			"tr_idtdVal", mp_fDbhToDiam10Intercept, p_oPop, true);
	...
	for (int i = 0; i < m_iNumSpecies; i++) {
		if (mp_fDbhToDiam10Slope[i] == 0) {
			...
			stcErr.sMoreInfo =
					"Diameter at 10 cm to DBH conversion may not equal 0.";
			throw(stcErr);
		}
	}
	...
	for (i = 0; i < m_iNumSpecies; i++) {
		...
		} else if (&clAllometry::CalcNonSpatDensDepExpAdultCrownRad
				== mp_AdultCrownRad[i]) {
			fTempRad = 1000;
	...
	for (i = 0; i < m_iNumSpecies; i++) {
		if (&clAllometry::CalcStandardSapAdultCrownRad
...
float clAllometry::CalcAdultDbh(float fHeight, const int &iSpecies) {
	...
	if (fHeight >= mp_fMaxTreeHeight[iSpecies]) {
		...
		stcErr.sFunction = "clAllometry::CalcAdultDbh";
		stcErr.sMoreInfo =
				"Cannot set tree height greater than maximum canopy height for this species.";
		throw(stcErr);
	}
...
float clAllometry::CalcSeedlingDiam10(float fHeight, const int &iSpecies) {
	...
	if (fHeight >= 30.1) {
		...
		stcErr.sFunction = "clAllometry::CalcSeedlingDiam10";
		stcErr.sMoreInfo =
				"Cannot set tree height greater than maximum tree height for this species.";
		throw(stcErr);
	}
...
////////////////////////////////////////////////////////////////////////////
// CalcSeedlingSizeClass()
////////////////////////////////////////////////////////////////////////////
int clAllometry::CalcSeedlingSizeClass(float fHeight, const int &iSpecies) {
	if (iSpecies >= m_iNumSpecies || iSpecies < 0) {
		//invalid species - throw an error
		modelErr stcErr;
		stcErr.iErrorCode = BAD_DATA;
		stcErr.sFunction = "clAllometry::CalcSeedlingSizeClass";
		std::stringstream s;
		s << "Unrecognized species " << iSpecies;
		stcErr.sMoreInfo = s.str();
		throw(stcErr);
...
//    throw(stcErr);
//  }
	return (*this.*mp_SeedlingSC[iSpecies])(fHeight, iSpecies);
}
...
float clAllometry::CalcStandardSapAdultHeight(const float &fDbh,
		const int &iSpecies) {
	float fHeight = (1.35
			+ ((mp_fMaxTreeHeight[iSpecies] - 1.35)
					* (1.0
							- exp(
									(double) (-mp_fSlopeAsympHeight[iSpecies]
											* fDbh)))));
	if (fHeight > 0.001)
		return fHeight;
	else
...
float clAllometry::CalcStandardSeedlingHeight(const float &fDiam10,
	...
	float fHeight = 0.1
			+ 30
					* (1.0
							- exp(
									(double) (-mp_fSlopeHeightDiam10[iSpecies]
											* fDiam10)));
	if (fHeight > 0.001)
		return fHeight;
	else
...
//////////////////////////////////////////////////////////////////////////////
// CalcStandardSeedlingSizeClass()
//////////////////////////////////////////////////////////////////////////////
int clAllometry::CalcStandardSeedlingSizeClass(const float &fHeight,
		const int &iSpecies) {
	int fSizeClass;
	if (fHeight < 0.1)
		fSizeClass = 0;
	else if (fHeight < 0.25)
		fSizeClass = 1;
	else if (fHeight < 0.75)
		fSizeClass = 2;
	else if (fHeight < 1.4)
		fSizeClass = 3;
	else
...
float clAllometry::CalcStandardSeedlingDiam10(const float &fHeight,
		const int &iSpecies) {
	float fDiam = (log(1 - ((fHeight - 0.1) / 30)))
			/ -mp_fSlopeHeightDiam10[iSpecies];
	if (fDiam > 0.001)
		return fDiam;
	return 0.001;
}
...
float clAllometry::CalcStandardSapAdultCrownRad(clTree *p_oTree) {
	...
	float fRad = mp_fAsympCrownRad[iSpecies]
			* pow(fDbh, mp_fCrownRadExp[iSpecies]);
...
float clAllometry::CalcChapRichSapAdultCrownRad(clTree *p_oTree) {
	...
	int iSpecies = p_oTree->GetSpecies();
	p_oTree->GetValue(m_oPop->GetDbhCode(iSpecies, p_oTree->GetType()), &fDbh);
	return mp_fCRCrownRadIntercept[iSpecies]
			+ (mp_fCRAsympCrownRad[iSpecies]
...
float clAllometry::CalcStandardSapAdultCrownDepth(clTree *p_oTree) {
	...
	p_oTree->GetValue(m_oPop->GetHeightCode(iSpecies, p_oTree->GetType()),
			&fHeight);
	float fCrownHeight = mp_fAsympCrownDepth[iSpecies]
			* pow(fHeight, mp_fCrownDepthExp[iSpecies]);
...
float clAllometry::CalcChapRichSapAdultCrownDepth(clTree *p_oTree) {
	...
	float fCHeight = mp_fCRCrownHtIntercept[iSpecies]
			+ (mp_fCRAsympCrownHt[iSpecies]
					* (pow((1 - exp(-mp_fCRCrownHtShape1[iSpecies] * fHeight)),
...
float clAllometry::CalcPowerSaplingDbh(const float &fHeight,
	...
	if (fHeight <= 0.001)
		return 0.001;
	float fDiam = pow(fHeight / mp_fPowerA[iSpecies],
			1 / mp_fPowerExpB[iSpecies]);
	//Convert to DBH
...
float clAllometry::CalcNonSpatDensDepExpAdultCrownRad(clTree *p_oTree) {
	...
	p_oTree->GetValue(
			m_oPop->GetHeightCode(p_oTree->GetSpecies(), p_oTree->GetType()),
			&fHeight);
	p_oTree->GetValue(
			m_oPop->GetDbhCode(p_oTree->GetSpecies(), p_oTree->GetType()),
			&fDbh);
	...
	if (m_fPlotDensity < 0) {
		...
		while (p_oNeigh) {
			p_oNeigh->GetValue(
					m_oPop->GetDbhCode(p_oNeigh->GetSpecies(),
	...
	while (p_oNeigh) {
		p_oNeigh->GetValue(
				m_oPop->GetHeightCode(p_oNeigh->GetSpecies(),
	...
	fCrownRad = mp_fNonSpatExpDensDepCRD1[iSpecies]
			* pow(fDbh, mp_fNonSpatExpDensDepCRA[iSpecies])
			* pow(fHeight, mp_fNonSpatExpDensDepCRB[iSpecies])
			* pow(fCHi, mp_fNonSpatExpDensDepCRC[iSpecies])
			* pow(m_fPlotDensity, mp_fNonSpatExpDensDepCRD[iSpecies])
			* pow(m_fPlotBasalArea, mp_fNonSpatExpDensDepCRE[iSpecies])
			* pow(fBAL, mp_fNonSpatExpDensDepCRF[iSpecies]);
...
float clAllometry::CalcNonSpatDensDepLogAdultCrownDepth(clTree *p_oTree) {
	...
	fCrownHeight =
			fHeight
					/ (1
							+ exp(
									mp_fNonSpatLogDensDepCHA[iSpecies]
											+ mp_fNonSpatLogDensDepCHB[iSpecies]
													* fDbh
											+ mp_fNonSpatLogDensDepCHC[iSpecies]
													* fHeight
											+ mp_fNonSpatLogDensDepCHD[iSpecies]
													* fCRi
											+ mp_fNonSpatLogDensDepCHE[iSpecies]
													* m_fPlotDensity
											+ mp_fNonSpatLogDensDepCHF[iSpecies]
													* m_fPlotBasalArea
											+ mp_fNonSpatLogDensDepCHG[iSpecies]
													* fBAL));
	...
	try {
		...
		for (i = 0; i < m_iNumSpecies; i++) {
			if (standard == p_iWhatFunction[i]) {
				...
				stcErr.sFunction = "clAllometry::GetData";
				std::stringstream s;
				s << "Invalid adult height-diam function code: "
						<< p_iWhatFunction[i];
				stcErr.sMoreInfo = s.str();
				throw(stcErr);
			}
		...
		if (iHowMany > 0) {
			...
			FillSpeciesSpecificValue(p_oElement, "tr_adultLinearSlope",
					"tr_alsVal", p_fTemp, iHowMany, m_oPop, true);
			for (i = 0; i < iHowMany; i++) {
				if (0 == p_fTemp[i].val) {
					modelErr stcErr;
					stcErr.iErrorCode = BAD_DATA;
					stcErr.sFunction = "clAllometry::GetData";
					stcErr.sMoreInfo =
							"No values in adult linear slope may be 0.";
					throw(stcErr);
		...
		if (iHowMany > 0) {
			...
			for (i = 0; i < iHowMany; i++) {
				if (0 == p_fTemp[i].val) {
					...
					stcErr.sFunction = "clAllometry::GetData";
					stcErr.sMoreInfo =
							"No values in adult reverse linear slope may be 0.";
					throw(stcErr);
			...
			FillSpeciesSpecificValue(p_oElement,
					"tr_adultReverseLinearIntercept", "tr_arliVal", p_fTemp,
					iHowMany, m_oPop, true);
			for (i = 0; i < iHowMany; i++) {
				mp_fAdultReverseLinearIntercept[p_fTemp[i].code] =
						p_fTemp[i].val;
			}
		}
	...
	try {
		...
		for (i = 0; i < m_iNumSpecies; i++) {
			if (standard_cr == p_iWhatFunction[i]) {
				mp_AdultCrownRad[i] =
						&clAllometry::CalcStandardSapAdultCrownRad;
			} else if (chapman_richards_cr == p_iWhatFunction[i]) {
				mp_AdultCrownRad[i] =
						&clAllometry::CalcChapRichSapAdultCrownRad;
			} else if (non_spat_exp_dens_dep_cr == p_iWhatFunction[i]) {
				mp_AdultCrownRad[i] =
						&clAllometry::CalcNonSpatDensDepExpAdultCrownRad;
				...
				stcErr.sFunction = "clAllometry::GetData";
				std::stringstream s;
				s << "Invalid adult crown radius-diam function code: "
						<< p_iWhatFunction[i];
				stcErr.sMoreInfo = s.str();
				throw(stcErr);
			}
		...
		if (iHowMany > 0) {
			...
			FillSpeciesSpecificValue(p_oElement, "tr_stdAsympCrownRad",
					"tr_sacrVal", p_fTemp, iHowMany, m_oPop, true);
			for (i = 0; i < iHowMany; i++) {
				mp_fAsympCrownRad[p_fTemp[i].code] = p_fTemp[i].val;
			}

			FillSpeciesSpecificValue(p_oElement, "tr_stdCrownRadExp",
					"tr_screVal", p_fTemp, iHowMany, m_oPop, true);
			for (i = 0; i < iHowMany; i++) {
				mp_fCrownRadExp[p_fTemp[i].code] = p_fTemp[i].val;
			}

			FillSpeciesSpecificValue(p_oElement, "tr_stdMaxCrownRad",
					"tr_smcrVal", p_fTemp, iHowMany, m_oPop, true);
			for (i = 0; i < iHowMany; i++) {
		...
		if (iHowMany > 0) {
			...
			FillSpeciesSpecificValue(p_oElement, "tr_nciCRCrowdingN",
					"tr_nccrnVal", p_fTemp, iHowMany, m_oPop, true);
			for (i = 0; i < iHowMany; i++) {
			...
			FillSpeciesSpecificValue(p_oElement, "tr_nciCRSizeEffectD",
					"tr_ncrsedVal", p_fTemp, iHowMany, m_oPop, true);
			for (i = 0; i < iHowMany; i++) {
			...
			for (i = 0; i < m_iNumSpecies; i++) {
				stringstream s;
				s << "tr_nciCR" << m_oPop->TranslateSpeciesCodeToName(i)
						<< "NeighborLambda";
				FillSpeciesSpecificValue(p_oElement, s.str(), "tr_ncrlVal",
						p_fTemp, iHowMany, m_oPop, true);
	...
	try {
		...
		for (i = 0; i < m_iNumSpecies; i++) {
			if (standard == p_iWhatFunction[i]) {
				...
				mp_SaplingHeight[i] =
						&clAllometry::CalcReverseLinearSaplingHeight;
				...
				stcErr.sFunction = "clAllometry::GetData";
				std::stringstream s;
				s << "Invalid sapling height-diam function code: "
						<< p_iWhatFunction[i];
				stcErr.sMoreInfo = s.str();
				throw(stcErr);
			}
		...
		if (iHowMany > 0) {
			...
			for (i = 0; i < iHowMany; i++) {
				if (0 == p_fTemp[i].val) {
					...
					stcErr.sFunction = "clAllometry::GetData";
					stcErr.sMoreInfo =
							"No values in sapling linear slope may be 0.";
					throw(stcErr);
		...
		if (iHowMany > 0) {
			...
			for (i = 0; i < iHowMany; i++) {
				if (0 == p_fTemp[i].val) {
					...
					stcErr.sFunction = "clAllometry::GetData";
					stcErr.sMoreInfo =
							"No values in sapling reverse linear slope may be 0.";
					throw(stcErr);
			...
			FillSpeciesSpecificValue(p_oElement,
					"tr_saplingReverseLinearIntercept", "tr_sarliVal", p_fTemp,
					iHowMany, m_oPop, true);
			for (i = 0; i < iHowMany; i++) {
				mp_fSaplingReverseLinearIntercept[p_fTemp[i].code] =
						p_fTemp[i].val;
			}
		}
		...
		if (iHowMany > 0) {
			...
			//Power function "a"
			FillSpeciesSpecificValue(p_oElement, "tr_saplingPowerA",
					"tr_sapaVal", p_fTemp, iHowMany, m_oPop, true);
			for (i = 0; i < iHowMany; i++) {
				mp_fPowerA[p_fTemp[i].code] = p_fTemp[i].val;
			}

			//Power function "b"
			FillSpeciesSpecificValue(p_oElement, "tr_saplingPowerB",
					"tr_sapbVal", p_fTemp, iHowMany, m_oPop, true);
			for (i = 0; i < iHowMany; i++) {
	...
	try {
		...
		for (i = 0; i < m_iNumSpecies; i++) {
			if (standard_cr == p_iWhatFunction[i]) {
				...
				stcErr.sFunction = "clAllometry::GetData";
				std::stringstream s;
				s << "Invalid sapling crown radius-diam function code: "
						<< p_iWhatFunction[i];
				stcErr.sMoreInfo = s.str();
				throw(stcErr);
			}
	...
	try {
		...
		for (i = 0; i < m_iNumSpecies; i++) {
			if (standard_cd == p_iWhatFunction[i]) {
				mp_SaplingCrownDepth[i] =
						&clAllometry::CalcStandardSapAdultCrownDepth;
			} else if (chapman_richards_cd == p_iWhatFunction[i]) {
				mp_SaplingCrownDepth[i] =
						&clAllometry::CalcChapRichSapAdultCrownDepth;
				...
				stcErr.sFunction = "clAllometry::GetData";
				std::stringstream s;
				s << "Invalid sapling crown height-diam function code: "
						<< p_iWhatFunction[i];
				stcErr.sMoreInfo = s.str();
				throw(stcErr);
			}
		...
		if (iHowMany > 0) {
			...
			FillSpeciesSpecificValue(p_oElement, "tr_stdAsympCrownHt",
					"tr_sachVal", p_fTemp, iHowMany, m_oPop, true);
			for (i = 0; i < iHowMany; i++) {
				mp_fAsympCrownDepth[p_fTemp[i].code] = p_fTemp[i].val;
			}

			FillSpeciesSpecificValue(p_oElement, "tr_stdCrownHtExp",
					"tr_scheVal", p_fTemp, iHowMany, m_oPop, true);
			for (i = 0; i < iHowMany; i++) {
		...
		if (iHowMany > 0) {
			...
			FillSpeciesSpecificValue(p_oElement, "tr_nciCDCrowdingN",
					"tr_nccdnVal", p_fTemp, iHowMany, m_oPop, true);
			for (i = 0; i < iHowMany; i++) {
			...
			FillSpeciesSpecificValue(p_oElement, "tr_nciCDSizeEffectD",
					"tr_ncdsedVal", p_fTemp, iHowMany, m_oPop, true);
			for (i = 0; i < iHowMany; i++) {
			...
			for (i = 0; i < m_iNumSpecies; i++) {
				stringstream s;
				s << "tr_nciCD" << m_oPop->TranslateSpeciesCodeToName(i)
						<< "NeighborLambda";
				FillSpeciesSpecificValue(p_oElement, s.str(), "tr_ncdlVal",
						p_fTemp, iHowMany, m_oPop, true);
				for (j = 0; j < iHowMany; j++) {
	...
	try {
		...
		for (i = 0; i < m_iNumSpecies; i++) {
			if (standard_cd == p_iWhatFunction[i]) {
				...
				stcErr.sFunction = "clAllometry::GetData";
				std::stringstream s;
				s << "Invalid adult crown height-diam function code: "
						<< p_iWhatFunction[i];
				stcErr.sMoreInfo = s.str();
				throw(stcErr);
			}
...
void clAllometry::SetupSeedlingHeightDiam(xercesc::DOMElement *p_oElement) {
	...
	try {
		...
		mp_SeedlingSC = new Ptr2AllometryI[m_iNumSpecies];
		...
		for (i = 0; i < m_iNumSpecies; i++) {
			...
			// size class allometry is not really an allometry (for now)
			mp_SeedlingSC[i] = &clAllometry::CalcStandardSeedlingSizeClass;
			...
			if (standard == p_iWhatFunction[i]) {
				...
				mp_SeedlingHeight[i] =
						&clAllometry::CalcReverseLinearSeedlingHeight;
				mp_SeedlingDiam[i] =
						&clAllometry::CalcReverseLinearSeedlingDiam10;
			} else {
				modelErr stcErr;
				stcErr.iErrorCode = BAD_DATA;
				stcErr.sFunction = "clAllometry::GetData";
				std::stringstream s;
				s << "Invalid seedling height-diam function code: "
						<< p_iWhatFunction[i];
				stcErr.sMoreInfo = s.str();
				throw(stcErr);
			}
		...
		if (iHowMany > 0) {
			...
			for (i = 0; i < iHowMany; i++) {
				if (0 == p_fTemp[i].val) {
					...
					stcErr.sFunction = "clAllometry::GetData";
					stcErr.sMoreInfo =
							"No values in seedling linear slope may be 0.";
					throw(stcErr);
		...
		if (iHowMany > 0) {
			...
			FillSpeciesSpecificValue(p_oElement,
					"tr_seedlingReverseLinearSlope", "tr_serlsVal", p_fTemp,
					iHowMany, m_oPop, true);
			for (i = 0; i < iHowMany; i++) {
				if (0 == p_fTemp[i].val) {
					modelErr stcErr;
					...
					stcErr.sFunction = "clAllometry::GetData";
					stcErr.sMoreInfo =
							"No values in seedling reverse linear slope may be 0.";
					throw(stcErr);
				}
				mp_fSeedlingReverseLinearSlope[p_fTemp[i].code] =
						p_fTemp[i].val;
			}

			//Reverse linear seedling intercept
			FillSpeciesSpecificValue(p_oElement,
					"tr_seedlingReverseLinearIntercept", "tr_serliVal", p_fTemp,
					iHowMany, m_oPop, true);
			for (i = 0; i < iHowMany; i++) {
				mp_fSeedlingReverseLinearIntercept[p_fTemp[i].code] =
						p_fTemp[i].val;
			}
		}
...
float clAllometry::CalcNCICrownRad(clTree *p_oTree) {
	...
	float fCrowdingEffect, //tree's crowding effect
			...
			fCrownRadius, fNeighX, fNeighY, //holders for the neighbor tree's X and Y
	...
	short int iNeighSpecies, iNeighType, //species and type for neighbor
			...
			iTargetType = p_oTree->GetType(), iDeadCode; //neighbor's dead code
	...
	sprintf(cQuery, "%s%f%s%f%s%f%s%f", "distance=",
			mp_fNCICRMaxCrowdingRadius[iTargetSpecies], "FROM x=", fTargetX,
	...
	while (p_oNeighbor) {
		if (p_oNeighbor != p_oTree) {
			...
			if (clTreePopulation::seedling != iNeighType
				...
				p_oNeighbor->GetValue(
						m_oPop->GetDbhCode(iNeighSpecies, iNeighType), &fNDbh);
				if (fNDbh >= mp_fNCICRMinNeighborDBH[iNeighSpecies]) {

					//Make sure the neighbor's not dead
					iDeadCode = m_oPop->GetIntDataCode("dead",
							p_oNeighbor->GetSpecies(), p_oNeighbor->GetType());
					if (-1 != iDeadCode)
					...
					if (notdead == iIsDead) {
						...
						p_oNeighbor->GetValue(
								m_oPop->GetXCode(iNeighSpecies, iNeighType),
								&fNeighX);
						p_oNeighbor->GetValue(
								m_oPop->GetYCode(iNeighSpecies, iNeighType),
								&fNeighY);

						//Get the distance between the two trees
						fDistance = p_oPlot->GetDistance(fTargetX, fTargetY,
								fNeighX, fNeighY);
						...
						if (0 != fDistance)
							...
							fNCI +=
									mp_fNCICRLambda[iTargetSpecies][iNeighSpecies]
											* (pow(fNDbh,
													mp_fNCICRAlpha[iTargetSpecies])
													/ pow(fDistance,
	...
	fCrownRadius = mp_fNCIMaxCrownRadius[iTargetSpecies] * fSizeEffect
			* fCrowdingEffect;
	return fCrownRadius;
...
float clAllometry::CalcNCICrownDepth(clTree *p_oTree) {
	...
	float fCrowdingEffect, //tree's crowding effect
			...
			fDistance, //distance between target and neighbor
			fNDbh, //neighbor's dbh
			fCrownDepth, fHeight, fNeighX, fNeighY, //holders for the neighbor tree's X and Y
			fTargetX, fTargetY; //holders for the target tree's X and Y location
	int iIsDead;
	short int iNeighSpecies, iNeighType, //species and type for neighbor
			iTargetSpecies = p_oTree->GetSpecies(), //target tree's species
	...
	//seedlings don't compete
	sprintf(cQuery, "%s%f%s%f%s%f%s%f", "distance=",
			mp_fNCICDMaxCrowdingRadius[iTargetSpecies], "FROM x=", fTargetX,
			"y=", fTargetY, "::height=", m_fMinSaplingHeight);
	p_oAllNeighbors = m_oPop->Find(cQuery);
				...
				if (fNDbh >= mp_fNCICDMinNeighborDBH[iNeighSpecies]) {
					...
					if (notdead == iIsDead) {
						...
						if (0 != fDistance)
							...
							//Add competitive effect to NCI
							fNCI +=
									mp_fNCICDLambda[iTargetSpecies][iNeighSpecies]
											* (pow(fNDbh,
													mp_fNCICDAlpha[iTargetSpecies])
													/ pow(fDistance,
	...
	fCrownDepth = mp_fNCIMaxCrownDepth[iTargetSpecies] * fSizeEffect
			* fCrowdingEffect;

	//Make sure crown depth is not greater than total tree height
	p_oTree->GetValue(m_oPop->GetHeightCode(iTargetSpecies, iTargetType),
			&fHeight);
	if (fCrownDepth > fHeight)
		fCrownDepth = fHeight;
