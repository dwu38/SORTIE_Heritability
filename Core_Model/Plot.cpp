...
void clPlot::GetData(DOMDocument *p_oDoc) {
	try {
		...
		FillSingleValue(p_oElement, "plot_precip_mm_yr", &m_fMeanAnnualPrecipMm,
				true);
		//plot mean annual temp, C
		FillSingleValue(p_oElement, "plot_temp_C", &m_fMeanAnnualTempC, true);
		...
		FillSingleValue(p_oElement, "plot_seasonal_precipitation",
				&m_fSeasonalPrecipitation, false);
		//water deficit, not required
		FillSingleValue(p_oElement, "plot_water_deficit", &m_fWaterDeficit,
				false);
		//july maximum temperature, not required
		FillSingleValue(p_oElement, "plot_july_maximum_temperature",
				&m_fJMxTemp, false);
		//january minimum temperature, not required
		FillSingleValue(p_oElement, "plot_january_minimum_temperature",
				&m_fJMnTemp, false);
		//annual snow, not required
		FillSingleValue(p_oElement, "plot_snow", &m_fSnow, false);
		//annual AET, not required
		FillSingleValue(p_oElement, "plot_AET", &m_fAET, false);
		//annual ASpck, not required
		FillSingleValue(p_oElement, "plot_ASpck", &m_fASpck, false);
...
///////////////////////////////////////////////////////////////////////////
// GetDistance()
// with torus conditions
// for each distance, check if it's shorter to wrap.
//////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////
// GetDistance()
// without torus conditions,
//the distance between two points is actually easier to compute without the torus conditions:
// we just have to compute the difference between the coordinates
///////////////////////////////////////////////////////////////////////////
float clPlot::GetDistance(float fFromX, float fFromY, float fToX, float fToY) {
	float fXDistance, fYDistance;

	//For each axis, find the shortest distance:
	if (fToX > fFromX) {
		fXDistance = fToX - fFromX;
	} else {
		fXDistance = fFromX - fToX;
	}

	if (fToY > fFromY) {
		fYDistance = fToY - fFromY;
	} else {
		fYDistance = fFromY - fToY;
	}
...
// GetAzimuthAngle()
///////////////////////////////////////////////////////////////////////////
float clPlot::GetAzimuthAngle(float fFromX, float fFromY, float fToX,
		float fToY) {
	float fDistX = GetXDistance(fFromX, fToX), fDistY = GetYDistance(fFromY,
			fToY), fAzimuth; //azimuth angle to return
...
// GetFastAzimuthAngle()
//////////////////////////////////////////////////////////////////////////
int clPlot::GetFastAzimuthAngle(float fFromX, float fFromY, float fToX,
		float fToY) {
	int i, //loop counter
			iAzimuth = -1; //azimuth value to return
	float fDistX = GetXDistance(fFromX, fToX), fDistY = GetYDistance(fFromY,
			fToY), fTanAzimuth; //tangent of the azimuth angle
	...
	if ((fDistY > 0) && (fDistX >= 0)) { //first quadrant
		...
		for (i = 0; i < 91; i++) {
			if (mp_fAziTans[i] <= fTanAzimuth
					&& mp_fAziTans[i + 1] > fTanAzimuth) {
				iAzimuth = i;
				break;
			}
		}

	} else if ((fDistX > 0) && (fDistY <= 0)) { //second quadrant
	...
	} else if ((fDistX <= 0) && (fDistY < 0)) { //third quadrant
	...
	} else if ((fDistX < 0) && (fDistY >= 0)) { //fourth quadrant
		...
		for (i = 271; i < 359; i++) {
			...
			if (iAzimuth < 0)
				iAzimuth = 359; //avoid going over the end of the array
		}
	}
	return iAzimuth;
}
