...
public:
	...
	/**
	 * Calculates size class according to the seedling diam10
	 * @param fDiam10  Diameter at 10 cm in cm
	 * @param iSpecies Species of tree for which to calculate seedling size class.
	 * @return Size class (0,1,2, or 3)
	 * @throw ModelErr If the species isn't recognized or if the height is greater
	 * than the maximum height for the species.
	 */
	int CalcSeedlingSizeClass(float fHeight, const int &iSpecies);
...
protected:
	...
	/**Typedef to all allometry calculating returning integer*/
	typedef int (clAllometry::*Ptr2AllometryI)(const float&, const int&);
	...
	/**Function pointer array for the appropriate seedling size class
	 * function.  Array size is number of species.*/
	Ptr2AllometryI *mp_SeedlingSC;
	 ...
	 * @return Height in meters.
	 */
	float CalcReverseLinearSaplingHeight(const float &fDbh,
			const int &iSpecies);
	...
	/**
	 * Calculates size class according to standard seedling height
	 * @param fHeight height in m for which to calculate size class.
	 * @param iSpecies Species of tree for which to calculate size class.
	 * @return Size class (0,1,2,3).
	 */
	int CalcStandardSeedlingSizeClass(const float &fHeight,
			const int &iSpecies);
	 ...
	 * @return Height in meters.
	 */
	float CalcReverseLinearSeedlingHeight(const float &fDiam10,
			const int &iSpecies);
	 ...
	 * @return DBH in cm.
	 */
	float CalcReverseLinearSaplingDbh(const float &fHeight,
			const int &iSpecies);
	 ...
	 * @return DBH in cm.
	 */
	float CalcReverseLinearSeedlingDiam10(const float &fHeight,
			const int &iSpecies);
