...
// Constructor()
//////////////////////////////////////////////////////////////////////////////
clSimManager::clSimManager(int iMajorVersion, int iMinorVersion,
		const string sAppPath) {
	try {
		...
		GoToNoDataState();
		throw(err);
	} catch (modelMsg &msg) { //non-fatal error
		throw(msg);
	} catch (...) {
		modelErr stcErr;
...
clSimManager::~clSimManager() {
	...
	delete mp_oPopulationManager;
	mp_oPopulationManager = NULL;
	delete mp_oGridManager;
	mp_oGridManager = NULL;
	delete mp_oBehaviorManager;
	mp_oBehaviorManager = NULL;
	delete mp_oPlot;
	mp_oPlot = NULL;
...
fileType clSimManager::GetFileType(string sFileName) {
	try {
		using namespace std;
		string strFileExtension, strFileNameCopy, strPlaybackExtension,
				strFileCodeTag = "fileCode", //the XML tag for the SORTIE file
				//code
		...
		if (filetest == NULL) { //file does not exist
		...
		strFileExtension = strFileNameCopy.substr(pos,
				strFileNameCopy.length());
		...
		if (strcmp(cXMLTest, cXMLDec) == 0 || strcmp(cXMLTest, cXMLCom) == 0) {
			...
			while (1) { //read all the file text until we find what we want
				...
				if (pos != string::npos) {
					...
					strFileCode = strLine.substr(
							pos + strFileCodeTag.length() + 2, 8);
		...
		if (strFileExtension == ".sgli" || strFileExtension == ".par"
				|| strFileExtension == ".out" || strFileExtension == ".stmf"
				|| strFileExtension == ".sll" || strFileExtension == ".hvr"
				|| strFileExtension == ".spb" || strFileExtension == ".hvs")
			return oldsortie;
...
unsigned long clSimManager::RunSim(int iNumStepsToRun) {
	try {
		...
		if (0 > iNumStepsToRun) {
			...
			stcErr.sMoreInfo =
					"Number of timesteps to run must be greater than zero.";
			throw(stcErr);
		}
		...
		//stop timesteps
			...
			iStartTimestep = m_iCurrentTimestep + 1;
			if (iNumStepsToRun < 1
					|| iNumStepsToRun >= (m_iNumTimesteps - m_iCurrentTimestep))
				iEndTimestep = m_iNumTimesteps + 1;
			else
		...
		//TIMESTEP LOOP
		for (iTimestep = iStartTimestep; iTimestep < iEndTimestep;
				iTimestep++) {

			//Pass a message indicating the timestep we just started
			stcMsg.iMessageCode = INFO;
			stringstream msg;
			msg << "Starting timestep " << iTimestep << " of "
					<< m_iNumTimesteps;
        ...
        SendMessage( stcMsg );
				...
				stcMsg.iMessageCode = INFO;
				stringstream msg;
				msg << "Timestep " << iTimestep << ": Starting behavior "
						<< p_oBehavior->GetName() << " at " << clock();
				stcMsg.sMoreInfo = msg.str();
				SendMessage(stcMsg);
				...
				for (int j = 0; j < iNumPopulations; j++) {
					...
					p_oPopulation =
							dynamic_cast<clPopulationBase*>(p_oTempObject);
...
void clSimManager::ReadParameterFile(DOMDocument *p_oDoc, fileType iFileType) {
	try {
		...
		if ( NULL == p_oDoc) { //nope - throw error
			modelErr stcErr;
...
void clSimManager::DoSetup(DOMDocument *p_oDoc) {
	try {
		...
		FillSingleValue(p_oElement, "yearsPerTimestep", &m_fNumYearsPerTimestep,
				true);
		...
		if (0 >= m_fNumYearsPerTimestep) {
			...
			stcErr.iErrorCode = BAD_DATA;
			stcErr.sMoreInfo =
					"Number of years per timestep must be a positive number.";
			throw(stcErr);
		}
...
void clSimManager::RunBatch(string sBatchFile) {
	...
	p_oBatchGroups = p_oBatchFileDoc->getElementsByTagName(
			XMLString::transcode("ba_parFile"));
	...
	for (i = 0; i < iNumBatchGroups; i++) {
		...
		p_oNodeList = p_oBatchElement->getElementsByTagName(
				XMLString::transcode("ba_fileName"));
		iNumNodes = p_oNodeList->getLength();
		if (0 == iNumNodes) { //no par filename - throw error
			modelErr stcErr;
			stcErr.sFunction = "clSimManager::RunBatch";
		...
		p_parFileList[i].sParFileName = XMLString::transcode(
				p_oNodeList->item(0)->getFirstChild()->getNodeValue());
		...
		if (file.bad()) {
			...
			stcErr.iErrorCode = BAD_FILE;
			std::stringstream s;
			s << "Couldn't open parameter file "
					<< p_parFileList[i].sParFileName;
			stcErr.sMoreInfo = s.str();
			throw(stcErr);
		} //end of if (file.bad())
		file.close();
		...
		p_oNodeList = p_oBatchElement->getElementsByTagName(
				XMLString::transcode("ba_numTimesToRun"));
		if (0 == p_oNodeList->getLength()) {
			modelErr stcErr;
			stcErr.sFunction = "clSimManager::RunBatch";
			stcErr.iErrorCode = DATA_MISSING;
			std::stringstream s;
			s << "ba_numTimesToRun for parameter file "
					<< p_parFileList[i].sParFileName;
			stcErr.sMoreInfo = s.str();
			throw(stcErr);
		} //end of if (0 == p_oNodeList->getLength())
		cNumRunsText = XMLString::transcode(
				p_oNodeList->item(0)->getFirstChild()->getNodeValue());
	...
	for (i = iStart; i < iNumBatchGroups; i++) {
		try {
			...
			p_oNodeList = p_oParFileDoc->getElementsByTagName(
					XMLString::transcode("ou_filename"));
			if (0 != p_oNodeList->getLength()) {
				strDetailedOutputFileName = XMLString::transcode(
						p_oNodeList->item(0)->getFirstChild()->getNodeValue());
				//trim off the file extension, if it exists
				if ("" != strDetailedOutputFileName) {
					strDetailedOutputFileName =
							strDetailedOutputFileName.substr(0,
									strDetailedOutputFileName.find(
											TARBALL_FILE_EXT));
				}
			} else
				strDetailedOutputFileName = "";
			//Short output
			p_oNodeList = p_oParFileDoc->getElementsByTagName(
					XMLString::transcode("so_filename"));
			if (0 != p_oNodeList->getLength()) {
				strShortOutFileName = XMLString::transcode(
						p_oNodeList->item(0)->getFirstChild()->getNodeValue());
				//trim off the file extension, if it exists
				if ("" != strShortOutFileName) {
					strShortOutFileName = strShortOutFileName.substr(0,
							strShortOutFileName.find(SHORT_OUTPUT_FILE_EXT));
				}
			} else
				strShortOutFileName = "";

			//Perform the number of runs specified
			cout << "\nBeginning runs for file "
					<< p_parFileList[i].sParFileName << ".\n" << flush;
			...
			for (j = iStart; j < p_parFileList[i].iNumRuns; j++) {
				...
				if (strDetailedOutputFileName != "") {
					...
					p_oFilenameNode->setNodeValue(
							XMLString::transcode(strNewFileName.str().c_str()));
				}
				if (strShortOutFileName != "") {
					strNewFileName.str("");
				...
				cout << "Run " << (j + 1) << " of " << p_parFileList[i].iNumRuns
						<< " completed.\n" << flush;
...
// CreateGrid()
/////////////////////////////////////////////////////////////////////////////
clGrid* clSimManager::CreateGrid(string sGridName, short int iNumIntVals,
		short int iNumFloatVals, short int iNumCharVals, short int iNumBoolVals,
		float fXCellLength, float fYCellLength) {
	try {
		//Make sure the grid manager has been created - if not, return NULL
		if (!mp_oGridManager)
			return NULL;
		return mp_oGridManager->CreateGrid(sGridName, iNumIntVals,
				iNumFloatVals, iNumCharVals, iNumBoolVals, fXCellLength,
				fYCellLength);
	} catch (modelErr &err) {
		GoToNoDataState();
		throw(err);
