...
clTreePopulation::clTreePopulation(clSimManager * p_oSimManager) :
  ...
  try
  {
    ...
    mp_iHeightCode = NULL;
    mp_iDiam10Code = NULL;
    mp_iSizeClassCode = NULL;
    mp_iDbhCode = NULL;
    mp_iParamG1Code = NULL;
    mp_iParamG2Code = NULL;
    mp_iParamM1Code = NULL;
    mp_iParamM2Code = NULL;
...
clTreePopulation::~clTreePopulation() {
  ...
  for (i = 0; i < m_iNumSpecies; i++) {
    ...
    if (mp_iSizeClassCode) {
      delete[] mp_iSizeClassCode[i];
      mp_iSizeClassCode[i] = NULL;
    }
    if (mp_iDbhCode) {
    ...
    if (mp_iParamG1Code) {
      delete[] mp_iParamG1Code[i];
      mp_iParamG1Code[i] = NULL;
    }
    if (mp_iParamG2Code) {
      delete[] mp_iParamG2Code[i];
      mp_iParamG2Code[i] = NULL;
    }
    if (mp_iParamM1Code) {
      delete[] mp_iParamM1Code[i];
      mp_iParamM1Code[i] = NULL;
    }
    if (mp_iParamM2Code) {
      delete[] mp_iParamM2Code[i];
      mp_iParamM2Code[i] = NULL;
    }
    if (mp_iCrownRadCode) {
  ...
  if (mp_iParamG1Code) {
    delete[] mp_iParamG1Code;
    mp_iParamG1Code = NULL;
  }
  if (mp_iParamG2Code) {
    delete[] mp_iParamG2Code;
    mp_iParamG2Code = NULL;
  }
  if (mp_iParamM1Code) {
    delete[] mp_iParamM1Code;
    mp_iParamM1Code = NULL;
  }
  if (mp_iParamM2Code) {
    delete[] mp_iParamM2Code;
    mp_iParamM2Code = NULL;
  }
  if (mp_iCrownRadCode) {
...
void clTreePopulation::ReadParameters(DOMDocument * p_oDoc) {
  try
  {
    ...
    mp_fMeanParamG1 = new float[m_iNumSpecies];
    mp_fStDevParamG1 = new float[m_iNumSpecies];
    mp_fNSHeritParamG1 = new float[m_iNumSpecies]; //narrow-sense heritability of the ParamG1
    mp_fMeanParamG2 = new float[m_iNumSpecies];
    mp_fStDevParamG2 = new float[m_iNumSpecies];
    mp_fNSHeritParamG2 = new float[m_iNumSpecies]; //narrow-sense heritability of the ParamG2
    mp_fMeanParamM1 = new float[m_iNumSpecies];
    mp_fStDevParamM1 = new float[m_iNumSpecies];
    mp_fNSHeritParamM1 = new float[m_iNumSpecies]; //narrow-sense heritability of the ParamM1
    mp_fMeanParamM2 = new float[m_iNumSpecies];
    mp_fStDevParamM2 = new float[m_iNumSpecies];
    mp_fNSHeritParamM2 = new float[m_iNumSpecies]; //narrow-sense heritability of the ParamM2
    ...
    FillSpeciesSpecificValue(p_oElement, "tr_meanParamG1", "tr_mpg1Val", mp_fMeanParamG1, this, true);
    FillSpeciesSpecificValue(p_oElement, "tr_stdevParamG1", "tr_spg1Val", mp_fStDevParamG1, this, true);
    FillSpeciesSpecificValue(p_oElement, "tr_nsheritParamG1", "tr_h2g1Val", mp_fNSHeritParamG1, this, true); //gets narrow-sense heritability of the ParamG1
    FillSpeciesSpecificValue(p_oElement, "tr_meanParamG2", "tr_mpg2Val", mp_fMeanParamG2, this, true);
    FillSpeciesSpecificValue(p_oElement, "tr_stdevParamG2", "tr_spg2Val", mp_fStDevParamG2, this, true);
    FillSpeciesSpecificValue(p_oElement, "tr_nsheritParamG2", "tr_h2g2Val", mp_fNSHeritParamG2, this, true); //gets narrow-sense heritability of the ParamG2
    FillSpeciesSpecificValue(p_oElement, "tr_meanParamM1", "tr_mpm1Val", mp_fMeanParamM1, this, true);
    FillSpeciesSpecificValue(p_oElement, "tr_stdevParamM1", "tr_spm1Val", mp_fStDevParamM1, this, true);
    FillSpeciesSpecificValue(p_oElement, "tr_nsheritParamM1", "tr_h2m1Val", mp_fNSHeritParamM1, this, true); //gets narrow-sense heritability of the ParamM1
    FillSpeciesSpecificValue(p_oElement, "tr_meanParamM2", "tr_mpm2Val", mp_fMeanParamM2, this, true);
    FillSpeciesSpecificValue(p_oElement, "tr_stdevParamM2", "tr_spm2Val", mp_fStDevParamM2, this, true);
    FillSpeciesSpecificValue(p_oElement, "tr_nsheritParamM2", "tr_h2m2Val", mp_fNSHeritParamM2, this, true); //gets narrow-sense heritability of the ParamM2
...
void clTreePopulation::DoTreeDataStructureSetup() {
  try
  {
    ...
    mp_iSizeClassCode = new short int*[m_iNumSpecies];
    mp_iDbhCode = new short int*[m_iNumSpecies];
    mp_iParamG1Code = new short int*[m_iNumSpecies];
    mp_iParamG2Code = new short int*[m_iNumSpecies];
    mp_iParamM1Code = new short int*[m_iNumSpecies];
    mp_iParamM2Code = new short int*[m_iNumSpecies];
    ...
    for (i = 0; i < m_iNumSpecies; i++) {
      ...
      mp_iSizeClassCode[i] = new short int[m_iNumTypes];
      mp_iDbhCode[i] = new short int[m_iNumTypes];
      mp_iParamG1Code[i] = new short int[m_iNumTypes];
      mp_iParamG2Code[i] = new short int[m_iNumTypes];
      mp_iParamM1Code[i] = new short int[m_iNumTypes];
      mp_iParamM2Code[i] = new short int[m_iNumTypes];
      ...
      for (j = 0; j < m_iNumTypes; j++) {
        ...
        //Float - X, Y, diameter, height and ParamG1, G2, M1 and M2 for seedling, sapling, adult, snag
    	//(seedling was 4, switch to 8, sapling, adult and snag was 6, switch to 10)
        //X, Y, diameter for stump
        //Crown rad and height for saplings, adults, snag
        //Nothing for anything else
    	 // changed: an integer added for seedlings (size class)
        if (j == seedling)
          mp_iNumTreeFloatVals[i][j] = 8;
        else if (j == sapling || j == adult || j == snag)
          mp_iNumTreeFloatVals[i][j] = 10;
        else if (j == stump)
        ...
        //Int - size class for seedlings
        else if (j == seedling) mp_iNumTreeIntVals[i][j] = 1;
        else
        ...
        mp_iDbhCode[i][j] = -1;
        mp_iParamG1Code[i][j] = -1;
        mp_iParamG2Code[i][j] = -1;
        mp_iParamM1Code[i][j] = -1;
        mp_iParamM2Code[i][j] = -1;
        mp_iDiam10Code[i][j] = -1;
        mp_iCrownDepthCode[i][j] = -1;
        mp_iCrownRadCode[i][j] = -1;
        mp_iSizeClassCode[i][j] = -1;
...
//          mp_iParamG1Code[i][j] = RegisterFloat("ParamG1", i, j);
//          mp_iParamG2Code[i][j] = RegisterFloat("ParamG2", i, j);
//          mp_iParamM1Code[i][j] = RegisterFloat("ParamM1", i, j);
//          mp_iParamM2Code[i][j] = RegisterFloat("ParamM2", i, j);
        }
      }
      ...
      //For seedlings, saplings, adults and snags - register ParamG1
      mp_iParamG1Code[i][seedling] = RegisterFloat("ParamG1", i, seedling);
      mp_iParamG1Code[i][sapling] = RegisterFloat("ParamG1", i, sapling);
      mp_iParamG1Code[i][adult] = RegisterFloat("ParamG1", i, adult);
      mp_iParamG1Code[i][snag] = RegisterFloat("ParamG1", i, snag);
      //For seedlings, saplings, adults and snags - register ParamG2
      mp_iParamG2Code[i][seedling] = RegisterFloat("ParamG2", i, seedling);
      mp_iParamG2Code[i][sapling] = RegisterFloat("ParamG2", i, sapling);
      mp_iParamG2Code[i][adult] = RegisterFloat("ParamG2", i, adult);
      mp_iParamG2Code[i][snag] = RegisterFloat("ParamG2", i, snag);
      //For seedlings, saplings, adults and snags - register ParamM1
      mp_iParamM1Code[i][seedling] = RegisterFloat("ParamM1", i, seedling);
      mp_iParamM1Code[i][sapling] = RegisterFloat("ParamM1", i, sapling);
      mp_iParamM1Code[i][adult] = RegisterFloat("ParamM1", i, adult);
      mp_iParamM1Code[i][snag] = RegisterFloat("ParamM1", i, snag);
      //For seedlings, saplings, adults and snags - register ParamM2
      mp_iParamM2Code[i][seedling] = RegisterFloat("ParamM2", i, seedling);
      mp_iParamM2Code[i][sapling] = RegisterFloat("ParamM2", i, sapling);
      mp_iParamM2Code[i][adult] = RegisterFloat("ParamM2", i, adult);
      mp_iParamM2Code[i][snag] = RegisterFloat("ParamM2", i, snag);
      ...
      //For seedlings - register size class
      mp_iSizeClassCode[i][seedling] = RegisterInt("SizeClass", i, seedling);
...
void clTreePopulation::CreateTreesFromInitialDensities(DOMDocument * p_oDoc) {
  try
  {
	...
	fParamG1, fParamG2, fParamM1, fParamM2, // new Params for the tree.
      ...
      for (j = 0; j < iNumClasses; j++)
      {
        ...
        if (strcmp("Seedling", cData) == 0)
        {
          ...
          for (k = 0; k < fNumTrees; k++)
          {
            ...
            //Get ParamG1
            fParamG1 = clModelMath::NormalRandomDraw(mp_fStDevParamG1[iSpecies]) + mp_fMeanParamG1[iSpecies] ;
            //Get ParamG2
            fParamG2 = clModelMath::NormalRandomDraw(mp_fStDevParamG2[iSpecies]) + mp_fMeanParamG2[iSpecies] ;
            //Get ParamM1
            fParamM1 = clModelMath::NormalRandomDraw(mp_fStDevParamM1[iSpecies]) + mp_fMeanParamM1[iSpecies] ;
            //Get ParamM2
            fParamM2 = clModelMath::NormalRandomDraw(mp_fStDevParamM2[iSpecies]) + mp_fMeanParamM2[iSpecies] ;
            //Create tree with zero diameter - allow CreateTree to randomize
            //us up a value
            CreateTree(fX, fY, iSpecies, seedling, 0, fParamG1, fParamG2, fParamM1, fParamM2);
        ...
        { //not the "seedling" class - creating saplings or adults
          ...
          if (fUpperLimit != 0)
          {
            ...
            for (k = 0; k < fNumTrees; k++)
            {
              ...
              // Get ParamG1 fot this tree
              fParamG1 = clModelMath::NormalRandomDraw(mp_fStDevParamG1[iSpecies]) + mp_fMeanParamG1[iSpecies];
              // Get ParamG2 fot this tree
              fParamG2 = clModelMath::NormalRandomDraw(mp_fStDevParamG2[iSpecies]) + mp_fMeanParamG2[iSpecies];
              // Get ParamM1 fot this tree
              fParamM1 = clModelMath::NormalRandomDraw(mp_fStDevParamM1[iSpecies]) + mp_fMeanParamM1[iSpecies];
              // Get ParamM2 fot this tree
              fParamM2 = clModelMath::NormalRandomDraw(mp_fStDevParamM2[iSpecies]) + mp_fMeanParamM2[iSpecies];
              //Figure out whether this is a sapling or adult
              if (fDiam < mp_fMinAdultDbh[iSpecies]) iType = sapling;
              else
                iType = adult;
              CreateTree(fX, fY, iSpecies, iType, fDiam, fParamG1, fParamG2, fParamM1, fParamM2);
            } // end of for (k = 0; k < fNumTrees; k++)
    ...
    for (iSpecies = 0; iSpecies < m_iNumSpecies; iSpecies++)
    {
      ...
      if (fDiam >= m_fNewSeedlingDiam10) {
        ...
        for (k = 0; k < fNumTrees; k++)
        {
          ...
          CreateTree(fX, fY, iSpecies, seedling, fDiam, fParamG1, fParamG2, fParamM1, fParamM2);
    ...
    for (iSpecies = 0; iSpecies < m_iNumSpecies; iSpecies++)
    {
      ...
      for (k = 0; k < fNumTrees; k++)
      {
        ...
        // Get ParamG1 for this tree
        fParamG1 = clModelMath::NormalRandomDraw(mp_fStDevParamG1[iSpecies]) + mp_fMeanParamG1[iSpecies];
        // Get ParamG2 for this tree
        fParamG2 = clModelMath::NormalRandomDraw(mp_fStDevParamG2[iSpecies]) + mp_fMeanParamG2[iSpecies];
        // Get ParamM1 for this tree
        fParamM1 = clModelMath::NormalRandomDraw(mp_fStDevParamM1[iSpecies]) + mp_fMeanParamM1[iSpecies];
        // Get ParamM2 for this tree
        fParamM2 = clModelMath::NormalRandomDraw(mp_fStDevParamM2[iSpecies]) + mp_fMeanParamM2[iSpecies];
...
void clTreePopulation::CreateTreesFromTreeMap(DOMDocument * p_oDoc) {
  try
  {
    ...
    ***p_ints = NULL, ***p_floats = NULL, //array for each data member type
    ...
    fX, fY, fDiam, fHeight, fParamG1, fParamG2, fParamM1, fParamM2, //For getting these required values
    ...
    //All species/type combos that are defined must have X, Y, ParamG1, ParamG2, ParamM1, ParamM2, and diam or
    //height (which can be used to calculate diam)
    for (i = 0; i < m_iNumSpecies; i++)
      for (j = 0; j < m_iNumTypes; j++)
      {
        fX = -1; fY = -1; fDiam = -1; fParamG1 = -1; fParamG2 = -1; fParamM1 = -1; fParamM2 = -1;
        if (p_ints[i][j] || p_floats[i][j] || p_chars[i][j] || p_bools[i][j])
        {
          for (k = 0; k < p_iNumFloats[i][j]; k++)
          {
            ...
            if (p_floats[i][j][k].iCodeForTree == mp_iParamG1Code[i][j]) fParamG1 = 1;
            if (p_floats[i][j][k].iCodeForTree == mp_iParamG2Code[i][j]) fParamG2 = 1;
            if (p_floats[i][j][k].iCodeForTree == mp_iParamM1Code[i][j]) fParamM1 = 1;
            if (p_floats[i][j][k].iCodeForTree == mp_iParamM2Code[i][j]) fParamM2 = 1;
            if (p_floats[i][j][k].iCodeForTree == mp_iHeightCode[i][j]) fDiam = 1;
            if (seedling == j)
            {
              if (p_floats[i][j][k].iCodeForTree == mp_iDiam10Code[i][j]) fDiam = 1;
              if (p_floats[i][j][k].iCodeForTree == mp_iSizeClassCode[i][j]) iSizeClass = 1;
            }
            else if (sapling == j)
            {
          ...
          if (fX < 0 || fY < 0 || fDiam < 0 || fParamG1 < 0 || fParamG2 < 0 || fParamM1 < 0 || fParamM2 < 0)
          { //required values absent - error
            modelErr stcErr;
            stcErr.iErrorCode = DATA_MISSING;
            stcErr.sFunction = "clTreePopulation::CreateTreesFromTreeMap" ;
            stcErr.sMoreInfo = "Cannot load map without X, Y, ParamG1, ParamG2, ParamM1, ParamM2, and diameter values.";
            throw(stcErr);
          }
        }
      }
    ...
    {
      fX = -1; fY = -1; fDiam = -1; fHeight = -1; iSizeClass = -1;
      ...
      //Load the values - first search for X, Y, ParamG1, ParamG2, ParamM1, ParamM2 and diam in the floats so we
      //can create the tree
      ...
      for (j = 0; j < p_oChildren->getLength(); j++)
      {
        ...
        if (iTreeCode == mp_iXCode[iSpecies][iType]) //this is X
          ...
          fY = fTemp;
        else if (iTreeCode == mp_iParamG1Code[iSpecies][iType]) // this is ParamG1
          fParamG1 = fTemp;
        else if (iTreeCode == mp_iParamG2Code[iSpecies][iType]) // this is ParamG2
              fParamG2 = fTemp;
        else if (iTreeCode == mp_iParamM1Code[iSpecies][iType]) // this is ParamM1
          fParamM1 = fTemp;
        else if (iTreeCode == mp_iParamM2Code[iSpecies][iType]) // this is ParamM2
              fParamM2 = fTemp;
        else if (iTreeCode == mp_iSizeClassCode[iSpecies][iType]) // this is SizeClass
          iSizeClass = fTemp;
      ...
      if (fDiam < 0 && fHeight > 0)
      {
        ...
        //Check to see if we have size class but not size nor diameter - if so, get a
        //diameter and a height from the size class value
        if (fDiam < 0 && fHeight < 0 && iSizeClass >= 0)
        {
          if (seedling == iType)
          {
        	if (iSizeClass == 0)
        		fHeight = 5;
        	else if (iSizeClass == 1)
        		fHeight = 12.5;
        	...
        	else
        	{
                ...
                stcErr.iErrorCode = BAD_DATA;
                stcErr.sFunction = "clTreePopulation::CreateTreesFromTreeMap" ;
                stcErr.sMoreInfo = "Seedlings must have size class between 0 and 3.";
                throw(stcErr);
        	}
            fDiam = mp_oAllom->CalcSeedlingDiam10(fHeight, iSpecies);
          }
        else
        {
            ...
            stcErr.iErrorCode = BAD_DATA;
            stcErr.sFunction = "clTreePopulation::CreateTreesFromTreeMap" ;
            stcErr.sMoreInfo = "Tree without Diam or Height, only SizeClass.";
            throw(stcErr);
        }
      ...
      //Whew.  OK, check to see that we have X, Y, ParamG1, ParamG2, ParamM1, ParamM2, and diam.  If yes, create a
      ...
      p_oTree = CreateTree(fX, fY, iSpecies, iType, fDiam, fParamG1, fParamG2, fParamM1, fParamM2);
      ...
      if (iSizeClass > -1)
      {
        p_oTree->SetValue(mp_iSizeClassCode[iSpecies][iType], iSizeClass, true, false);
      }
      ...
      //*********************Floats*********************
      //For floats - skip X, Y, ParamG1, ParamG2, ParamM1, ParamM2, or diameter values
      sVal = XMLString::transcode("fl");
      p_oChildren = p_oTreeInMap->getElementsByTagName(sVal);
      XMLString::release(&sVal);
      for (j = 0; j < p_oNodeList->getLength(); j++)
      {
        ...
        if (iTreeCode != mp_iXCode[iSpecies][iType] && //it's not X
            ...
            iTreeCode != mp_iParamG1Code[iSpecies][iType] && //it's not ParamG1
			iTreeCode != mp_iParamG2Code[iSpecies][iType] && //it's not ParamG2
            iTreeCode != mp_iParamM1Code[iSpecies][iType] && //it's not ParamM1
			iTreeCode != mp_iParamM2Code[iSpecies][iType] && //it's not ParamM2
...
// UpdateTree(int)
//////////////////////////////////////////////////////////////////////////////
void clTreePopulation::UpdateTree(clTree * p_oTree, short int iCode, int iValue, bool bUpdateNow, bool bUpdateAllometry) {
...
//  }
////////////////////////////////////////////////////////////////////////////////////////////////////////////
	//For ints - changed to allow allometry (if the int is SizeClass for seedlings, we need to update diam10 and height)
  try
  {
    float fSize, //for calculating size values
	fAllomValue; //for calculating allometric values
    ...
    if (bUpdateAllometry)
    {
      ...
      {
        if (mp_iSizeClassCode[iSp][iType] == iCode)
        { //update SizeClass
          ...
          if (iValue < 0) {
            ...
            stcErr.sMoreInfo = "Attempt to set seedling Size Class below zero.";
            throw(stcErr);
          }
          p_oTree->mp_iIntValues[iCode] = iValue;
          //calculate height based on height class: set the height to the middle height in the class
          // we don't use this height for now
          // extra small (size at 0.1 because that's the minimum size for a tree )
          if (iValue == 0) fSize = 0.1;
          // small
          else if (iValue == 1) fSize = 0.175;
          // medium
          else if (iValue == 2) fSize = 0.50;
          // large
          else if (iValue == 3) fSize = 1.075;
          // too large, this seedling will transfer to sapling
          else fSize = 1.41;
          p_oTree->mp_fFloatValues[mp_iHeightCode[iSp][iType]] = fSize;
          //calculate diam10 based on height
          //fAllomValue = mp_oAllom->CalcSeedlingDiam10(fSize, iSp);
          p_oTree->mp_fFloatValues[mp_iDiam10Code[iSp][iType]] = 0.001;
        ...
        {
          //Re-do the allometry based on sapling equations, based on the height
          if (mp_iSizeClassCode[iSp][iType] == iCode)
          { //calculate dbh based on height
            fAllomValue = mp_oAllom->CalcSaplingDbh(p_oTree->mp_fFloatValues[mp_iHeightCode[iSp][iType]], iSp);
          ...
          if (p_oTree->mp_fFloatValues[mp_iDbhCode[iSp][sapling]] >= mp_fMinAdultDbh[iSp])
          {
            ...
            if (mp_iSizeClassCode[iSp][iType] == iCode) //size class was updated
              iCode = mp_iHeightCode[iSp][sapling]; //make it the height code
            iType = sapling;

            //Re-do the allometry based on adult equations, keeping constant the
            //thing that was assigned (size = 1.41 in that case)
             fAllomValue = mp_oAllom->CalcAdultDbh(1.41, iSp);
    ...
    else
    {
      ...
      else if (sapling == p_oTree->m_iType && mp_iHeightCode[iSp][iType] == iCode && iValue < 4)
      {
...
void clTreePopulation::UpdateTree(clTree * p_oTree, short int iCode,
  ...
  try
  {
    ...
    int iAllomValue; //for calculating allometric values
    unsigned short int iSp = p_oTree->GetSpecies(), iType = p_oTree->GetType();
    bool bAllometryUpdated = false;
    ...
    //Verify that we're not trying to change X or Y or ParamG1 or ParamG2 or ParamM1 or ParamM2 - if so, throw an error.
    if (iCode == mp_iXCode[iSp][iType] || iCode == mp_iYCode[iSp][iType] ||
    		iCode == mp_iParamG1Code[iSp][iType] || iCode == mp_iParamG2Code[iSp][iType]||
    		iCode == mp_iParamM1Code[iSp][iType] || iCode == mp_iParamM2Code[iSp][iType])
    {
      modelErr stcErr;
      stcErr.iErrorCode = ILLEGAL_OP;
      stcErr.sFunction = "clTreePopulation::UpdateTree" ;
      stcErr.sMoreInfo = "X, Y, ParamG1 and ParamG2, ParamM1 and ParamM2 cannot be changed.";
      throw(stcErr);
    }
    ...
    if (bUpdateAllometry)
    {
      ...
      if (seedling == p_oTree->m_iType)
      {
        ...
        { //update diam10
          ...
          //calculate size class based on height
          iAllomValue = mp_oAllom->CalcSeedlingSizeClass(fAllomValue, iSp);
          p_oTree->mp_iIntValues[mp_iSizeClassCode[iSp][iType]] = iAllomValue;
          bAllometryUpdated = true;
        }
        ...
        { //update Height
          ...
          p_oTree->mp_fFloatValues[mp_iDiam10Code[iSp][iType]] = fAllomValue;
          //calculate size class based on height
          iAllomValue = mp_oAllom->CalcSeedlingSizeClass(fValue, iSp);
          p_oTree->mp_iIntValues[mp_iSizeClassCode[iSp][iType]] = iAllomValue;
          bAllometryUpdated = true;
        }
        else
...
// GetMeanParamG1()
//////////////////////////////////////////////////////////////////////////////
float clTreePopulation::GetMeanParamG1(int iSpecies) {
  if (iSpecies < 0 || iSpecies >= m_iNumSpecies)
  {
    //invalid species - throw an error
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    stcErr.sFunction = "clTreePopulation::GetMeanParamG1" ;
    std::stringstream s;
    s << "Unrecognized species " << iSpecies;
    stcErr.sMoreInfo = s.str();
    throw(stcErr);
  }
  return mp_fMeanParamG1[iSpecies];
}

//////////////////////////////////////////////////////////////////////////////
// GetMeanParamG2()
//////////////////////////////////////////////////////////////////////////////
float clTreePopulation::GetMeanParamG2(int iSpecies) {
  if (iSpecies < 0 || iSpecies >= m_iNumSpecies)
  {
    //invalid species - throw an error
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    stcErr.sFunction = "clTreePopulation::GetMeanParamG2" ;
    std::stringstream s;
    s << "Unrecognized species " << iSpecies;
    stcErr.sMoreInfo = s.str();
    throw(stcErr);
  }
  return mp_fMeanParamG2[iSpecies];
}

//////////////////////////////////////////////////////////////////////////////
// GetMeanParamM1()
//////////////////////////////////////////////////////////////////////////////
float clTreePopulation::GetMeanParamM1(int iSpecies) {
  if (iSpecies < 0 || iSpecies >= m_iNumSpecies)
  {
    //invalid species - throw an error
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    stcErr.sFunction = "clTreePopulation::GetMeanParamM1" ;
    std::stringstream s;
    s << "Unrecognized species " << iSpecies;
    stcErr.sMoreInfo = s.str();
    throw(stcErr);
  }
  return mp_fMeanParamM1[iSpecies];
}

//////////////////////////////////////////////////////////////////////////////
// GetMeanParamM2()
//////////////////////////////////////////////////////////////////////////////
float clTreePopulation::GetMeanParamM2(int iSpecies) {
  if (iSpecies < 0 || iSpecies >= m_iNumSpecies)
  {
    //invalid species - throw an error
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    stcErr.sFunction = "clTreePopulation::GetMeanParamM2" ;
    std::stringstream s;
    s << "Unrecognized species " << iSpecies;
    stcErr.sMoreInfo = s.str();
    throw(stcErr);
  }
  return mp_fMeanParamM2[iSpecies];
}

//////////////////////////////////////////////////////////////////////////////
// SetMeanParamG1()
//////////////////////////////////////////////////////////////////////////////
void clTreePopulation::SetMeanParamG1(int iSpecies, float fNewMeanParamG1) {
	mp_fMeanParamG1[iSpecies] = fNewMeanParamG1;}

//////////////////////////////////////////////////////////////////////////////
// SetMeanParamG2()
//////////////////////////////////////////////////////////////////////////////
void clTreePopulation::SetMeanParamG2(int iSpecies, float fNewMeanParamG2) {
	mp_fMeanParamG2[iSpecies] = fNewMeanParamG2;}

//////////////////////////////////////////////////////////////////////////////
// SetMeanParamM1()
//////////////////////////////////////////////////////////////////////////////
void clTreePopulation::SetMeanParamM1(int iSpecies, float fNewMeanParamM1) {
	mp_fMeanParamM1[iSpecies] = fNewMeanParamM1;}

//////////////////////////////////////////////////////////////////////////////
// SetMeanParamM2()
//////////////////////////////////////////////////////////////////////////////
void clTreePopulation::SetMeanParamM2(int iSpecies, float fNewMeanParamM2) {
	mp_fMeanParamM2[iSpecies] = fNewMeanParamM2;}

//////////////////////////////////////////////////////////////////////////////
// GetStDevParamG1()
//////////////////////////////////////////////////////////////////////////////
float clTreePopulation::GetStDevParamG1(int iSpecies) {
  if (iSpecies < 0 || iSpecies >= m_iNumSpecies)
  {
    //invalid species - throw an error
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    stcErr.sFunction = "clTreePopulation::GetStDevParamG1" ;
    std::stringstream s;
    s << "Unrecognized species " << iSpecies;
    stcErr.sMoreInfo = s.str();
    throw(stcErr);
  }
  return mp_fStDevParamG1[iSpecies];
}

//////////////////////////////////////////////////////////////////////////////
// GetStDevParamG2()
//////////////////////////////////////////////////////////////////////////////
float clTreePopulation::GetStDevParamG2(int iSpecies) {
  if (iSpecies < 0 || iSpecies >= m_iNumSpecies)
  {
    //invalid species - throw an error
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    stcErr.sFunction = "clTreePopulation::GetStDevParamG2" ;
    std::stringstream s;
    s << "Unrecognized species " << iSpecies;
    stcErr.sMoreInfo = s.str();
    throw(stcErr);
  }
  return mp_fStDevParamG2[iSpecies];
}

//////////////////////////////////////////////////////////////////////////////
// GetStDevParamM1()
//////////////////////////////////////////////////////////////////////////////
float clTreePopulation::GetStDevParamM1(int iSpecies) {
  if (iSpecies < 0 || iSpecies >= m_iNumSpecies)
  {
    //invalid species - throw an error
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    stcErr.sFunction = "clTreePopulation::GetStDevParamM1" ;
    std::stringstream s;
    s << "Unrecognized species " << iSpecies;
    stcErr.sMoreInfo = s.str();
    throw(stcErr);
  }
  return mp_fStDevParamM1[iSpecies];
}

//////////////////////////////////////////////////////////////////////////////
// GetStDevParamM2()
//////////////////////////////////////////////////////////////////////////////
float clTreePopulation::GetStDevParamM2(int iSpecies) {
  if (iSpecies < 0 || iSpecies >= m_iNumSpecies)
  {
    //invalid species - throw an error
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    stcErr.sFunction = "clTreePopulation::GetStDevParamM2" ;
    std::stringstream s;
    s << "Unrecognized species " << iSpecies;
    stcErr.sMoreInfo = s.str();
    throw(stcErr);
  }
  return mp_fStDevParamM2[iSpecies];
}
...
// GetNSHeritParamG1()
//////////////////////////////////////////////////////////////////////////////
float clTreePopulation::GetNSHeritParamG1(int iSpecies) {
  if (iSpecies < 0 || iSpecies >= m_iNumSpecies)
  {
    //invalid species - throw an error
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    stcErr.sFunction = "clTreePopulation::GetNSHeritParamG1" ;
    std::stringstream s;
    s << "Unrecognized species " << iSpecies;
    stcErr.sMoreInfo = s.str();
    throw(stcErr);
  }
  return mp_fNSHeritParamG1[iSpecies];
}

//////////////////////////////////////////////////////////////////////////////
// GetNSHeritParamG2()
//////////////////////////////////////////////////////////////////////////////
float clTreePopulation::GetNSHeritParamG2(int iSpecies) {
  if (iSpecies < 0 || iSpecies >= m_iNumSpecies)
  {
    //invalid species - throw an error
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    stcErr.sFunction = "clTreePopulation::GetNSHeritParamG2" ;
    std::stringstream s;
    s << "Unrecognized species " << iSpecies;
    stcErr.sMoreInfo = s.str();
    throw(stcErr);
  }
  return mp_fNSHeritParamG2[iSpecies];
}

//////////////////////////////////////////////////////////////////////////////
// GetNSHeritParamM1()
//////////////////////////////////////////////////////////////////////////////
float clTreePopulation::GetNSHeritParamM1(int iSpecies) {
  if (iSpecies < 0 || iSpecies >= m_iNumSpecies)
  {
    //invalid species - throw an error
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    stcErr.sFunction = "clTreePopulation::GetNSHeritParamM1" ;
    std::stringstream s;
    s << "Unrecognized species " << iSpecies;
    stcErr.sMoreInfo = s.str();
    throw(stcErr);
  }
  return mp_fNSHeritParamM1[iSpecies];
}

//////////////////////////////////////////////////////////////////////////////
// GetNSHeritParamM2()
//////////////////////////////////////////////////////////////////////////////
float clTreePopulation::GetNSHeritParamM2(int iSpecies) {
  if (iSpecies < 0 || iSpecies >= m_iNumSpecies)
  {
    //invalid species - throw an error
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    stcErr.sFunction = "clTreePopulation::GetNSHeritParamM2" ;
    std::stringstream s;
    s << "Unrecognized species " << iSpecies;
    stcErr.sMoreInfo = s.str();
    throw(stcErr);
  }
  return mp_fNSHeritParamM2[iSpecies];
}
...
clTree * clTreePopulation::CreateTree(float fX, float fY, int iSp, int iType,
    float fDiam, float fParamG1, float fParamG2, float fParamM1, float fParamM2) {

	short int iSizeC = 0; // size class for new seedlings (0 for extra small seedlings, less than 10 cm)
  //Validate the data
  ...
  if (seedling == iType)
  {
    //If the diam10 wasn't passed, get a minimum one
    if (0 == fDiam)
    	fDiam = 0.001;
//      fDiam = GetRandomDiam10Value();
	 //commented to allow for diam10 equal to 0.001 (extra-small seedlins < 10cm)
    ...
    //Set the X and Y (and ParamG1, ParamG2, ParamM1, ParamM2) values directly, but let SetValue do the allometry
    //updates for us (not based on DBH anymore but on the size Class)
    p_oTree->mp_fFloatValues[mp_iXCode[iSp][iType]] = fX;
    p_oTree->mp_fFloatValues[mp_iYCode[iSp][iType]] = fY;
    p_oTree->mp_fFloatValues[mp_iParamG1Code[iSp][iType]] = fParamG1;
    p_oTree->mp_fFloatValues[mp_iParamG2Code[iSp][iType]] = fParamG2;
    p_oTree->mp_fFloatValues[mp_iParamM1Code[iSp][iType]] = fParamM1;
    p_oTree->mp_fFloatValues[mp_iParamM2Code[iSp][iType]] = fParamM2;
    p_oTree->SetValue(mp_iDiam10Code[iSp][iType], fDiam, false);
    p_oTree->SetValue(mp_iSizeClassCode[iSp][iType], iSizeC, false,true);
  ...
  else if (stump == iType)
  {
    ...
    //Set X, Y, and DBH values (and ParamG1, ParamG2, ParamM1, ParamM2)
    p_oTree->mp_fFloatValues[mp_iXCode[iSp][iType]] = fX;
    p_oTree->mp_fFloatValues[mp_iYCode[iSp][iType]] = fY;
...
// GetParamG1Code()
//////////////////////////////////////////////////////////////////////////////
short int clTreePopulation::GetParamG1Code(int iSpecies, int iType) {
  //validate the species and type codes
	  if (iSpecies < 0 || iSpecies >= m_iNumSpecies || iType < 0 || iType
	      >= m_iNumTypes) { //throw an error
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    stcErr.sFunction = "clTreePopulation::GetParamG1Code";
    std::stringstream s;
    s << "Unrecognized species or type. Species = " << iSpecies << " Type = " << iType;
    stcErr.sMoreInfo = s.str();
    throw stcErr;
  }
  return mp_iParamG1Code[iSpecies][iType];
}

//////////////////////////////////////////////////////////////////////////////
// GetParamG2Code()
//////////////////////////////////////////////////////////////////////////////
short int clTreePopulation::GetParamG2Code(int iSpecies, int iType) {
  //validate the species and type codes
	  if (iSpecies < 0 || iSpecies >= m_iNumSpecies || iType < 0 || iType
	      >= m_iNumTypes) { //throw an error
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    stcErr.sFunction = "clTreePopulation::GetParamG2Code";
    std::stringstream s;
    s << "Unrecognized species or type. Species = " << iSpecies << " Type = " << iType;
    stcErr.sMoreInfo = s.str();
    throw stcErr;
  }
  return mp_iParamG2Code[iSpecies][iType];
}

//////////////////////////////////////////////////////////////////////////////
// GetParamM1Code()
//////////////////////////////////////////////////////////////////////////////
short int clTreePopulation::GetParamM1Code(int iSpecies, int iType) {
  //validate the species and type codes
	  if (iSpecies < 0 || iSpecies >= m_iNumSpecies || iType < 0 || iType
	      >= m_iNumTypes) { //throw an error
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    stcErr.sFunction = "clTreePopulation::GetParamM1Code";
    std::stringstream s;
    s << "Unrecognized species or type. Species = " << iSpecies << " Type = " << iType;
    stcErr.sMoreInfo = s.str();
    throw stcErr;
  }
  return mp_iParamM1Code[iSpecies][iType];
}

//////////////////////////////////////////////////////////////////////////////
// GetParamM2Code()
//////////////////////////////////////////////////////////////////////////////
short int clTreePopulation::GetParamM2Code(int iSpecies, int iType) {
  //validate the species and type codes
	  if (iSpecies < 0 || iSpecies >= m_iNumSpecies || iType < 0 || iType
	      >= m_iNumTypes) { //throw an error
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    stcErr.sFunction = "clTreePopulation::GetParamM2Code";
    std::stringstream s;
    s << "Unrecognized species or type. Species = " << iSpecies << " Type = " << iType;
    stcErr.sMoreInfo = s.str();
    throw stcErr;
  }
  return mp_iParamM2Code[iSpecies][iType];
}
...
// GetSizeClassCode()
//////////////////////////////////////////////////////////////////////////////
short int clTreePopulation::GetSizeClassCode(int iSpecies, int iType) {
  //validate the species and type codes
  if (iSpecies < 0 || iSpecies >= m_iNumSpecies || (iType
      != seedling)) { //throw an error
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    stcErr.sFunction = "clTreePopulation::GetSizeClassCode";
    std::stringstream s;
    s << "Unrecognized species or type. Species = " << iSpecies << " Type = " << iType;
    stcErr.sMoreInfo = s.str();
    throw stcErr;
  }
  return mp_iSizeClassCode[iSpecies][iType];
}
...
void clTreePopulation::CreateTreesFromTextTreeMap(xercesc::DOMDocument *p_oDoc) {
  ...
  try
  {
    ...
    float fX, fY, fDiam, fHeight = 0, fParamG1, fParamG2, fParamM1, fParamM2, fTemp;
    int iSpecies, iType, iCode, iTemp, iSizeClass = 0;
    ...
    // changed to read ParamG1, ParamG2, ParamM1 and ParamM2
    iNumColumns -= 10; //this is how many are extra
    ...
    //Read lines
    in >> fX >> fY >> sSp >> sTp >> fDiam >> fHeight >> fParamG1 >> fParamG2 >> fParamM1 >> fParamM2 >> iSizeClass;
    ...
    while (!in.eof()) {
      ...
      if (iSizeClass < 0) {
        ...
        stcErr.sFunction = "clTreePopulation::CreateTreesFromTextTreeMap";
        std::stringstream s;
        s << "The file \"" << sFileName << "\" has an invalid tree size class: \"" << iSizeClass << "\".";
        stcErr.sMoreInfo = s.str();
        throw(stcErr);
      }
