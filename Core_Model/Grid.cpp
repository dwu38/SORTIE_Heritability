...
clGrid::clGrid(clSimManager *p_oSimManager, const std::string sGridName,
		short int iNumIntVals, short int iNumFloatVals,
		short int iNumStringVals, short int iNumBoolVals, float fXCellLength,
		float fYCellLength) :
	...
	try {
		...
		if (0 == fYCellLength)
			m_fYCellSize = fXCellLength;
		else
			m_fYCellSize = fYCellLength;
		...
		for (i = 0; i < m_iNumXCells; i++) {
			...
			for (j = 0; j < m_iNumYCells; j++) {
				...
				if (m_iNumIntVals == 0)
					mp_gridVals[i][j].p_iIntVals = NULL;
				else {
				...
				//Float data members
				if (m_iNumFloatVals == 0)
					mp_gridVals[i][j].p_fFloatVals = NULL;
				else {
				...
				//String data members
				if (m_iNumStringVals == 0)
					mp_gridVals[i][j].p_sStringVals = NULL;
				else {
					mp_gridVals[i][j].p_sStringVals =
							new string[m_iNumStringVals];
					for (k = 0; k < m_iNumStringVals; k++)
						mp_gridVals[i][j].p_sStringVals[k] = "";
				}
				//Bool data members
				if (m_iNumBoolVals == 0)
					mp_gridVals[i][j].p_bBoolVals = NULL;
				else {
...
void clGrid::ChangePackageDataStructure(short int iNumIntVals,
		short int iNumFloatVals, short int iNumCharVals,
	...
	try {
		...
		if (!m_bPackageChangeAllowed) {
			...
			stcErr.sMoreInfo =
					"Package data structure may not be changed after packages have been created.";
			throw(stcErr);
		}

		//Delete all the old label arrays
		if (mp_sPackageIntLabels)
			delete[] mp_sPackageIntLabels;
		mp_sPackageIntLabels = NULL;

		if (mp_sPackageFloatLabels)
			delete[] mp_sPackageFloatLabels;
		mp_sPackageFloatLabels = NULL;

		if (mp_sPackageStringLabels)
			delete[] mp_sPackageStringLabels;
		mp_sPackageStringLabels = NULL;

		if (mp_sPackageBoolLabels)
			delete[] mp_sPackageBoolLabels;
		mp_sPackageBoolLabels = NULL;
...
// GetPointOfCell
//////////////////////////////////////////////////////////////////////////////
void clGrid::GetPointOfCell(short int iCellX, short int iCellY, float *fX,
		float *fY) {
	try {
		float fTemp;

		//Make sure the cell points are valid
		if (iCellX < 0 || iCellX >= m_iNumXCells || iCellY < 0
				|| iCellY >= m_iNumYCells) {
			modelErr stcErr;
			stcErr.iErrorCode = BAD_DATA;
		...
		if (iCellX < (m_iNumXCells - 1) && iCellY < (m_iNumYCells - 1)) {
			...
			*fY = (iCellY * m_fYCellSize) + (m_fYCellSize / 2);
			...
			if (iCellX == m_iNumXCells - 1)
				fTemp = (m_fXPlotLength - ((m_iNumXCells - 1) * m_fXCellSize))
						/ 2;
			else
				fTemp = m_fXCellSize / 2;
			*fX = (iCellX * m_fXCellSize) + fTemp;

			if (iCellY == m_iNumYCells - 1)
				fTemp = (m_fYPlotLength - ((m_iNumYCells - 1) * m_fYCellSize))
						/ 2;
			else
				fTemp = m_fYCellSize / 2;
...
void clGrid::GetValueOfCell(int iX, int iY, short int iCode,
	...
	try {
		...
		if (iCode < 0 || iCode >= m_iNumIntVals) {
			...
			stcErr.sMoreInfo = s.str();
			throw(stcErr);
		} else if (iX >= m_iNumXCells || iX < 0 || iY >= m_iNumYCells
				|| iY < 0) {
			modelErr stcErr;
			stcErr.iErrorCode = BAD_DATA;
			stcErr.sFunction = "clGrid::GetValueOfCell (int)";
...
float clGrid::GetAverageValue(float fX, float fY, short int iCode,
	...
	int
			iNumGridCells = 0, //the number of grid cells included in the average
			...
			iMaxYGridCell = 0, iGoodX, iGoodY, //grid numbers corrected for torus
	...
	if (0 > iCode || (bFloat && iCode >= m_iNumFloatVals)
		...
		stcErr.sMoreInfo = s.str();
		throw(stcErr);
	} else if (fX >= m_fXPlotLength || fX < 0 || fY >= m_fYPlotLength
			|| fY < 0) {
		modelErr stcErr;
		stcErr.sFunction = "clGrid::GetAverageValue";
		stcErr.iErrorCode = BAD_DATA;
	...
	if (fRadius >= (m_fYCellSize * m_iNumYCells / 2)) {
		iMaxYGridCell = (int) (floor(
				floor(fY / m_fYCellSize) + (float) (m_iNumYCells / 2.0)));
		iMinYGridCell = (int) (floor(
				floor(fY / m_fYCellSize) - (float) (m_iNumYCells / 2.0)) + 1);
	} else {
	...
	for (iYGrid = iMinYGridCell; iYGrid <= iMaxYGridCell; iYGrid++) {
		...
		if ((iYGrid * m_fYCellSize) < fY)
			fXDist = fRadius * fRadius
					- ((fY - ((iYGrid + 1) * m_fYCellSize))
							* (fY - ((iYGrid + 1) * m_fYCellSize)));
		else
			fXDist = fRadius * fRadius
					- ((fY - (iYGrid * m_fYCellSize))
							* (fY - (iYGrid * m_fYCellSize)));
		...
		if (fXDist > 0) {
			...
			if (fXDist >= (m_fYCellSize * m_iNumYCells / 2)) {
				iMaxXGridCell = (int) floor(
						floor(fX / m_fXCellSize)
								+ (float) (m_iNumXCells / 2.0));
				iMinXGridCell = (int) floor(
						floor(fX / m_fXCellSize) - (float) (m_iNumXCells / 2.0))
						+ 1;
			} else {
		...
		if (iYGrid >= m_iNumYCells)
			iGoodY = iYGrid - m_iNumYCells;
		else if (iYGrid < 0)
			iGoodY = iYGrid + m_iNumYCells;
		else
			iGoodY = iYGrid;
		...
		for (iXGrid = iMinXGridCell; iXGrid <= iMaxXGridCell; iXGrid++) {
			...
			if (iXGrid >= m_iNumXCells)
				iGoodX = iXGrid - m_iNumXCells;
			else if (iXGrid < 0)
				iGoodX = iXGrid + m_iNumXCells;
			else
				iGoodX = iXGrid;
...
// GetAverageIntValue()
/////////////////////////////////////////////////////////////////////////////
float clGrid::GetAverageIntValue(float fFromX, float fFromY, float fToX,
		float fToY, short int iCode) {
...
// GetAverageFloatValue()
/////////////////////////////////////////////////////////////////////////////
float clGrid::GetAverageFloatValue(float fFromX, float fFromY, float fToX,
		float fToY, short int iCode) {
...
// GetAverageValue()
/////////////////////////////////////////////////////////////////////////////
float clGrid::GetAverageValue(float fFromX, float fFromY, float fToX,
		float fToY, short int iCode, bool bFloat) {
	float fAverage = 0;  //the average value to be returned
	...
	iNumCells = 0, iYGrid, iXGrid; //loop counters
	...
	if (0 > iCode || (bFloat && iCode >= m_iNumFloatVals)
		...
		stcErr.sMoreInfo = s.str();
		throw(stcErr);
	} else if (fFromX >= m_fXPlotLength || fFromX < 0 || fToX >= m_fXPlotLength
			|| fToX < 0 || fFromY >= m_fYPlotLength || fFromY < 0
...
float clGrid::GetEndXOfCell(int iX) {
	...
	if (fValue < m_fXPlotLength)
		return fValue;
	else
		return (m_fXPlotLength - VERY_SMALL_VALUE);
}
...
float clGrid::GetEndYOfCell(int iY) {
	...
	if (fValue < m_fYPlotLength)
		return fValue;
	else
		return (m_fYPlotLength - VERY_SMALL_VALUE);
}

/////////////////////////////////////////////////////////////////////////////
// SetValueAtPoint()
////////////////////////////////////////////////////////////////////////////
void clGrid::SetValueAtPoint(float fX, float fY, short int iCode,
		float fValue) {
	try {
...
void clGrid::SetValueOfCell(int iX, int iY, short int iCode, float fValue) {
	try {
		...
		if (iCode < 0 || iCode >= m_iNumFloatVals) {
			...
			mp_gridVals[iX][iY].p_fFloatVals[iCode] = fValue;
	} //end of try block
	catch (modelErr &err) {
		throw(err);
	} catch (modelMsg &msg) {
...
void clGrid::SetValueOfCell(int iX, int iY, short int iCode, int iValue) {
	try {
		...
		if (iCode < 0 || iCode >= m_iNumIntVals) {
			...
			mp_gridVals[iX][iY].p_iIntVals[iCode] = iValue;
	} //end of try block
	catch (modelErr &err) {
		throw(err);
	} catch (modelMsg &msg) {
...
void clGrid::SetValueOfCell(int iX, int iY, short int iCode, bool bValue) {
	try {
		...
		if (iCode < 0 || iCode >= m_iNumBoolVals) {
			...
			mp_gridVals[iX][iY].p_bBoolVals[iCode] = bValue;
	} //end of try block
	catch (modelErr &err) {
		throw(err);
	} catch (modelMsg &msg) {
	...
	try {
		...
		struct stcCodeTrans {     //for translating the data member codes in map
			short int iCodeInMap,     //what the code is in the map
				...
				*p_floats = NULL,  //array for each data member type
				*p_chars = NULL, *p_bools = NULL, *p_cohInts = NULL,
				*p_cohFloats = NULL, //packages
		...
		int iNumGridVals,           //Number of grid values in the map file
				...
				iNumElements, iNumCellVals, //Number of values of a particular data type in
				//one cell
				iNumPackages,           //Number of packages for a cell
				i, j, k, m,             //loop counters
				iTemp = 0;              //For initializing grid values
		short int iNumInts, iNumFloats, iNumStrings, iNumBools, iNumCohInts,
				iNumCohFloats, iNumCohStrings, iNumCohBools, iMapCode, //codes for values read from the map file
		...
		if (0 == p_oNodeList->getLength())
			...
			cData = XMLString::transcode(
					p_oElement->getFirstChild()->getNodeValue());
			fMapXLen = atof(cData);
			delete[] cData;
			cData = NULL;
		} //end of else
		...
		if (fMapXLen != p_oPlot->GetXPlotLength()
			...
			std::stringstream s;
			s
					<< "A plot dimension for this map doesn't match the plot.  Map X = "
			...
			if (iNumCohFloats > 0) {
				...
				for (i = 0; i < iNumCohFloats; i++) {
					...
					cData = XMLString::transcode(
							p_oElement->getAttributeNode(sVal)->getNodeValue());
					XMLString::release(&sVal);
					p_cohFloats[i].iCodeForGrid = GetPackageFloatDataCode(
							cData);
		...
		if (0 != p_oNodeList->getLength()) {
			...
			if (iNumCohStrings > 0) {
				...
				for (i = 0; i < iNumCohStrings; i++) {
					...
					XMLString::release(&sVal);
					sTemp = cData;
					p_cohChars[i].iCodeForGrid = GetPackageStringDataCode(
							sTemp);
		...
		for (i = 0; i < m_iNumXCells; i++)
			for (j = 0; j < m_iNumYCells; j++) {
				for (k = 0; k < m_iNumIntVals; k++)
					SetValueOfCell(i, j, k, iTemp);
				for (k = 0; k < m_iNumFloatVals; k++)
					SetValueOfCell(i, j, k, fTemp);
				for (k = 0; k < m_iNumStringVals; k++)
					SetValueOfCell(i, j, k, "");
				for (k = 0; k < m_iNumBoolVals; k++)
					SetValueOfCell(i, j, k, bTemp);
				//Delete all packages
				p_oPackage = mp_gridVals[i][j].p_oPackage;
				while (p_oPackage) {
		...
		for (i = 0; i < iNumGridVals; i++) {
			...
			if (NULL == p_oAttr) {
				...
				stcErr.sFunction = "clGrid::ReadMapFile";
				stcErr.sMoreInfo =
						"Can't find X attribute for a cell for grid ";
			...
			for (j = 0; j < iNumCellVals; j++) {
				...
				if (-1 == iGridCode) { //unrecognized code - throw error
					...
					stcErr.iErrorCode = BAD_DATA;
					std::stringstream s;
					s << "Unrecognized int code " << iMapCode << " in map "
							<< m_sNameString;
					stcErr.sMoreInfo = s.str();
					throw(stcErr);
				}
			...
			for (j = 0; j < iNumCellVals; j++) {
				...
				if (-1 == iGridCode) { //unrecognized code - throw error
					...
					stcErr.iErrorCode = BAD_DATA;
					std::stringstream s;
					s << "Unrecognized float code " << iMapCode << " in map "
							<< m_sNameString;
					stcErr.sMoreInfo = s.str();
					throw(stcErr);
				}
			...
			for (j = 0; j < iNumCellVals; j++) {
				...
				if (-1 == iGridCode) { //unrecognized code - throw error
					...
					stcErr.iErrorCode = BAD_DATA;
					std::stringstream s;
					s << "Unrecognized char code " << iMapCode << " in map "
							<< m_sNameString;
					stcErr.sMoreInfo = s.str();
					throw(stcErr);
				}
			...
			for (j = 0; j < iNumCellVals; j++) {
				...
				if (-1 == iGridCode) { //unrecognized code - throw error
					...
					stcErr.iErrorCode = BAD_DATA;
					std::stringstream s;
					s << "Unrecognized bool code " << iMapCode << " in map "
							<< m_sNameString;
					stcErr.sMoreInfo = s.str();
					throw(stcErr);
				}
				cData = XMLString::transcode(
						p_oElement->getFirstChild()->getNodeValue());
				if (strcmp(cData, "true") == 0)
					bTemp = true;
				else if (strcmp(cData, "false") == 0)
					bTemp = false;
			...
			for (m = 0; m < iNumPackages; m++) {
				...
				if (p_oPackage) {
					...
					p_oPackage = CreatePackageOfCell(iXGridVal, iYGridVal);
				...
				for (j = 0; j < iNumCellVals; j++) {
					...
					if (-1 == iGridCode) { //unrecognized code - throw error
						...
						stcErr.iErrorCode = BAD_DATA;
						std::stringstream s;
						s << "Unrecognized package " << "int code " << iMapCode
								<< " in map " << m_sNameString;
						stcErr.sMoreInfo = s.str();
						throw(stcErr);
					}
					cData = XMLString::transcode(
				...
				for (j = 0; j < iNumCellVals; j++) {
					...
					if (-1 == iGridCode) { //unrecognized code - throw error
						...
						stcErr.iErrorCode = BAD_DATA;
						std::stringstream s;
						s << "Unrecognized package " << "float code "
								<< iMapCode << " in map " << m_sNameString;
						stcErr.sMoreInfo = s.str();
						throw(stcErr);
					}
					cData = XMLString::transcode(
				...
				for (j = 0; j < iNumCellVals; j++) {
					...
					if (-1 == iGridCode) { //unrecognized code - throw error
						...
						stcErr.iErrorCode = BAD_DATA;
						std::stringstream s;
						s << "Unrecognized package char code " << iMapCode
								<< " in map " << m_sNameString;
						stcErr.sMoreInfo = s.str();
						throw(stcErr);
					}
				...
				for (j = 0; j < iNumCellVals; j++) {
					...
					if (-1 == iGridCode) { //unrecognized code - throw error
						...
						stcErr.iErrorCode = BAD_DATA;
						std::stringstream s;
						s << "Unrecognized package bool code " << iMapCode
								<< " in map " << m_sNameString;
						stcErr.sMoreInfo = s.str();
						throw(stcErr);
					}
...
short int clGrid::RegisterInt(const string sLabel) {
	...
	if (!m_bPackageDataChanged)
		RegisterDataMember(sLabel, m_iNumPackageIntVals, mp_sPackageIntLabels);
	return iReturnCode;
}
...
short int clGrid::RegisterString(const string sLabel) {
	short int iReturnCode;
	iReturnCode = RegisterDataMember(sLabel, m_iNumStringVals,
			mp_sStringLabels);
	//We may need to set up a package data member too
	if (!m_bPackageDataChanged)
...
short int clGrid::RegisterPackageInt(const string sLabel) {
	return RegisterDataMember(sLabel, m_iNumPackageIntVals,
...
short int clGrid::RegisterPackageFloat(const string sLabel) {
	return RegisterDataMember(sLabel, m_iNumPackageFloatVals,
			mp_sPackageFloatLabels);
}
...
short int clGrid::RegisterPackageString(const string sLabel) {
	return RegisterDataMember(sLabel, m_iNumPackageStringVals,
			mp_sPackageStringLabels);
}
...
short int clGrid::RegisterPackageBool(const string sLabel) {
	return RegisterDataMember(sLabel, m_iNumPackageBoolVals,
			mp_sPackageBoolLabels);
}
...
const string clGrid::GetBoolDataLabel(short int iCode) {
	...
	if (iCode < 0 || iCode >= m_iNumBoolVals)
		...
		return mp_sBoolLabels[iCode];
}
//----------------------------------------------------------------------------
const string clGrid::GetPackageIntDataLabel(short int iCode) {
	//If the code's invalid, return an empty string
	if (iCode < 0 || iCode >= m_iNumPackageIntVals)
		return "";
	else
		return mp_sPackageIntLabels[iCode];
}
//----------------------------------------------------------------------------
const string clGrid::GetPackageFloatDataLabel(short int iCode) {
	//If the code's invalid, return an empty string
	if (iCode < 0 || iCode >= m_iNumPackageFloatVals)
		return "";
	else
		return mp_sPackageFloatLabels[iCode];
}
//----------------------------------------------------------------------------
const string clGrid::GetPackageStringDataLabel(short int iCode) {
	//If the code's invalid, return an empty string
	if (iCode < 0 || iCode >= m_iNumPackageStringVals)
		return "";
	else
		return mp_sPackageStringLabels[iCode];
}
//----------------------------------------------------------------------------
const string clGrid::GetPackageBoolDataLabel(short int iCode) {
	//If the code's invalid, return an empty string
	if (iCode < 0 || iCode >= m_iNumPackageBoolVals)
		return "";
	else
		return mp_sPackageBoolLabels[iCode];
}
//----------------------------------------------------------------------------
...
// Constructor
//////////////////////////////////////////////////////////////////////////////
clPackage::clPackage(clGrid *p_oParentGrid, clGrid::stcRecords *p_oParentCell) {
	try {
		int k;        //loop counters
		...
		//Size and initialize the arrays
		if (mp_oParentGrid->m_iNumPackageIntVals == 0)
			mp_iIntVals = NULL;
		else {
		...
		//Float data members
		if (mp_oParentGrid->m_iNumPackageFloatVals == 0)
			mp_fFloatVals = NULL;
		else {
		...
		//Char data members
		if (mp_oParentGrid->m_iNumPackageStringVals == 0)
			mp_sStringVals = NULL;
		else {
			mp_sStringVals =
					new string[mp_oParentGrid->m_iNumPackageStringVals];
			for (k = 0; k < mp_oParentGrid->m_iNumPackageStringVals; k++) {
				mp_sStringVals[k] == "";
			}
		}
		//Bool data members
		if (mp_oParentGrid->m_iNumPackageBoolVals == 0)
			mp_bBoolVals = NULL;
		else {
...
void clPackage::SetValue(short int iCode, int iValue) {
	try {
		...
		if (iCode < 0 || iCode >= mp_oParentGrid->m_iNumPackageIntVals) {
			...
			std::stringstream s;
			s << "Invalid data code:  " << iCode << " for grid "
					<< mp_oParentGrid->m_sNameString;
			stcErr.sMoreInfo = s.str();
			throw(stcErr);
		}
