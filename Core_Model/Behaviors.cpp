...
#include <ClimateNCIMortality.h> //formerly JulMaxPrecipMortality.h
#include <iostream>
#include <sstream>
#include "Behaviors.h"
...
#include "ClimateSeedlingHeightGrowth.h" //formerly SeedlingsGrowth.h
#include "ClimateNCIGrowth.h"
#include "ClimateGenGrowth.h"
...
#include "ClimateSeedlingHeightSurvival.h" //formerly SeedlingsSurvival.h
...
void clBehaviorManager::CreateBehavior(behaviorData * p_data)
{
  try
  {
    ...
    else if (p_data->sNameString.compare("AnnualClimateImporter") == 0)
    {
      clAnnualClimateImporter * p_oObj = new clAnnualClimateImporter(mp_oSimManager);
      mp_oObjectArray[iIndex] = p_oObj;
    }

    else if (p_data->sNameString.compare("ParamChange") == 0)
    {
    	clParamChange * p_oObj = new clParamChange(mp_oSimManager);
    	mp_oObjectArray[iIndex] = p_oObj;
    }
    ...
    else if (p_data->sNameString.compare("LinearGrowth diam only") == 0
        || p_data->sNameString.compare("LinearGrowth") == 0)
    {
      clLinearGrowth * p_oGrowth = new clLinearGrowth(mp_oSimManager);
      mp_oObjectArray[iIndex] = p_oGrowth;
    }
    else if (p_data->sNameString.compare("ClimateDepGrowth diam only") == 0
        || p_data->sNameString.compare("ClimateDepGrowth") == 0)
    {
      clClimateDepGrowth * p_oGrowth = new clClimateDepGrowth(mp_oSimManager);
      mp_oObjectArray[iIndex] = p_oGrowth;
    }
//    else if (p_data->sNameString.compare("SeedlingsGrowth diam only") == 0
//        || p_data->sNameString.compare("SeedlingsGrowth") == 0)
//    {
//      clSeedlingsGrowth * p_oGrowth = new clSeedlingsGrowth(mp_oSimManager);
//      mp_oObjectArray[iIndex] = p_oGrowth;
//    }
    else if (p_data->sNameString.compare("ClimateNCIGrowth diam only") == 0
        || p_data->sNameString.compare("ClimateNCIGrowth") == 0)
    {
      clClimateNCIGrowth * p_oGrowth = new clClimateNCIGrowth(mp_oSimManager);
      mp_oObjectArray[iIndex] = p_oGrowth;
    }
    else if (p_data->sNameString.compare("JanMinPrecipGrowth diam only") == 0
        || p_data->sNameString.compare("JanMinPrecipGrowth") == 0)
    {
      clJanMinPrecipGrowth * p_oGrowth = new clJanMinPrecipGrowth(mp_oSimManager);
      mp_oObjectArray[iIndex] = p_oGrowth;
    }
    else if (p_data->sNameString.compare("ClimateGenGrowth diam only") == 0
        || p_data->sNameString.compare("ClimateGenGrowth") == 0)
    {
      clClimateGenGrowth * p_oGrowth = new clClimateGenGrowth(mp_oSimManager);
      mp_oObjectArray[iIndex] = p_oGrowth;
    }
    else if (p_data->sNameString.compare("ClimateGridGrowth diam only") == 0
        || p_data->sNameString.compare("ClimateGridGrowth") == 0)
    {
      clClimateGridGrowth * p_oGrowth = new clClimateGridGrowth(mp_oSimManager);
      mp_oObjectArray[iIndex] = p_oGrowth;
    }
    ...
    else if (p_data->sNameString.compare("SeedlingsHeightGrowth height only") == 0)
    {
      clSeedlingsHeightGrowth * p_oGrowth = new clSeedlingsHeightGrowth(mp_oSimManager);
      mp_oObjectArray[iIndex] = p_oGrowth;
    }
    ...
    else if (p_data->sNameString.compare("LogisticMortality") == 0)
    {
      clLogisticMortality * p_oMort = new clLogisticMortality(mp_oSimManager);
      mp_oObjectArray[iIndex] = p_oMort;

    }
    else if (p_data->sNameString.compare("ClimateDepMortality") == 0)
    {
      clClimateDepMortality * p_oMort = new clClimateDepMortality(mp_oSimManager);
      mp_oObjectArray[iIndex] = p_oMort;

    }
    else if (p_data->sNameString.compare("ClimateNCIMortality") == 0)
    {
      clClimateNCIMortality * p_oMort = new clClimateNCIMortality(mp_oSimManager);
      mp_oObjectArray[iIndex] = p_oMort;

    }
//    else if (p_data->sNameString.compare("JulMaxPrecipMortality") == 0)
//    {
//      clJulMaxPrecipMortality * p_oMort = new clJulMaxPrecipMortality(mp_oSimManager);
//      mp_oObjectArray[iIndex] = p_oMort;
//
//    }
    ...
    //replaced SeedlingsSurvival with ClimateSeedlingHeightSurvival below
    else if (p_data->sNameString.compare("ClimateSeedlingHeightSurvival") == 0)
    {
      clClimateSeedlingHeightSurvival * p_oMort = new clClimateSeedlingHeightSurvival(mp_oSimManager);
      mp_oObjectArray[iIndex] = p_oMort;
    }
    ...
    else if (p_data->sNameString.compare("DisperseParamChange") == 0)
    {
      clDisperseParamChange * p_oDisperse = new clDisperseParamChange(mp_oSimManager);
      mp_oObjectArray[iIndex] = p_oDisperse;
    }
    else if ((p_data->sNameString.compare("GapDisperseLinear") == 0)
        || (p_data->sNameString.compare("NonGapDisperseLinear") == 0))
    {
      clSpatialDispersalLinear * p_oDisperse = new clSpatialDispersalLinear(mp_oSimManager);
      mp_oObjectArray[iIndex] = p_oDisperse;
    }
